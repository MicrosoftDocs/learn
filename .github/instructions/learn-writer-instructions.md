...
# Phase 9: Create video transcript

<workflow>
  Based on the learn unit content created, generate a detailed transcript for a video that effectively conveys the key concepts and information.

  ## Define the Key Topic

  Decision criteria: What is the one thing they need to remember after watching this video?

  **Purposeful**: It explains what it is and why it matters.
  **Actionable**: The video provides practical steps learners can immediately apply. It shows how to do the thing, ideally with a demo, checklist, or framework.
  **Trackable**: Challenge, question or reflection

  Make sure you get an answer on these 3 questions before moving to the next step!

  ## Define The hook

  Grab the viewers attention from the start with a hook. You can pick (one) from:

  1) Ask a question (but don't answer it yet).
  2) Tell a quick story that grabs my attention.
  3) Make a bold statement.

  Make it short, because the audience attention span is limited.

  ## Define the transcript

  Make the transcript clear, engaging and effecting. It needs to be dynamic, direct and designed for spoken delivery. Fluent and naturally when read aloud. Use "You", "We", and "Us" to create a converational engagement feel. Keep jargon and accronyms in check. If the audience isn't already familiar with the term, define it before using it. 

  The presenter of the video can use a touch of humor to help break through the monotony and ensure that critical learning messages actualy stick in the learners mind. 

  Do not deviate from the key topic defined earlier. Do not invent any new information. Stick to the content topics provided by the user.

  Length of the video should be maximum 4 minutes.

  ## Define the call to action

  At the end of the video do a call to action and optionally refer back to the hook. If in the hook, you started with a question, now is the time to answer it. The call to action could also be somehting like "pause this video and try this out right now".

  ## Now it's time to split the transcript into scenes. 

  The transcript should be split into multiple chunks of 20-30 seconds. 
  Output only the transcript, no additional commentary. No need to add scene titles.

  ## Output transcript

  Store the transcript without headings or any additional comments into a markdown file. 

  Output location: `/learn-pr/wwl-data-ai/(module)/videos/(unit-id).md`
</workflow>

# Phase 10: Create video using Synthesia

<workflow>
  Now that we have the transcript (with the paragraphs/scenes), we can create the synthesia video. Use the #synthesia-crt-video tool with this JSON:

  ```json
  {
    "test": false,
    "title": "<markdown-filename.md>",
    "visibility": "private",
    "aspectRatio": "16:9",
    "input": [
      {
        "scriptText": "<script>",
        "avatar": "b00c69d4-e253-4843-a32b-71832943c83a",
        "background": "workspace-media.f3dbe147-e326-49c1-bc57-e1b5a4d511d9"
      }
    ]
  }
  ```

  - Every paragraph/scene should be a separate scriptText in the input array.
  - The title element should be the same as the name of the transcript markdown file you just used (without extension .md).

  Once you created the video, get the id from the synthesia response and create a url like this:

  https://app.synthesia.io/#/video/<id>
