...
- Use default setup to quickly configure CodeQL analysis for code scanning on your repository. The default setup will handle choosing the languages to analyze, query suite to run, and events that trigger scans with the option to manually configure the languages and query suites. This setup option will run code scanning as a Github Action.
- Use advanced setup to add the CodeQL workflow directly to your repository. This generates a customizable workflow file which uses the [github/codeql-action](https://github.com/github/codeql-action/) to run the CodeQL CLI as a Github Action.
...
5. Review the default options and if needed select the "Edit" button in the bottom left corner of the new window to customize how CodeQL will run.
...
Above we enabled code scanning using the default setup, which will run code scans as a Github Action without needing to maintain a workflow file. The other option is "advanced setup," which will generate the default workflow file that can be edited for advanced configuration and additional steps. We will cover using the advanced setup for configuring code scanning in a later unit.
