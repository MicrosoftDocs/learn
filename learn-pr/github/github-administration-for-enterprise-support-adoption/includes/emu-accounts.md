...
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **Ownership**  | Specifically created for and controlled by the identity provider. | Multiple authorization and authentication initiatives required in GitHub to ensure secure enterprise access. |
...
| **Policies**   | Policies can be created at the enterprise level which apply to all users on the account. | Users are required to confirm compliance with GitHub user policies. |
...
| **Metadata**   | User GitHub metadata is controlled by the enterprise and cannot be modified by managed users (e.g. email addresses, display names, usernames). | Enterprise users create their own account, provide email, and choose their GitHub handle. |
...
There are several unique ways you can leverage EMU accounts within your organization, but here are some common use cases for EMUs found within most companies.

#### Onboard new employees

Employees are constantly joining the company all across the organization, departments, and teams. As an administrator, you can provision an EMU account for a new employee with all of their required permissions in the morning of their first day. This also allows you to assign a GitHub handle that matches their work email to make identification easy across all systems. All of this occurs within the IdP and not within GitHub. The benefits of this are to configure these settings in the same automatic flow as their benefits, HR, and expense systems to create a fast, efficient, and consistent onboarding.

#### Off board employees

On the contrary to onboarding, when an employee leaves the company an administrator can instantly suspend their GitHub account with the same flow used to turn off the rest of their work access and systems like HR, email, internal documents, benefits, etc. Since EMUs are governed by the IdP, this creates an efficient, fast, and secure method to remove access to sensitive source code when the employee leaves the company without lingering GitHub access.
