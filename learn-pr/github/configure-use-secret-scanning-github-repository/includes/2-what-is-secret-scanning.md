...
Push protection prevents secret leaks by scanning for highly identifiable secrets before they are pushed. When a secret is detected in code, contributors are prompted directly in their IDE or command line interface with remediation guidance to ensure that the secret is not inadvertently exposed. To proceed, contributors must either remove the secret(s) from the push or, if needed, bypass the protection. 
