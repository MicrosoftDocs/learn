...
![Diagram that shows real time feeds sending data to stream processing applications, then to an output. An arrow shows data moving to storage from the stream processing information to storage, as well as optional storage and queries going to storage from the output.](../media/streaming-2.png)
