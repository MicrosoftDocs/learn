...
Azure provides predefined roles that describe common (tasks? TODO). Each role has an associated set of access permissions that relate to that role. When you assign individuals or groups to one or more roles, they receive all of the associated access permissions. Role-based access control is then applied to a _scope_, which is a resource or set of resources that this access applies to.
...
Azure RBAC is enforced on any action that's initiated against an Azure resource that passes through the Azure Resource Manager control plane (TODO: Define "control plane"?).

You typically access the Azure Resource Manager from the Azure portal, Azure Cloud Shell, Azure PowerShell, and the Azure CLI. Azure RBAC does not enforce access permissions at an application or data level. Application security must be handled by your application.
...
You can apply Azure RBAC to an individual person or a group of people. You can also apply Azure RBAC to other special identity types such as service principals and managed identities. These identity types are used by applications and services to automate access to Azure resources.
...
* Cost and Billing
    ...
    People in this team track and report on technology-related spend and managed the organization's internal budgets.
* Security Operations
...
(Tom) I like this idea. And maybe we can use it in the KC unit.
</div>

## How do I manage Azure RBAC permissions?

You manage access permissions on the **Access control (IAM)** blade in the Azure portal. This blade shows who has access to that scope and their role. You can also grant or remove access from this blade.
