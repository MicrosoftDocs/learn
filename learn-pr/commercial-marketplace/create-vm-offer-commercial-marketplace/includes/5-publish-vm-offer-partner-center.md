We've covered virtual machine (VM) offer basics and concepts to help create your VM. Now we'll spend some time on an overview of the Partner Center publishing process. In this process, you'll put all the pieces together, providing the information needed to create and publish your offer.
...
When you publish your offer, you'll be able to identify a preview audience that gets access to review your VM offer before it's available in Azure Marketplace. For more information, review our [documentation on adding a preview audience for your VM offer](/partner-center/marketplace/azure-vm-create-preview).
...
- Add open ports (which are public ports that will be automatically opened on a deployed VM). You can specify the ports individually or via a range along with the supported protocol: TCP, UDP, or both. Be sure to use a hyphen if specifying a port range (such as 80-150).
