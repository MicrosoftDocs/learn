The team would like to try using an *Azure Resource Manager template* to automate the deployment of their _Space Game Web_ application infrastructure. This includes the Azure SQL Server logical instance and the Azure SQL Database as well as the App Service Plan and App Service.

The WebApp is still in development and database schema changes will happen. The database administrator (DBA) is responsible for the integrity of the database and will need to approve any changes made to the schema before they are applied.

In the [Manage database changes in Azure Pipelines](https://docs.microsoft.com/learn/modules/manage-database-changes-in-azure-pipelines/azure-pipelines=true) module you created a pipeline with a stage that built the .NET Core webApp project and the SQL Server Data Tools database project. Then added a stage to discover the changes made to the database schema. The next stage that you added applied those schema changes if the DBA approved them. Then the WebApp was deployed.
...
The _Build_ stage has a job to build the WebApp and a job to build the database project. ![Callout 1](../../shared/media/callout-01.png) The database project build produces a _.dacpac_ file that is a build artifact.
...
In that module, you created the infrastructure manually before you ran the pipeline. The _dev_, _test_, and _staging_ stages all had identical infrastructure and shared a common database. As the web application project is maturing, the infrastructure needs of each of these stages is changing. The team is looking at Azure Resource Manager templates to automate deploying infrastructure to each stage.

In this section you:

> [!div class="checklist"]
> * Learn about Infrastructure as Code using Azure Resource Manager templates
> * Design an Azure Resource Manager template for an Azure App Service and Azure SQL Database to use in Azure Pipelines.
> * Understand how to use Azure Key Vault to store and retrieve secrets using Azure Resource Manager templates and Azure Pipelines.
> * Learn what is needed to add an Azure Resource Manager template to Azure Pipelines.
...
**Tim:** I was hoping you could do that. Automation to the rescue again. But what is *Azure Resource Manager templates*?

### Infrastructure as Code using Azure Resource Manager templates

Recall that Azure Resource Manager is the interface for managing and organizing cloud resources. A Resource Manager template defines all the Resource Manager resources in a deployment allowing you to deploy a Resource Manager template into a resource group as a single operation. A link to a Learn module on Azure Resource Manager templates is provided at the end of this module.
...
**Andy:** We will need to create template and parameter files and change the pipeline to use them. The only parameter so far is just the resource name suffix representing the stage we are deploying to. For example, _dev_ for the development stage.
...
*Mara searches on her laptop*

**Mara:** Everything we had to create manually the last time we worked with the database changes in the pipeline can now go in the template file. Even deploying the database using the .bacpac file. So that means we will provision the SQL Server instance, the Azure SQL database, the App Service plan, and the App Service. I propose we create just the infrastructure we need for one stage. Let's start with the _dev_ stage. Then we can reuse this template for the other stages, just change the parameters we give the template.
...
**Tim:** But what about the administrator password for the SQL Server instance? I don't want that in the files that are pushed to the GitHub repo.

*Andy does some typing on his laptop*

### Use Azure Key Vault to store and retrieve secrets using Azure Resource Manager templates and Azure Pipelines

*Azure Key Vault* is a tool used to securely store secrets. For example, you can store your API keys and passwords or anything that needs tight control over who has access to it.
...
**Mara:** Now we need a task that will handle the template deployment. I found this one, [AzureResourceManagerTemplateDeployment@3](https://github.com/microsoft/azure-pipelines-tasks/blob/master/Tasks/AzureResourceManagerTemplateDeploymentV3/README.md?azire-pipelines=true). It has parameters for the template file and the template parameter file.

We also need to think about setting the connection string in the App Service. We could do that in the template, but then when we deploy the WebApp, it will get overwritten. So I think we need a task to set that after the WebApp is deployed.
...
**Tim:** We can get to it in the pipeline using the [AzureKeyVault@1](https://docs.microsoft.com/azure/devops/pipelines/tasks/deploy/azure-key-vault?view=azure-devops&azure-poral=true) task.
...
**Andy:** Any other concerns? No? Then I think we have a plan.
