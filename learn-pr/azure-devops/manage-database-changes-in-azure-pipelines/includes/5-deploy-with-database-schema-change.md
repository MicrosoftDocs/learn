...
1. You still need to add data to the new column before you can try the new web site.
    ...
    1. Paste the file contents into **Query 1** and select **Run**. Add the new data to the table. Insert sql script. FavoriteMapData.sql in query editor.
    ...
    1. In **Query 2** add the following T-SQL.
        ...
        ```
        Verify that the `favoriteMap` column is populated.
