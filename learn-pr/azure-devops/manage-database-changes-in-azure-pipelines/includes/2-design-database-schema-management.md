...
**Andy:** After the database is in place, we can modify the website to read from the database instead of from local files. This is a big milestone for us, so we need to discuss a few topics. First, let's choose a database technology. We'll need to consider a lot of things, so bear with me.

## Choose a database technology

When considering a data storage solution, you can choose from several options. Here are some of the guiding factors that can help you choose:

### The type of data you're storing

Data can be structured, semistructured, or unstructured.

* _Structured_ data is relational data that fits into tables with columns and rows. For example, think of a point-of-sale system. You might have a table for product data that has columns that define how you describe your products. For example, columns can be named Product Name, SKU Number, Color, and so on.

* _Unstructured_ data includes documents such as text files, photos, and videos. This format is for quick access to an entire file and not for processing records.

* _Semistructured_ data is nonrelational data that fits into hierarchies by using tags. This format is good for data whose description might change. Your product data is semistructured if you can't use the same columns to describe all of your products the way you can with structured data. Usually, this data is stored in a JSON or XML file format.
...
> [!NOTE]
> At the end of this module, you'll find resources to help you choose the right data storage option.
...
**Mara:** That sounds good. I've used SQL Server Data Tools to create a database project. I will add a database project to our *Tailspin.SpaceGame.Web* solution. The project will have the SQL scripts we need to create the tables. We can use it to make sure that changes to the database schema in the project are checked against the current database schema.
