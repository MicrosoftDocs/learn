...
**Tim**: It's great that we're able to identify and fix issues so early in the process. I don't know how I would've found this information on my production systems. I'll take this to our security consultant. Perhaps we can all work together to identify and fix the most important issues.
...
A relatively easy bug to fix relates to replacing bold `<b>` tag with the `<strong>` tag, as you saw earlier.

![Bug details on SonarCloud](../media/3-sonar-scan-bug.png)

See if you can locate this issue in the source code based on the report. Once you fix the issue and commit the change to the `master` branch on GitHub, you'll see the issue disappear the next time the scanner runs.
