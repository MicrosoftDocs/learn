...
1. From the **Artifacts explorer**, expand the **drop** folder.
    ...
    As an optional exercise, you can download this .zip file to your computer and explore its contents. To do so, select the .zip file, click the ellipsis (**...**), and then click **Download**.
...
Just like with other programming languages, variables enable you to do things like:
...
1. From Visual Studio Code, modify **azure-pipelines.yml** like this.
    ...
    * `buildConfiguration` &mdash; this specifies the build configuration
    * `wwwrootDir` &mdash; this specifies the path to the **wwwroot** directory.
    * `dotnetSdkVersion` &mdash; this specifies the .NET Core SDK version to use.
