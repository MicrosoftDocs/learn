In this part, you'll learn about packaging code to make it easier to share. You'll discover why you should make packages, what kinds of packages you can create, where you can host the packages, and how you access them once they are hosted. You'll also learn about package versioning as well as Azure Artifacts.

But first, let's check in and see how the team is doing. Andy has called the team together to talk about a potential change to their code to help out another team.

**Andy**: Hi everyone. I was chatting with the team who's working on the back end system for _Space Game_. They could use the models we use for the web site in a back end application they plan to write.

**Amita**: What do you mean by models?

**Andy**: As you know, the _Space Game_ web site is an ASP.NET Core application. It uses the Model-View-Controller, or MVC, pattern to separate data from how that data is displayed in the user interface. I was thinking we could create a package that contains our model classes so that any application can use them.
...
**Tim**: I'd love to help, but first I have some questions. I'm new to this and I want to understand how this all works.

## What is a package?

A _package_ contains reusable code that you or other developers make available to others to use in their projects.
...
Either way, packages are typically compressed to ZIP or a similar format. Package systems will often define a unique file extension such as .nupkg or .jar to make its use clear. Compression can help reduce download time and also provides a single file to manage.
...
Tim asks, "Why not simply duplicate the code in both projects?"

One reason to create a package instead of duplicating code is to prevent _drift_. When code is duplicated, each copy can quickly diverge to satisfy the needs of each application. It becomes difficult to migrate changes from one copy to the others. In other words, you lose the ability to improve the code in ways that benefits everyone.
...
One tradeoff is that with a package, you have another codebase to test and maintain. Another tradeoff is that you must be careful when adding features. Generally speaking, a package should contain features that benefit many kinds of applications. For example, [Json.NET](https://www.newtonsoft.com/json?azure-portal=true) is an extremely popular NuGet package for .NET that enables you to work with JSON files. Json.NET is open source, so the community can propose improvements and report issues.

However, when multiple applications can benefit from the same code, these tradeoffs pay off because you have just one codebase, set of tests, and build process to manage.
...
* **RubyGems**: packages for Ruby libraries

## Where are packages hosted?

You can host packages on your own network or use a hosting service. A hosting service is often called a _package repository_ or _package registry_. Many of these services provide free hosting for open source projects.

Building on the previous example, here are popular hosting services for a few popular kinds of packages.
...
A _package feed_ refers to your package repository server. This server can be on the internet or behind your firewall, on your network. For example, you can [host your own NuGet feeds](https://docs.microsoft.com/nuget/hosting-packages/overview?azure-portal=true) using hosting products such as Azure Artifacts and MyGet. You can also host packages on a file share.

When you host packages behind the firewall, you can include feeds to your own packages. You can also cache packages that you trust on your network when your systems cannot connect to the internet.
...
Semantic Versioning is a popular versioning scheme. NuGet packages follow Semantic Versioning. Here's the format:
...
Here is an example of the package dependencies from our Tailspin-SpaceGame-Web project file. In this project, we depend on packages like Newtonsoft.Json, and you can see that we specify version 12.0.1 of that package.
...
**Mara:** It seems to me it would make sense for us to host the new Models package in Azure Artifacts. We are all part of the Azure DevOps organization already so authentication would be easier than trying to set it all up on a different package manager.

**Andy:** I looked into that before the meeting and it seems straight forward to me. I agree with Mara.

**Amita:** What is Azure Artifacts?

**Andy:** Azure Artifacts is a repository in your Azure DevOps organization where you can manage the dependencies for your codebase. Azure Artifacts can store your artifacts and your binaries. It provides a container for groups of dependencies called a *feed*. This allows developers that have access to the feed to easily consume or publish packages.

## How do I create a package and use it in the pipeline?

**Tim:** So if I am understanding right, the application code uses packages from Nuget already. We are going to create our own package and host it in Azure Artifacts. Can you draw out the pieces and how they will work together? I'm having a hard time picturing the whole process.
...
Our application might want to use this newer version of the package. In that case, we update the application to reference the newer version and run the tests locally to make sure this new version works with our application. Once we are satisfied that all is working well,  we submit the application change to the pipeline and it will build with the new version of the package dependency.
