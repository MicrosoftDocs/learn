In this section, you follow along with Tim and Mara as they explore load testing and add load tests to the pipeline. The load tests use Apache JMeter. It simulates many users who are accessing the web app simultaneously. The tests will fetch the web content from the app running on Azure App Service, in the _Staging_ environment.

Tim starts by bringing up the Apache JMeter user interface on his laptop and running a basic test plan. Then, he and Mara export the test plan to a file that can be run from the command line. Finally, they add tasks to Azure Pipelines that run the load tests during _Staging_.
...
Tim brings up Apache JMeter on his laptop, which runs Linux. Here's what it looks like.

![The Apache JMeter user interface](../media/4-apache-jmeter-new.png)

He then creates a new test plan file, named *LoadTest.jmx*. To it, he adds a **Thread Group**. Each simulated user runs on its own thread; a thread group controls the number of users and the number of requests each user makes.

This example shows 10 simulated users (threads); each user makes 10 requests. This makes a total of 100 requests.
...
Next, Tim adds **Http Request Defaults** and an **Http Request** sampler to the thread group. He provides the host name of the _Space Game_ website that's running in their **staging** environment on Azure App Service.
...
**Tim:** This is a pretty basic test plan. There are many other options we can explore later. I think 100 requests is a good place to start in terms of getting meaningful results.

### Run the test plan

**Tim:** JMeter enables you to run many different kinds of tests. One possibility is to run your test plan from the JMeter graphical user interface. But for load tests, the JMeter documentation recommends that you run the test plan from the command line.
...
Before we do that, one part remains. We can visualize the results if we provide them in a format Azure Pipelines understands. Azure Pipelines can parse an XML file that contains test results, but the file needs to be in a supported format. Those include CTest, JUnit (including PHPUnit), NUnit 2, NUnit 3, Visual Studio Test (TRX), and xUnit 2. I bet we can write an XSLT that converts the JMeter results to JUnit.

**Tim:** Let's give it a try.

## Transform the report to JUnit

XSLT stands for _XSL Transformations_, or _eXtensible Stylesheet Language Transformations_. An XSLT file resembles an XML file, but it enables you to transform an XML document to another XML format. Here, Mara and Tim write an XSLT that converts JMeter output to JUnit.
...
The XSLT that Mara and Tim create checks these requirements. Here's what it looks like:
...
* Each HTTP request time.
    ...
    It does this by selecting the `t` attribute from each `httpSample` element. (`./httpSample/@t`)
* Each failure message.

    It does this by selecting all `failureMessage` nodes from the document. (`./httpSample/assertionResult/failureMessage`)

The XSLT also sets the threshold value to 1,000 ms. This is the maximum response time that Tim defined earlier.
...
1. The maximum response time, and a failure if more than 10% of requests exceed the threshold of 1,000 ms.
...
**Mara:** The tests succeeded! The test results stay in the pipeline; they don't go in source control. But your test plan does go in source control. And because it's a plain text file and not a binary file, we can examine the Git history to track how the test plan changes over time.
