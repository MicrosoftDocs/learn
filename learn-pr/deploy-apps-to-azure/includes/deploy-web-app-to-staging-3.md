To use CI/CD best practices, we should change our workflow so that the build definition deploys to a staging slot rather than to production. Create a staging slot for your web app in the Azure portal:
...
6. After the deployment slot has been created, select it from the list of available slots. The overview for the deployment slot is shown, including the URL. The deployment slot is added to name of your web app, such as *https://mywebapp-staging.azurewebsites.net*:
