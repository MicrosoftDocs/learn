...
In unit 2 we've seen that there are plenty of problems that can be formulated as a search problem. Now that we've familiar with Grover's search algorithm and its implementation, let's revisit the question: are all search problems going to benefit from the quantum speedup offered by this algorithm?

It turns out that this is not the case. We've seen that Grover's algorithm doesn't rely on the internal structure of the function $f$. However, when you try to solve a problem, you try to approach it from different angles, including analyzing the problem structure and looking for the ways to exploit it to speed up your solution. It turns out that search problems often have a well-defined problem structure that can be exploited to get a classical algorithm that runs much faster. 
...
This gives us the first part of the answer: the problems best poised to benefit from Grover's algorithm are the ones for which we can easily verify whether the given $x$ is the answer, but we cannot easily exploit problem structure to find $x$.
