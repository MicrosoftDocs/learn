...
This observation gives us the first part of the answer: the problems that are best poised to benefit from Grover's algorithm are the ones for which we can easily verify whether the given $x$ is the answer, but we cannot easily exploit problem structure to find $x$.
