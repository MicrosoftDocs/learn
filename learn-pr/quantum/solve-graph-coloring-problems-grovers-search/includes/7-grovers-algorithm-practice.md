In unit 2, you learned that there are plenty of problems that can be formulated as a search problem, and therefore be solved using Grover's algorithm. Now that you're familiar with Grover's search algorithm and its implementation, let's revisit the question. Can all search problems benefit from the quantum speedup offered by Grover's algorithm? 
