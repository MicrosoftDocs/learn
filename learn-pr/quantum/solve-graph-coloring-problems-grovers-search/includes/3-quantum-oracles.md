...
## Performing computations on a superposition state
...
This will convert a basis state $|x\rangle|y\rangle$ into $|x\rangle|y \oplus f(x)\rangle$ ($\oplus$ is addition modulo 2). 
...
* A lot of quantum algorithms call for using the first approach, encoding the classical function values in phases of basis states, since this simplifies expressing the algorithm. 
* On the other hand, the second approach, encoding the classical function values in the states of extra qubits, makes implementing the classical computations easier.
...
A quantum oracle is a "black box" operation which is used as input to another algorithm (in this case Grover's search algorithm, but the term is broadly used [in classical computing](https://en.wikipedia.org/wiki/Oracle_machine) as well). 
...
For such algorithms, their runtime analysis is done in terms of the number of oracle calls (i.e., function evaluations), rather than in terms of primitive operations the algorithm will do.
