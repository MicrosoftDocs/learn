...
In the previous units, you learned that how quantum entanglement works. can be an excellent resource for quantum communication. In this unit, you learn how to use entanglement as a resource for qunatum communication through one of the most famous applications of entanglement: the quantum teleportation protocol.

Quantum teleportation uses entanglement to transfer the state of a qubit from one location to another. The state of one qubit is transferred to another qubit, but the qubit itself doesn't physically move. This is an important thing to remember! The information of the state of a qubit is transferred to another qubit, which is used as a vessel to write the information of the message qubit on.
...
Alice and Bob work together in the same company. Alice is based in Seattle, and Bob is based in Los Angeles. They are working on a project that requires them to share quantum information. They decide to use quantum teleportation to send information between them.
...
Alice also has another qubit, called the message qubit, that she wants to send to Bob by teleportation. The state of the message qubit is unknown to both Alice and Bob. The unknown qubit state is

$$\ket{m}=\alpha \ket{0}_m + \beta \ket{1}_m$$

where $\alpha$ and $\beta$ are complex numbers, and the subscript $m$ denotes that this is the messenger qubit.

The whole system consists of the entangled qubits, $A$ and $B$, and the the message qubit, $m$. The state of the three-qubit system is
...
The message qubit is in the unknown state $\alpha \ket{0}_m + \beta \ket{1}_m$, so after Alice applies the CNOT gate, her qubits are in a superposition of the four Bell states. The overall state of the three qubits is
...
When Alice measures the message qubit and her qubit $A$ in the Bell basis, she projects her qubits into one of the four Bell states. Because Alice's qubits are entangled with Bob's qubit, Alice's measurement results are correlated with Bob's. When Alice measures her qubits, Bob's qubit is also projected into the correlated state.
...
Because Bob's qubit, $B$, is entangled with Alice's qubit, $A$, Bob now knows the state of his own qubit without the need to measure it. However, Bob wants his qubit to be in the same state as the message qubit that Alice wants to teleport. Remember that at this point, neither Alice nor Bob know the state of the message qubit.

### Bob applies quantum operations
...
The quantum operations Bob applies can be a Pauli $X$ gate, a Pauli $Z$ gate, both gates, or no gates.
...
> [!IMPORTANT]
> Applying a gate to a qubit isn't the same as measuring a qubit. When Bob applies a gate, he doesn't measure his qubit. But he does affect the state of the qubit.
