...
> [!NOTE]
> The symbol :::no-loc text="$\\otimes$"::: means that we take the tensor product of the kets on the left with the kets on the right.

### Alice entangles her qubits

Alice uses a CNOT gate to entangle the message qubit, :::no-loc text="$m$":::, with her other qubit, :::no-loc text="$A$":::. The message qubit is the control qubit, and Alice's other qubit is the target qubit. This creates a three-qubit entangled state.
...
The message qubit is in the unknown state :::no-loc text="$\\alpha \\ket{0}_m + \\beta \\ket{1}_m$":::, so after Alice applies the CNOT gate, her qubits are in a superposition of the four Bell states. Here's the overall state of the three qubits:
...
Alice then measures the message qubit and her other qubit, :::no-loc text="$A$":::. She measures her qubits in the Bell basis, which consists of the four Bell states, :::no-loc text="$\\lbrace \\ket{\\phi^+}, \\ket{\\phi^-}, \\ket{\\psi^+}, \\ket{\\psi^-} \\rbrace$":::.

When Alice measures the message qubit and her qubit :::no-loc text="$A$"::: in the Bell basis, her qubits are found to be in one of the four Bell states, each with equal probability. Because Alice's qubits are entangled with Bob's qubit, Alice's measurement results are correlated with Bob's. When Alice measures her qubits, Bob's qubit is also projected into the correlated state.

For example, if Alice measures her qubits and observes the state :::no-loc text="$\\ket{\\phi^-}$":::, then Bob's qubit is projected into the state :::no-loc text="$\\alpha\\ket{0}_B - \\beta\\ket{1}_B$":::.
...
Because Bob's qubit, :::no-loc text="$B$":::, is entangled with Alice's qubit, :::no-loc text="$A$":::, Bob now knows the state of his own qubit without the need to measure it. However, Bob wants his qubit to be in the same state as the message qubit that Alice wants to teleport. Remember that neither Alice nor Bob knows the state of the message qubit.

### Bob applies the necessary quantum operations

Next, Bob applies quantum operations to his qubit, :::no-loc text="$B$":::, so that it has the same state as the message qubit. The operations that Bob applies depend on what Alice told him by phone.
...
The quantum operations that Bob applies are either a Pauli :::no-loc text="$X$"::: gate, a Pauli :::no-loc text="$Z$"::: gate, both gates, or no gates.

For example, if the result of Alice's measurement is :::no-loc text="$\\ket{\\phi^-}$":::, then Bob knows that his qubit is in the state :::no-loc text="$(\\alpha \\ket{0}_B - \\beta \\ket{1}_B)$":::. He needs to apply only a Pauli :::no-loc text="$Z$"::: gate to recover the original state of the message qubit. Here are the operations that Bob needs to apply to his qubit for each measurement result that Alice sends him:

| Alice measures | Bob applies                           |
|----------------|---------------------------------------|
| :::no-loc text="$\\ket{\\phi^+}$"::: | No operation                          |
| :::no-loc text="$\\ket{\\phi^-}$"::: | Pauli Z gate                          |
| :::no-loc text="$\\ket{\\psi^+}$"::: | Pauli X gate                          |
| :::no-loc text="$\\ket{\\psi^-}$"::: | Pauli X gate followed by Pauli Z gate |
