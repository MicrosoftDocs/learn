...
1. Modify *Program.qs* like the following example, and then save the file:
    ...
    Here, you use a `for` loop to act on each qubit sequentially. Q# has classical flow control capabilities. For more information about Q# flow control statements, see the [QDK documentation](https://docs.microsoft.com/azure/quantum/user-guide/libraries/standard/control-flow).
