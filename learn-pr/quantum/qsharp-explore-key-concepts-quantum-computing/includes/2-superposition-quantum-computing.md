...
A handy notation to describe quantum states is the Dirac bra-ket notation. In this notation, we describe the possible states of quantum systems by using kets: $\ket{ }$.

For example, $\ket{0}$ and $\ket{1}$ are two possible states of a qubit. A qubit in the state $\ket{\psi} = \ket{0}$ means that the probability of observing `Zero` when we measure the qubit is 100 percent. Similarly, a qubit in the state $\ket{\psi} =\ket{1}$ produces `One` after we measure it.
