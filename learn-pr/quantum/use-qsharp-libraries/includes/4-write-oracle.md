...
This problem can be easily solved by a classical computer (not to mention by hand), but let's assume that it can't be. Instead, consider how we might solve it with a quantum computer.
...
As mentioned earlier, we can straightforwardly bring the target register from $|0\rangle$ to the number state $|9\rangle$ by using `ApplyXorInPlace`. 
Then, the only step left is to perform the mapping by using `MultiplyAndAddByModularInteger`.
