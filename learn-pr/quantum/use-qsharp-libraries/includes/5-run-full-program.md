...
However, since we're only interested in the number states $0 - 9$, we have no use for the remaining states $10-15$ which our 4 qubits can represent.
Therefore, we'll only utilize and reflect about the uniform superposition $\sum_{i=0}^9 |i \rangle$.
...
You can think of this as analogous to the previous module's use of `ApplyToEachA(H, register)` to do prepare the uniform superposition over all possible number states.
...
Since we only consider the 10 possible digits as our search space, we'll provide this an argument of 10.
...
Although it typically returns a correct answer with a very high probability, there's often a finite chance that the measurement results in an incorrect solution. 
...
Next, you'll need to have access to all the operations, so be sure to include the `open` statements (we include the corresponding operations they provide in case you were curious): 
...
In case other spacefarers may need this same code, it's valuable to document it properly.
In the next unit you'll add your own API documentation to some of these operations. 
