...
:::image type="content" source="../media/2-data-storage-options.svg" alt-text="The common local storage options available to a .NET MAUI app.":::
...
- The **Library** folder. This folder is returned by the `FileSystem.AppDataDirectory` property, as described above.Use the **Library** folder when you're storing data that's app-generated.

- The **Documents** folder. The code snippet below shows how to reference this folder in the **docFolder** variable. Use the **Documents** folder to store user-generated data only. (This data is created in direct response to a user action). For example, if you were creating a text-editing application such as Microsoft Word, you would store the document in the Documents folder.
