...
In this unit, you'll compare using XAML with defining the UI layout using C# code. You'll also learn about some of the benefits of using XAML as a markup language to define your UI.
...
For example, you can use Hypertext Markup Language (HTML) to create a webpage that you can display in a web browser. You don't need to understand all the tags we use in the following example; what's important to see is that this code describes a document that has the text "Hello World!" as its content.
...
> [!NOTE]
> The `InitializeComponent` method in the page constructor reads the page's XAML description, loads that page's various controls, and sets their properties. You only call this method if you define a page using XAML markup. The previous example showing how to create the UI using C# code doesn't invoke `InitializeComponent`.

This structure allows for the separation of design and behavior. The UI's entire declaration is contained in a single dedicated source file, which is separate from the UI behavior. Additionally, the XAML markup provides greater clarity for a developer trying to understand the application's look and feel.
...
This approach also allows a specialist UI designer to work on updating the UI's look and feel by using XAML editing tools separately from a developer updating the UI logic.
