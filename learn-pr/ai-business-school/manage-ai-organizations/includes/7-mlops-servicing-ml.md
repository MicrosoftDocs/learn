...
If the model is a new version of a deployed model, testing ensures that it performs better than the existing model for key metrics. After initial tests, we recommend doing A/B testing. This involves deploying the model to only a small percentage of traffic and comparing to determine whether the new model performs better. If the model fails, it's sent back to training to be reworked. If it performs well, it continues to deployment.  

It's critical to keep a "human in the loop." This means maintaining human oversight of the retraining and outcomes of the new model. If model retraining is entirely automated, bias can be introduced over time or outcomes can shift. It's also necessary to validate the model again. Test it against the business use case and determine whether it still produces valuable outcomes.
