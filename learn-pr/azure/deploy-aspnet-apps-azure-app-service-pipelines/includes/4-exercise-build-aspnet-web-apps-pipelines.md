...
> [!NOTE]
> You'll need a valid email address, such as one associated with a Microsoft LiveID user account.
...
1. On the **Create your account** page, specify an email address, a password you want to associate with your new GitHub account, and any additional information necessary to complete this step, and select **Create account**.
...
1. If you're using a Microsoft account, on **Link your GitHub credentials to your Microsoft account**, select **Next**. When prompted, enter the code included in another email delivered to your email account, select **Sign in**, and, to complete the setup, select **Got it**.
   ...
   > [!NOTE]
   > If you're prompted to set up your Azure DevOps organization details, verify your name, email address, and location, and then select **Continue**.
...
Next, you'll create your own repository from a GitHub template repository, containing the sample ASP.NET app, into your GitHub account and then generate an Azure DevOps build pipeline using it as its source.
...
1. Switch to the browser tab displaying the **Deploy ASP.NET web apps with Azure Pipelines** Azure DevOps project, in the vertical menu bar along the left edge of the project page, select the **Pipelines** icon and select **Create Pipeline** on the **Create your first Pipeline** pane.
1. On the **Where is your code?** pane, select **GitHub YAML**.

   :::image type="content" source="../media/4-where-is-your-code-github-yaml.png" alt-text="Screenshot of the New pipeline pane displaying the option to use the GitHub repository as the source for a YAML pipeline.":::
...
On the **Review your pipeline YAML** pane, scroll down to the **Steps:** section and review the list of auto-generated tasks by hovering with the mouse pointer over task headers to identify the purpose of each task:
...
1. Because you won't be running tests in this exercise, scroll down to the bottom of the pipeline and remove the entire **VSTest@2** task section:
   ...
   > [!NOTE]
   > Next, you'll add a step that will publish artifacts generated by the build.
...
1. On the **Review your pipeline YAML** pane, in the toolbar, select **Save & run** and select **Save and run** on the **Save and run** pane.
1. On the pipeline run pane, in the **Jobs** section, select **Job**, then track the execution of the individual tasks in the newly created pipeline.
1. Verify that all tasks completed successfully.

:::image type="content" source="../media/4-completed-build-job.png" alt-text="Screenshot of the Azure DevOps Pipeline job execution logs displaying a successfully completed job.":::
