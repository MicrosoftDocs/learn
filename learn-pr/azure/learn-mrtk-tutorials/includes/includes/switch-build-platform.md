## [Unity 2020 + openxr](#tab/openxr)

1. In the Unity menu, select **File** > **Build Settings...** to open Build Settings.

    :::image type="content" source="../../media/new-project-progress.png" alt-text="Screenshot of Unity Build Settings... menu path." lightbox="../../media/new-project-progress.png":::

1. In  Build Settings, select **Universal Windows Platform** and complete these settings:

    * Set **Target device** to *HoloLens*
    * Set **Architecture** to *ARM 64*
    * Set **Build Type** to**D3D Project*
    * Set **Target SDK Version** to *Latest Installed*
    * Set **Minimum Platform Version** to *10.0.10240.0*
    * Set **Visual Studio Version** to *Latest installed*
    * Set **Build and Run on** to **USB Device**
    * Set **Build configuration** to *Release* (there are known performance issues with Debug)
    
 1. Select **Switch Platform**.

    :::image type="content" source="../../media/universal-windows-platform-settings-switch.png" alt-text="Screenshot of Unity Build Settings with Universal Windows Platform settings." lightbox="../../media/universal-windows-platform-settings-switch.png":::

1. When Unity has finished switching the platform, select **X** to close Build Settings:

    :::image type="content" source="../../media/close-icon.png" alt-text="Screenshot of Unity Build Window with close icon highlighted." lightbox="../../media/close-icon.png":::

## [Unity 2019/2020 + windows xr plugin](#tab/winxr)

1. In the Unity menu, select **File** > **Build Settings...** to open Build Settings:

    :::image type="content" source="../../media/unity-build-settings.png" alt-text="Screenshot of Unity Build Settings... menu path." lightbox="../../media/unity-build-settings.png":::

2. In Build Settings, select **Universal Windows Platform**, and then select **Switch Platform**.

    :::image type="content" source="../../media/switch-platform.png" alt-text="Screenshot of Unity Build Settings with UWP selected to switch platform from Standalone." lightbox="../../media/switch-platform.png":::

3. When Unity has finished switching the platform, select **X** to close Build Settings.

    :::image type="content" source="../../media/close-icon.png" alt-text="Screenshot of Unity Build page with close icon highlighted." lightbox="../../media/close-icon.png":::

## [Legacy wsa](#tab/wsa)

1. In the Unity menu, select **File** > **Build Settings...** to open Build Settings.

    :::image type="content" source="../../media/unity-build-settings.png" alt-text="Screenshot of Unity Build Settings... menu path." lightbox="../../media/unity-build-settings.png":::

1. In Build Settings, select **Universal Windows Platform**, and then select **Switch Platform**.

    :::image type="content" source="../../media/switch-platform.png" alt-text="Screenshot of Unity Build Settings window with UWP selected to switch platform from Standalone." lightbox="../../media/switch-platform.png":::

1. When Unity has finished switching the platform, select **X** to close Build Settings.

    :::image type="content" source="../../media/close-icon.png" alt-text="Screenshot of Unity Build Settings with close icon highlighted." lightbox="../../media/close-icon.png":::
  
