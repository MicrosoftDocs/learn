SProbably you're more than curious how a deployment of a Java application in Azure can look like! No worries! We got you.
...
Before you decide which Service is the best for you to use, have a look at the following flowchart. It should be simple to follow. It can help you decide if using for example an Azure App Service is the best solution for your application code.
...
As you as a Java developer are used to, you'll find all the relevant code in the src folder. It's following the Spring Boots architecture with Object, Controller and Repository classes and they are separated by functionality.
...
That example is using Java 8. So our App Service, Linux or Windows should use Java 8 as well. As the platform does not matter, we will use Linux for our example.

Regardless of the Java version. We have to add some Azure dependencies to the `pom.xml` file. That can be done automatically for us by running

```bash
mvn com.microsoft.azure:azure-webapp-maven-plugin:1.11.0:config
```

Running that command will ask for some options to be specified. Those will be stored automatically in our `pom.xml`.
...
The `pom.xml` file has now added the required plugin for the automatic maven based deployment to Azure.
...
Great! We are already prepared for the deployment to Azure App Service.

```bash
mvn package azure-webapp:deploy
```

This command will run the tests and after that automatically spin up an Azure App Service on Azure and deploy the packaged application.
