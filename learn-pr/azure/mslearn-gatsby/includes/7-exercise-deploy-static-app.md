...
Before we can deploy our app we first need to make it into a Git repository and push it to GitHub. That can easily be done through the following steps.
...
This will initialize a GitHub repo.
...
Please replace the `<user>` above with your GitHub user name. Next, run the `push` command like so:
...
- **Resource group**, all things provisioned needs to belong to a logical bucket, a resource group. Either create one or select an existing
- **Name**, this is name your app will get
- **Region**, this is what Azure region you want to deploy the app on
- **SKU**, SKU stands for Stock Keeping Unit. It's a pricing tier that indicates how much you will be charged
- **GitHub**, we need to authorize Static apps service as a 3rd party application to read this information from GitHub. Once authorized we are able to select the specific GitHub repository we want to use as the template.

Now that we filled in all the needed fields. Click `Sign in with GitHub`. This will create an area below we you are asked to select
...
The important field to fill in is `App artifact location`. This is the location of the folder containing our static assets. In the case of Gatsby this is the `/build` folder. We don't need to do anything more in this screen as The Static apps service will call the `build` command in our `package.json` that carries out `gatsby build`. Next we click `Review + create` and we should see the following image:

![Deploy - review](../media/gatsby-review.png)

The above image is an overview of all the choices made. If you are happy with all the selections made then click `Create`. This will start to provision your Static app. Once done you will come to a portal screen looking like so:
