...
The scaffolded Gatsby project consists of some parts that you need to understand to work with Gatsby effectively and efficiently.
...
This component creation will lead to a route being created at `/hello` and it will render the component content, in this case the string `Hello` between two div tags.
...
For this query, we will use data stored in `gatsby-config.js`. Opening this file up, we can see that the first piece of information available is the following JavaScript:
...
There you will find a view looking like the following image below:

TODO GraphQL view image
...
Note how the `data` input parameters are populated by Gatsby ad how we in our rendering section just need to focus on frilling down in our data to find the data we need `data.site.siteMetadata.title`.
