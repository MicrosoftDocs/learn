This exercise provides an overview of setting up the collection, processing, and storage of IoT telemetry generated, by using an Azure IoT Central application. For more in-depth knowledge and tutorials regarding these topics, refer to:
...
1. Start a web browser, navigate to the [Welcome to IoT Central](https://apps.azureiotcentral.com/?azure-portal=true) page.
...
1. On the **New application** page, configure the following settings, and then select **Create**.
   ...
   | Application URL | Enter a unique name consisting of lower-case letters, digits and/or dashes, starting with a letter. |
...
In this task, you'll create an Azure Function app that implements the HTTP webhook functionality. You'll leverage this functionality to forward the telemetry data from the Azure IoT Central application by creating an application rule.
...
1. On the **Basics** tab of the **Create Function App** blade, configure the following settings, and then select **Next: Hosting >**:
   ...
   | Function App name | Enter a unique name that consists of lower-case letters, digits, and/or dashes, and starts with a letter. |
   | Publish | Select **Code**. |
...
1. Back on the **HttpTrigger1 \| Integration** blade, in the vertical menu bar, in the **Developer** section, select **Code + Test**.
1. On the **HttpTrigger1 \| Code + Test** blade, replace the existing code with the following content, and THEN select **Save** to save your changes:
   ...
   :::image type="content" source="../media/6-azure-function-app-function-code-test.png" alt-text="Screenshot of the Add function blade of the Azure Function app in the Azure portal":::

   > [!NOTE]
   > The function logs the JSON-based payload of the HTTP request and constructs individual JSON elements to be stored in an individual document of the target Cosmos DB collection corresponding to the function output you configured. The **pk** element, which consists of the concatenated deviceId property and date of the collected telemetry data point, could be used as the partition key.

1. On the **HttpTrigger1 \| Code + Test** blade, select **Get function URL** and record its value.

   :::image type="content" source="../media/6-azure-function-app-function-url.png" alt-text="Screenshot of the HttpTrigger1 | Code + Test blade of the Azure Function app in the Azure portal, with the Get function URL pop-up window.":::
...
1. On the **Rules** page, select **+ New** and configure the following settings:
   ...
   > [!NOTE]
   > The value of the temperature telemetry is intentionally artificially low to invoke the webhook functionality and trigger the function. In real-life scenarios, you'd adjust it to correspond to the condition that warrants the collection of data that you intend to store in the target Cosmos DB collection.
...
1. In the vertical menu, in the **Developer** section, select the **Monitor** entry, and then, on the **HttpTrigger1 \| Code + Test** blade, select the **Logs** tab.
1. Verify that the logs pane displays the messages generated in response to the HTTP trigger originating from the Azure IoT Central application webhook you configured earlier in this task.
   ...
   :::image type="content" source="../media/6-azure-function-app-function-monitor-log.png" alt-text="Screenshot of the function logs containing entries representing webhook calls generated by devices registered with Azure IoT Central IoT Central application.":::
