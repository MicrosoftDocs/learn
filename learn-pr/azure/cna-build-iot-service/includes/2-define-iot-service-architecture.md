...
- Azure Data Lake Storage. This distributed data store can process large amounts of data without requiring its transformation or schema definition. Its extensive processing capabilities, available through integration with Azure Data Lake Analytics, make it a suitable choice for big data analytics scenarios.
- Azure SQL Database. This managed relational data store is optimized for scenarios that require transactional support and enforceable schema constraints, and involve complex queries.
- Azure Cosmos DB. This cloud-native NoSQL database offers features such as multiple-region writes, autoscaling, support for multiple consistency models, schema-on-read, and latency guarantees for read and write operations.
...
- Azure Synapse. This managed analytics service is optimized for massive parallel processing workloads. Azure Synapse typically hosts aggregated data and acts as the data source for business intelligence (BI) tools.
