Azure Blob Storage lets you serve files to your users in a fast and reliable way, which can be a key resource to having a responsive website that your users love. You can use Blob Storage to cover your applicatin's static file needs, whether that's uploading images, or serving images to the user. While serving files is the easy part, uploading images requires to grant access to your users to your Blob Storage Account. Of course Azure offers a safe and secure solution to do just that. In this module, we are going to explore image uploading in a secure way. Imagine the following scenario.
