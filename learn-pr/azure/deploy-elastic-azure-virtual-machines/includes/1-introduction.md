Among Elastic on Azure deployment options, Elastic on Azure VMs provides users the most control over the underlying Azure infrastructure. Teams with established orchestration practices around Azure IaaS tend to choose this option. In cases where customers want to migrate on-premises Elasticsearch on Linux deployments to Azure, they may pick this option to minimize re-architecture. 
...
Through the course of this module, we will discuss the steps and considerations for migrating an Elastic workload to an Azure VM. You will need to set up and use your own pay-as-you-go Azure subscription to participate in the exercises, which may incur a nominal fee. To keep costs low, you can either sign up for a free trial or set the VM to the smallest size and be sure to take it down when the exercise is over.
