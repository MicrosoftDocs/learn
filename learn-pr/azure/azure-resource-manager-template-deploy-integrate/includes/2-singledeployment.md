Whether you are deploying your templates in your test environment or in production, the ability to deploy from the Azure Cloud Shell or the command line is crucial. This unit covers the following ways to deploy your templates:

* Azure PowerShell
* Azure CLI
* GitHub Actions

There will be times where you need to deploy from different locations, for example:
...
When you deploy a template, you need to specify a resource group that contains the resources. Before running the deployment command, create the resource group with either Azure CLI or Azure PowerShell.
...
In the majority of cases, you target your deployment to a resource group (which will be the focus of this module). However, when you need to apply policies or role assignments across a larger scope, you can target your deployment to subscription, management group, or tenant deployments.
