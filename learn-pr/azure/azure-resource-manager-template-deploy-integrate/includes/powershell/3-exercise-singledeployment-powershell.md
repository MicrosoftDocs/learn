...
1. To get the subscription ID. The command will list your subscriptions and their IDs in an easy to read table format. The subscription ID is the second column. Look for Concierge Subscription and copy the second column. It will look something like *cf49fbbc-217c-4eb6-9eb5-a6a6c68295a0*
...
To pass inline parameters to your deployment, you need to provide the names of the parameter with the `New-AzResourceGroupDeployment` cmdlet. In the next exercise, you will pass the parameters as `Inline` parameters and as a parameter file.

1. Using the same template as the last exercise, you will construct a hashtable that will contain the values for the required template parameters.
