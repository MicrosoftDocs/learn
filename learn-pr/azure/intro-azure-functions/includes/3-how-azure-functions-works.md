...
Azure Functions is a serverless compute solution that allows you to write less code, and because it’s a platform a PaaS you don’t have to worry about hosting your app, and you save on costs.
...
In the Consumption and Premium plans, Functions scales CPU and memory resources by adding additional function app instances. The number of instances is determined based on the number of events that trigger a function. All functions within a function app share resource within an instance and scale at the same time.
...
| Function runtime           | Azure Functions currently supports several versions of the runtime host. Functions also supports many different runtimes such as .NET Core, Node.js, Java, PowerShell and Python. During development, you can install the Functions runtime on your local computer and run functions locally by using the Azure Functions Core Tools.                                             |
