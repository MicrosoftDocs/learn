...
- **Scheduled tasks**. That could use the same time-based triggers. In this case, there could be jobs that needs to be done at certain intervals like data cleaning for example or look for a piece of data.
...
- **Analyse IoT stream**. It’s possible to collect and process data from IoT devices.
...
There are many more cases where Azure Functions makes sense to use but hopefully the above can serve as inspiration how you can innovate in the cloud when applying an event-based reasoning and architecture.
