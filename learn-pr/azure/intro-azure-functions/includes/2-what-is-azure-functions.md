Azure Functions is a cloud-based compute service. Functions provides event-driven and scalable serverless compute for Azure. In some cases, you’re only billed for the time your function code runs.
...
- Events: EventGrid and Event Hubs produces events that can trigger your code.
- HTTP, code can be triggered by Web requests as well as Web Hooks
...
Bindings are ways to simplify coding for input and output data. While you can use client SDKs to connect to services from your function code, Functions provides bindings to simplify these connections. Essentially bindings are connection code you don’t have to write. You can integrate with many services on Azure and solve integration problems and automate business processes.

Bindings comes in two flavours, input, and output. An output binding provides a way to write data to the data source to for example place a message on a queue or a new row in a database. Input bindings can be used to pass data to your function from a data source different than the one that triggered the function.
