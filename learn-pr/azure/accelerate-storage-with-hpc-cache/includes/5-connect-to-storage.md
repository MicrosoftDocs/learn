After the Azure HPC Cache instance is created, it's time to point it to your data. Ideally there's an ExpressRoute or high-speed connection between Azure and the data.

From the Azure portal, select the HPC Cache resource. In the left menu, choose **Storage Targets**.

## Storage targets

An HPC Cache “storage target” is the storage location where your HPC data is kept, like a NetApp or EMC Isilon appliance.

1. Select **Add Storage Target** in the upper-left corner.
...
1. In the new pane, provide a name for your storage target. This name doesn't have the strict requirements that other fields have and can include spaces.
...
The usage model names should help you choose the best option. In general, the options at the top provide the better performance, but might not have the latest data. Choose the usage model based on how frequently the source data changes.
