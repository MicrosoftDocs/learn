...
C# & .NET is popular choice for building server-side logic but client-side apps are often built using web UI frameworks, commonly using JavaScript. Using multiple languages and toolsets requires multiple skillsets, and often requires hiring two separate teams. As well, code to transfer and represent data needs to be built in both languages and kept in sync.
...
Blazor WebAssembly can also be configured as Progressive Web Applications. This allows your app to be used offline.
