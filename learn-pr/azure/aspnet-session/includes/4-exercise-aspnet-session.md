...
1. In the **Solution Explorer** window, right-click the **eShopLegacyWebForms** project, then select **Manage NuGet Packages**.
    ...
    > ![Screenshot of the shortcut menu for the eShopLegacyWebForms project in the Solution Explorer window with Manage NuGet Packages selected.](..\media\4-manage-nuget.png)
...
1. In the **NuGet Package Manager** window, select **Browse**. In the **Search** box, enter **Microsoft.Web.RedisSessionStateProvider**. Select the **Microsoft.Web.RedisSessionStateProvider** package and  select **Install**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Screenshot of the NuGet Package Manager window installing the Microsoft.Web.RedisSessionStateProvider package.](..\media\4-install-package.png)

1. If the **Preview Changes** window appears, select **OK**.

    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Preview Changes window with OK selected.](..\media\4-preview-changes.png)

1. In the **License Acceptance** window, select **I Accept**.

    > [!div class="mx-imgBorder"]
    > ![Screenshot of the License Acceptance window with I Accept selected.](..\media\4-license.png)
...
1. In the **Solution Explorer** window, expand the **Models** folder, expand the **Infrastructure** folder, and then select the **dbo.catalog_brand_hilo.Sequence.sql** file.
    ...
    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Solution Explorer window with the dbo.catalog_brand_hilo.Sequence.sql file selected.](..\media\4-edit-sql-file.png)
...
1. If you aren't currently signed in to your Azure account in Visual Studio, in the upper right of the toolbar, select **Sign in**. Follow the instructions and sign in.
    ...
    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Visual Studio toolbar with Sign in selected.](..\media\4-toolbar-sign-in.png)

1. In the **Solution Explorer** window, right-click the **eShopLegacyWebForms** project, then select **Publish**.

    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Solution Explorer window with the Publish command on the shortcut menu for the eShopLegacyWebForms project selected.](..\media\4-project-publish.png)
...
1. If you haven't previously deployed the web app and configured the connection to the Azure SQL Database server, complete the following substeps. Otherwise, skip to step 7.
    ...
    1. Switch back to the Azure PowerShell window. Run the following command to allow your computer to access the Azure SQL Database server that you created in the setup unit. Replace *YourIPAddress* with the public IP address of your computer.
        ...
        > [!NOTE]
        > Make sure you specify the IP address of your computer, not the IP address of the host running Cloud Shell.

    1. Return to the **Pick a publish target** window in Visual Studio and select **Azure App Service** > **Select Existing** > **Create Profile**.

        > [!div class="mx-imgBorder"]
        > ![Screenshot of the Pick a publish target window, preparing to create a new profile for publishing the web app to Azure App Service.](..\media\4-pick-publishing-target.png)
    ...
    1. In the **App Service** window, select your Azure subscription. Set **View** to **Resource type**. Expand **Web App**, select the Azure web app that you created during the setup unit, and then select **OK**.
        ...
        > [!div class="mx-imgBorder"]
        > ![Screenshot of the App Service window with the Azure web app created earlier selected.](..\media\4-select-app-service.png)

    1. On the **Publish** page, select **Edit**.

        > [!div class="mx-imgBorder"]
        > ![Screenshot of the Publish page with **Edit** selected.](..\media\4-publish-edit.png)

    1. On the **Connection** page of the **Publish** dialog box, select **Next**.

        > [!div class="mx-imgBorder"]
        > ![Screenshot of the Connection page of the Publish dialog box with Next selected.](..\media\4-publish-next.png)

    1. On the **Settings** page, by the **Remote connection string** text box, select the ellipsis button (**...**).

        > [!div class="mx-imgBorder"]
        > ![Screenshot of the Settings page of the Publis dialog box with the ellipsis button for the remote connection string selected.](..\media\4-publish-ellipsis.png)

    1. In the **Destination Connection String** dialog box, specify the values in the following table, and select **OK**.
        ...
        > [!div class="mx-imgBorder"]
        > ![Screenshot of the Destination Connection String dialog box with the settings required to connect to the eShop database running in Azure SQL Database.](..\media\4-connection-string.png)
    ...
    1. Back on the **Settings** page, select **Use this connection string at runtime (update destination web.config)** if it isn't already selected, then select **Save**.
        ...
        > [!div class="mx-imgBorder"]
        > ![Screenshot of the Settings page of the Publish dialog box with Save selected.](..\media\4-publish-save.png)

1. On the **Publish** page, select **Publish**.

    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Publish page with Publish selected.](..\media\4-webapp-publish.png)
...
1. Select the **Show portal menu** button, then select **All resources**.

    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Show portal menu button in the Azure portal with the All resources option selected.](..\media\4-all-resources.png)
...
1. On the **Overview** pane for the cache, select **Console**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Overview pane for the cache with Console selected.](..\media\4-cache-overview.png)

1. In the **Redis Console** window, at the **\>** prompt, type **monitor**, then select the <kbd>Enter</kbd> key.

    > [!div class="mx-imgBorder"]
    > ![Screenshot of the Console window with the Redis monitor active.](..\media\4-start-monitor.png)
