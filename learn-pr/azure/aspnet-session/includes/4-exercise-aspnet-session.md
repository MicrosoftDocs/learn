...
2. Define a PowerShell variable containing the name of the Azure Cache for Redis instance to be created. 
...
4. Run the following command to check the provisioning state of the cache. Repeat this command every 30 seconds until it displays the message ***Succeeded***.

    ```PowerShell
    (Get-AzRedisCache -ResourceGroupName $resourcegroupname -Name $rediscachename).ProvisioningState
    ```

5. Retrieve the primary access key for the cache, and record it to use later.
...
2. In the **Solution Explorer** window, right-click the **eShopLegacyWebForms** project, and then click **Manage NuGet Packages**.
...
3. In the **NuGet Package Manager** window, click **Browse**. In the **Search** box, type **Microsoft.Web.RedisSessionStateProvider**, select the **Microsoft.Web.RedisSessionStateProvider** package, and then click **Install**.
...
4. If the **Preview Changes** window appears, click **OK**.

    > [!div class="mx-imgBorder"]
    > ![Image of the **Preview Changes** window. The user has clicked **OK**](..\media\4-preview-changes.png)

5. In the **License Acceptance** window, click **I Accept**.

    > [!div class="mx-imgBorder"]
    > ![Image of the **License Acceptance** window. The user has clicked **I Accept**](..\media\4-license.png)


6. In the **Solution Explorer** window, select the **Web.config** file.

7. In the **web.config** file, find the **sessionState** section. It looks like this:
...
8. Remove the commented out content, and amend the entry for **MySessionStateStore** as shown below. Replace *\<youralias>\>* with the value of the *$useralias* PowerShell variable, and replace *\<primarykey\>* with the value of the *$rediskey* PowerShell variable.
...
9. Find the **\<sessionState mode="InProc" />** element near the start of the **\<system.web>** section, and comment it out, as shown below:
...
6. If you haven't previously deployed the web app and configured the connection to the Azure SQL Database, either complete the following substeps, or go to Step 7 below.
...
9. Wait while the web app is built and deployed to the App Service. When deployment is complete, the web app will start and Visual Studio opens a web browser. Browse the website to verify that the web app operates in the same manner as it did before, when running locally. Leave the page for the web app open in the browser.

## View cache activity

1. Sign in to the [Azure portal](https://portal.azure.com/learn.docs.microsoft.com?azure-portal=true).

2. On the hamburger menu, click **All resources**.

    > [!div class="mx-imgBorder"]
    > ![Image of the hamburger menu in the Azure portal. The user has clicked **All resources**.](..\media\4-all-resources.png)

3. On the **All resources** page, click the Azure Cache for Redis instance you created earlier. It will have the name **\<useralias\>-workshop-cache**.

3. On the **Overview** blade for the cache, click **Console**.

    > [!div class="mx-imgBorder"]
    > ![Image of the **Overview** blade for the cache. The user has clicked **Console**.](..\media\4-cache-overview.png)

4. In the **Redis Console** window, at the **\>** prompt, type **monitor**, and press return.

    > [!div class="mx-imgBorder"]
    > ![Image of the **Console** window. The user started the Redis monitor.](..\media\4-start-monitor.png)

5. Return to the browser window for the web app, and browse through the details of products.
...
6. Return to the **Redis Console** window for the cache. You should see messages displaying the cache activity that took place while you browsed products. The keys that have **_Data** and **_Internal** suffices identify session state information.

    > [!div class="mx-imgBorder"]
    > ![Image of the **Console** window, displaying the activity generated while browsing the web app.](..\media\4-monitor-activity.png)

7. Press Enter to stop monitoring activity.