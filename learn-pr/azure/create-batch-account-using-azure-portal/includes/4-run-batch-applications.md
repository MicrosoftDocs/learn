...
Azure Batch uses parallel tasks to split a job across compute nodes. Azure Batch is especially well-suited to running large-scale parallel and high-performance computing batch jobs. Batch service handles everything for you, which includes managing and scheduling all the nodes and applications that are required to run your scenarios.

With a smaller number of nodes in a pool, you can maximize the resource usage by running more than one task simultaneously. Certain workloads may see shorter job times and lower costs when multiple tasks share compute nodes.

Some scenarios may require you to minimize data transfers for tasks that are able to share data. You can dramatically reduce the data transfer charges by copying shared data to a smaller number of nodes and execute tasks in parallel on each node. This reduces the time taken to transfer data to all nodes instead of sharing data to large number of nodes.

## Enable parallel task execution

Parallel task execution controls how many tasks can be handled concurrently by a single node in a given pool.

In Batch, parallel task execution is controlled by slots. Tasks have a property called `RequiredSlots`, which denote how resource-intensive a given task is. Resource-intensive tasks require more slots than resource-light tasks.

When creating a pool, you specify how many task slots are available per node by setting the `taskSlotsPerNode` property. This property governs how resource-intensive the tasks are that can run concurrently on a given node. For example, if a pool's `taskSlotsPerNode` property is set to 16, then the tasks running concurrently on a given node will never require more than 16 slots. This means that, for example, two nodes that require 8 slots can run at the same time (2 * 8 = 16), or 3 tasks that require 5 slots (3 * 5 = 15), but not 5 tasks that require 4 slots (because 5 * 4 = 20, which is greater than 16).

At max, the `taskSlotsPerNode` property can be up to 4x the number of vCPUs that a node has. To figure out how many vCPUs are available on a node, see [Sizes for virtual machines in Azure](https://docs.microsoft.com/azure/virtual-machines/sizes). Note that after the `taskSlotsPerNode` property has been set, it can't be modified---changing it requires creating a new pool.
