...
To use private endpoints, a Batch account needs to be configured appropriately when it's created. Public network access configuration must be disabled. After the Batch account is created, private endpoints can be created and associated with the Batch account.
