...
Azure Arc relies on a locally installed agent to establish a logical connection between the local resource and Azure. This agent is the Azure Connected Machine Agent. The Azure Connected Machine agent package contains several logical components, which are bundled together: 
...
Arc-enabled servers can leverage a breadth of capabilities, so you can bring Azure services to your servers anywhere: across on-premise, multi-cloud, and edge environments. These capabilities serve myriad use cases across governance, security, and observability needs. 
...
Arc-enabled servers can leverage VM Extensions. VM extensions are lightweight software components that automate post-operating system deployment configuration and automation tasks. Traditionally, VM extensions were available only on Azure VMs, but now it is possible to use selected ones on Azure Arc enabled servers. 
