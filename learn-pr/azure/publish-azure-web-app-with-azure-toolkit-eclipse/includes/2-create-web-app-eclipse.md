Eclipse provides project templates for building static and dynamic web projects. Static web projects include files that contain HTML and client-side JavaScript code. Dynamic web projects typically include files that contain Java Server Pages (JSP), which run at the server. Azure App Service supports both static and dynamic web projects.

In this unit, you'll learn the basics for how to use Eclipse to create dynamic web projects. In the next unit, you will complete an exercise where you create a dynamic web project, which you can deploy as an Azure web app.
...
- To declare static variables, you would use a pair of `<%!` and `%>` tags, and you can initialize variables at the same time. For example, in the following code excerpt, *counter* is an integer variable, and *currencies* is a set that contains the details of each currency that is available in the *java.util* library.
