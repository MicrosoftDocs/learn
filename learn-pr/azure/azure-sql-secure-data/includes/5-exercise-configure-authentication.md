...
1. Once you've configured your databases and gotten connected, your next step might be to add other users and grant access. As in SQL Server, you can add new logins and users.
...
1. Select **Connect** > **Database Engine** and configure the main page so you are connecting to your Azure SQL Database logical server with Login `BobLovesTN` and Password `Tanehill1!`.  

1. You must also set the database name, which you can do by going to **Options** > **Additional Connection Parameters**, where you can enter `Initial Catalog=AdventureWorks`. You must do this manually, because BobLovesTN doesn't have access to scan the server to select a database.  

1. Select **Connect** and confirm you're able to access the database.  

1. As a clean-up step, right-click on the connection from BobLovesTN and select **Disconnect**.  
