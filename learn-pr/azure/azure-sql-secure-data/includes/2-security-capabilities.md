SQL Server and Azure SQL services are known for the importance they put on security, specifically as being enterprise-class. In this unit, you learn about the various security capabilities related to network security and access management. In the units that follow, you get hands-on experience with some of these capabilities.
...
Allowing access to Azure services doesn't, however, allow anything outside of Azure (for example, your on-premises environment) to have access.

The most secure configuration is to set **Allow Azure services and resources access to this server** to **No**, which blocks all connections and networks apart from ones you explicitly added with the various options that follow in the next sections.

### Firewall rules

Your next option is to apply firewall rules. You might get results similar to the **Allow Azure services and resources access to this server** setting except that, for each service (in the following image, a virtual machine (VM)), you need to add a unique firewall rule to allow it to connect. Firewall rules also enable your on-premises environment to connect to the Azure SQL Database resource, because you can add the rules for machines and applications in your on-premises environment.
...
The result would be `203.0.113.1`, the public IP address of the Azure VM. Even though we're using firewall rules, all the connections being made are public.
...
By using this command, you can get details that are related to the DNS infrastructure. Your results would be similar to the following output:  
...
> [!NOTE]
> In the preceding output block, **Address: 168.63.129.16** is a virtual public IP address that's used to facilitate a communication channel to Azure platform resources. It applies to all regions and all national clouds, and it doesn't change.
...
If you use the preceding command, your results are slightly different with the private endpoint configured:  
...
Additionally, the service blocks you from directly connecting by using anything other than `aw-server.database.windows.net`.  

## Access management

After you work out the networking access, the next layer to consider is access management.
...
Depending on how your organization configures the Microsoft Entra instance, you can connect to it by using any of the following three methods (for example, in SSMS):
