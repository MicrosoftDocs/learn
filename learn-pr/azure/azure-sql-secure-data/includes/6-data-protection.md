...
Encrypted connections are forced by Azure SQL Database, with the option to additionally specify the inbound Transport Layer Security (TLS) minimum version (>1.0, >1.1, or >1.2) required. For Azure SQL Managed Instance, a TLS version of >1.2 is forced for outbound connections, and you have the same options for inbound TLS as for Azure SQL Database. The recommended task is to force encryption on the client to avoid server negotiation, as well as to not trust the server certificate as a best practice.

Transparent Data Encryption (TDE) provides encryption for data at rest and is on by default for all new Azure SQL Databases. It can be configured for all deployment options via a switch in the Azure portal, as shown here:  

:::image type="content" source="../media/6-transparent-data-encryption-enabled.png" alt-text="Screenshot of confirming TDE is on.":::  

At the server or instance level, you can also choose to use a service-managed key or use Bring Your Own Key (BYOK). The default is to let the Azure service manage your key. Azure will automatically generate a key to encrypt your databases, and it will manage the key rotations. You've seen how to do this with the Azure portal, but you can also use PowerShell, the Azure CLI, Transact-SQL (T-SQL), or REST APIs.

:::image type="content" source="../media/6-transparent-data-encryption-key.png" alt-text="Server view of TDE options":::  

You can, alternatively, use Bring Your Own Key (BYOK) and take advantage of an Azure key vault. In this scenario, you (not Azure) are responsible for and in full control of a key lifecycle management (key creation, rotation, deletion), key usage permissions, and auditing of operations on keys.
...
Let's take a look at example where the data includes sensitive information, such as social security numbers, email addresses, and salary values. You can apply a mask to those columns with the Azure portal or the following T-SQL commands:
...
Some users, for example, if they're assigned to a role such as *DataOfficers* (this is an example, not an official role), might need to be able to view the masked data. They can be given `UNMASK` privileges with the following T-SQL command:
