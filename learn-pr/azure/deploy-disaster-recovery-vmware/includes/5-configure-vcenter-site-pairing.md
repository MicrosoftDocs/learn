...
2. In the jump box VM, open a browser and connect to the private cloud vCenter and NSX-T Manager using credentials that you can retrieve from the **Identity** pane in your Azure VMware Solution private cloud.
   ...
   [!NOTE] The first time you attempt to connect to vCenter, you’ll be prompted to use either a Flash-based vSphere web client or an HTML5-based client. We recommend you use the HTML5-based client.

3. In the vSphere web client, select **Menu**, and then select **Site Recovery**.

4. In the **Site Recovery** window, verify that the status **OK** displays for both **vSphere Replication** and **Site Recovery Manager**, and then select **Open Site Recovery**.
...
[!NOTE] The following ports should be open to provide cloud-to-cloud recovery: 
...
[!NOTE] To enable bidirectional protection and reprotect, you can configure reverse mappings to map the objects on the recovery site back to their corresponding objects on the protected site. 
...
2. On the **Site Recovery** home tab, select a site pair, and then select **View Details**.
   ...
   [!NOTE] You might be prompted to sign in with **cloudadmin** credentials.
...
5. On the **Virtual machines** page, select one or multiple VMs that you want to protect, and then select **Next**.

6. On the **Target datastore** page, configure the protected VM’s **Disk format** and **VM storage policy**, and then select **Next**.

7. On the **Replication settings** page, select **Recovery point objective**, and optionally, enable point-in-time instances, network compression, or encryption of the data. and then select **Next**.
...
[!NOTE] You can configure replications for powered-off VMs, but the data synchronization begins when the VM is powered on. While the source VM is powered off, the replication displays a **Not active** status. 
...
You can create a protection group containing multiple VMs that VMware SRM protects collectively. Protection groups are included in the recovery plan that specifies how VMware SRM recovers the VM contained in the protection group.
...
4. On the **Name and direction** page, enter a name and description for the protection group, select a direction, and then select **Next**.

5. On the **Protection group type** pane, select **Individual VMs (vSphere Replication)**, and then select **Next**.

6. Select VMs from the list to add to the protection group, and then select **Next**.

   [!NOTE] Only VMs that you configure for vSphere Replication and that aren’t already in a protection group are in the list.
...
4. On the **Name and direction** page, enter the name and description of the recovery plan, select the direction of the replication, and then select **Next**.

5. On the **Protection Groups** page, select **Protection groups for individual VMs or datastore groups**, select one of the previously created protection groups, and then select **Next**.

6. On the **Test Networks** page, select the networks to use while running tests on this plan, and then select **Next**.

7. On the **Ready to complete** page, review the settings, and then select **Finish**.

### Testing the recovery plan

After you have created a recovery plan, you can now test or run the recovery plan. When you test the recovery plan, the source VM continues to operate in the source Azure VMware Solution, and a replica of that VM is created in the recovery site in the test network.

Use the following procedure to test the recovery plan:

1. Select the recovery plan, and then select **Test**.
2. On the **Confirmation options** page, ensure that you select the **Replicate recent changes to recovery site** check box, and then select **Next**.
3. On the **Ready to complete** page, review the selected settings, and then select **Finish**.
...
1. Select the recovery plan, and then select **Run**.
