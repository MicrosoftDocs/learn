...
In this module, you'll learn about the different ways you can combine data in Kusto queries to provide your team with the information they need to increase product awareness and grow sales.
...
In the table, the *CustomerKey* and *ProductKey* are highlighted to indicate that they're unique identifiers and will be used to combine records between tables.
...
| `rightanti`, `rightantisemi` | Right anti join and semi variant<br />**Schema**: All columns from the left table<br />**Rows**: All records from the right table that don't match records from the left table | :::image type="icon" source="../media/2-join-right-outer.png" border="false"::: |

You'll notice that the default join kind is `innerunique` and doesn't need to be specified. Nevertheless, it's best practice to always explicitly specify the join kind for clarity.

As you progress through the units, you'll also learn about the `arg_min()` and `arg_max()` aggregation functions, the `as` operator as an alternative to the `let` statement, and the `startofmonth()` function to assist with grouping data by month.
