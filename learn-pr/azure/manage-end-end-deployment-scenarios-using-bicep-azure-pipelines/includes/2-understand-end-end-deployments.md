Pipelines are flexible tools that you can configure in many different ways to suit your needs. In this unit, you'll learn about how a single pipeline can be used to deploy an entire solution, including configuring the Azure infrastructure as well as performing other deployment operations.

## How many pipelines?

In some organizations, the team that manages the Azure environment is different to the team that develops the code that runs in the environment. In these situations, it's often tempting to create multiple pipelines, each owned by the team holding responsibility for their particular area. For example, you might have one pipeline to deploy the Bicep code that deploys your website's Azure resources, and another pipeline that deploys the website application.

Although this might give you some flexibility in how you manage the pipelines, it can be challenging to keep everything in sync. For example, suppose your website team needs a new setting on their App Service app to enable a feature they're building. The application deployment pipeline can't run until the infrastructure deployment pipeline has completed successfully.
...
Many Azure resources have two different *planes* for access. The *control plane* deploys and configures the resource. The *data plane* enables you to access the resource's functionality.

When you create and deploy Bicep files, you typically only interact with the control plane. In Azure, the control plane is Azure Resource Manager. You use Resource Manager to define the configuration of each of your resources. But your pipeline often needs to do more than just interact with the control plane. For example, you might need to:
...
When you create a deployment pipeline that interacts with the data plane of your resources, there are two common approaches: Resource Manager deployment scripts, and pipeline scripts and tasks.
