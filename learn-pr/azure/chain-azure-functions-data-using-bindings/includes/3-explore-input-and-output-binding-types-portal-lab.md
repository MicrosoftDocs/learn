...
1. Deployment can take some time. So, stay in the notification hub and  watch for a **Deployment succeeded** message similar to the following message.
 ...
 1. Once the function app is deployed, go to **Home > All resources** in the portal. The function app will be listed with type **App Service** and has the name you gave it. Select the function app from the list to open it.
...
>[!TIP]
> You can view the addtional code by clicking the drop down in the middle of the **Code + Test** screen.
...
1. Select **+ Add input** under the **Inputs** column. A list of all possible input binding types is displayed as shown in the following screenshot.
    ...
    Take a moment to consider each of these input bindings by clicking on the drop down and how you might use them in a solution. There are a lot to choose from.

1. We'll get back to adding input bindings later in the module but, for now, select **Cancel** to dismiss this list.

1. Select **+Add Output** under the **Outputs**. A list of all possible output binding types is displayed if you click the drop down.
