...
In this step, you'll open the log stream generated by your ASP.NET Web app by using Azure CLI commands from Cloud Shell.

1. In Cloud Shell, to open the log stream, replacing **\<_your-number_\>** with the random number that was generated to uniquely identify your app, run the following command.
...
1. In Cloud Shell, you should now see log entries generated by each page in the ASP.NET app; if you do not see any log data, repeat the previous step, and wait a few minutes.
...
1. Click in Cloud Shell, and to stop the log stream session, press <kbd>Ctrl+C</kbd>.
