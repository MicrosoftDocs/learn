...
> [!NOTE] The Azure Connected Machine agent sends a heartbeat message to the Azure Arc service every 5 minutes. The Azure Arc service considers the connected machine offline if the it stops receiving  heartbeat messages. The machine is then marked as **Offline** until heartbeats resume, at which time, the machine is marked as **Online**.
...
In case of Azure Arc enabled servers, the agent allows you to enhance management of Windows and Linux computers residing in on-premises datacenters or hosted by third party private and public cloud providers. The Azure Connected Machine agent supports the following versions of the Windows and Linux operating systems:
