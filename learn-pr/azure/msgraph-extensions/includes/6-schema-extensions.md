...
+ If you don't have a verified vanity domain, specify only the schema ID (for example, `teamBondingApp`) in your request body. Microsoft Graph transforms this ID and prefixes it in the format *ext{8-random-alphanumeric-chars}_{schema-name}*. An example of a new schema ID might be `extkvbmkofy_teamBondingApp`.
...
If you're using Graph Explorer for the exercise unit, you must explicitly specify the **owner** property in the request body for the Create request.
...
A schema extension definition has a lifecycle that affects how the property can be used. This lifecycle is managed through the **status** property and can be one of the following states: `InDevelopment`, `Available`, or `Deprecated`.
...
Schema extensions are loosely coupled to their owner apps but are also available for use by other tenants where the app has been consented to. For more information, see the [Extension status](#extension-status) table.
...
You manage the schema extension properties on user profiles through the same HTTP methods that are used to manage users.
