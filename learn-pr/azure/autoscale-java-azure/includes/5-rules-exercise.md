...
In your sample Azure Spring Apps application, when your application was created, it already triggered a scale-out action on the customer service microservice.
...
In your https://shell.azure.com bash window, run the following commands to set your spring apps instance name (same Azure spring Apps service name you used in the previous exercise):

```bash
export SPRING_APPS_SERVICE=<spring-apps-instance-name>
```

Next, in the bash window, run the following commands to execute transactions against your spring apps customers-service
...
> [!IMPORTANT]
> Your Azure Spring Apps might not scale as autoscale will try to estimate what the final state will be after it scaled. This means autoscale would have to immediately scale again, if the average tomcat request count remains the same or even falls only a small amount.
