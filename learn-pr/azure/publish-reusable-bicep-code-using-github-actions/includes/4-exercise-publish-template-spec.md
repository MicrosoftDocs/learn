...
1. Open the file named *template-spec-linux-app-service.yml*.
   ...
   Notice the workflow definition includes two triggers. In this exercise, you won't modify the Bicep file for the template spec, so the `push` trigger never fires. To try out your workflow, you manually invoke it by using the `workflow_dispatch` trigger.
