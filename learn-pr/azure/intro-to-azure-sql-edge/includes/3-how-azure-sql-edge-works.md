...
Azure SQL Edge is a containerized Linux application that runs on a process that's based on x64 architecture. The Azure SQL Edge startup memory footprint is less than 500 megabytes (MB). Within this small footprint, you can design and build apps that run on many IoT devices.
...
- **Architecture:** Device architecture supports architectures based on x64.
