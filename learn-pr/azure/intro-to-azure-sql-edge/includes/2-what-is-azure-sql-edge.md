...
| IoT apps can run in insecure environments                    | Since many IoT apps might run in cars, hospitals, and factories, security and privacy are important. Therefore, the platform selected to work with data in these environments must be secure. It must be trusted be users and developers alike. |
