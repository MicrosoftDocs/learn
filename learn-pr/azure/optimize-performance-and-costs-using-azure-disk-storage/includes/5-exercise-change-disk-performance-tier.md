...
Suppose you run an accounting application that stores the data on Azure VMs running SQL Server. The application runs periodic jobs that demand higher performance for short periods, but otherwise experience baseline performance most of the time. To manage these performance requirement, you need to:

- Create a managed disk and add that to a VM.
- Change the disk performance settings on a VM.

In this exercise, you'll make the changes to a VM by using the Azure portal. First, sign in to the portal, and then create a VM with an additional managed data disk.
...
5. Retain the defaults for the remaining fields, and then select  **Next : Disks >**.
6. On the  **Disks**  pane, notice the VM has one disk, the OS disk.
...
14. After reviewing your new VM settings, select  **Create**  to start deploying your new VM.
    ...
    > [!Note]
    > VM creation can take a few minutes as it creates the various resources (storage, network interface, and so on) required to support the VM. Wait until the VM has deployed before continuing with the exercise.
...
9. Proceed with the next steps in the wizard and retain all the default settings. When you're done, select  **Finish**.
...
In this exercise, you used the Azure portal to add a data disk on a new VM and to change the performance of the data disk. <!--Marjan, do we need to add a summary statement like this? I don't remember if we did this for exercises in other modules. But we had a Clean up resources section in other exercise. Please confirm. -->
