...
- **Pre-existing resource**.  When you specify a resource in a template and deploy it, one of two things happens. Either the resource is deployed or it's not deployed if it already exists. Checking whether it exist is something ARM does for you, it's implicit. The question is if you can leverage this mechanism to your advantage when you reason about _how_ you can check the preexistence of something.
