...
Azure Monitor receives data from multiple target resources such as applications, operating systems, Azure resources, Azure subscriptions, or Azure tenants. The nature of the resource defines which data type, or types are available. A data type will be one of metric, log, or metric and log.
...
You'll learn about the three different signal types – metrics, activity logs, and logs – that you can use to monitor your environment.
...
- **ALERT DETAILS**
    ...
    - The **severity** of the alert if the criteria or logic test evaluates 'true'. Severity is defined in the range 0 to 4. Severity levels are:
      - 0: Critical
      - 1: Error
      - 2: Warning
      - 3: Informational
...
### Filtering alerts 
From the alert summary window, you'll see a satellite view of the alerts being reported in Azure Monitor. You filter this view to reduce the volume of alerts being reported to a specific subset that you're interested in.   If you have enabled smart groups, they can also be filtered on – you'll see more on this subject later.
The following can also be used to filter the view:
