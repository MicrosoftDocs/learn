...
1. You want an alert when any virtual machine in your resource group is deleted. Select the **<rgn>[sandbox resource group name]</rgn>** resource group, and then select **Done**. The **Create alert rule** pane reappears with the Sceop target resource showing **All Virtual machines**.
...
1. In the title bar, select the **Notifications** icon and wait until vm1 is successfuly deleted.
...
1. You should see three verbose alerts that were generated by deleting **vm1**. Select that row. The **All alerts** pane appears, listing the VM was deletd alerts created by deleting **vm1**.

    ![Screenshot that shows all alerts with affected resource, monitor service, and signal type.](../media/7-vm-rg-deleted-alert.png)

The alerts list shows alerts for the VM that you deleted, along with the details: the alert name, everity, state, affected resource, monitor service, and signaly type.
