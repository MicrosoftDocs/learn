As an Azure solution architect, you realize that, by using metric, log, and activity log alerts, a large number of alerts are generated – this makes it difficult to identify the critical and essential issues. You want to explore the options available in Azure Monitor. Is there a mechanism that might reduce the alert noise and make the task of managing alerts easier?
...
To change the state, select the **change smart group** state<!--CE:I think this needs a bold or quote mark recognition.--> as shown here.
...
Smart groups are automatically created when a pattern of alerts is detected by Azure Monitor. Alerts are either added automatically to an existing smart group or a new smart group is created. 
