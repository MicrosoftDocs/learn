...
Azure Data Lake Storage overcomes these data access barriers, thereby enabling so-called *data democratization* where data is accessible to all users, not just those with technical knowledge. It does this by enabling your organization to store all your data formats (including raw data) in a single location. Eliminating data silos enables users to use a tool such as Azure Data Explorer to access and work with every data item in their storage account.
