Importing data into a search index is the first step in enabling your company to use search. With data loaded into an index, the next step is learning how to query it.

In this unit, you'll see how to write simple search queries in the Search explorer, and then append extra parameters to manipulate those results.
...
- **Service endpoint and index collection:** Usually the URL such as `https://(your-service-name).search.windows.net/indexes/(your-index-name)/docs`.
- **API version:** Enables you to choose which version to send the request to.
- **API Key:** Used to authenticate the request.
- **Query type:** - One of two choices, either simple or full, with simple syntax being the default option.
- **Query:** - Will contain search terms and then a combination of sorting statements or filtering statements.
...
This query is trying to find any video in your company's catalog that is calm and relaxing, but the person searching wants to exclude iyengar yoga and hot pilates.
...
By default, the search engine will match any of the terms in the query. A title containing just `calm` would be a match. In this example, using `-"iyengar yoga"` would lead to the search results including all the titles that don't have the exact string "iyengar yoga" in it.
...
Breaking down the query into its components, the last part uses the Lucene syntax's ability to specify a field in a document, (in this case, `Difficulty`), to match a given term. The value can be a single search term or phrase, or in this example, a range of terms.
...
The proximity search `"term1 term2 term3"~3` tells the search engine that these terms must be found in a string no more than three words apart from each other. In the previous example, `color~2` tells the search engine to look for all words that are up to two characters away from `color`. Running the query would match with the words colored, colors, colour, colon, and colours.

Fuzzy matching works by looking for similar strings, either replacing, adding, or removing characters in them.

If you need more specific control over string composition, Lucene also offers the power of matching against regular expressions. You can use forward slashes around the regular expression. For example, `/[yt]oga/` would match on all videos with the words yoga or toga in the title.

### Filter data

Filtering in Azure Cognitive Search is an additional criteria specified by a `$filter` expression. Filtering is executed before your search query is parsed so your query runs on a subset of the documents in the index. For example, a filter could be used to restrict the exercise videos returned in the catalog based on their difficulty level. You specify your filter criteria using OData expressions.
...
This search introduces a new parameter, `search`, which is where you put the query expression. The parameter is included here for clarity as the number of options increase. It can be omitted.
...
Your web developers would like to provide a sidebar on the search page to enable your customers to filter the results based on their tags. They need to have a list of the tags on videos, and the total number of videos with that tag, to add drilldown capabilities.
...
`&facet=fieldname`: Returns a facet structure in the JSON results for the specified field. For example, `&facet=Tags` includes an array in the results. This array could be used by the website, enabling users to select the length of videos.
