...
This will return _all_ virtual machines defined in this subscription. The output can be limited to a specific resource group through the `--resource-group` parameter. If you try this, you'll find that the default results are always in JSON form which can be harder read, so you can add the `--output table` flag to limit the information to a simple table.
...
## A quick stop at JMESPath queries

JMESPath is an industry standard query language built around JSON objects. The simplest query is to specify an _identifier_ which selects a key in the JSON object.
...
We can also add specific qualifiers, `people[?age > '25']` which would return just the `Fred` and `Wilma` objects. 
...
Finally, we can constrain the results by adding a select: `people[?age > '25'].[name]` which would return just the names:
...
JMESQuery has several other interesting query capabilities - there's an online tutorial available on the [JMESPath.org site](http://jmespath.org/tutorial.html).
...
This query technique will work with any Azure CLI command and can be used to pull specific bits of data out on the command line. It is useful for scripting as well - for example, you can pull a value out of your Azure account and store it in an environment or script variable. If you decide to use it this way, a useful flag to add is the `--output tsv` parameter (which can be shortened to `-o tsv`). This will return the results in Tab-Separated Values (TSV) which only includes the actual data values with tab separators.
