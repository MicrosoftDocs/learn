...
Once you have the connection secret, you can directly store it in an **app setting** for the App Service. When you need to rotate the secret, you need access to the original Azure resource (such as Cosmos DB) and the Azure App Service.
...
If you want to restrict access to the App Service, you may choose to store the secret in an **Azure Key Vault** then reference the secret by the App Service as an app setting. When you need to rotate the secret, you only need access to the Azure Key Vault. 
