...
When a threat like this is detected, Defender for Cloud will generate a _security alert_.
...
Microsoft Defender for Cloud provides a variety of alerts that align with the stages of the _cyber kill chain_. The cyber kill chain is a series of steps that trace the stages of a cyberattack from the early reconnaissance stages to the exfiltration of data. The kill chain was created by Lockheed Martin and is modeled off a military framework established to identify and engage enemy targets.
...
Defender for Cloud alerts attempts to detect and recognize known behaviors in each phase of the kill chain and provide the SecOps team an opportunity to stop a cyberattack in progress. Post breach, Defender for Cloud can provide the necessary details to be able to identify the exfiltration and close off compromised systems.
