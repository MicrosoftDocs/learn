...
In the last unit, you connected to the desktop of your Ubuntu DSVM. One of the icons on the desktop is labeled "Jupyter." If you run this, it will start a Jupyter notebook server using the base environment provided by conda. 

The conda package manager is a popular way to manage Python programming environments. It is the default environment manager used by the DSVM and the DSVM comes with several environments preconfigured to work with popular data science and ML packages. As an example, let's run through initializing, activating, and using PyTorch running with Python 3.6.
...
After startup, the Firefox browser on your DSVM should start and open to `localhost:8888/tree`, showing the home directory of your DSVM user. 
...
It is possible to "resize" a virtual machine so that the image runs on more or less powerful hardware. If you had created the a non-GPU DSVM in your own subscription and that subscription included GPU resources, you could switch to the more powerful hardware without losing any of your data or configuration. 
