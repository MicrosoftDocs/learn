...
1. In the left menu pane, select **Databases**, and under *Popular offers*, select **SQL Database**.
...
1. Run the `bcp` utility to create a format file from the schema of the `dbo.Courses` table in the database. The format file specifies that the data will be in character format (`-c`) and separated by commas (`-t,`).
