...
The bulk copy `bcp` utility is a command-line utility that you can use to import and export data from a database. For importing data, `bcp` requires three things:
...
The `bcp` utility has several parameters that control the functionality of the utility. You can specify the target table (`<database>.<schema>.<table>`), specify that the data is to be imported (`format nul -c -f mytable.fmt -t,`), and the connection details to your database (`-S <server>.database.windows.net -U <username> -P <password>`). Full syntax and command line parameters for the utility can be found in the [help documentation for bcp](https://docs.microsoft.com/sql/tools/bcp-utility).
