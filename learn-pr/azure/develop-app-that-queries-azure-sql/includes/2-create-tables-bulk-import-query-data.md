...
When you create a database, you will be prompted for the server to use to host the database. A server is an instance of the SQL Database service. You can create a new server, or you can select an instance that you have created previously. If you create a new server, you'll be asked to specify a server admin username and password. You use these credentials to connect to the server to perform administrative tasks and access the databases on this server.
...
If sign-in is successful, you'll see a *1>* prompt. You can enter SQL commands over several line and run them by typing *GO*.
...
The **bcp** utility is very flexible inasmuch as the source data can be in almost any format. The format file indicates the layout of the data, whether it is binary or character-based, the type and length of each item, how the data is separated, and so on. The format file also specifies how to map each item in the file to a column in the table. It is important to define the contents of this file correctly, otherwise your data might not be imported, or data could be read into the wrong columns.
...
The best way to create a format file for import is to use the **bcp** command to generate it for you. The **bcp** command can create a format file based on the schema of the target table in the database. You can then edit this file to match the data in the source file.
...
The first argument to the **bcp** command is the fully qualified name of the target table, in the format *\<database\>.\<\schema\>.\<table\>*. Unless you created your own schema, tables will be held in the *dbo* schema by default. The *format nul* argument indicates that we are using bcp to generate a format file rather than importing or exporting data. The *-c* flag specifies that we will be importing data held in a text file (8 bit character data). If your file uses a multi-byte character set, you can specify the *-w* flag instead. If your source data is in binary format, use *-n*. The *-f* flag names the format file to be created. If this file already exists, it'll be overwritten. The *-t* flag specifies the field separator in the source file; a comma in this case. The *-S* flag is the Azure SQL Database instance holding the database, and the *-U* and *-P* arguments specify the username and password for connecting to the database server.

If you look at the contents of the **mytable.fmt** format file generated by this command, it'll look like this:
...
Now the data in field 2 in the source file will be mapped to the first column in the databaase, and field 1 will be mapped to the second column.
...
The *in* flag specifies that we are using **bcp** to import data; you can use *out* to transfer data from a database to a file.. The *-F 2* flag indicates that the import operation should start at line 2 of the source file. Remember, the first line contains headers rather than data.
