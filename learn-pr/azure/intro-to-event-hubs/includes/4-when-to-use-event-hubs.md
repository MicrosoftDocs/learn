...
Event Hubs directly integrates with numerous Azure services, which can make development of pipelines easier and safer. Three important integrations are Capture support, integration with Event Grid, and integration with Stream Analytics.

Capture support provides a built-in consumer that can be configured to save every message, or a subset thereof, directly into a storage account. This storage account can be used as an emergency archive or a slow processing channel (also called a cold path) for data whose analysis is not time-sensitive. This provides some assurances when consumers may be slow to process data or must be temporarily taken offline.

Event Hubs provides direct integration with Event Grid, which is an Azure-standard means of receiving notifications from one service and using this to trigger other services. For example, when a message is received by Event Hubs, this can pass to Event Grid, which wakes an Azure Function or Logic App that acts as the true consumer.
