...
A hypothetical company that manages its global retail operations in the cloud might benefit from Azure Cosmos DB. The company's engineers might need to guarantee that point-of-sale (POS) devices can persist data to a cloud database with short response time to each request. If the company maintains a database of inventory stock, it might want to ensure that this database is highly available (to avoid customer dissatisfaction) and globally distributed (to ensure that customers globally see a responsive webpage). 

Furthermore, the load on the company's global retail system might vary dramatically throughout the year and is especially aligned with retail holidays. The company knows that the load on its database will peak only in the weeks around retail holidays. It's looking for a pricing model where it pays for what it consumes rather than paying for peak consumption.
