...
1. In the **Tables** list, expand **Azure Resources** category, and move the mouse over **Azure Activity** table, and then select **Preview data.**
2. In the **AzureActivity** window select **See in query editor.** This can be useful to ensure that this is the data that you&#39;re expecting before you actually run a query with it.
...
![](RackMultipart20201113-4-15gkrx6_html_5f147df2d04af023.png)

1. Scroll down to the **Caller activities** table, that shows activities run from your users or security principals. By clicking on every column, you can sort the results presented in the table.
2. Scroll up to the header bar in the **Azure Activity-** _sentinelname_ page. Select the **Edit** button, to switch the workbook in the editing mode. Notice on the right side of the page, several **Edit** buttons appear.
3. Select the first **Edit** from the top. That will bring editing pane for one of the steps in the workbook. You can further customize the presentation of the elements, by adjusting the style, reordering them in different order.
4. You can add additional parameters with different types, such as text, drop down, multi-values or similar.
5. Select **Add parameter.**
6. In the **New Parameter** page, enter the following values:
...
1. Confirm that the query result return two type of events based on the level: **Informational, Warning.**

![](RackMultipart20201113-4-15gkrx6_html_443b55e77c61fb63.png)

1. Select **Save,** to commit the changes in the step.
2. Notice that the parameter step now includes one additional parameter name, called **Level.**
3. When you done all the changes in the step select **Done Editing.**
4. If you select the three dots right from the **Edit** button, the new drop-down menu will appear. From that menu, you can move this step, in different part of the workbook. You can also clone or remove the step from the workbook.
5. From the header bar select the icon to **Save as** the customized workbook.
6. In the title provide name for the new workbook and then select **Save.**
7. Your new workbook will be created in the **My workbooks** tab. If you don&#39;t see as listed, select **Refresh** button.
