...
There are several Microsoft products that provide data loss prevention. In today's world, most commonly used is Security & Compliance Center in Microsoft 365 platform. With a data loss prevention (DLP) policy in the Security & Compliance Center <!--- Although Security & Compliance Center is still available, Microsoft has split its functionality to Security Center and Compliance Center - both are available directly from Microsoft 365 admin center, while Security & Compliance is available if you follow additional links. It is true that Compliance Center still points to Security & Compliance Center for certain tasks, but it also provides quite a few features that can be managed just in Compliance Center (and not in Security & Compliance Center). My suggestion would be to rather use Compliance Center in this module, except you are talking about specific feature that can be managed only in Cecurity & Compliance Center. (everything that is described in this paragraph can be done in Compliance Center. There are two instances of "Security & Compliance Center" that you can replace in this paragraph. --->, you can identify, monitor, and automatically protect sensitive information across Microsoft 365 services and applications.
