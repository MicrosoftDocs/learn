...
Your VMs will be automatically onboarded to these participating services when you use the best practices configuration profiles. Following is a list of participating services:
...
If you want to customize the best practice services and settings, you can use the custom profile option. Custom profiles allow you to customize the services and settings you want to apply to your machines. This option is useful when your IT requirements differ from the best practices. For instance, if you don't want to use the Microsoft Anti-malware solution because your IT organization requires you to use a different anti-malware solution, you can toggle off Microsoft Anti-malware when creating a custom profile.
...
For example, when a machine is conformant with its assigned configuration profile, its status will read *Conformant*. When a machine drifts from the configuration profile, Automanage automatically brings it back into compliance.
