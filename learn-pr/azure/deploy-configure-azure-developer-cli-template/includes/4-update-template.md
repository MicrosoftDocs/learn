...
1. Change the header display text on line 42 from `Todo` to `Ticket manager` and save your changes.
...
1. Return to a command prompt window set to the root of your directory, and then run the `azd deploy` command. Wait for a moment while `azd` redeploys your code.
...
Your team is planning to add the ability for users to upload files with their ticket. These files might include images or screenshots of an issue they are encountering, or other service documents. You want to add an Azure Storage account to your environment that will be used to hold these files. You also want to make sure this storage account is included during the initial setup when this template is provisioned to future environments. You can meet these requirements by modifying the infrastructure as code files for your template using the following steps:
...
1. Save your changes, and then run the `azd provision` command. `azd provision` will use the infrastructure as code files in your template to update your Azure environment, but it will not redeploy any of your app source.
...
> [!NOTE]
> When working with `azd`, keep in mind that infrastructure as code files are idempotent, meaning no matter how many times you run them, the desired state will remain consistent. Bicep or Terraform will apply changes to Azure so that your environment matches the state of your infrastructure files, but will not make changes to resources that already match the desired state. By default, resources will also not be deleted if they are removed from the infrastructure as code files - they will only be created or updated.
