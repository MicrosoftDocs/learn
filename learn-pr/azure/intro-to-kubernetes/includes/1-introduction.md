...
The standard container management runtime is focused on managing individual containers. However, there are times where you'll want to scale and have multiple containers working together. Managing multiple containers are challenging as several factors need consideration. Suppose you need to handle load balancing, security, network connectivity, and deployment. It's common to use a container management platform such as Kubernetes to help make the management process easier.
...
- Basic knowledge of installing software using a command-line interface
