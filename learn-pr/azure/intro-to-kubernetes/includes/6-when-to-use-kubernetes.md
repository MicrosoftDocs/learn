...
The drone tracking solution is built as microservices that are designed as loosely coupled, collaborative services. You're deploying these services separately from each other to simplify the solution's design and maintenance. Here is the current configuration of your solution.
...
- A cache services that store frequently requested information displayed on the website
...
The change in developer thinking has resulted in several processes and business benefits for the company. For example, better use of hosted compute resources, new features have improved time to market, and improved customer reach.

However, several container management challenges that lead your company to investigate container orchestration solutions. Your teams found that scaling the tracking application to a handful of deployments was relatively easy, but scaling and managing many instances is hard.

There are several other aspects to also consider. For example, dealing with failed containers, storage allocation, network configuration, application secrets management, and so on.
...
Not all applications need to run in Kubernetes, and as a result, Kubernetes may not be a good fit for your company.

For example, the effort in containerization and deployment of a monolithic application may be more than the benefits of running the application in Kubernetes. A monolithic architecture can't easily use features such as individual component scaling or updates.
...
Your development teams will have to embrace modern design concepts when developing and designing applications. For example, using microservices and the containerization of these services.  Teams also have to experiment with container and orchestration environments to make the best use of all the options available.
