Once you've selected a workload to either migrate to or create in Azure SQL, you need to plan your deployment, deploy accordingly, and verify that the deployment . In this unit, you'll learn various methods for each step in the process.  
...
Behind the scenes, for Azure SQL Managed Instance, Azure deploys a dedicated ring (sometimes referred to as a *virtual cluster*) for your service. This helps in providing security and native virtual network support. However, because of this, deployment and scaling operations can take longer. For example, when you scale up or down, Azure deploys a new virtual cluster for you and then seeds it with your data. You can think of every instance as running on a single virtual machine. Azure SQL Instance pools were introduced to help with this, because you can pre-deploy a "pool" of dedicated resources, making deploying into a pool and scaling within a pool much faster (and with a higher packing density).  
...
For Azure SQL Managed Instance, you then might check the ERRORLOG, which is a very common thing to do in SQL Server on-premises or in an Azure VM. This capability is not available in Azure SQL Database.  
...
What you may notice is there are two queries that are not available in Azure SQL Database related to OS system information and OS process memory. This is because with Azure SQL Database, some thing related to the OS are abstracted away from you, since you're just getting a database.  
