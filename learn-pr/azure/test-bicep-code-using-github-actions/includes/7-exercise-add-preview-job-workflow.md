...
1. Between the **validate** and **deploy** jobs, add the following definition for the **preview** job:
   ...
   > [!NOTE]
   > There is currently a bug in the Azure CLI action that requires you set the version number explicitly. This bug will be fixed soon.

1. Update the **deploy** job to make it depend on the **preview** job:

   :::code language="yaml" source="code/7-workflow.yml" range="57-58, 60-75" highlight="3" :::
