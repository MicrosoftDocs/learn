...
1. **Fast:** Containers don't have to boot an entire operating system, so they can start much faster--typically in seconds.
...
* **Fault isolation**: If an individual microservice becomes unavailable, it won't disrupt the entire application, provided that upstream microservices are designed to handle faults correctly.
...
A *stateless* service is one where each request and reply can be understood in isolation. You can imagine a very simplistic calculator service, where you send a calculation to be performed (2+2, for example) and receive a single answer (4). If you wanted to perform an additional calculation on that result (4 x 2, for example), you would manually send a request to compute 4 x 2 and receive 8. However, the service wouldn't be aware you were using the initial calculation's result.
...
![Diagram that shows the scope of Azure Service Fabric, including orchestration, programming models, automatic scaling, and more, PNG.](../media/2-service-fabric-overview.png)
