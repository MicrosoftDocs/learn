The Azure Blueprint is backed by the globally distributed Azure Cosmos DB, which means that the blueprint objects are replicated to multiple Azure regions with low latency, high availability, and consistent access to your objects. All of this is available to you at no cost, because the only costs for Azure Blueprints will be associated with the resources deployed through a blueprint, like a virtual machine, storage, database, etc.
...
These next sections will dive into some of the details of each one.
...
* The **Azure Resource Manager (ARM) template** you would like to deploy in order to automate resource creation
