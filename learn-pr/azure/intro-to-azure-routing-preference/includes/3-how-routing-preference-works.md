Before you can decide if routing preference can acheieve the cost savings of the factory network traffic project, you need to understand how the routing preference options work in Azure.
...
For example, if traffic from a service hosted in the Central US region is destined for a user in Singapore, the traffic exits the Microsoft network at the Edge POP closest to the Central US region and then traverses the public internet to the user in Singapore.
...
As we learned in the previous unit, routing preference unmetered The routing preference unmetered service is available for Content Delivery Network (CDN) providers who have their customers' origin content hosted on Azure.
