...
1. Create a custom file called *Custom-ParameterNaming.test.ps1*. Give the file the following content:
...
You have a custom test written at this point. However, your template file's naming doesn't fulfill the requirement. So you expect the upcoming test run to fail. Ensure that's the case by taking the following steps.
