...
3. Save the file with <kbd>Ctrl+S</kbd> and close the editor with <kbd>Ctrl+Q</kbd>.
...
1. Run `func start` to start the local functions host. As with `func new`, your shell should still be in the `loan-wizard` directory.
    ...
    At the end of the output, you'll see a message that the simple-interest function is available at the URL `http://localhost:7071/api/simple-interest`. If you see an error message, use <kbd>Ctrl+C</kbd> to stop the host and make sure that the contents of your index.js file are the same as the sample above.
...
1. Stop the background functions host with the `pkill` utility:
...
1. Close the editor with <kbd>Ctrl+Q</kbd>.
