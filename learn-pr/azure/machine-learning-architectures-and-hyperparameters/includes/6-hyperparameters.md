...
You can further customize the criterion used to split nodes. For example, setting the _minimum purity decrease_ means that a node can only be split if it will improve the model by a certain amount or more. There are several related hyperparameters that can block new nodes being created, such as the maximum depth or the minimum number of samples at a node.
