In your customer's API, the traffic to the application API runs unencrypted. Your customer wants the application to use HTTPS instead of HTTP. You could enhance the application code to support HTTPS, but you don't have access to the original application.
...
The sidecar pattern is a powerful concept in container-based architectures that lets you decompose application functionality in different container images that run together in the same container group. Containers in the same container group share some properties, such as the underlying network stack. In a Container Instances container group, each container can take over part of the functionality required by the application.

In this unit, you use NGINX, a web server that can be used as a reverse proxy in front of a web application, to offload secure socket layer (SSL)  functionality. Encrypted traffic enters the Azure Container Instances container group in the NGINX container, and NGINX hands over the decrypted traffic to the actual application container.

![Diagram that shows topology of Azure Container Instances with NGINX sidecar.](../media/3-nginx-sidecar.png)

## Create NGINX configuration

1. The first thing you need is a digital certificate for data encryption. In this unit, you use a self-signed certificate, but in production you'd use a digital certificate generated by a Certificate Authority. In Cloud Shell in the Azure portal, run the following code to get the certificate.
...
1. Create a file containing the configuration for NGINX. You don't need to understand every detail of the file, but the following aspects are important:

    - The `proxy_pass` property points to `http://127.0.0.1:8080`, where the application container is. The NGINX and application containers inside the container group share the same network namespace, so they can communicate with each other by using the `localhost` or `127.0.0.1` address.
    ...
    - The sidecar container listens to the TCP port 443.
...
You use YAML to deploy the container group. Complex configurations such as sidecars require using YAML instead of Azure CLI.
...
1. Create the YAML file by combining all the information you stored in variables in this unit in a single file. Be aware of the following points:
    ...
    - The SQL password is passed as a secure environment variable, so it's not exposed after creating the container instance.
    - The NGINX container mounts the config volume in the */etc/nginx/* directory, where the certificates are expected to be found. The volume contents are specified as secrets, which is why you base64-encoded the variables earlier.
    ...
    Use YAML to specify the following properties for the container instance:
...
1. After you put all the required configuration in the YAML file, run the following command to create the container instances:
...
1. Extract the private IP address of the container instance and access it via HTTPS from the test VM. Use the flag `-k` with `curl` so it disables certificate validation, because you're using self-signed certificate in this unit. The `api/healthcheck` endpoint of the API should respond with an `OK`.
