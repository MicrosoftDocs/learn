...
The Git CLI and the `git log` command provide many arguments you can use to view information about your commits and files. However, it's often easier to use Visual Studio Code to view the commit history for a file. In the **Explorer** pane in Visual Studio Code, you can select and hold (or right-click) a file in your repository and select **View Timeline**. The **Timeline** pane opens, and shows a list of each commit that affected that file. When you click a commit, you see the exact changes to the file. You'll see how to use this in the next exercise.
