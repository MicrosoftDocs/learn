...
A pull request is effectively a controlled merge of two branches, and GitHub or Azure Repos can enforce policies about who can merge and what kinds of changes can be merged. A typical team workflow would involve a team member making changes to their code on a branch, then creating a pull request to ask someone else to merge their changes into the **main** branch. Other team members can see the list of changes in the PR, and they can even provide reviews on the changes before they accept them. PRs provide a way to provide quality control around your team's code.
