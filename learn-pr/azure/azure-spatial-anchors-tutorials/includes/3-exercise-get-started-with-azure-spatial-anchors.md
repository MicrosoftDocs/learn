...
First, follow the steps in [Initializing your project and deploying your first application](/learn/paths/beginner-hololens-2-tutorials/) in the learn introduction module, excluding the Build your application to your device instructions, which includes the following steps:
...
2. Select **MixedRealityToolkit** object in the Hierarchy window, then use the **Add Component** button in the Inspector window to add the following components:
    ...
    * DisableDiagnosticsSystem (Script)
    ...
    > [!Warning]
    > There's a known issue with ASA v2.9.0 and v2.10.0-preview.1 that requires two additional objects to be placed in the scene. Use the **Add Component** button in the inspector window to add an AR Camera Manager (Script) and an AR Session (Script) to the **MixedRealityToolkit** object. Be sure to disable the Camera that is created automatically while adding the AR Camera Manager (Script) by unchecking the box next to the **Camera** object in the inspector window. This issue will be addressed in the full release of ASA v2.10.0.
    > [!Note]
    > When you add the AR Anchor Manager (Script) component, the AR Session Origin (Script) component is automatically added, as it's required by the AR Anchor Manager (Script) component.
...
3. In the Hierarchy window, select the **CreateAzureAnchor** button, then in the Inspector window, configure the **Button Config Helper(Script)** component's **On Click ()** event as follows:
...
4. In the Hierarchy window, select the **RemoveLocalAnchor** button, then in the Inspector window, configure the **Button Config Helper (Script)** component's **On Click ()** event as follows:
...
5. In the Hierarchy window, select the **FindAzureAnchor** button, then in the Inspector window, configure the **Button Config Helper (Script)** component's **On Click ()** event as follows:
...
6. In the Hierarchy window, select the **DeleteAzureAnchor** button, then in the Inspector window, configure the **Button Config Helper (Script)** component's **On Click** () event as follows:
...
Azure Spatial Anchors cannot run in Unity, so to test the Azure Spatial Anchors functionality, you need to build the project and deploy the app to your device.

> [!Tip]
> For a reminder on how to build and deploy your Unity project to HoloLens 2, you can refer to the [Building your application to your HoloLens 2](/learn/modules/learn-mrtk-tutorials/1-7-exercise-hand-interaction-with-objectmanipulator?ns-enrollment-type=LearningPath&ns-enrollment-id=learn.azure.beginner-hololens-2-tutorials) instructions.
...
3. With the newly added RoverModule_Complete object still selected in the Hierarchy window, drag it onto the **ParentAnchor** object to make it a child of the ParentAnchor object:
    ...
    >[!Tip]
    >There are a variety of user-experience flows for repositioning experiences, including the use of a repositioning object (such as the cube used in this tutorial), the use of a button to toggle a bounds control that surrounds the experience, the use of position and rotation gizmos, and more.
