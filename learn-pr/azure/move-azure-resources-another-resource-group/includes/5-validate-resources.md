...
Let's have a high-level look at the process for validating a move using the REST API.
...
To submit this POST request with the correct body by using the Azure CLI, execute this command:
...
If your request is accepted, the API returns a status code of 202, as shown below:
...
To submit this GET request by using the Azure CLI, execute this command:

``` bash
az rest --method get --uri <location-url>
```

If your move validates as successful, you'll get a 204 status code. Otherwise, you'll receive the following error message if your move wouldn't be successful:
