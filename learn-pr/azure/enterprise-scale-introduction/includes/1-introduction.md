...
Similarly, an organization can turn on an Azure resource when it's needed and turn it off when it's not. Azure provides broader options beyond on/off configurations, much like a dimmer switch that lets you control how much light to provide at any moment in time.
...
The same idea applies to resources in Azure subscriptions, which must be properly organized for operational efficiency. Enterprise-scale architecture provides a set of prescriptive, proven guidelines to define a sustainable platform architecture for landing zones. At a library, we know where to find a book and, when we're done with it, where to return it. Likewise, we build landing zones so we know where to go, and we provision resources when we need them.  
