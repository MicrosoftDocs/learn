...
Your database is secure by default. If you add your current IP address to the firewall rules, you can connect to and query your database using your username and password.
...
The service runs the standard Community PostgreSQL in the compute container. Compute containers can be scaled dynamically - meaning Azure only charges for the resources that are needed, when they're needed. For example, hosting of a small database for a few dollars a month that can then be scaled as needs grow. The compute container is easily configured via the Azure portal with firewall and security rules, performance insights, and configuration recommendations.
...
Servers can also be replicated on up to five read-only *read replica* servers, improving the performance and scale of read-intensive workloads. A common scenario is to have business intelligence and analytical workloads use the read replica as the data source for reporting.
...
When you're assessing a trial of your payment app, creating a read-only server for analytics is useful as you can show stakeholders analytics, without impacting the transaction times for users. Storage `auto-grow` keeps costs down, while never running out of storage.

## How PostgreSQL extensions work

PostgreSQL allows you to extend the functionality of your database by using extensions. After being loaded in the database, extensions can function like built-in features. Due to the open-source nature of PostgreSQL, there are too many extensions available to list on this page. A hyperlink to the full list is available in Unit 7 - Summary.
