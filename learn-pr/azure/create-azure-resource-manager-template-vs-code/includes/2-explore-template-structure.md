...
To deploy a local template, you need to have either [Azure PowerShell](/powershell/azure/install-az-ps?view=azps-4.2.0&azure-portal=true) or the [Azure CLI](/cli/azure/install-azure-cli?view=azure-cli-latest&azure-portal=true) installed locally.
...
A CI/CD pipeline automates the creation and deployment of development projects, which includes ARM template projects. The two most common pipelines used for template deployment are [Azure Pipelines](../../paths/deploy-applications-with-azure-devops/index.md?azure-portal=true) or [GitHub Actions](../../paths/automate-workflow-github-actions/index.md?azure-portal=true).
