...
Next, select the **Fork** button near the upper right of the webpage.

![Screenshot of a GitHub repository webpage with a callout highlighting the Fork button.](../media/3-exercise-clone-branch-01.png)
...
You need the URL of your fork to clone it to your local hard drive. While at the root of this repository, copy the URL of your address bar. It will be in the format of `https://github.com/{Owner}/{Repository}`.
...
Select <kbd>Ctrl+Shift+P</kbd> (MacOS: <kbd>Cmd+Shift+P</kbd>) to open the **Command Palette**.

In the Search box, enter **clone**.  The drop-down option should list **Git: Clone** as the selected option.  Select <kbd>Enter</kbd>.

At the prompt, enter the repository URL by selecting <kbd>Ctrl+V</kbd> (MacOS: <kbd>Cmd+V</kbd>). This action pastes the URL that you copied from the **Clone with HTTPS** window in Step 2.  Select <kbd>Enter</kbd>.

> [!NOTE]
> The Command Palette provides access to many Git features with helpful prompts.  If you're curious about what you can accomplish from the Command Palette, open it by selecting <kbd>Ctrl+Shift+P</kbd> (MacOS: <kbd>Cmd+Shift+P</kbd>), then enter **git** to display a list containing dozens of commands.
...
A window is displayed at the lower right asking whether you want to open the closed repository.  Select **Open**.

All the repository's code files are displayed in the Visual Studio Code **Explorer** view.

> [!NOTE]
> An additional window might open at the lower right, suggesting that you install one or more Visual Studio Code extensions for the files in this workspace.  You can safely dismiss it for now by selecting the **Close** button (**X**).
...
- On the left is the name of the current working branch.  If you modify any tracked files in the working branch, the Status Bar adds an asterisk symbol (*) next to the branch name.
   ...
   ![Screenshot of the Status Bar with an arrow pointing to the asterisk next to the working branch name.](../media/3-exercise-clone-branch-03.png)
...
- On the right is an icon that switches from a circle with arrows (**Synchronize Changes**) to a cloud with an arrow pointing upward (**Publish Changes**).
   ...
   ![Screenshot of the Status Bar with an arrow pointing to the Synchronize Changes icon.](../media/3-exercise-clone-branch-04.png)

   And the following image shows you what the **Publish Changes** icon looks like:

   ![Screenshot of the Status Bar with an arrow pointing to the Publish Changes icon.](../media/3-exercise-clone-branch-05.png)
...
Start typing the name of the new branch, **update-response**.  As you type, notice that the existing branch names disappear.

After you type **update-response**, select <kbd>Enter</kbd>.  The Command Palette prompts you to select the branch that you will base your new branch on.  Select **master**.
...
When you save your changes to the file by using <kbd>Ctrl+S</kbd>, the blue circle will disappears and a new blue icon with a number appears on the Source Control Icon, indicating the number of pending changes (unstaged or staged) that are ready to be committed.  The number increments as you make more changes to the files.
...
- To quickly run `git pull` and `git push` commands on an existing branch, select the **Synchronize Changes** button.
- To quickly run a `git push` command on a new branch, select the **Publish Changes** button.
