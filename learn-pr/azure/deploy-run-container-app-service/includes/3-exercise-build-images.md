...
1. Specify the values in the following table for each of the properties, and then click **Create**:
...
1. Wait until the container registry has been created before continuing.

## Build a Docker image and upload it to Azure Container Registry

1. In the Cloud Shell window on the right, run the following command to download the source code for the sample web app. This web app is simple; it presents a single page containing static text and a carousel control that rotates through a series of images.
...
1. Run the following command. This command sends the folder's contents to Azure Container Registry, which will use the instructions in the Dockerfile to build the image and store it. Replace `<container_registry_name` with the name of the registry you created earlier. Take care not to leave out the `.` character at the end of the command.

    ```bash
    az acr build --registry <container_registry_name> --image webimage .
    ```

    The Dockerfile contains the step-by-step instructions for building a Docker image from the source code for the web app. Azure Container Registry runs these steps. The build process generates a number of messages as it proceeds. It should complete after a couple of minutes without any errors or warnings.

## Examine Azure Container Registry
...
2. Under **Services**, click **Repositories**. You should see a repository named **webimage**.

3. Click the **webimage** repository. It should contain an image with the **latest** tag. This is the Docker image for the sample web app.
 ...
 The Docker image containing your web app is now available in your registry for deployment to App Service.