...
Azure provides a robust platform for developers to build modern applications. However, cloud-based applications may face challenges such as latency, intermittent connectivity, and regulatory compliance. Operator Nexus offers the following benefits for operating secure carrier-grade network functions at scale:
...
- **Integrated platform for compute, network, and storage:** Operator Nexus provides an end-to-end (E2E) platform from infrastructure to tenant for applications, eliminating the need for separate provisioning of compute, network, and storage. For example, networks associated with the compute infrastructure can be automatically provisioned across the compute and network infrastructure without requiring other teams.
...
Operator Nexus is managed using the same tools that users already use for managing Azure subscriptions. Operators can choose an Azure region that supports Operator Nexus for any on-premises Operator Nexus instance. The diagram illustrates the component architecture of Operator Nexus.
...
The use of Azure tools enables the design, development, deployment, and management of services and applications in hybrid scenarios in a consistent manner, regardless of their location. This minimizes administrative, development, and operational overheads.

Operator Nexus is a Kubernetes-based platform that utilizes CNCF compliant Azure Kubernetes and other CNCF services, such as Kubevirt and Multus. This allows Telco operators to benefit from the latest advancements in cloud-based containerization and microservices technologies. Kubevirt, a Kubernetes-managed virtualization engine, enables both CNFs and VNFs to be hosted on the same bare metal servers.
