It's very common to execute a piece of logic at a set interval. For example, imagine you're a blog owner and you notice that your subscribers are not reading your most recent posts. You decide that the best action is to send an email once a week letting them know that there's new content to read. You decide that you want to implement this logic using an Azure Function and use a timer trigger to invoke your function every seven days.

Here, you will learn how to create a timer trigger and instruct the trigger to run at a consistent interval.
...
A timer trigger can be created completely inside the Azure portal. Inside your Azure Function, you'll select **timer trigger** from the list of predefined trigger types, enter your logic that you want to execute, and supply a *Timestamp parameter name* and *CRON expression*.
