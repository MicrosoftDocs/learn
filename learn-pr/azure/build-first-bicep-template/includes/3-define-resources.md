...
* Next, you'll give the resource a *symbolic name*. In the example, the resource's symbolic name is `storageAccount`. Symbolic names are used within Bicep to refer to the resource, but they won't show up in Azure.
* `Microsoft.Storage/storageAccounts@2019-06-01` is the *resource type* and *API version* of the resource. `Microsoft.Storage/storageAccounts` tells Bicep you're declaring an Azure storage account. `2019-06-01` is the version of the Azure Storage API Bicep will use when it creates the resource.
  > [!TIP]
