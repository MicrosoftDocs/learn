The web development team are busy finalizing the new company website.

In the past, you've used third-party vendors to register and host your domain details. Which means the domain name resides with the domain name registrar. One of the critical requirements of this redevelopment is to host the domain on your Azure servers.
...
Think of the DNS as a lookup table, much like using a phone book to find a telephone number. Only, in this case, the phone number is the IP address of the server hosting your website, web service, or load balancer.

The DNS uses a global directory hosted on servers around the world. Microsoft is part of that network, and provides a DNS service through Azure DNS.
...
- Maintaining a cache of recently accessed/used domain names and their IP addresses. Which enables it to provide faster responses to a local domain lookup request. If it cannot find the requested domain, it passes the request to another DNS server until either the domain is resolved or a reasonable search period has elapsed. The local domain name cache exists for a short period.
...
- If the domain cannot be found after a reasonable number of DNS checks, it responds with a "domain cannot be found" (404) error.

### IPv4 and IPv6

Every computer, server, or network enabled device on your network, whether on-premises or externally available to the internet has an Internet Protocol (IP) address. An IP address, within your domain, will be unique. There are two standards of IP address: IPv4 and IPv6. 
...
- **IPv6** is a relatively new standard and will eventually replace IPv4. It is made up of eight groups of hexadecimal numbers, each separated by a colon. For example: fe80:11a1:ac15:e9gf:e884:edb0:ddee:fea3.

Today, more and more network devices are provisioned with both an IPv4 and an IPv6 address. The DNS name server is now capable of resolving domain names to both IPv4 and IPv6 addresses.

### DNS settings for your domain

Whether the DNS server for your domain is hosted by a third-party or managed in-house, it will need to configure for the subdomains, web, email, and other services you're using.
...
In addition to three record types above there are also these record types: wildcards, CAA (Certificate Authority), NS (Name Server), SOA (Start of Authority), SPF (Sender policy framework), SRV (server locations), and TXT.

The SOA and NS records are created automatically when you create a DNS zone using Azure DNS.

### Record sets

In a typical configuration, there will be multiple record types. However, some record types support the concept of record sets, or resource record sets.  Record sets allow for multiple resources to be defined in a single record. For example, here is an A record that has one domain with two IP addresses:
...
## What is Azure DNS in Azure

Microsoft provides a globally distributed name server infrastructure for hosting and managing your domains called Azure DNS. Built on Azure monitoring technology, it allows seamless management of all your domains using your existing Azure credentials.

The Azure DNS will act as the **start of authority** (SOA) for your domain.

At present, you cannot use Azure DNS to register a domain name. You will still need to use a third-party domain registrar to register future domains.
...
At this time, Azure DNS does not support Domain Name System Security Extensions or DNSSEC.  If this security extension is required, you should host those portions of your domain with a third party.

### Security Features

Azure DNS provides a number of security features:

- Role-based access control, which gives fine-grained control for user access to Azure resources. You can monitor their usage, control the resources and services they have access to. It's great for segregating duties and controlling what your team can and cannot do.
...
Azure DNS is integrated into the Azure portal. Once you've logged in using your existing credentials, you will be able to manage your Azure DNS alongside your existing resources. Using Azure DNS seamlessly integrates into your exiting billing and subscriptions. 
