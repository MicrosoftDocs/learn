You're responsible for many production applications at Adatum, a company that designs and builds recreational submarines. Currently, Adatum has several three-tier applications that run on IaaS virtual machines (VMs). These applications have been migrated from on-premises datacenters that are hosted across various Azure virtual networks.
...
- You have one application on a single VM that doesn't experience variance in utilization and doesn't have an architecture that allows load balancing.
