...
- Deliver software features **quickly**: DevOps processes and tools enbrace the concept of failing fast. Bulding MVPs or prototypes to quickly validate whether the feature being worked on goes in the right direction is core to the concept of DevOps.
...
As you can see DevOps embodies the same core concepts of a healthy innovation culture: adopting its methodology is key to get to an agile innovation cycle.
...
- Applications can be rearchitected into microservices to allow innovating on their components idividually, without affecting the rest.
- Containers enable reliable application deploymens across multiple platforms and environments
- Kubernetes is a cloud-agnostic orchestration platform to run containerized applications
- Azure Application Services is an Azure offering that can run web-based workloads with a minimum management overhead. It offers many features like serverless or application slots, specifically targeted to speed up the innovation cycle.

Tailwind Traders has decided to start the redesign of their ecommerce application into a microservices architecture. The first application subsystem they will separate from the main "monolith" is the payment service, since this has been identified as a critical area where their competition are offering better value to customers. After the payment subsystem, additional application components will be converted into independent microservices. The microservices will talk to each other using REST APIs.
