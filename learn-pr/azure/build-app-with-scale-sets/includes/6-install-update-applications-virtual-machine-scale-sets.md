...
To use a custom script extension with the Azure CLI, you'll create a configuration file that defines the files to get and the commands to run. This file is in JSON format.  
...
You can use the same `az vmss extension set` command shown previously to deploy the updated app, but this time, you'll reference the new configuration file:
...
The VMs are updated according to the upgrade policy for the scale set. You'll specify this policy when you first create the scale set. The upgrade policy can have one of the following three modes:
