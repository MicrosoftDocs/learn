...
Here, you will deploy a simple Azure Resource Manager (ARM) template from a sample repository that deploys a Virtual Network (VNet) in your sandbox subscription.  Once the deployment is complete, you will then deploy a template that has the following changes made to it:

1. **Address space**. A change of the address space in a virtual subnet
1. **Subnet**. An additional Subnet in the virtual network
...
Now that you've deployed the template, you're ready to test the *what-if* operation. This time you will deploy a template that changes the virtual network you deployed in the first part.  The difference in the second templates are:

- The original tag was removed
- A subnet has been removed
- The address prefix has changed.
...
1. Use the following PowerShell command to run the deployment in complete mode, with a confirmation prompt:
