...
Administrators must rely on remote management for the administration and maintenance of an organization's Azure resources, which include VMs, and the apps installed on those VMs. It's important to consider the ability to connect securely to these resources and apps without exposing them to the internet. You can use Azure Bastion to remotely connect to and manage hosted VMs without exposing management ports to the internet. However, some administrators have addressed this requirement by using *jump servers*, which are sometimes called *jump boxes*. In this unit, you'll determine whether Azure Bastion can replace jump boxes as a method for providing secure remote management access.
...
- The network traffic flow between the internet, the jump box, and the target VMs is managed by an NSG.
...
The jump box is a VM running a server operating system, so you'll need to:
