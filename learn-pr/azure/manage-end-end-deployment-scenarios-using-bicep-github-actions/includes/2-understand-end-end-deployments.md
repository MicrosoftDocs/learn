...
Instead, it's often better to create a single workflow that deploys everything required for your solution, even if the components are managed by different people or different teams. You can use tools such as Git and GitHub to coordinate your work. When a new feature is added, you can use a branch to make the necessary changes to your Bicep file. And when the change is ready to be integrated and released, a single workflow performs all the steps necessary to build and deploy the solution, which reduces the chance of things getting out of sync.
...
When you create a deployment workflow that interacts with the data plane of your resources, you can use any of three common approaches: Resource Manager deployment scripts, workflow scripts, and workflow actions.
...
A workflow ordinarily creates and configures your Azure resources by deploying a Bicep file. The subsequent parts of the workflow then interact with the data plane of those resources. To be able to interact with the resources, the workflow actions and steps need information about the Azure resource that was created.
