...
To export data, you'll complete the following steps:
...
1. **Check job status**
   ...
   - Transfer data: Microsoft copies the data from the disks into the specified storage account. This can take some time, depending on the volume of data and number of disks. There's no SLA for this process, but it should be complete in 7 to 10 days after receipt of the disks. Microsoft updates the job status to indicate that the data is being transferred. When the transfer is complete, Microsoft changes the job status to indicate that the data is now available in Azure Storage.
