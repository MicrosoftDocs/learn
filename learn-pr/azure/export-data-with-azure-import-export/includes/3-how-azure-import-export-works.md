To use Azure Import/Export, you create a job that specifies the data that you wish to import or export. You then prepare the disks to use to transfer the data. For an import job, you write your data to these disks and send them to an Azure datacenter. Microsoft uploads the data for you. For an export job, you prepare a set of blank disks, and send them to an Azure datacenter. Microsoft copies the data to these disks and ships them back to you.
...
In this unit, you'll learn how to use your own disk drives to export data between your on-premises storage and Azure storage. You'll see how to create import and export jobs, and how you can monitor the progress of an import or export job.

## Export data from Azure

You can use the Import/Export service to export data from Azure blob storage only. You can't export data held in Azure file storage.
...
- WAImportExport version 1 installed from the Microsoft Download Center.
- An active shipping carrier account like FedEX or DHL. This is required to ship drives to an Azure datacenter.
- A set of disks that you can send to an Azure datacenter to copy the data from Azure storage.
...
1. **Ship**
   ...
   Send your disks to the Azure datacenter identified by the export job. The datacenter is identified based on the location of your storage account. You can check the number of disks required for the export job using the *PreviewExport* argument of the WAImportExport command. You provide the details of your export job as parameters to this command.
...
1. **Check job status**
   ...
   - Package disks: Microsoft prepares the disks for shipping.  The drives are encrypted using BitLocker.
   - Ship: Microsoft sends the disks back to the return address specified in the export job, and update the status of the job again.
...
1. **Prepare disks**
   ...
   1. Open a command prompt window. Move to the folder where you installed the WAImportExport tool. Run the following command to retrieve the BitLocker key for the drive:

      ```cmd
      manage-bde -protectors -get <DriveLetter>:
      ```

   1. Prepare the disks using the WAImportExport command. This command can take a significant amount of time (hours and possibly days), depending on the amount of data:
...
1. **Create job**
   ...
   Create an import job by using the Azure portal or the REST API. Provide the following information:
    - Name for the job
    - Storage account where to import the data
    - Shipping address of the Azure datacenter for the storage account
    - Return address where Microsoft should send the drive back
    - List of drives containing the data for the job
    - BitLocker key used to encrypt the data for each drive

1. **Ship**

   Send the drives to the Azure datacenter using one of the supported carriers like DHL or FedEx. The carrier provides a tracking number. You add this tracking number to the import job
...
1. **Check job status**
   ...
   - Transfer data: Microsoft copies the data from the disks into the specified storage account. This can take some time, depending on the volume of data and number of disks. There's no SLA for this process, but it should be complete between 7 and 10 days after receipt of the disks. Microsoft updates the status of the job to indicate that the data is being transferred. When the transfer is complete, Microsoft changes the status of the job to indicate that the data is now available in Azure storage.
   - Package disks: Once the data has been uploaded to Azure storage, Microsoft repackages your disks. The job status is updated.
   - Ship: Microsoft sends the disks back to you, using your selected carrier. The job status is changed to indicate that the job has completed.
