...
*Locking* refers to a client's ability to prevent other clients from updating a specific file or section of a file. NFSv3 does not have its own locking mechanism, and what locking that does exist is performed by ext network services called Network Lock Manager (NLM) and Network Status Monitor (NSM). These servers run on clients and services and manage all lock interaction. These services run outside of the NFS service and generate their own network protocol traffic. Further, these locks are not exclusive/mandatory but are rather known as *advisory locks*. The locks are tracked but not enforced. Applications are expected to handle contention and locking issues.

## NFSv4.x ##

NFSv4.x introduces some key improvements to NFS. For our purposes, we will focus on the main ones.

### NFSv4 and Kerberos ###

NFSv4 introduced full support for the use of Kerberos for authentication of users and encryption of all NFS traffic. There was an attempt to integrate Kerberos with NFSv3 but due to the use of NLM/NSM and other external protocols only the data payloads were encrypted.
...
A key consideration here is that not all NFSv4-compatible services support Kerberos. Without Kerberos, you are still able to use other features of NFSv4 but will not authenticate or encrypt data. When choosing your solution be sure to verify if you need this capability and if the options support Kerberos.

### NFSv4 and network ports ###

One advantage of NFSv4 over NFSv3 is that NFSv4 uses a single TCP port (port 2049) for all traffic. If you have an NFSv4 environment in an Azure vnet or located in your datacenter and need to provide access between firewalls, it is much simpler to support NFSv4.
...
NFSv4 introduces the concept of Access Control Lists (ACLs). Each file/folder can have an ACL applied to it. Each ACL consists of one or more Access Control Entries (ACEs). This model is familiar to Windows NTFS administrators as it follows the same model.

If your current environment uses NFSv4 ACLs, you will need to determine which storage solution best matches this requirement.
...
You will need to evaluate your current environment, determine your group management, network, and security requirements to determine the appropriate storage solution.

The table below is an NFS comparison based on the above discussion:
