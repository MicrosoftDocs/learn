...
You have a genomics workload that you'd like to run in Azure. Your data is generated by on-premises gene sequencers to a local NAS environment. On-premises researchers consume the data for various different uses. Perhaps those researchers also wish to consume the results of the HPC workload you intend to run in Azure, but some of those researchers use on-premises workstations to do so. Let's also assume that fresh genomic data is generated regularly, and so you have a limited interval to execute the current workload before the data needs replaced/refreshed.
...
The main questions when attempting to run HPC workloads in Azure include:
...
- If they do, are they able to operate on the data in Azure, or are they requiring the data is stored on-premises?
