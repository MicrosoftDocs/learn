...
The following image depicts the layers of a container image. You'll notice at the top layer is the read/write Flight Booking System for Airline Reservations web application layer, which is built on top of the previous read-only layers, all of which are resulting from the commands in the Dockerfile.
