...
Multi-stage builds handle processes very efficiently. Specifically, processes that have no dependencies are executed in parallel. For example, in this case, the download of Tomcat in Stage 0 and the installation and compilation of Maven in Stage 1 are completely independent processes. Therefore, when you execute `docker build`, the Tomcat process and the Maven process are carried out concurrently. However, the Final Stage copies the artifacts from Stage 0 and Stage 1, so it cannot be executed until the previous processes are completed. Thus, the Final Stage waits for the previous processes to finish and proceeds when it becomes possible to do so.
