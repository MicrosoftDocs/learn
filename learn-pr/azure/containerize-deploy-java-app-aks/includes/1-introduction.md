...
In this module, you’ll first containerize a Java app. For convenience, we’ve selected an existing Java app for you to use, from the open internet, shared  under the [MIT license](https://github.com/git/git-scm.com/blob/main/MIT-LICENSE.txt). [Flight Booking System for Airline Reservations](https://github.com/Azure-Samples/containerize-and-deploy-Java-app-to-Azure), a fully responsive, sample web-based Flight Booking System based on Turkish Airlines, built with the Model View Controller (MVC) Architecture made using Java Servlets, Java Server Pages (JSPs).

You’ll then construct a Dockerfile and write the Docker instructions needed to build a container image.

You’ll then run the container image locally and test the app.

You’ll then push the container image into Azure Container Registry and deploy it to Azure Kubernetes Service.
