You want to ensure the back up and restore jobs you put in place offer a way to recover your companies high risk servers. With this requirement in mind, you want to investigate the best way to implement back up for your VMs.
...
In this unit, you'll explore all the methods of backing up machines provided by Azure Backup, and make a decision on which to implement.

## Definition of a Snapshot

For Azure VMs, Azure Backup uses different extensions for each supporting operating system:
...
- **Crash-consistent consistency**:
    - Occurs typically if the VM in question is shut down at the time of the backup
...
## Definition of a Recovery Services vault
...
A Recovery Services vault is the storage used to manage and store the backup data in Azure. Typically a vault consists of data copies, configuration information for virtual machines, server, and workstation workloads. Recovery Services vaults can also be used to hold backup data for Infrastructure as a Service (IaaS) virtual machines. The vaults provide a single place to manage your backups, and are accessible in context to the resource they are protecting. For example, with the vault you can:

- Monitor Azure IaaS virtual machines backups; hybrid scenarios are also supported, so on-premises machines protected with Azure Backup can also be monitored
...
### Azure VM Backup agent (Azure Backup extension)

![Azure Backup Architecture](../media/3-azure-vm-backup-architecture.png)

The Azure Backup extension is the default backup choice, because it's the most simple and straight forward method to quickly back up an Azure VM. No other action is needed by the administrator other than to configure the backup job. During the first backup, a *VMSnapshot* (for Windows) or *VMSnapshotLinux* (for Linux) extension is installed. These extensions take snapshots of the VM's entire disk, which means they don't enable file or folder level restores. The snapshots are created and stored in an Azure Recovery services vault. 

If your company's VMs are in Azure, and you don't require file level restores, your company should use the Azure Backup extension.

### Microsoft Azure Backup Server (MABS) protection agent

![Azure MABS Architecture](../media/3-azure-mabs-backup-architecture.png)

The MABS agent installed on an Azure-based or on-premises VM allows that machine to back up to an Microsoft Azure Backup server. The MABS agent can back up and restore SQL and other application services. Workloads like Exchange and SharePoint can also be supported by using the **AzureBackupWindowsWorkload** extension. Machines and workloads are backed up to a Microsoft Azure Backup Server, although this doesn't move these backups to an Azure Recovery Services vault; if you need to move your backups to an Azure Recovery Services vault, you'll need to install the MARS agent.

### Microsoft Azure Recovery Services (MARS) agent

![Azure MARS Architecture](../media/3-azure-mars-backup-architecture.png)

The MARS agent is used by Azure Backup to back up **Windows** files, folders, and system state data to a Recovery Services vault. The agent is installed manually on the machines that you are backing up. The MARS agent enhances the levels of backup if installed alongside the Azure Backup extension on an Azure VM. MARS enables on-premises Windows machines to back up directly to an Azure Recovery Services vault.
...
1. The snapshot is transferred to your Recovery vault within Azure.
    ...
    - After the first full backup, Azure Backup will identify the blocks of data that have changed and only backup up that information rather than the whole VM a second time.
