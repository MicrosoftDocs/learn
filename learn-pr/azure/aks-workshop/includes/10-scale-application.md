...
1. Run the load test using the command below, setting the duration of the test to 120 seconds, simulating up to 500 requests per second.
 ...
 1. Watch the Horizontal Pod Autoscaler working.
    ...
    In a few seconds, we'll see the HPA transition to deploying more replicas, scaling up from 1 to 8 to accommodate the load. Use `CTRL+C` to stop watching.
...
1. Change the `resources.requests` and `resources.limits` for the container to be 2000 m, meaning one core. The section should now look like this.
...
1. When we configure the cluster autoscaler, we should see it dynamically adding and removing nodes based on the cluster utilization. To change the node count, use the `az aks update` command and specify a minimum and maximum value. Make sure to use the same resource group from earlier, for example, **aksworkshop**.
