...
- **Create Subscription**: You must follow the standard Azure procedure to [create a subscription](/azure/cost-management-billing/manage/create-subscription). Ensure you have owner of contributor-level access to this Azure subscription by navigating to **Home > Subscriptions > Your-subscription > Resource providers** in the Azure portal.
...
- **Allocate Subnet and IP addresses**: Azure Private 5G Core requires a management network, an access network, and one or more data network. These networks may be part of the same larger network, or they can be separate. The approach you use depends on your traffic separation requirements.
