...
> [!NOTE]
> At this point, the client library will has not attempted to connect to Azure or validate the connection string and access key being used.  It has simply constructed a lightweight client object used to perform operations against Azure Blob Storage.  Only when an operation is invoked against the storage account will a network call be made.
...
Now run the application to have it create the container in the storage account.  Since we used the `CreateIfNotExists()` method, you can run your program multiple times but the container will only be created on the first run.
...
Once you have a client object, you can use the methods on the client object to perform actual work. Methods that make network calls are intentionally asynchronous. The library uses Promises to return asynchronous results. For that reason, you need to mark your main function as async as shown below.
