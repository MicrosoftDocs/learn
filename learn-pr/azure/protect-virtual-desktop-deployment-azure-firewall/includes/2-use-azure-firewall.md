To prevent unauthorized network traffic in your Azure Virtual Desktop environment, you can limit outbound traffic with Azure Firewall. In this unit, you'll learn how Azure Firewall filters this traffic.
...
Azure Firewall denies traffic that reaches it unless an applicable rule allows the traffic. Route tables determine which traffic reaches the firewall. Firewall Policy rules then evaluate network and application traffic.
...
For Azure Virtual Desktop to work, session hosts need outbound access to required service and supporting endpoints. The host pool might also need outbound internet access for your users. You can use Azure Firewall to filter this outbound traffic. Keep gateway and broker traffic on a direct `WindowsVirtualDesktop` service-tag route to avoid disconnections during firewall scale-in.
