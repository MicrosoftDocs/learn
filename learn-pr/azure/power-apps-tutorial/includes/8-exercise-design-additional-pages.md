...
11. Replicate the same procedure for **Notes_carpets**.
    ...
    > [!Note]
    > We won't be including the **View in MR** feature for the **Carpets** category. For the **Notes_carpets** page, exclude adding the **Gallery control** to store photos taken during the **View in MR** session.
...
3. Add a **Button** at the bottom of the screen. Configure the **Text** property of the button by adding **Shop more**. Let's direct the user to the home page: select the **Shop more** button, and then add the following line in the **OnSelect** property.
...
1. Select the **Data** tab and select **+ Add data**. Expand the **Connectors** and then select **Office 365 Outlook** to add it as one of the connectors for this application.
...
2. Open **Order_products** and then configure the **OnSelect** property of the **Confirm** button by adding the following lines:
    ...
    > [!Note]
    > The names used in the above function follow the naming conventions used in this application at the time of development. You can customize the function according to your application.
