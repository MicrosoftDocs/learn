...
Service-level encryption uses Microsoft-managed keys by default unless you decide to onboard using Customer Key. The Microsoft-managed key option is currently enabled by default for Exchange Online, SharePoint Online, and OneDrive for Business. Your data is always encrypted at this default level at a minimum. Microsoft manages the cryptographic keys, including the root keys for service encryption.

With Customer Key, you supply the root keys for service encryption, and you manage these keys by using Azure Key Vault. Using the keys you provide, Customer Key encrypts:
