...
When you start a wizard to create an Azure Route Server, you will be presented with the following set of options:
...
Choose the instance name for the Azure Route Server, and then select the region where your virtual network resides. Ensure that you select the proper region, or you won’t be able to select a virtual network.
...
After creating the Azure Route Server object, you need to create its peers so it can exchange route information. In the Azure Route Server portal, you should navigate to **Peers**, and then select **Add a peer**. To add a peer, you need to provide a peer name, ASN number, and the IP address of the peer interface that’s accessible to Azure Route Server. The following screenshot depicts an example of these settings:

:::image type="content" source="../media/5-add-peer.png" alt-text="Add peer section with name, ASN, and IPV4 address boxes filled in.":::

After you add one or more peers to the Azure Route Server object, it will start learning and exchanging routes by using BGP. However, you can’t use the Azure portal to review these routes. If you want to check the routes that Azure Route Server received from its peers, you need to use the **Get-AzRouteServerPeerLearnedRoute** cmdlet. For this command, you need to provide the name of your Azure Route Server and resource group. You’ll also need to provide the name of a peer, as routes are listed on a per-peer basis. For example:
