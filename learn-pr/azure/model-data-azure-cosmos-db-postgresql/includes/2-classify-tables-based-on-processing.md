The trial of the Woodgrove Bank application went well, and now it is time to prepare the app for growth. Woodgrove Bank is expanding, adding a corporate office and branches throughout Europe. Their contactless payment app is about to see growth in the number of users as well as the locations. With a substantial growth in users, the amount of transactions will grow exponentially. This is a case where it makes sense to distribute the data across multiple servers rather than put the burden of growth on a single server. Azure Cosmos DB for PostgreSQL can assist in cases when coming from a standalone PostgreSQL environment and needing to grow at scale.

## Initial standalone relational data schema

As the Woodgrove Bank development team needed a quick proof of concept,  they trialed their payment application with a single, non-distributed PostgreSQL database. Prior to their trial, they initially had 2 tables:

* payment_users - Users using the application
* payment_events - Events for the users - such as GiftFunds, SendFunds, and RequestFunds

This was the initial database architecture:
...
Storing the data in a relational database, the Woodgrove Bank team realized they needed to normalize their data for better storage. They migrated their event types and merchants to their own tables. This is the normalized architecture:
...
![Diagram of an Azure Cosmos DB for PostgreSQL cluster with a coordinator node and two worker nodes. The worker nodes show separate pieces of data for the same table, grouped by their 'merchant_id' field.](../media/shards-illustrated.png)
...
![Diagram of SQL query processing in the Azure Cosmos DB for PostgreSQL environment. The SQL query enters a cluster via the coordinator node. The coordinator node directs the query to the required worker nodes.](../media/distributed-query-processing.png)
...
Before choosing a distribution column, it is important to be aware of how the tables and relationships from a standalone PostgreSQL translate to a distributed environment. There are many things to consider when migrating from a standalone environment to a distributed environment:
...
Each of these tables start on the coordinator node and are created using the regular PostgreSQL CREATE TABLE syntax.
...
Distributed tables can be joined with reference tables. However, distributed tables cannot be joined with local tables.
...
* There will be more users than merchants. A merchant can have a lot of payment events for a single user. A merchant can have a lot of users as customers.
...
* The merchants list is not anticipated to grow at a rate where duplication would hurt performance.

The `payment_merchants` table makes sense to be a reference table. So for the Woodgrove Bank tables, they have settled on the following:
