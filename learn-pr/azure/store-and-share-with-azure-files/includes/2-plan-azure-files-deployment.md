Moving a company's shared files into Azure Files, a service that enables you to create and use network file shares in the cloud, requires an analysis of the options and a plan for the implementation. Importantly, how are you going to access and update files? You could choose to replace your existing Server Message Block (SMB) file shares with their equivalent in Azure Files. The other option is to set up an instance of Azure File Sync, which offers more flexibility on how files are secured and accessed.

The finance company has given you guidance on their requirements. Their main concern to minimize the impact on existing applications, systems, and users. The company wants a drop-in replacement for their existing SMB protocol file shares. Their intention is no code changes will be required to support the moved data.

In this unit, you'll explore the features offered by Azure Files and understand the different ways you can access your files. A company's data needs to be resilient to failures. You'll investigate the different redundancy options and choose the right one for the finance company. Finally, knowing the Azure Files features you need, you'll create a plan to migrate the existing files.
