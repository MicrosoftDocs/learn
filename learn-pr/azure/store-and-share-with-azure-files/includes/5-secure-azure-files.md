Companies still have choices to secure and control the access to file shares even if you're using a shared key to connect to them.
...
In this unit, you'll investigate ways to secure access to Azure file shares from on-premises machines. You'll check to ensure that data is encrypted in transit to Azure. With secure access in place, you'll look at how the finance company can make use of Azure AD to control access to the file shares. As an extra level of protection for the shared files, you'll also explore the snapshot feature.

## Secure access from on-premises

Azure file shares only support mounting them from SMB version 3.0. There are known vulnerabilities with SMB 1.0, and Microsoft recommends you either uninstall or disable this version from all the machines you're responsible for.

If you need to remove SMB 1, use this PowerShell command: `Remove-WindowsFeature -Name FS-SMB1`.

When mounting Azure files shares, Windows needs to communicate over port 445. Organizations may block network access over 445 by default. If your organization has this policy, consider requesting to open the port. There's nothing inherently insecure about this specific port, and with SMB 1 disabled, the risks of an SMB-based attack dramatically reduces.

### Configure IP-based firewall rules

To limit access to your on-premises networks, you'll need to know your public-facing IP address. With this information, enable the storage accounts firewall to allow access from selected networks. You can then add your public IP address to the list of allowed addresses. You can do this in the portal, or use a PowerShell command:
...
Using the commands in the previous exercise creates an Azure storage account with secure transfer enabled by default. If for some reason your file shares have been created without this feature enabled, it's easy to do that in the portal. 
...
The finance company is running their new reporting application on a VM hosted on Azure, and that's why they'd like to set up Azure AS DS authentication to access the file shares. They'd like to do this in advance of them moving more of their processing into the cloud.

The benefits of Azure AD DSi are that you can manage access with role-based access controls, and files inherit their existing NTFS DACLs.
