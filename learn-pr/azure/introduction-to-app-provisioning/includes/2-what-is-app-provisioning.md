...
Microsoft Entra ID also supports provisioning users into applications hosted on-premises or in a virtual machine, without having to open firewalls. If an application supports System for Cross-domain Identity Management (SCIM), or you've built a SCIM gateway to connect to your legacy application, you can use the Microsoft Entra provisioning agent to directly connect with your application and automate provisioning and deprovisioning. If you have legacy applications that don't support SCIM and rely on a Lightweight Directory Access Protocol (LDAP) user store or a SQL database, Microsoft Entra ID can support those as well.
