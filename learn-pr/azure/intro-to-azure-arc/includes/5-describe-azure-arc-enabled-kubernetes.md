...
1. Meet the prerequisites. You'll need:
    ...
    - To meet a number of [network requirements](https://aka.ms/networking-requirements?azure-portal=true).
...
> [!NOTE]
> For more detailed information about connecting an Azure Arc-enabled Kubernetes cluster, follow the guidance in the [Connect an Azure Arc-enabled Kubernetes cluster (Preview)](https://aka.ms/connect-cluster?azure-portal=true) tutorial.
...
- [Deploy configurations using GitOps on Arc enabled Kubernetes cluster (Preview)](https://aka.ms/use-gitops-connected-cluster?azure-portal=true).
- [Deploy Helm Charts using GitOps on Arc enabled Kubernetes cluster (Preview)](https://aka.ms/use-gitops-with-helm?azure-portal=true).
