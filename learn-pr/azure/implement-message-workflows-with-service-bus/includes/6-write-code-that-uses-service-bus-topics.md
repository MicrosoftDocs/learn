...
Let's describe what happens when a user cancels a bicycle order. This is a little different from placing an initial order, where our workflow wanted to wait until the order cleared payment processing before sending the order to the local storefront. For the cancel operation, we're going to notify both the storefront *and* the payment processor at the same time. This approach minimizes the chances that we waste delivery driver time.
