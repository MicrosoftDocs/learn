You've decided to use an Azure Service Bus topic to distribute sales performance messages in your salesforce application. Sales personnel use the app on their mobile devices to send messages that summarize sales figures for each area and time period. Those messages are distributed to web services that are located in the company's operational regions, including the Americas and Europe.

You've already implemented the necessary infrastructure in your Azure subscriptions for the topic. Now, you want to write the code that sends messages to the topic and write code that retrieves messages from a subscription. Then, you send a message to a topic and retrieve the message for a specific subscription.
...
1. In the Code Explorer window to the left, expand **performancemessagesender**. 
...
1. In the Code Explorer, right-click **performancemessagesender**, and then select **Open in Integrated Terminal**. 
1. In the terminal pane, at the command prompt, confirm that you are in the **performancemessagesender** folder, and then enter the following command: `dotnet build`.  
1. Now, run the application by running the following command: `dotnet run`. As the program runs, messages are printed to the console indicating that the app is sending a message. Each time you run the app, another message is added to the topic, and a copy becomes available for each subscription.
...
If you replace `Americas` with `EuropeAndAsia` and run the command again, you see that both subscriptions have the same number of messages.
...
1. In the Code Explorer page, expand **performancemessagereceiver**. 
...
1. In the Code Explorer, right-click **performancemessagereceiver**, and then select **Open in Integrated Terminal**. 
1. In the terminal pane, at the command prompt, confirm that you are in the **performancemessagereceiver** folder, and then enter the following command: `dotnet build`.  
...
If you replace `Americas` with `EuropeAndAsia` in this code to see the current message count for the `EuropeAndAsia` subscription, you see that the message count is `1`. In the preceding code, only `Americas` was set to retrieve topic messages, so that message is still waiting for `EuropeAndAsia` to retrieve it.
