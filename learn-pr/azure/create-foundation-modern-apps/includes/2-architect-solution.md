...
Knowing that such feed is available, the other thing to do is to figure out how to be notified when a bus is close enough to you so that you know that you have to start to walk to the bus station to make sure you'll catch the bus on time. One option to do that is to create a [geofence](https://en.wikipedia.org/wiki/Geo-fence) around a couple of buses stops before yours, for example, so that you can be notified when the bus is leaving it. If you can even get a notification when that happens, you don't even have to keep checking on a map where the bus is. When you'll get the notification, you'll now it's time to leave.
