...
As database is one of the main part of a solution, ability to have it part of such practices is a key point of a modern and agile application.
...
Having the database part of a CI/CD pipeline means that you want to deploy to create or update the structure - and maybe even some of the data - of your database as a fully automated, reproducible and determinist manner, so that you can do this process any time you need - even many times per day - and get consistent results.
...
Another option, is when you don't actually write T-SQL code, but use C# or Python or Node and the entities defined in you solution (for example: a Book, a Customer) to automatically generate the database for you. Using a specific tool, that usually comes with the platform or the framework you are using, you can make sure that every time you change any of the used entities - for example you add a new field, or even a new entity altogether - their new structure will be reflected to the database. Here's some reference to well know languages, tools and frameworks that supports a Code-First approach:
...
With this approach, the developer takes care of writing and maintaining all the scripts needed to create and change the database over time. After a script has been deployed in productions it is never changed and a new one is created. Each script contains the code needed to evolve the database to the new schema. One a completely new database needs to be deployed, all scripts must be executed in the correct sequence to make sure database is created and the evolved correctly. Tools like [DbUp](https://dbup.github.io/) helps in making sure that once a script has been deployed, is not applied again to the same database in subsequent executions.
