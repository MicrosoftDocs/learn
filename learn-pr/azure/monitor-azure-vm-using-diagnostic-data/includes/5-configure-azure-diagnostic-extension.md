Azure Monitor includes VM Insights, curated workbooks of useful VM metrics visualizations for managing your VM health. VM Insights also collects client and workload metrics. In this unit, you enable VM Insights and use it to monitor host performance and guest OS metrics.
...
VM Insights installs *Azure Monitor Agent*, an agent that collects data from inside VMs. You use data collection rules (DCRs) to select which guest metrics Azure Monitor Agent collects and where it sends the data.
...
1. For **Data collection rule**, keep the default. Ensure that **Processes and dependencies (Map)** is set to **Enabled**.
1. Select **Configure**.
...
When the deployment finishes, confirm that the Azure Monitor Agent and the Dependency Agent are installed by looking under **Extensions + applications** at the bottom of the VM's **Overview** page, on the **Properties** tab.
