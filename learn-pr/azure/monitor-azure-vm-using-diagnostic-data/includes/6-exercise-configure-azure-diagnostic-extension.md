The [Azure Monitor Agent](/azure/azure-monitor/agents/agents-overview) that you installed for VM Insights uses a Data Collection Rule (DCR) to provide a comprehensive view of client performance data in its prebuilt workbook. While metrics monitor system performance, to understand the reason for a performance anomaly, you have to analyze log data to see which events caused or contributed to the anomalous data. In this unit, you set up a DCR that uses the Azure Monitor Agent to collect Linux VM Syslog event data.
