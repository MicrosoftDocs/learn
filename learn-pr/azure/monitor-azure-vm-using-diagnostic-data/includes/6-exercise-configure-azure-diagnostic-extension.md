This unit describes how you can use Azure Monitor Data Collection Rules (DCRs) to collect event log data from VM clients. The [Azure Monitor Agent](/azure/azure-monitor/agents/agents-overview) that you installed for VM Insights already uses a DCR to provide a comprehensive view of performance data in its prebuilt workbook. Now, you set up a DCR to collect Linux VM Syslog event data.
