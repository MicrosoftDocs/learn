...
Azure offers a straightforward and flexible way to replicate services and network loads to the cloud. As the cloud is off premises, it is not impacted by the same risks that on-premise data centers can face such as earthquakes and loss of power. Azure offers two tools that assist in providing DR to on-premises environments, **Azure Site Recovery**, and **Azure Backup**.
...
Relevant features of Azure Site Recovery are:
...
<!-- TODO replace the above taken from https://docs.microsoft.com/en-us/azure/site-recovery/physical-azure-architecture -->

There are several components that need to be set up to enable Site Recovery, do them both in the Azure portal and the on-premises environment:

- **Networking**:  A valid Azure network is required, unless otherwise specified the migrated VMs connect to it by default 
...
![Azure Migrate Architecture](../media/2-replication-architecture.png)
<!-- TODO replace the above taken from https://docs.microsoft.com/en-us/azure/site-recovery/physical-azure-architecture -->
