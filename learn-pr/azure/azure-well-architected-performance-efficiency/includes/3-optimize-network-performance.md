Network performance can have a dramatic impact on a user's experience. In complex architectures with many different services, minimizing the latency at each network hop can have a huge impact on the overall performance. In this unit, you'll learn about the importance of network latency and how to reduce it within your architecture. We'll also discuss strategies to minimize network latency between their Azure resources as well as between their users and Azure.

## The importance of network latency

Latency is a measure of delay. Network latency is the time that it takes for data to travel between a source to a destination across a network, and the time that it takes for data to travel from the source to a destination and for the destination to respond is commonly known as a round-trip delay.

In a traditional datacenter environment, latency may be minimal since resources often share the same location and a common set of infrastructure. The time taken to get from source to destination is typically lower when resources are physically close together.

In comparison, a cloud environment is built for scale. Cloud-hosted resources may not be in the same rack, datacenter, or region. This distributed approach can have an impact on the round-trip time of your network communications. While all Azure regions are interconnected by a high-speed fiber backbone, the speed of light is still a physical limitation. Calls between services in different physical locations will still have network latency directly correlated to the distance between them.

On top of this, depending on the communication needs of an application, more round trips may be required. Each round trip comes with a latency tax, with each round trip adding to the overall latency. The following illustration shows how the latency perceived by the user is the combination of the roundtrips required to service the request.
...
Imagine that you work for a healthcare organization that is pilot testing a new patient booking system. This system is running on several web servers and a database, all of which are located in the West Europe Azure region, and the scope of your pilot test is available only for users in Western Europe. This architecture minimizes your network latency, as all of your resources are colocated inside a single Azure region.

Suppose that your pilot testing of the booking system was successful, and the scope of your pilot test has been expanded to include users in Australia. When the users in Australia view your website, they will incur the additional round-trip time that is necessary to access all of the resources that are located in West Europe, and their end-user experience will be decreased to due to the additional network latency.

To address your network latency issues, your IT team decides to host another front-end instance in the Australia East region. While this design helps reduce the time for your web servers to return content to end users in Australia, their experience is still decreased since there's significant latency for data that is being transferred between the front-end web servers in Australia East and the database in West Europe.
...
The goal here is to minimize the network latency between each layer of the application. How you will improve your network latency depends on your application and data architecture, but Azure provides mechanisms to resolve latency issues through several services.

## Latency between users and Azure resources

You've looked at the latency between your Azure resources, but you should also consider the latency between your users and your cloud application. You're looking to optimize delivery of the front end-user interface to your users, so let's take a look at some ways to improve the network performance between your end users and your application.

### Use a DNS load balancer for endpoint path optimization

In our example scenario, your IT team created an additional web front-end node in Australia East. However, end users have to explicitly specify which front-end endpoint they want to use. As the designer of a solution, you want to make the experience as smooth as possible for users.

Azure Traffic Manager could help. Traffic Manager is a DNS-based load balancer that enables you to distribute traffic within and across Azure regions. Rather than having the user browse to a specific instance of your web front end, Traffic Manager can route users based upon a set of characteristics:
...
Traffic Manager profiles can also be nested. For example, you could initially route your users across different geographies (such as Europe and Australia) using geographic routing, and then route them to local front-end deployments using the performance routing method.

Recall that the organization in our example scenario has deployed a web front end in West Europe and another front end in Australia. Let's assume that they have deployed Azure SQL Database with their primary deployment in West Europe, and a read replica in Australia East. Let's also assume the application can connect to the local SQL instance for read queries.
...
Your website will likely be using some form of static content, either whole pages or assets such as images and videos. This static content could be delivered to end users faster by using a content delivery network (CDN), such as Azure CDN. 
...
You can always use the public internet to connect users to your services, but internet performance can vary and may be impacted by outside issues. On top of that, you may not want to expose all of your services over the internet, and you may want a private connection to your Azure resources. Site-to-site VPN over the internet is also an option, but VPN overhead and internet variability can have a noticeable impact on network latency for high throughput architectures.
...
If we consider our example scenario once again, your team decides to further improve end-user experience for users who are in their facilities by provisioning an ExpressRoute circuit in both Australia East and West Europe. This gives their end users a direct connection to their booking system, and ensures the lowest latency possible for their application.

Considering the impact of network latency on your architecture is important to ensure the best possible performance for your end users. In this unit, we've taken a look at some options to lower network latency between end users and Azure and between Azure resources.
