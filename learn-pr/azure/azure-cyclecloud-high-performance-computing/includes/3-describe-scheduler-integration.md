...
Azure CycleCloud facilitates deployment of schedulers in Azure, which, in turn, distribute and manage jobs running on clusters composed of Azure resources. Effectively, Azure CloudCycle does not function as a scheduler. Rather, it functions as an intermediary between schedulers and the underlying platform. However, as that intermediary, Azure CycleCloud also simplifies development of autoscaling functionality for the corresponding schedulers by providing a REST API-based programming interface and a Python-based client library. 
