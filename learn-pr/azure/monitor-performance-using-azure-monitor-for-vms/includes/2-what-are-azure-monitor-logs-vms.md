...
| ---- | ---- | ---- |
| Azure Monitor Agent | Collects monitoring data from guest operating systems on VMs and delivers data to Azure Monitor logs and/or metrics | Over time, this agent will replace the Log Analytics Agent and Azure diagnostic extension listed below. While this agent introduces new capabilities, it does not support all the monitoring scenarios covered by earlier agents. You should understand the [tradeoffs](https://docs.microsoft.com/azure/azure-monitor/agents/azure-monitor-agent-overview#should-i-switch-to-azure-monitor-agent) before switching to the Azure Monitor agent. |
| Log Analytics agent | Collects logs and performance data for virtual machines in Azure, other clouds, or on-premises | Allows for the onboarding of Azure Monitor VM Insights, Azure Security Center, and Azure Sentinel. The agent also works in conjunction with Azure Automation accounts to onboard Azure Update Management and Azure Automation State Configuration, along with Azure Automation Change Tracking and Inventory.
