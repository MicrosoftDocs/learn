...
| Dashboards | You can pin the results of any query to an Azure dashboard. A pinned query on a dashboard allows visualization of log and metric data together.
| Export | You can import Azure Monitor data into Excel or Power BI. Prior to the import, you'll create a log query that defines all data to export.
| PowerShell | You can run a PowerShell script from a command line or an Azure Automation runbook. The script retrieves log data from Azure Monitor and determines the data to collect based on the query.
...
Tabular operators are predominantly how you'll interact with and query monitoring data. Tabular operators such as count, distinct, evaluate, join, limit, and project are all relevant in formulating queries. Because the KQL was originally written for Azure Data Explorer, there are a number of additional ways to manipulate data that are outside the scope of this Learn module. The language is powerful and can perform complex analysis on generated data.
...
You can use a preview pane that provides insight into a table's content, along with additional information and metadata.
...
Starting with a table name is the correct way to configure log queries. Using a table name defines a clear scope for the query, which improves query performance and result relevance.

The KQL is case sensitive. Language keywords are typically written in lowercase, such as *consume*, *count*, *distinct*, *evaluate*, *join*, *limit*, and *project*. When you use table or column names in a query, make sure to use the correct case. If you're ever stuck, the schema pane will show how to use correct casing for the table name.

## Basic query understanding

One of the tables Azure Monitor captures is the Heartbeat table. This table contains a number of useful columns. Heartbeat captures data on everything, including OS type, OS major version, resource ID, and resource group. To understand all the columns that make up this table, run a query with only Heartbeat.
...
In the results pane at the bottom of logs, you'll find some of the following fields in the preceding screenshot: TimeGenerated [UTC], SourceComputerId, ComputerIP, Computer, Category, OSType, and so on. Each of these columns is a data point on which you can filter.

You can apply the tabular operators to help the query return more relevant and specific information you want to examine. Recall that tabular operators help you filter on extracted data. There seem to be many results received from Heartbeat in this example. You know from the previous module that only two virtual machines are reporting to the Log Analytics workspace. To make sure there are only two virtual machines reporting to the workspace, let's add the distinct operator to this query.
