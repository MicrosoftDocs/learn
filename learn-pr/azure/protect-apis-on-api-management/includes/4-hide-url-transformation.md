...
The Government census API exposes details about the URL from which the API is being called from. This could allow a malicious user to attempt to access the census data by bypassing the API Management gateway and exposing a less secure endpoint.As lead developer, you want to mask these URLs within the response body of the API.

Here, you will learn how to use API Management policies that manipulate the content of API reponse headers and bodies.

## Why transform a response?

The response body of an API call contains the data that is being requested. In the case of the Census API, for example, the response body contains the JSON data for the respondants. You can also see how the body contains URL links to view individual people:
...
To achieve this, we will create a transformation policy.
...
| **Mask URLs in content** | Re-writes links in the response body so that they point to the equivalent link through the gateway. |
