...
Often in a legacy IT environment, one backup solution with more or less one configuration. Azure makes it easy to deploy a backup vault for each workload if needed. With seperate backup vaults it also becomes easy to handle both permissions to backup data and show back for the backup cost.
