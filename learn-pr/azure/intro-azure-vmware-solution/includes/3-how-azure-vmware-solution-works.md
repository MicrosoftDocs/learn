...
In partnership with VMware, Microsoft covers the lifecycle management of VMware software (ESXi, vCenter, and vSAN). Microsoft also works with VMware for the lifecycle management of NSX-T appliances and bootstrapping the network configuration, such as creating the Tier 0 gateway and enabling north/south routing. 
...
You can use Azure Storage services in workloads that are running in your private cloud. These services include:
- Storage accounts 
- Azure Table Storage
- Azure Blob Storage
 
The following diagram shows the available Azure Storage services that you can use with Azure VMware Solution.

:::image type="icon" source="../media/3-storage-overview.png" border="false" alt-text="Diagram that shows Azure VMware Solution and the available Azure Storage services for it.":::
