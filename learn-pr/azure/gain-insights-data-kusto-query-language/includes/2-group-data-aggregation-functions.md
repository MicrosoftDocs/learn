...
An aggregate function allows you to make these comparisons by grouping values of multiple rows to form a single summary value. The type of summary value depends on the specific function you use, and could be an average, maximum, minimum, or median, just to name a few. 
...
In the next units, we'll learn about some of the most common aggregate functions, and then build complex queries using the `let` statement to gain insights into a sample dataset containing US meteorological data.