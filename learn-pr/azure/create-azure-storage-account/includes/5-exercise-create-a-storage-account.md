...
1. Select **Next : Advanced**. Enter the following values for each setting.
    ...
    | Minimum TLS version | *Version 1.2* from dropdown list. TLS 1.2 is the most secure version of TLS and is used by Azure Storage on public HTTPS endpoints. TLS 1.1 and 1.0 is supported for backwards compatibility. See *Warning* at end of table. |
...
1. Select **Next : Data protection**. Enter the following value for the setting.
    ...
    |---|---|
    | **Recovery**| |
    | Enable soft delete for blobs | *Uncheck*. Soft delete lets you recover your blob data in many cases where blobs or blob snapshots are deleted accidentally or overwritten. |
