Protecting your cloud assets is one of the primary goals for security groups. One of the primary ways unauthorized users get access to systems is by obtaining a valid username/password combination. Azure can help mitigate this with several features of Microsoft Entra ID, including:
...
* **Microsoft Entra password protection**: You can block commonly used and compromised passwords via a global banned-password list.

* **Microsoft Entra smart lockout**: Smart lockout helps lock out malicious hackers who are trying to guess your users’ passwords or use brute-force methods to get in. It recognizes sign-ins coming from valid users and treats them differently than the sign-ins of malicious hackers and other unknown sources.
...
Using Microsoft Entra multifactor authentication increases identity security by limiting the impact of credential exposure. To fully authenticate, a malicious hacker who has a user's password would also need their phone or their fingerprint or face. Authentication with only a single factor is insufficient, and without authentication from Microsoft Entra multifactor authentication, a malicious hacker is unable to use those credentials to authenticate. You should enable Microsoft Entra multifactor authentication wherever possible, because it adds enormous benefits to security.
