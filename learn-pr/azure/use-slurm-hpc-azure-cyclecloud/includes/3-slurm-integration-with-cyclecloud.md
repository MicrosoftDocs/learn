...
- Cluster Configuration: When creating a new cluster in Azure CycleCloud, you can choose Slurm as the cluster scheduler. This means that CycleCloud will configure the cluster resources (e.g., VMs, storage, networking) and deploy the Slurm scheduler on the master node (or controller node) of the cluster.
