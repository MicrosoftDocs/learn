...
Suppose that your lenses database, which is implemented in Azure Storage tables, has been working well. The company has recently expanded into several new countries, and you've found that the performance of the database isn't always as good in those countries as it is in your country. Your technical director has asked you to determine if there's a way to eliminate these problems without rewriting the app that uses the database.

Here, you'll learn how Azure Cosmos DB provides a more scalable alternative to Azure Storage tables and doesn't require developers to rewrite code.
...
In this module, we'll concentrate on Azure Storage tables. This service provides a way to store semi-structured data in the cloud. The data is highly available to clients because it's replicated to multiple nodes or locations.
...
| **Zone-redundant storage** | Data is replicated to three storage clusters in a single region. This data remains available if a single data center fails. Your data may be unavailable if there's a region-wide outage.
| **Geo-redundant storage** | Data is replicated to a secondary region, hundreds or thousands of miles from the primary region. This data remains available even if there's a region-wide outage.
...
There are some differences in behavior between Azure Storage tables and Azure Cosmos DB tables to remember if you're considering a migration. For example:

- You're charged for the capacity of an Azure Cosmos DB table as soon as it's created, even if that capacity isn't used. This charging structure is because Azure Cosmos DB uses a reserved-capacity model to ensure that clients can read data within 10 ms. In Azure Storage tables, you're only charged for used capacity, but read access is only guaranteed within 10 seconds.
- Query results from Azure Cosmos DB aren't sorted in order of partition key and row key as they are from Storage tables.
...
- Table names are case-sensitive in Azure Cosmos DB. They aren't case-sensitive in Storage tables.
...
Each organization has different priorities for their NoSQL database system. Once you've identified those priorities, use this table to help you choose whether to use Azure Storage tables or Azure Cosmos DB tables to persist data for your applications:

| Priority | Azure Storage tables | Azure Cosmos DB tables |
| --- | --- | --- |
| Latency | Responses are fast, but there's no guaranteed response time. | < 10 ms for reads, < 15 ms for writes. |
...
| Data consistency | Strong in the primary region. If you're using read-access geo-redundant replication, it may take time for changes to reach the secondary region. | You can choose from five different consistency levels depending on your needs for availability, latency, throughput, and consistency. |
...
If you've decided to move to Azure Cosmos DB, and you currently have data in one or more Azure Storage tables, you must consider how to move that data into Azure Cosmos DB. Microsoft provides two tools to complete this task:
