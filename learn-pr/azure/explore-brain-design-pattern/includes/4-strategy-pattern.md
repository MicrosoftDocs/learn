...
Because each of these scenarios has completely different goals, each of these phases has a different set of strategies that are used during each scenario. Typically, you would use opening strategies for the opening phase, mid game strategies for the mid game, and check-mating strategies for the end game. Each strategy shares the same control actions, i.e. moving the chess pieces on the chess board.
...
Heuristics strategies are important to discover from subject matter experts as they will be the skills or modules of your brain design.
...
| Rail     | The yard is "dirty" with misplaced containers            | Trains cannot be loaded until containers are moved         | Allocate more hostlers to move containers within the yard    |
