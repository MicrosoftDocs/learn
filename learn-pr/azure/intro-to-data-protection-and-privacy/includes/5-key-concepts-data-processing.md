...
- Customer data. All data, including text, sound, video, image files, or software that is provided by, or on behalf of, a customer through use of the enterprise service. Customer data includes:

  - Identifiable information of end users; for example, user names and contact information in Azure Active Directory (Azure AD)

  - Customer content that a customer uploads to or creates in specific services. Some examples include customer content in an Azure Storage account, customer content of an Azure SQL database, or a customer's virtual machine (VM) image in Azure Virtual Machines.
...
A controller is obligated to promptly consider each DSR and provide a substantive response either by taking the requested action, or by providing an explanation for why they cannot accommodate the DSR. A controller should consult with their own legal or compliance advisers regarding the proper disposition of any given DSR. Completing a DSR might involve several processes that are subject to your organization's GDPR compliance rules. These processes include:
