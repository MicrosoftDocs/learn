...
- A unique name inside the workflow. In the previous example, the trigger is named *manual*, but you could replace this with a more meaningful identifier.
...
The following example shows the definition of another *Request* trigger that is for a Logic App receiving HTTP POST requests. A POST request typically provides a request body, containing the data to be posted. The request body in this example contains a customer name and address, comprising the street and city.
...
| *Workflow* | Runs another Logic App. |
