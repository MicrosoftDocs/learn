...
- **High availability**. Using HA configurations provides better control on application uptime for planned and unplanned failovers. If there's a failure in one VM, the other VMs can maintain full access to the disk.
...
- Shared managed disks don't natively offer a fully managed file system that's accessible by using a server message block (SMB) or an NFS. You must use a cluster manager, such as Windows Server Failover Cluster (WSFC) or Pacemaker, that manages cluster-node communication and write locking.
