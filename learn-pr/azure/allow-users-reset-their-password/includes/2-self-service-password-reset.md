...
It's critical to verify the identity of a user before you allow a password reset. Malicious users may exploit any weakness in the system to impersonate that user. Azure supports six different ways to authenticate reset requests.

As an administrator, you choose the methods to use when you configure SSPR. It's highly recommended that you enable two or more of these methods so that users can choose the ones that they can use easily. The methods are:
...
For the security question method, you can specify a minimum number of questions that the user must set up to register for this method, and a minimum number of questions that they must answer correctly to reset their password.
...
- The security question option is the least recommended method because the answers to the security questions may be known to other people. Only use the security question method in combination with at least one other method.
...
There are several editions of Azure AD: Free, Premium P1, and Premium P2. The password reset functionality you can use depends on your edition.
