...
You can deploy SSPR with password writeback by using [Azure AD Connect](active-directory/authentication/tutorial-enable-sspr-writeback.md) or [cloud sync](azure/active-directory/authentication/tutorial-enable-cloud-sync-sspr-writeback.md), depending on the needs of users. Each option can be deployed side-by-side in different domains to target different sets of users. This helps existing users on-premises to writeback password changes while adding an option for users in disconnected domains because of a company merger or split. Users from an existing on-premises domain can use Azure AD Connect while new users from a merger can use use cloud sync in another domain. Cloud sync can also provide higher availability because it doesn't rely on a single instance of Azure AD Connect. For a feature comparison between the two deployment options, see [Comparison between Azure AD Connect and cloud sync](/azure/active-directory/cloud-sync/what-is-cloud-sync.md#comparison-between-azure-ad-connect-and-cloud-sync).
