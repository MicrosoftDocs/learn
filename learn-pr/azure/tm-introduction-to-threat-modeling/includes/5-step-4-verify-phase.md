...
Then, make sure the default configuration settings from Cloud Providers, Operating Systems and Components are changed to meet all security requirements. Examples include enabling Azure SQL DB transparent data encryption to protect data on disk, use Role Based Access Control (RBAC) to assign permissions to users, groups and applications and enable Windows Firewall across all profiles.
...
The last part involves running both manual and automated tests. At Microsoft, services are subject to a verification process before deployment, and may consist of automated scanners, code reviews, and penetration tests. The process can be enforced before each deployment or across time intervals, like every 6-12 months. You may want to have shorter verification cadences if you answer yes to any of the following questions:
