...
|**System environment**|Will the system be built on the cloud or on-premise? On which OS will it be built? Will containers be used? Is the system an application, service, or something entirely different?|
...
|**Identity & access control**|How will the system help secure those accounts? Will it rely on Microsoft Entra ID? Will it use features like Access Control Lists (ACL), multi-factor authentication (MFA), and Session control?|
...
|**Logging, monitoring and backing up**|What are the mechanisms the system uses to log security events, monitor for anomalies, and back up system data? Which event types will it capture?|
...
|**Data**|What type of data will the system create or handle? What will the data classification type be? How will the system trust data sources? How will it parse data? What will be the expected input and output behaviors? How will validation be handled? How will data be encrypted across all states?|
...
> [!NOTE]
> If you have a dedicated security team, schedule a whiteboarding session with them to go over the initial design. It will save you a considerable amount of time.
