...
Host Groups are the first unique layer in a dedicated host deployment. They're a convenient way to visualize, organize, and manage a dynamic environment which can mix different hosts.
...
Size families are a list of related VM sizes which would normally run on a shared machine. You'll only be able to provision VMs on the host which are in the same VM family. If you want different hardware that's beyond what your selected size family can support, you'll have to add another host to your host group.
