...
1. Run the `az deployment group create` command to deploy the template:
    ...
    The command can take a few minutes to run. While the command runs, you can [examine the ARM template](https://github.com/Azure/azure-quickstart-templates/blob/master/quickstarts/microsoft.compute/vm-simple-linux/azuredeploy.json) from a separate browser tab if you'd like.
