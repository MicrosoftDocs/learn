...
- A backend pool of VM addresses
...
### Front-end IP address and backend pool
...
Load Balancer selects the IP address of a VM from the backend pool using an algorithm designed to distribute requests evenly. Load Balancer generates a hash by using the following information:
