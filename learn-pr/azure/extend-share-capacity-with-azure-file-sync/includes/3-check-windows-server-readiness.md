...
In this unit, you'll see what the system requirements are for servers that you want to use with Azure File Sync. You'll see how to run system checks by using PowerShell.
...
Use the Azure File Sync evaluation tool to evaluate the system's OS and files to ensure compatibility with the service. This tool is an Azure PowerShell cmdlet that checks for potential problems with the file system and data sets, like unsupported characters or OS version.
...
Use PowerShell to download the latest versions of `PackageManagement` and `PowerShellGet`. These packages allow the installation of preview modules. You can then install the `Az.StorageSync` PowerShell module that allows the data set and system requirements to be tested and saved in a comma-separated file.
