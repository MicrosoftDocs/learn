...
The most common cause for files not syncing is invalid file names. The assessment tool should help you identify problems with existing file names. However, if new files are created with invalid characters after the sync is set up, you'll need to identify them with the monitoring tools.
