...
- A **sync group** outlines the replication topology for an Azure file share. All server endpoints located in the same sync group are kept in sync with each other. If you have multiple Azure file shares that must be in sync and managed with Azure File Sync, you'd create two sync groups and different endpoints.
...
- **Azure File Sync agent** is a downloadable package that allows Windows Server to be synced with an Azure file share. The agent has three components:
