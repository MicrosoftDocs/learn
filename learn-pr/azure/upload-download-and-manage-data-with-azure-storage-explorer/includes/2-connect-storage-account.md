You want to enable your engineers to manage the data stored in your Azure storage. But you don't want to burden them with learning complex tools. Thankfully, Azure now provides you with the capability to manage your Azure Storage through a single app called Azure Storage Explorer.

Here, you'll learn about Azure Storage Explorer, how you can use it manage your data from multiple storage accounts and subscriptions, the Azure Stack, and data held in both Cosmos DB and Azure Data Lakes. You'll learn different ways of using Azure Storage Explorer to connect to your data.
...
- Azure storage manager is free to download and use
...
Blob storage is used to store unstructured data as a blob. Azure blob storage offers two access types: hot and cold. Hot is used for frequently accessed data, and cold is used for less frequently access data.

**Table storage**

Table storage is used to store NoSQL, semi-structured data. While its called Table storage, it uses a schemaless design based on groups of entities.

**File Storage**

File Storage is a file-sharing service. Your organization could use File Storage as part of a migration process to move data to the cloud. It can also be used to store logs, diagnostic data, and application settings.

**Data Lake Storage**

Azure Data Lake is designed for large data volumes and can store both unstructured and structured data. It's based on Apache Hadoop. Azure Data Lake has strong integration with Azure HDInsight and Azure Data Analytics.
...
You can use Azure Storage Explorer to connect to both an Azure Cloud or an Azure Stack. Access the Azure Stack requires a direct connection provided by a Base-64 encoded root certificate. This certificate needs importing into Azure Storage Explorer. 

The following steps provide an overview of how to get the certificate and apply it through Azure Storage Explorer.

To get the certificate:

- Start a Microsoft management console (MMC) session on your Azure Stack, or locally via a VPN to the Azure Stack

- Select **File**, then **Add/Remove snap-in**, and **Certificates** from the available snap-ins list

  ![Screenshot showing the Microsoft management console, certificate selection dialogue](../media/2-mmc-snap-in-certificate.png)

- Next, you need to select the correct **Computer account** and **local computer** option

- Now you need to find the **AzureStackSelfSignedRootCert**, which you'll find here:  **Console Root\Certificated (Local Computer)\Trusted Root Certification Authorities\Certificates**

- Finally, you'll need to export the certificate as a **Base-64 encoded X.509 (.CER)** file

Applying the certificate:

- Launch Storage Explorer and don't connect to an Azure Storage account
- From the **Edit** menu, select **SSL Certificate** and use the **Import Certificate** option
- Import the root certificate you exported earlier
- You'll need to restart Storage Explorer for the certificate to be applied correctly
- When Storage Explorer running, check to see if the **Target Azure Stack API** is selected

You can use Storage Explorer to manage your Azure Stack storage accounts in the same way you use cloud-based Azure.

### Manage your local emulators

During the development phase of your project, you may not want your developers to incur additional costs through the use of Azure Storage accounts. In those cases, you can use a locally based storage emulator. Storage Explorer supports two emulators: Azure Storage emulator and Azurite.

- Azure Storage Emulator makes use of a local instance of Microsoft SQL Server 2012 Express localDB. It emulates the Azure Table, Queue, and Blob services. 
...
To connect to an Azure Data Lake service, you'll need to the URI associated with the Data Lake. Presently, only Data Lake Storage Gen 1 us supported by Storage Explorer. Data Lake Storage Gen 2 is currently in preview, and support will be available shortly through Storage Explorer. Using a URI allows you to access resources, not in your subscription. Once you have the URI of the resource you want to access, you can connect to it using the **Data Lake Storage Gen1** option.
...
There are many ways to connect your Azure Storage Explorer instance to your Azure resources.
...
- Attach to an Azure Data Lake by URI
...
- Select **add a resource via Azure AD**, and choose the Azure tenant and the associated account
- When prompted provide the type of resource, you're connecting to
- Review and verify the connection details, and select **Connect**
...
To use this connection method, you'll need to have a shared access signature URI to the required Storage account. You'll need a SAS URI whether you want to us a file share, table, queue, or blob container. A SAS  URI can be obtained either from the Azure portal or from Storage Explorer.

To add a SAS connection, you need to:

- Launch Azure Storage Explorer
- **Connect** to your Azure storage
- Select the connection type: **shared access signature (SAS) URI**
- Provide a meaningful name for the connection
- When prompted provide the **shared access signature URI**
- Review and verify the connection details, and select **Connect**

Once you've successfully added a connection, it appears in the resource tree as a new node. The connection node can be found in resource tree in the branch: **Local & attached** > **Storage Accounts** > **Attached Container** > ***Service***.

### Connect using a storage account name and key

To quickly connect to a storage account on Azure, you can use the account key associated with the storage. You can find the storage access keys from the Azure portal by going to the correct storage account page and selecting **access keys**.

To add a connection, you need to:

- Launch Azure Storage Explorer
- Connect to your Azure storage
- Select the connection type: **storage account name and key**
- Provide a meaningful name for the connection
- When prompted provide the name of the storage account and either of the account keys needed to access it
- From the provided list, select the Storage domain you want to use.
- Review and verify the connection details, and select **Connect**
