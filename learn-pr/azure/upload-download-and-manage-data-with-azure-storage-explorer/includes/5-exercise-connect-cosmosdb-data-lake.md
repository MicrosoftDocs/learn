You've seen how simple it's to create and manage blob and queue resources in your Azure Storage account. Now you want to push your understanding further and see how it connects to your developers Cosmos DB and Azure Data Lakes.
...
1. From the Azure Cloud Shell, use these commands to create a Cosmos DB.
   ...
   az cosmosdb create --name $NAME \
    ...
    --resource-group learn-16287428-fa2d-44b0-8291-cfeb1ce093aa
   ```

   This command takes a few minutes to run, while it creates a new Cosmos DB for you.
1. When it has completed, you'll see something similar to  JSON response. These details also tell you the name of the cosmos DB. The database name has been marked so that you can find it more easily.
   ...
   Make a note of your Cosmos DB name. You'll need it next. In this example, the cosmos DB name is cosmos26829.

### Obtain your Cosmos DB connection String

1. Now, you need to obtain the primary master key for your Cosmos DB. From you, Azure Cloud Shell, use the following command.
   ...
   **NOTE**: replace the cosmos26829 with the name of the database.
...
1. You can now build your connection string. It requires the name of the Cosmos DB, in our example, that is cosmos26829. Replace the actual name you got when you created the database. It also needs the Primary Master Key that you've just acquired. Add these items to this template.
   ...
   Please store the connection string in notepad or a similar text editor, as you'll need it in the next section.
...
1. You'll see a **Cosmos DB Accounts** folder. Right-click on it and select **Connect to Cosmos DB...** option.
...
1. You'll now see a Summary view, showing the Account label and name, the account endpoint, and the account key. Verify they're correct, and select **Connect**.
...
1. You're now connected to your Cosmos DB instance.
...
1. Select the Cosmos instance you created previously. Right-click on it and select **Create Database**.
...
1. You've now created a database called **products** in your Cosmos DB.

### Create a collection in Cosmos DB database

Now that you've got a Cosmos DB database, its time to create a collection. When you create a collection, you'll need to know the collection ID or name, and the maximum storage capacity for your collection.
...
1. Next, you have to specify whether this collection has a finite amount of storage space or whether it will grow to match the size of the collection. A fixed option gives you a maximum collection size of 10 GB. 
1. The last field is Throughput, and you'll accept the default of 400.
1. Select **OK** when done.
1. You'll now see that the region's collection added under the production database. You can add as many collections as you want.
...
1. The new document shows in the ID panel. 
...
1. If you haven't already done so, you'll need to install a preview extension. Use the **az extensions** CLI command.
...
Now you've created a V2 Storage account; you'll need to reconnect to your Azure Storage area.
...
1. Which creates an empty container name field for you to complete. Give it the name **myfilesystem**.
...
1. Select the **Upload** option, and choose **Upload Files...**
1. The upload dialogue is displayed.

   <img src="../media/5-adls-upload-file.png" alt="Screenshot of the upload files dialogue" style="zoom: 75%;" />
