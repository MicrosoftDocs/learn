...
2. Go into the repo folder locally by running the following cd command.
...
4. To test that our app deployed correctly, first copy and paste the first URL from Cloud Shell output into your favorite browser. The browser should display the swagger UI for our app and declare the following RESTful endpoints:
...
Finally, copy the last URL from Cloud Shell output. This is the swagger JSON URL, which you will need in the next exercise.
...
1. In the left menu pane, select **Integration**, and under *Popular products*, select **API management**. The **Create API Management** pane appears.
...
1. In the **Basics** tab, enter the following values for each setting.
   ...
   > [!NOTE]
   > You're using the consumption plan because it is much faster to create whilst testing. The overall experience is very similar to the other pricing tiers.
