...
Once all your targeted virtual machines are replicated and into Azure, you can test your virtual machines to ensure everything works before you migrate them into production. The process runs a prerequisite check, prepares for the test, creates a new test virtual machine, and starts it. This process will take a few minutes.

## Migrating the virtual machines into production

Once you’re ready for the production migration, select **Migrate** from the replicating machines window. That process will prompt you to shut down the virtual machine to avoid any data loss and perform a final replication. We recommend doing this during off-peak business hours, because the virtual machine will be down for a few minutes.
