...
As an example, the **Contoso-web-sales Metrics** chart displayed below, demonstrates the ability to select the **Requests** metric for our web app. The list of metrics is available automatically for your resource. There is no requirement to install other software or services. You find Metrics under the Monitoring category in each resource panel that you have in your Azure subscription.
