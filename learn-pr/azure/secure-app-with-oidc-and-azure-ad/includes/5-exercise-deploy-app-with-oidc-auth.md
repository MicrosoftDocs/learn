...
In this exercise, you'll use Azure CLI commands in the Azure Cloud Shell to the right to configure an application to authenticate users against your Azure AD tenant and to deploy the web app.
...
1. Replace the values the following fields to configure the application to communicate with Azure AD:
...
1. Make sure you are still in the project module (~/mslearn-secure-app-with-oidc-and-azure-ad/app), and then run the following command to build and deploy the initial web app.
...
1. When deployment completes successfully, you should see ten lines of JSON output. Copy the URL from the output. You'll need this address to configure the app registration.

## Configure the app registration

Now let's configure the app registration to authorize the URL for the web service.

1. In the [Azure portal](https://portal.azure.com/learn.docs.microsoft.com?azure-portal=true), verify that you are signed in to the **Learn Module AAD Tenant**. If not, select your user name in the upper-right corner, and then select **Switch directory**. Select the **Learn Module AAD Tenant** directory that you created earlier. Sign in if you're prompted.

1. From the Home page or the resurce menu, select **Azure Active Directory**. The **Learn Module AAD Tenant** Azure Active Directory Overview pane appears.
...
1. From the menu, under **Manage**, select **Authentication**.

1. In the **Web** section under *Redirect URIs* select **Add URI** and then paste the URL from the Cloud Shell output in the previous section into the text box, and append the suffix `/signin-oidc`. Ensure that the URI starts with https.
