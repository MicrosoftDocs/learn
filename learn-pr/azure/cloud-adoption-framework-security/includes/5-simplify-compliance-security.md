Organizations can speed up implementing the security they need for cloud with the Azure Security Benchmark and the Landing Zones in the Microsoft Cloud Adoption Framework. The Benchmark represents Microsoft's recommended security best practices and is integrated into the Landing Zones to simplify the implementation of these best practices.
...
- New to Azure and are looking for security best practices to ensure a secure deployment of Azure services and application workloads.
...
- Security baselines: When available, benchmark recommendations for Azure services will include Azure Security Benchmark recommendations that are tailored specifically for that service.
...
- **Establish** guardrails to automate secure configurations and enforce compliance with Azure Security Benchmark (and other requirements in your organization) with Azure Policy.
