...
1. On the **Overview** pane, in the top menu bar, select **Browse**. The browser displays a *page can't be found* page. This is expected because the Web API doesn't implement a user interface.

1. In your browser's address bar, paste the Web API test URL that you previously copied, and press <kbd>Enter</kbd>. The browser displays a result in JSON format. Notice that the result includes the server time.

1. In your browser's address bar, paste the Swagger URL that you previously copied, and press <kbd>Enter</kbd>. The browser displays the Swagger page for your *Board Gaming API*. Keep this browser tab open for later use.

1. In your browser's address bar, paste the Swagger JSON URL that you copied earlier. The browser displays the OpenAPI specification in JSON format.
...
1. In the left menu pane, select **Web**, and then select **API Management** from the **Featured** list. The **Create API Management** pane appears.
...
1. On the [Azure portal](https://portal.azure.com/learn.docs.microsoft.com?azure-portal=true) menu or from the **Home** page, select **All resources**, and then select the new *Azure Cache for Redis*. Your **Azure Cache for Redis** pane appears.

1. Check the **Essentials** section at the top of the pane. If the status has not yet reached **Running**, you'll have to wait until the Redis cache deployment has completed. You can refresh this pane every couple of minutes to check the status.

1. After setup is complete, in the middle menu pane, under **Settings**, select **Access keys**. The **Access keys** pane for your Azure Cache for Redis instance appears.
...
1. In the middle menu pane, under **Deployment + Infrastructure**, select **External cache**. The **External cache** pane for your API Management service appears.

1. In the top menu bar, select **Add**. The **External cache** pane reappears for your API Management service.
...
1. In the middle menu pane, Under **APIs**, select **APIs**. The **APIs** pane for your API Management service appears.
...
1. Select **Send** to resend the request. Notice that the time in the response has not changed, because the cached response has been served.
...
1. In the **Inbound processing** section, select **</>**.
...
1. Select **Send** to resend the request. As before, notice that the time in the response has not changed, because the cached response has been served.
