...
In the Azure portal, you'd go to the subscription or resource group to which you want the custom role scope to apply, then go to **Access control (IAM)** and select **Add** > **Add custom role**.
...
To create the role in Azure PowerShell, you'd run the following command:
...
To update a custom role, you can use Azure CLI or Azure PowerShell. You'll walk through the specific steps to update your custom role definition in the next unit. But in general, after you update the JSON file with your changes, you'd run one of the following commands.
...
In Azure PowerShell, you'd run the following command with the path to the updated JSON file:
...
In Azure CLI, you'll use the following command with the name of the custom role.

```azurecli
   az role assignment delete --role "role name"
   ```

In Azure PowerShell, you'll use the cmdlet `Remove-AzRoleAssignment`. The command may look something like the following:
