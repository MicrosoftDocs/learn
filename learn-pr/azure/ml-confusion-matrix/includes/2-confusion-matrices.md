Data can be thought of as continuous, categorical, or ordinal (categorical but with an order). Confusion matrices are a means of assessing how well a categorical model performs. For context as to how these work, let’s first refresh our knowledge about continuous data. Through this we can see how confusion matrices are simply an extension of the histograms we already know.
...
In some respects, categorical data are not so different from continuous data. We can still produce histograms to assess how commonly values appear for each label. For example, a binary label (true/false) might appear with frequency like so:
...
This tells us that there are 200 samples which are ‘person’, 400 which are ‘animal’, and 100 which are ‘tree’.
...
Let’s explain these one by one, replacing these acronyms with actual values. Note that blue squares mean the model made a correct prediction, and red squares men the model made an incorrect prediction.
