...
There is an NSG rule that is blocking outbound traffic from the backend subnet to everywhere over the ports 80, 443 and 3389. Let's reconfigure that rule now:
...
1. In the RDP client, connected to **FrontendVM**, at the command prompt, execute the following two commands:
