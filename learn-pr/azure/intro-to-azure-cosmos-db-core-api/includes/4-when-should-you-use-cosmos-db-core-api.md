...
For enterprise scenarios, Azure Cosmos DB has a comprehensive suite of financially backed [service level agreements (SLAs)](https://azure.microsoft.com/support/legal/sla/cosmos-db/) that cover throughput, consistency, availability, and latency.

## Common use cases for the Azure Cosmos DB SQL API

### IoT/telemetry

IoT sensor workloads can be diverse and unpredictable. These workloads require a database platform that is responsive and capable of elastic scale to meet the needs of as many, or as few, data collection devices deployed at any point in time. In many cases, IoT workloads can also write massive volumes of data and would benefit from a write-optimized database platform. Azure Cosmos DB is optimized for write-heavy workloads. Operations on a single document are atomic with fast and predictable indexed write speeds. Azure Cosmos DB is also globally distributed and elastic to meet the needs of various IoT workloads.
...
1. Azure Cosmos DB that stores the aggregated results from Azure Databricks
1. Azure Synapse Analytics that performs near real-time analysis over the data in Azure Cosmos DB

:::image-end:::

In this example, a growing number of IoT devices are sending sensor data to an Azure IoT Hub. Azure Databricks then ingests and aggregates the data for storage in Azure Cosmos DB. Azure Synapse Analytics then performs near real-time analysis over the data using [Azure Synapse Link for Azure Cosmos DB](/azure/cosmos-db/synapse-link).

### Retail/marketing

Azure Cosmos DB is a great fit for retail and marketing workloads that can experience dramatic and unexpected swings in usage at any point throughout the year. The elastic scale of Azure Cosmos DB ensures that the database platform can handle requests during peak usage, and save money during non-peak times.

> [!NOTE]
> Microsoft's e-commerce platform, which includes Windows Store and Xbox Live, uses Azure Cosmos DB extensively.
...
1. An Azure Cosmos DB account with a container for inventory data and a container for shopping cart data
1. An Azure Search instance that builds a searchable catalog by indexing the Azure Cosmos DB account with inventory data.

:::image-end:::

In this example, a JavaScript web application, built on content stored in Azure Blob Storage, uses Azure Cosmos DB as it's backing database. Multiple accounts are used to manage different facets of the solution such as the shopping cart, inventory, or catalog. Azure Cosmos DB data is then indexed by Azure Search to provide a rich search experience to end users.
...
1. A globally distributed Azure Cosmos DB account

:::image-end:::

In this example, a user is using a URL to access a web site in their browser. The URL points to Azure Traffic Manager, which then uses a built-in algorithm to determine which Azure App Service endpoint to redirect the user to. Since Azure Cosmos DB is capable of global distribution, you only need one account that is replicated across multiple regions.
...
One key part of your game's success is the ability for users to keep in touch with their friends both in and out of the game environment. Your team has worked hard on a sophisticated in-game messaging system. With Azure Cosmos DB's tight integration with other Azure services, you can consider building solutions that use message data in Azure Cosmos DB to send notification to your user's mobile devices when they receive an in-game message.

:::image type="complex" source="../media/4-gaming-scenario.svg" alt-text="Gaming scenario diagram" border="false":::
...
1. Azure Cosmos DB storing game event data
...
1. Azure Notification Hub, triggered by Azure Functions, that notifies the user of new messages
1. A mobile device to receive notifications

:::image-end:::

This example is similar to the example from the introduction of this module. To build on the first example, your team has decided to introduce Azure Cosmos DB as the database of choice. Now, your team can use Azure Databricks to prepare and aggregate data for deeper analysis in analytical engines. Your team can also use services such as Azure Functions to react to data events with Azure Cosmos DB, and then send notifications to mobile devices using Azure Notification Hubs.
