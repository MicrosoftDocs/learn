...
As a fast NoSQL database with a flexible API, Azure Cosmos DB SQL API is well-suited for high-performance applications with global ambition. This makes Azure Cosmos DB SQL API great for web, retail, IoT, gaming, and mobile applications that require flexibility, low response time, and transactions at massive volume or velocity.
