...
In this module, you may have created an Azure Kubernetes Service cluster. If you want to remove the service so that there's no continued charge against your account, following steps below to clean up the resources.
...
1. Enter the name of the resource group to confirm. Select **Delete** to delete all of the resources that you created for this module.

1. From a cloud sell, rRun the `kubectl config delete-context` command to remove the deleted cluster's context. Here's an example of the complete command. Remember to replace the name of the cluster with your cluster's name.
