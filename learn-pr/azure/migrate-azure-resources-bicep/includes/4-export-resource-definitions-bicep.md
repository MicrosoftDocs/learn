...
2. The exported template may include some default resource properties that are normally not specified in a standard deployment. You will need to consider removing these properties prior to re-deployment.

3. The exported template may not include all of the parameters you will need to make the template reusable. Many of these parameters may be hard coded into the template.

#### Export template from a deployment

The Azure portal provides the ability to export a template of a resource prior to it's initial deployment or from it's deployment history.

If you have ever deployed a resource manually from the Azure portal, you may have noticed the option to **Download a template for automation** prior to the deployment of the resource. This option exports an JSON ARM template based on the names and properties you have set while building the resource in the portal.

You can also export a JSON ARM template based on its deployment history. As mentioned before, Azure Resource Manager tracks all resources and resource deployments. To export a resource template from it's deployment history click **Deployments** from the resource group blade. You may see multiple deployments listed. Click the name of the deployment that you want to export, click **Template** on the deployment blade, and click **Download** to save a copy of the template.

When exporting a template from a deployment, consider the following:

1. The exported template shows the state of the resource(s) at the time of deployment. It will not include any changes made after deployment.

2. You will not have the option to select specific resources from a multi-resource deployment. This option will download all resources that were part of the initial deployment.

3. The template wil only include resource properties needed for deployment. In most cases, you can consider this template ready to use.

4. The template will include parameters that will allow you to re-deploy the template in multiple environments.
