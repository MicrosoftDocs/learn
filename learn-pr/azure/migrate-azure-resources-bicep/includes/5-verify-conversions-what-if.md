Recently, your company acquired a smaller competitor. This company deployed its Azure resources using JSON ARM templates and the Azure portal. You have just completed a project converting the JSON ARM templates and manually deployed resources to Bicep. Before using these converted templates to deploy resources, you want to verify what changes will be made to your deployed Azure resources. You want to use the ARM template deployment what-if operation to confirm any changes resulting from a template re-deployment. You also want to learn about any documentation resources that may help increase the fidelity of your converted templates.
...
To help you verify your converted templates before deployment, you can use the Azure Resource Manager template deployment what-if operation. When you use the what-if operation, it compares the current state of your environment with the desired state defined in the template. The tool outputs the list of changes that will occur *without* applying the changes to your environment. This process can help increase your confidence level in your deployments.
...
The [Azure Quickstart Templates](https://azure.microsoft.com/resources/templates/) repository is a collection of community contributed templates. This database of searchable templates provide examples of many Azure resources and solutions. In some instances, both a JSON ARM template and a Bicep ARM template are available to view. These templates can be used as a reference point to help you build and verify your templates for deployment.

For example, suppose you want to use a quickstart template that builds an App Service Plan and an App Service as a reference for one of your templates. After searching the repository, you'll notice that each quickstart template has the option to deploy the template directly to Azure, or browse the template on GitHub.
...
Keep in mind that the Azure quickstart templates are community contributions. Some of the examples may be out of date as features are added to Azure services on a regular basis. The examples may also include resources and properties that may not be necessary for your templates.
