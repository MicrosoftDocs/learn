...
The JavaScript API exposes a class named `CosmosClient` that acts as the access point to Cosmos DB. You use a `CosmosClient` object to obtain a handle on databases and collections. When you have access to a collection, you can query and manipulate documents.
...
The following code snippet shows how to run a query against a collection. This example finds the documents for all students who have a specified academic year:
