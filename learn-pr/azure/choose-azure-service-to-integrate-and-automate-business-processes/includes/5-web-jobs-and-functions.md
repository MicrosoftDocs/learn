...
1. Are new parts required?
    1. New parts are required, but we don't have the parts in stock.
        ...
        1. Parts arrive.
    1. The technician fits new parts.
...
- **Cost**: With WebJobs, you pay for the entire VM or App Service Plan that hosts the job. Azure Functions can run on a consumption plan, so you only pay when the function runs. Because this process only kicks off when a bike is returned, we might save money by selecting Azure Functions.
