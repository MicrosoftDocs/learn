...
After you configure public IP settings, you need to select a virtual network and a subnet that’ll be used with this NAT gateway. You must select at least one subnet. You can add and remove subnets after creating the NAT gateway. Optionally, you can add tags to  Azure NAT Gateway objects and download a template for automation before you select the option to create the Azure NAT Gateway gateway.

When you create an instance of the Azure NAT Gateway service, the subnet you selected is reconfigured, so it sends all outbound traffic to the NAT gateway service. There’s no need to create routes, because this happens automatically.
