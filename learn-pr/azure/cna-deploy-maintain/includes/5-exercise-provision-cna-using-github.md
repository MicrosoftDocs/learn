This exercise takes you through the process of implementing continuous integration and continuous deployment (CI/CD) of a sample containerized Node.js web app into an AKS cluster. You will also test this functionality by modifying directly one of the app files in order to trigger the automatic build and deployment processes. 
...
Before you can proceed with deployment of Azure resources by using GitHub actions, you need to register the **Microsoft.AlertsManagement** resource provider, which the workflow references during provisioning of an AKS cluster.
...
Now you are ready to proceed with the implementation of a CI/CD pipeline by leveraging Azure DevOps Starter.
...
1. On the **Start fresh with a new application** tab of the **DevOps Starter** blade, verify the presence of the **Setting up DevOps starter with GitHub** label. In case the label states **Setting up DevOps starter with Azure DevOps**, select the **here** link, on the **DevOps starter settings** blade, select **GitHub** and select **Done**.
1. On the **Start fresh with a new application** page of the **DevOps Starter** blade, select the **Node.js** tile and then select **Next: Framework >**.
    ...
    ![Screenshot that shows the Start fresh with a new application page of the DevOps Starter blade in the Azure portal](../media/5-azure-devops-starter-node.js-app.png)
...
1. On the **Choose an application framework** page of the **DevOps Starter** blade, select the **Express.js** tile and then select **Next: Service >**.
    ...
    ![Screenshot that shows the Choose an application framework page of the DevOps Starter blade in the Azure portal](../media/5-azure-devops-starter-express.js-framework.png)
...
1. On the **Select an Azure service to deploy the application** page of the **DevOps Starter** blade, select the **Kubernetes Service** tile and then select **Next: Create >**.
    ...
    ![Screenshot that shows the Select an Azure service to deploy the application page of the DevOps Starter blade in the Azure portal](../media/5-azure-devops-starter-kubernetes-service.png)

1. On the **Select Repository and Subscription** page of the **DevOps Starter** blade, select the **Authorize** button.

    ![Screenshot that shows the Select Repository and Subscription page of the DevOps Starter blade in the Azure portal with the Authorize button](../media/5-azure-devops-starter-select-repo-sub-authorize.png)

1. In the **Authorize Azure GitHub Actions** pop-up window, select **Authorize AzureGitHub Actions**.

    ![Screenshot that shows the Authorize Azure GitHub Actions pop-up window.](../media/5-azure-github-actions-authorize.png)

1. If prompted to confirm access, in the **Password** textbox, type the password of your GitHub account and select **Confirm password**.
1. Back on the **Select Repository and Subscription** page of the **DevOps Starter** blade, specify the following settings:

    | Setting | Value |
    | --- | --- |
    | Organization | The name of the Azure DevOps organization you will be using in this exercise. |
    ...
    ![Screenshot that shows the Select Repository and Subscription page of the DevOps Starter blade in the Azure portal](../media/5-azure-devops-starter-select-repo-sub-settings.png)

1. On the **Select Repository and Subscription** page of the **DevOps Starter** blade, select **Additional settings**, on the **Additional settings** blade, review the **Kubernetes Service** and **Container Registry** sections and select **OK** without making any changes.

    ![Screenshot that shows the Additional settings blade in the Azure portal](../media/5-azure-devops-starter-select-repo-sub-additional-settings.png)
...
1. Open another tab in the same web browser window, navigate to [GitHub](https://github.com) and sign in by using your GitHub account.
1. From the GitHub page, navigate to the list of the repositories and select the newly created repository **cna-devops-05**.

    ![Screenshot that shows the cna-devops-05 repository entry in GitHub](../media/5-github-repositories.png)

1. On the **cna-devops-05** repository page, select the **Actions** tab.
1. On the **Actions** tab, in the list of workflow runs, select the ellipsis next to the **Adding workflow file** entry and, in the pop-up menu, select **Cancel run**.

    ![Screenshot that shows the Cancel run menu option of the Adding workflow file workflow run in GitHub](../media/5-github-cancel-run.png)
...
1. On the page displaying the content of the **devops-starter-workflow.yml** file, select the **Edit this file** icon in the form of a pencil.
    ...
    ![Screenshot that shows the Edit this file menu option of the workflow file in GitHub](../media/5-github-edit-workflow-file.png)
...
1. In the upper-right corner of the page, select **Start commit** and, in the **Commit changes** pop-up window, select **Commit changes**.
    ...
    ![Screenshot that shows the Edit this file menu option of the workflow file in GitHub](../media/5-github-edit-start-commit.png)
...
1. On the **cna-devops-05** blade, in the **GitHub Workflow** section, select **Authorize** and, on the **GitHub Authorization** blade, select **Authorize** again.
    ...
    ![Screenshot that shows the Authorize button in the GitHub Workflow section of the Azure DevOps starter blade in the Azure portal](../media/5-azure-devops-starter-select-github-workflow-authorize.png)
...
1. Switch again to GitHub, navigate back to the **Actions** tab, select the **Update devops-starter-workflow.yml** run entry in the list of jobs, select the **Build and push image to container registry** entry to review the detailed status of the build process. Wait until the build completes and then, in the list of jobs, select the **Deploy application to AKS** entry to review the detailed status of the deployment process.

    ![Screenshot that shows the status of the build job in GitHub](../media/5-github-build-job-status.png)
...
1. Switch back to the web browser window displaying the Azure portal and monitor the build and deployment process from the **cna-devops-05** DevOps starter blade, including the GitHub workflow and Azure resources.
    ...
    ![Screenshot that shows the progress of the build and deployment jobs on the Azure DevOps starter blade in the Azure portal](../media/5-azure-devops-starter-deployment-progress.png)

    > [!NOTE]
    > Wait for the deployment to complete. This might take about 5 minutes.

    ![Screenshot that shows the completed build and deployment jobs on the Azure DevOps starter blade in the Azure portal](../media/5-azure-devops-starter-deployment-completed.png)

    ![Screenshot that shows the completed build and deployment jobs in GitHub](../media/5-github-deployment-completed.png)
...
1. Verify that the web browser displays the **Success!** page.
    ...
    ![Screenshot that shows the sample web app in its original form.](../media/5-sampleapp-original.png)

1. Switch to the browser tab displaying your GitHub repository and switch to the **Code** tab.
1. On the **Code** tab, in the list of folders, select **Application**, then select **views**, and finally select **index.pug**.
1. On the page displaying the content of the **index.pug** file, select the **Edit this file** icon in the form of a pencil.

    ![Screenshot that shows the Edit this file menu option of the index.pug file in GitHub](../media/5-github-edit-index.png)
...
1. At the bottom of the page, select **Commit changes**.
1. In GitHub, navigate back to the **Actions** tab, select the **Update index.pug** run entry in the list of jobs, select the **Build and push image to container registry** entry to review the detailed status of the build process. Wait until the build completes and then, in the list of jobs, select the **Deploy application to AKS** entry to review the detailed status of the deployment process.

    ![Screenshot that shows the status of the build job in GitHub](../media/5-github-build-job-status.png)

1. Once the deployment completes, switch to the browser window displaying the sample web app, refresh the web page, and verify that it contains the text **GitHub repository has been successfully updated**.

    ![Screenshot that shows the sample web app in its modified form.](../media/5-sampleapp-updated.png)
...
Congratulations! You completed the second exercise of this module. In its exercise, you stepped through the process of implementing continuous integration and continuous deployment (CI/CD) of a sample containerized Node.js web app into an AKS cluster. You also tested this functionality by modifying directly one of the app files in order to trigger the automatic build and deployment processes. Once you completed your testing, you deleted the Azure resources deployed in this exercise in order to avoid extra charges.