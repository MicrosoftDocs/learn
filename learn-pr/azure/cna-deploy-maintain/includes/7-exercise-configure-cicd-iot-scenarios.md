This exercise takes you through the process of implementing CI/CD of a sample IoT application to Azure IoT Edge. As in the previous exercise, you'll use Azure DevOps Starter for this purpose, but you'll rely on Azure DevOps to provide the code repository and implement the corresponding CI/CD pipelines.

> [!NOTE]
> This exercise illustrates the use of Azure DevOps Starter, which considerably simplifies the implementation of CI/CD pipelines. For detailed information regarding authoring such pipelines in Azure DevOps and GitHub, refer to the links provided at the end of this module.
...
For the sake of simplicity, you'll implement the Azure IoT Edge CI/CD pipeline by using the functionality provided by Azure DevOps Starter.
...
1. On the **Start fresh with a new application** tab of the **DevOps Starter** blade, verify the presence of the **Setting up DevOps starter with Azure DevOps** label. In case the label states **Setting up DevOps starter with GitHub**, select the **here** link, on the **DevOps starter settings** blade, select **Azure DevOps**, and then select **Done**.
1. On the **Start fresh with a new application** page of the **DevOps Starter** blade, select the **Java** tile, and then select **Next: Framework >**.
    ...
    :::image type="content" source="../media/7-azure-devops-starter-java.png" alt-text="Screenshot of the Start fresh with a new application page of the DevOps Starter blade in the Azure portal, with the Java tile selected." border="false":::
...
1. On the **Choose an application framework** page of the **DevOps Starter** blade, select the **Simple IoT** tile, and then select **Next: Service >**.
    ...
    :::image type="content" source="../media/7-azure-devops-starter-simple-iot-framework.png" alt-text="Screenshot of the Choose an application framework page of the DevOps Starter blade in the Azure portal, with the Simple IoT tile selected." border="false":::
...
1. On the **Select an Azure service to deploy the application** page of the **DevOps Starter** blade, select the **IoT Edge** tile, and then select **Next: Create >**.
    ...
    :::image type="content" source="../media/7-azure-devops-starter-iot-edge-service.png" alt-text="Screenshot of the Select an Azure service to deploy the application page of the DevOps Starter blade in the Azure portal, with the IoT Edge tile selected." border="false":::
...
1. On the **Almost there!** page of the **DevOps Starter** blade, specify the following settings:
    ...
    :::image type="content" source="../media/7-azure-devops-starter-almost-there.png" alt-text="Screenshot of the Almost there page of the DevOps Starter blade in the Azure portal." border="false":::

1. On the **Almost there!** page of the **DevOps Starter** blade, select **Additional settings**.
1. On the **Additional settings** blade, specify the following configuration, while leaving the other settings with their default values:
...
1. In the Azure DevOps portal, select the **Azure DevOps** logo in the beginning of the page, and then, on the page displaying the list of projects, select the **cna-devops-07** entry.
1. On the **cna-devops-07** project pane, in the vertical menu, select **Pipelines** and then, in the list of recent pipeline runs, select the **cna-devops-07-iothub - CI** entry.
...
1. On the **cna-devops-07-iothub - CI** pane, select **Edit** to display the graphical pipeline editor pane.
    ...
    :::image type="content" source="../media/7-azure-devops-pipeline-edit-start.png" alt-text="Screenshot of the graphical pipeline editor pane of the Azure DevOps portal." border="false":::
...
1. On the **cna-devops-07-iothub - CI** pane, on the **Tasks** pane, in the list of tasks, select the **Azure CLI: Get AR credentials** entry and then select the **+** sign next to the **Agent job 1** entry.
    ...
    :::image type="content" source="../media/7-azure-devops-pipeline-editor-acr-credentials.png" alt-text="Screenshot of the Azure CLI: Get AR credentials task selected in the graphical pipeline editor pane the Azure DevOps portal." border="false":::

1. In the **Add tasks** pane, search for the **Use Python version** task, select it in the list of results, and then select **Add**.

    :::image type="content" source="../media/7-azure-devops-pipeline-editor-use-python-add.png" alt-text="Screenshot of the Use Python version task with the Add button in the graphical pipeline editor pane the Azure DevOps portal." border="false":::
...
1. In the list of tasks of the pipeline, select the newly added **Use Python 3.x** task, set its **Display name** to **Use Python 3.6** and its **Version spec** to **3.6**.
    ...
    :::image type="content" source="../media/7-azure-devops-pipeline-editor-use-python-set.png" alt-text="Screenshot of the Use Python 3.6 task with the display name and version spec set in the graphical pipeline editor pane the Azure DevOps portal." border="false":::

1. On the **cna-devops-07-iothub - CI** pane, on the **Tasks** pane, select the **+** sign next to the **Agent job 1** entry.
1. In the **Add tasks** pane, search for the **PowerShell** task, select it in the list of results, and then select **Add**.

    :::image type="content" source="../media/7-azure-devops-pipeline-editor-use-powershell-add.png" alt-text="Screenshot of the PowerShell task with the Add button in the graphical pipeline editor pane the Azure DevOps portal." border="false":::
...
1. In the list of tasks of the pipeline, select the newly added **PowerShell Script** task, set its **Type** to **Inline** and its **Script** to **pip install -U iotedgedev**.
    ...
    :::image type="content" source="../media/7-azure-devops-pipeline-editor-powershell-set.png" alt-text="Screenshot of the PowerShell Script task with the type and script set in the graphical pipeline editor pane the Azure DevOps portal." border="false":::

1. On the **cna-devops-07-iothub - CI** pane, on the **Tasks** pane, select **Save & queue**, in the drop-down menu, select **Save**, and then, in the **Save build pipeline** pane, select *Save**.
1. In the vertical menu next to the **cna-devops-07-iothub - CI** pane, in the **Pipelines** section, select **Releases**, ensure that the **cna-devops-07 - CD** release pipeline is selected, and then select **Edit**.
    ...
    :::image type="content" source="../media/7-azure-devops-release-pipeline-edit-button.png" alt-text="Screenshot of the cna-devops-07-iothub - CD pipeline with the Edit button in the Azure DevOps portal." border="false":::

1. On the **Pipeline** tab of the **All pipelines > cna-devops-07-iothub - CD** pane, on the edge of the rectangle representing the **qa** stage, select the oval shape representing the pre-deployment conditions applicable to this stage.

    :::image type="content" source="../media/7-azure-devops-release-pipeline-qa-pre-deployment-conditions.png" alt-text="Screenshot of the cna-devops-07-iothub - CD pipeline with the pre-deployment conditions of the qa stage in the Azure DevOps portal." border="false":::

1. On the **Pre-deployment conditions** pane, select the **Manual only** option.

    :::image type="content" source="../media/7-azure-devops-release-pipeline-qa-manual-only.png" alt-text="Screenshot of the cna-devops-07-iothub - CD pipeline with the pre-deployment conditions of the qa stage set to Manual only in the Azure DevOps portal." border="false":::

    > [!NOTE]
    > This will stop the automatic deployment following the completion of the **dev** stage. This change is applied for the sake of simplicity, to eliminate the need to apply changes the **qa** and **prod** stages of the pipeline, which would be equivalent to those you have to apply to the **dev** stage. In general, rather than using the **Manual only** option, you could rely on the results on the smoke test to determine whether the deployment can proceed to the subsequent stage.
...
1. On the **Pipeline** tab of the **All pipelines > cna-devops-07-iothub - CD** pane, in the rectangle representing the **dev** stage, select the link indicating the number of jobs and tasks in that stage.
    ...
    :::image type="content" source="../media/7-azure-devops-release-pipeline-dev-jobs-tasks.png" alt-text="Screenshot of the cna-devops-07-iothub - CD release pipeline in the Azure DevOps portal with the jobs and tasks of the dev stage selected." border="false":::<!--Marcin, image we have in media folder is	7-azure-devops-release-pipeline-dev-jobs-tasks-default-pool.png-->

    > [!NOTE]
    > The deployment pipeline generated by Azure DevOps Starter includes a number of components that will need to be updated for it to run successfully. You'll apply these updates next.
...
1. In the **Add tasks** pane, in the **Search** text box, enter **Azure CLI**, in the list of results, select **Azure CLI**, and then select **Add** to add the **Azure CLI** task directly above the **Azure Deployment: Create Azure IoT Hub** task.
1. Select the newly added **Azure CLI** task, in the task details pane, and then configure the following settings:
...
1. In the vertical menu next to the **All pipelines > cna-devops-07-iothub - CD** pane, select **Repos** and, in the folder hierarchy of the repository, select **edgeSmokeTest.sh**.

    > [!NOTE]
    > This will display the content of the **edgeSmokeTest.sh** file.

1. In the pane displaying the content of the **edgeSmokeTest.sh** file, select **Edit**, replace both occurrences of `azure-cli-iot-ext` with `azure-iot`, add the `# ` string in front of the line **91** `validateDevicesModulesRunning`, select **Commit**, and then, on the **Commit** pane, select **Commit** again.

    > [!NOTE]
    > The validateDevicesModulesRunning function fails due to an issue with the underlying module. By commenting it out, you'll be able to observe the successful completion of the release pipeline to the **dev** stage. In real-world scenarios, you should troubleshoot the status of the deployed module, but in this exercise, the focus is on the CI/CD functionality.

    :::image type="content" source="../media/7-azure-devops-repo-edgesmoketest-edit.png" alt-text="Screenshot of the edgeSmokeTest.sh script in the cna-devops-07 repository in the Azure DevOps portal." border="false":::

    > [!NOTE]
    > This will automatically trigger the build pipeline.

1. In the vertical menu next to the Azure DevOps portal, select **Pipelines**.
...
1. On the job run pane, monitor the progress of the build pipeline.
    ...
    > [!NOTE]
    > Wait for the completion of the build pipeline. This might take about five minutes.

    :::image type="content" source="../media/7-azure-devops-portal-build-completed.png" alt-text="Screenshot of the build pipeline completed in the Azure DevOps portal." border="false":::

1. In the Azure DevOps portal, in the vertical menu, in the **Pipelines** section, select **Releases**, ensure that the **cna-devops-07 - CD** release pipeline is selected, and then, in the list of releases, select **Release-1**.
1. On the **cna-devops-07-iothub - CD > Release-1** pane, select the **Logs** button directly following the rectangle representing the **dev** stage of the release pipeline.

    :::image type="content" source="../media/7-azure-devops-portal-release-progress.png" alt-text="Screenshot of the progress of the dev stage of the release pipeline in the Azure DevOps portal." border="false":::<!--Marcin, this image and the next are the same - should this one be a different one?-->

1. On the **cna-devops-07-iothub - CD > Release-1 > dev** pane, monitor the logs and the detailed progress of the release pipeline.

    :::image type="content" source="../media/7-azure-devops-portal-release-progress.png" alt-text="Screenshot of the progress of the release pipeline in the Azure DevOps portal." border="false":::

    > [!NOTE]
    > Wait for the deployment to complete. This might take about five minutes.

    :::image type="content" source="../media/7-azure-devops-portal-release-progress-completed.png" alt-text="Screenshot of the progress of the release pipeline in the Azure DevOps portal." border="false":::
...
Congratulations! You've completed the third and final exercise of this module. In this exercise, you stepped through the process of implementing a multi-stage CI/CD pipeline which builds and deploys an IoT application to Azure IoT Edge. As in the previous exercise, you used Azure DevOps Starter for this purpose, but this time, you used Azure DevOps to provide the code repository and implement the corresponding CI/CD pipelines. After you completed your testing, you deleted the Azure resources deployed in this exercise to avoid extra charges.
