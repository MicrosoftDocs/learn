...
Logic Apps does support this, but not directly. You need to use a feature called "groups" to do it. A *group* is a sequence of expressions combined with either **AND** or **OR**. You can't mix **AND** and **OR** within a group.
...
A *switch* action compares a value against several cases and executes only the one that matches. A *switch* can include a *default* action that runs if there's no match.
...
A *foreach* loop action processes an array. It performs the same actions on each array item. By default, the actions for each array element run in parallel, although you can control this in the loop's configuration.
