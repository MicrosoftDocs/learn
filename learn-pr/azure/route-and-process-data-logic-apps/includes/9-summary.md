...
The Azure Logic Apps integration platform let us automate this business process by creating a logic app workflow. We mapped each step in our workflow to available components. Our workflow used the Twitter trigger to detect mentions of our product and launch the subsequent steps, based on the tweet sentiment. The Azure Cognitive Services action let us analyze whether the tweets were positive or negative. A control action helped us decide where to route the tweet based on sentiment. Finally, we used actions to insert a row in SQL Server and send an Outlook email.
