...
In Defender for IoT, select the Alerts page. This lists all of the alerts generated by the sensor. To triage the alerts, you need to analyze the **severity** and **status** settings automatically assigned to it. There are 4 severities of alerts that are assigned by the sensor, which can be modified. They are: *Critical, Major, Minor, Warning*. Alerts also have a status of *New, Active, Closed*. It is recommended that you start with the critical level alerts first.
...
After analyzing the details of the alert, update the status or other settings as needed. Alerts can have 5<!--4? --> different statuses.
...
You can also analyze the PCAP files of the raw data for the alert, which might also give you more information about the alert. These are downloaded from the Alert Detail's page, and should then be analyzed using a PCAP file reader.

A comment can be added to the alert to leave important messages for other team members, document important information about the alert or make decisions. <!-- I dont see where this is done? Who can tell me? -->

Other changes can be made to ports and VLAN names, marking an ICS subnet and configuring DHCP address ranges. <!-- these are for the device sensor itself, not the alert. Should it still be included? -->
