...
1. Run the `azd init` command with the `--template` parameter to clone an existing templated and perform a few initial configurations.
...
4. Modify the application features or update the Azure resource templates and then run `azd up` again to provision your changes.

Other `azd` commands are available to complete additional tasks and expand on this workflow. Some of the most common `azd` commands you'll use include:
...
The Azure Developer CLI is designed around standardized project templates. Each template contains everything `azd` needs to provision and deploy your application. When you run a command such as `azd up`, the tool uses the project template assets to execute various workflow steps, such as provisioning or deploying resources to Azure. You can examine the structure of an `azd` template to understand how this works.

<pic>

All `azd` templates include the following assets:

* **`infra` folder** - A folder that contains all of the Bicep files for the `azd` template. Bicep is an infrastructure as code language that is used to create and configure Azure resources in a declarative syntax. Bicep files are executed by `azd` to create the Azure resources required to host your app.
...
* **`.azure` folder** - A folder for storing essential Azure configurations, such as the location to deploy resources or other subscription information.

Most `azd` templates also optionally include one or more of the following folders:

* **`.devcontainer` folder** - This folder allows you to setup a devcontainer environment for your application. This is a common development environment approach that is not specific to `azd`.
