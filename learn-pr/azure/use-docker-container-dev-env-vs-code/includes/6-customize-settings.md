...
This section specifies machine specific settings that will be copied into the container. of the container and which file to use as the dockerfile. You'll recognize the `Dockerfile` as being the other file in the ".devcontainer" folder. The context, and args are options that get passed through to the Docker container when it gets built.
