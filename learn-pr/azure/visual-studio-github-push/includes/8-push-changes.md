...
The main panel in the **Git Repository** window shows the **Local History** for the selected branch. Above **Local History** are two optional sections:
...
Visual Studio doesn't allow you to push commits if your local branch is behind the remote branch. If you try to push, a dialog prompts you to pull before pushing. It includes a **Pull and Push button**. 
...
Sometimes, you might want to overwrite the commits on the remote branch with your own local commits. This can occur if you realize that the commits you uploaded aren't correct, and you amend them. 

You can use the `--force` flag in Git, but it's a dangerous command because it deletes any remote commits that might have been added since you last pulled. Instead, Visual Studio provides the modified flag `--force-with-lease`. This is a safer option because it ensures that you don't overwrite someone else's work by force pushing. To enable the command, go to **Git** > **Settings** and select the **Enable push --force-with-lease** checkbox. 
...
The next time you try pushing with unpulled commits, the dialog that prompts you about pulling before pushing includes a **Force Push** button.
