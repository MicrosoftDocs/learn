...
When Visual Studio initializes your repository, it first runs a `git init` command. The operation creates a .git subdirectory at the root of your project, which contains all of the necessary Git metadata for the new repository. Besides the .git folder, Visual Studio won't alter anything else in your project with the initialization. 
...
The .gitattributes file helps Git understand file types in your repository better. It's an optional but important file that Visual Studio creates for you by using its own custom configuration.
...
All GitHub repositories created through Visual Studio are set to private by default. This means only you can view the repository and commit to it. To change this setting, add contributors to private repositories and manage access via the repository's **Settings** page on GitHub.com. Clearing the checkbox switches the repository to public, allowing anyone on the internet to view your repository and its contents on GitHub.com. But you can still choose who can contribute to it directly. 

Apart from pushing to GitHub, you can push to any other remote endpoint (such as Azure DevOps) by using the **Existing remote** tab. You can also use the **Local only** tab to initialize the Git repository locally. In the latter case, Visual Studio just adds the .git folder and doesn't make any commits.