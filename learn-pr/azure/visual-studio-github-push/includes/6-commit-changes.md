...
Under the branch picker is the number of outgoing and incoming commits. Whenever you make commits, you'll notice the first number increment higher to indicate commits you might want to push to your remote branch. And whenever you fetch, you can see if there are any unpulled commits from the remote tracking branch. Selecting the link takes you to the **Git Repository** window, where you can see the details of the outgoing and incoming commits, along with the branch's local history. 
...
Staging a file is the equivalent of the `git add` command. You can use the feature to focus only on the parts of your pending changes that you want to commit now and decide which changes you want to save for a later commit. 
...
After you stage files, the next logical step is to commit the staged files. You can enter a commit message in the **Commit** text box, and then select **Commit Staged** or the keyboard shortcut Ctrl+Enter to create a commit. The **Commit** button also has a dropdown menu where you can select **Commit and Push** or **Commit and Sync** to perform those actions in a single command. 
...
Visual Studio can help you undo any commit that you make. You might want to erase the local work that you just did and start from scratch if you've made a mistake. In that case, you can `git reset` a commit, which removes it from the commit graph and orphans it. 
...
You should never reset a commit that has been pushed up to a public repository. Removing a commit this way will make it look like a chunk of history is missing when other developers try to push their commits and will cause all sorts of problems. The safe way to undo a public commit is to use `git revert`. With Visual Studio, reverting a commit creates a new second commit that undoes the changes applied in the first commit.
...
Visual Studio has the functionality to stash changes. You can stash all changes or just the unstaged changes by using the Commit button's dropdown menu. 
