...
3. In the Project pane, navigate to **Packages** > **MRTK Standard Assets** > **Audio** > **HoloLens2** folder to locate the audio clips:
...
7. In the Hierarchy pane, select the RoverExplorer > Buttons > **Reset** button object, then in the Inspector pane, configure the PressableButton **OnClicked ()** event as follows:
...
3. In the Hierarchy pane, expand the Menu > **Buttons-GridLayout (Buttons)** object to reveal the three buttons, rename the third button to **BoundsControl_Enable**, and then in the Hierarchy window, configure the **BoundsControl_Enable** game object as follows:
    ...
    * Select the **Frontplate > AnimatedContent > Icon > UIButtonFontIcon** object and change the **Icon** to the 'four arrows' icon on the **Font Icon Selector** component
