...
Our app secrets are secure in the vault, but we still need to keep a secret or certificate outside of the vault to access them! This issue is called the *bootstrapping problem*, and Azure has a solution for it.
...
Enabling a managed identity for a web app requires only a single Azure CLI command with no configuration. You do it later when we set up an App Service app and deploy it to Azure. Before that, though, we're going to apply our knowledge of managed identities to write the code for our app.
