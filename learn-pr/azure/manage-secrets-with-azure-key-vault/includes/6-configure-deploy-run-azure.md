...
1. In Azure Cloud Shell, to create an App Service plan, run the following command.
...
For deploying to Azure, you'll follow the App Service best practice of putting the VaultName configuration in an app setting instead of a configuration file. To create the app setting, run this command. Make sure to enter both your unique app name to the `--name` parameter, and unique vault name to the `--settings` parameter.
...
Enabling managed identity on an app is a one-liner &mdash; to enable it on your app, run the following command. Make sure to enter your unique app name to the `--name` parameter.
