...
By default, all document properties in an Azure Cosmos DB are indexed.
...
- **None**:  No index is created. Queries are expensive on collections that aren't indexed. If you're using your Azure Cosmos DB collection to read records directly rather than querying the collection, then it's possible to avoid the overhead of indexing.

## Indexing mode for the Orders collection
