Containerization is one of the current predominant technological trends not only across public and private clouds, but also in on-premises datacenters. Running containers at scale requires the use of an orchestrator, which automates scheduling, deployment, networking, scaling, health monitoring, and management of containers. While there are a number of orchestrators that provide all of these features, Kubernetes has become a de facto standard for new containerized deployments, embraced by a wide range of vendors, including Microsoft. As the number of Kubernetes deployment grows, it becomes increasingly challenging to provide a consistent, scalable management model for Kubernetes clusters and their workloads. Azure Arc aims to address this challenge. 
## Scenario
