...
You can create two types of managed identity: *system-assigned managed identity* and *user-assigned managed identity*.

### System-assigned managed identity

You enable system-assigned managed identities directly on an Azure service instance, such as a VM. When you enable managed identities, Azure creates a service principal through Azure Resource Manager. A system-assigned managed identity has its lifecycle linked to the resource instance where it was created. For example:
...
User-assigned managed identities are created as standalone Azure resources. They are independent of any app or service instance. When a user-assigned managed identity is provisioned, Azure creates a service principal just as it does for a system-assigned managed identity.
...
- If your web app is deployed on 10 front-end VMs, you create a user-assigned managed identity for the app, grant the managed identity the necessary rights, and then associate it with all 10 VMs.
- If you used system-assigned managed identity, you would need 10 identities, and you would have to manage access for each one.
