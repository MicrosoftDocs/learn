As the lead system engineer and Azure administrator who’s tasked with resolving current connectivity issues with Azure VMs, your first step is to understand the technological background and the capabilities of Azure Virtual Network NAT.

Azure Virtual Network NAT is a fully-managed cloud service that runs in Azure. It’s highly resilient, scalable, and easy to configure. When you use Azure Virtual Network NAT with your existing virtual networks in Azure, individual VMs or other Azure resources can remain fully private, unless they host services that accept inbound connections from the internet. All outbound connectivity that’s initiated from your virtual network uses the Virtual Network NAT's static public IP addresses.
...
NAT is also used in homes and local networks. If you have a home router that connects you to the internet, it very likely has NAT implemented, so that all your devices are routed to the internet by using just one public IP address. NAT also hides your internal address space so all outbound traffic seems to be coming from a single public IP address, which is usually assigned to a router or gateway device.
...
1. When a device on the private network establishes a connection to a resource on the internet, the NAT replaces the internal device IP address in the packet header with the external IP address of the NAT device.
...
:::image type="content" source="../media/2-network-address-translation.png" alt-text=" The process of network address translation between a host and server." border="false":::
...
:::image type="content" source="../media/2-network-address-translation-private-public.png" alt-text=" Azure Virtual Network NAT is assigned to two subnets on a virtual network and translates private IP addresses to public IP. " border="false":::
