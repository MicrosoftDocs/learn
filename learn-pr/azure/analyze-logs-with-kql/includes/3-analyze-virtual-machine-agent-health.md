...
1. Which data in the `Heartbeat` table is relevant to your analysis and how do you want to transform and organize this data?
    ...
    | --- | --- | --- | --- |
    | `TimeGenerated` | Indicates when the virtual machine generated each log. | <ul><li>Identify machines that have been active recently.</li><li>Find the last log generated for each machine and check whether it was generated in the last few minutes.</li></ul> | <ul><li>`where TimeGenerated >ago(48h)`.</li><li>`max(TimeGenerated)`.</li></ul> |
    | `Computer` |Unique identifier of the machine. | Summarize results by machine. | `summarize` | 
