...
| `CounterName` | Holds the names of all performance counters in the table. | <ul><li>Monitor counters related to free space.</li><li>Rename `% Used Space` to `% Free Space` (in parallel, convert the related `CounterValue`)</li></ul>. | `where CounterName == "Free Megabytes" or CounterName =="% Free Space" or CounterName == "% Used Space"`<br/>To simplify the results and facilitate further analysis:<ul><li>Change `% Used Space` to `% Free Space` (`CounterName = iff(CounterName=="% Used Space", "% Free Space", CounterName)`).</li><li> Change `Free Megabytes` to `OverallFreeSpaceInGB` (`CounterName= iff(CounterName=="Free Megabytes", "OverallFreeSpaceInGB", CounterName)`) </li><li> </li></ul>For more information, see [where operator](/azure/data-explorer/kusto/query/whereoperator) and [== (equals) operator](/azure/data-explorer/kusto/query/equals-cs-operator).|
