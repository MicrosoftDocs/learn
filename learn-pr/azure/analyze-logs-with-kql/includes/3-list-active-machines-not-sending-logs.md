...
| `OSType` | The type of operating system running on the virtual machine. | Identify agent type for Log Analytics agents, which are different for each OS type. | `summarize by... OSType`<br/>For more information, see [summarize operator](/azure/data-explorer/kusto/query/summarizeoperator). |
...
1. Retrieve all logs from the past 48 hours:
    ...
    :::image type="content" source="../media/kql-log-analytics-heartbeat-table-48h.png" alt-text="Screenshot showing the results of a query for all record generated in the past 48 hours on the Heartbeat table." lightbox="../media/kql-log-analytics-heartbeat-table-48h.png":::
...
1. Find the last log generated by each machine and summarize by computer, agent type, and operating system:     
    ...
    :::image type="content" source="../media/kql-log-analytics-heartbeat-table-last-log.png" alt-text="Screenshot showing... the Heartbeat table." lightbox="../media/kql-log-analytics-heartbeat-table-last-log.png":::

1. To find out which computers haven't sent logs in the last five minutes, filter away all logs generated in the last five minutes:
    ...
    :::image type="content" source="../media/kql-log-analytics-heartbeat-table-not-sending-logs.png" alt-text="Screenshot showing... the Heartbeat table." lightbox="../media/kql-log-analytics-heartbeat-table-not-sending-logs.png":::
...
1. Manipulate the query results to present the information more clearly. 
    ...
    :::image type="content" source="../media/kql-log-analytics-heartbeat-table-manipulate-results.png" alt-text="Screenshot showing... the Heartbeat table." lightbox="../media/kql-log-analytics-heartbeat-table-manipulate-results.png":::
...
1. Rename the the `Version` column to `AgentVersion` for clarity, add another `summarize` line to find unique combinations of agent type, agent version, and operating system type, and use the KQL `make_set()` aggregate function to list all computers running each combination of agent type and agent version: 
    ...
    :::image type="content" source="../media/kql-log-analytics-heartbeat-table-make-set.png" alt-text="Screenshot showing... the Heartbeat table." lightbox="../media/kql-log-analytics-heartbeat-table-make-set.png":::
