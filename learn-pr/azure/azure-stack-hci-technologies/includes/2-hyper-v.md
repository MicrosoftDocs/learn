...
The following diagram depicts the high-level architecture of Hyper-V. The Hyper-V hypervisor takes full control of the hardware virtualization capabilities (orange arrow), <!--Marjan, we cannot refer to color indicators as color blind or visually impaired people will nto be able to make this out. Can we delete this or change this to "virtualization capabilities (depicted by the arrow going from the CPU layer to the Hypervisor layer)--> and does not expose them to the guest OS.
