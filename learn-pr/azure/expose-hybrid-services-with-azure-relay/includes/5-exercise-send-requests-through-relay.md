...
Having created a relay for your credit checking service, you now want to enable your on-premises listener component to connect to the relay, so that it receives requests and can send responses. You also want to complete the code in the client app, which will send credit check requests to the relay and wait for responses. By connecting to the relay service in this way, you can expose the service to components in the cloud, without opening a port on your on-premises firewall, which protects the credit checking service.

Here, you'll write the Azure Relay code in both the sender and listener applications.

## Clone the sample apps

You have two command-line apps that will send messages to, and listen for message from, the relay that you created earlier. Most of the code is finished, but you must add the Azure Relay code to the apps. Let's start by obtaining the code:
...
1. Now, use `git` to clone the sample apps:
    ...
    <!-- TODO: Replace this URL with the correct one when the repo has been created -->
...
First, let's ensure that the listener app can connect to the relay. The app needs the address of the namespace, the name of the hybrid connection, the key name, and the primary connection key that you saved in a text file:
...
You must add the credit check service to your relay as a listener, so that it will receive requests sent to the relay. Your code must configure the listener to subscribe to events, process any requests, and formulate responses. The credit check service, for example, must obtain the name from the request and then respond with the credit history for that person. When you have set the listener up, call the `OpenAsync()` method to start listening for requests:
...
Apps that want to check a customer's credit record, send a request to the relay, which forwards it to the listener. As for the listener, we have to configure the sender app with connection details for the relay:
...
To request a credit check, the sender app sends a name in an HTTP GET request to the relay. Let's write the code that formulates that request now:
...
The listener and sender apps are now complete. You can now run both apps, use the sender to request a credit check, and then observe the results in both apps:
