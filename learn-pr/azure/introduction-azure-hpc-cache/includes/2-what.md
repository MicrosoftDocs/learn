...
A storage target is where your data is kept. Often, these are network attached storage appliances. There are specific storage targets that the HPC Cache supports like Azure NetApp Files, physical NetApp appliances, EMC Isilon appliances, or Linux storage servers.
...
For example, running a stock market simulation using historical stock market data would be a good use for a caching service. The dataset doesn't change and you can have hundreds of compute clients running thousands of scenarios.
