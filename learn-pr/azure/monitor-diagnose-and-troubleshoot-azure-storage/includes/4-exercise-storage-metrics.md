...
2. In the **Search the Marketplace** box, type **Azure storage account**, and then press Enter.

3. On the results page, click **Storage account**.

4. On the **Storage account** page, click **Create**.

5. On the **Create storage account** page, specify the following values, and then click **Review + create**.
...
6. On the **Validation passed** page, click **Create**, and wait for the storage account to be deployed.

## View storage metrics in Azure portal

1. In the **Azure portal**, navigate to your storage account.

2. In the menu pane, under **Monitoring (classic)**, click Select **Diagnostic settings (classic)**.

3. In the **Diagnostic settings (classic)** pane, verify that the **Status** is set to **On**. Also verify that **Hour metrics** for the blob service are enabled, but **Minute metrics** are disabled by default.

4. Examine the properties of the file, table, and queue services. The default configuration should be similar to those for the blob service.
...
2. Switch to the Azure Cloud Shell window. This window should display a PowerShell prompt.

3. Create an Azure storage context object by running the following command. Specify your storage account name, and account key where indicated:

    ```powershell
    $context = New-AzureStorageContext -StorageAccountName <your-storage-account-name> -StorageAccountKey <your-storage-account-key>
    ```

4. Run the `Set-AzStorageServiceMetricsProperty` command to configure minute-level storage metrics, with a retention period of 7 days:
...
6. Return to the Azure portal. On the **Diagnostics settings (classic)** page for your storage account, verify that the **Minute metrics** for the blob storage service are enabled:
...
2. In the blobs pane, click **+Container**.

3. Add a container named **testcontainer**, using the **Private** access level, and then click **OK**.

4. In the menu pane for your storage account, under **Settings**, click **Access keys**. Make a note of the connection string for **key1**.

5. Return to the PowerShell prompt in Azure Cloud shell.

6. Run the following command to switch to download the sample code for this exercise:
...
9. Wait for 1 minute, and then repeat the command.
...
2. In the menu pane, under **Monitoring (classic)**, click **Metrics (classic)**. 

3. In the **Show data for last** bar, click **1 hour**. In the **Show data for** list, select **Blobs**, and de-select **Files**, **Tables**, and **Queues**. You should see a set of graphs similar to those in the following image. Each graph shows three peaks, corresponding to each time that you ran the sample application.
...
3. On the **View** menu, click **Account Management**.

4. In the **SETTINGS** pane, click **Add an account**.

5. In the **Connect to Azure Storage** dialog box, click **Use a connection string**, and then click **Next**.

6. In the **Attach with Connection String** dialog box, set **Display name** to the name of your storage account, enter the connection string for your account that you noted earlier, click **Next**, and then click **Connect**.

7. In the **SETTINGS** pane, click **Apply**.
...
9. Click **\$MetricsMinutePrimaryTransactionsBlob**. This table contains the minute-by-minute summary of the transactions performed in blob storage.

10. In the toolbar, click **Column Options**, and select the following columns:
