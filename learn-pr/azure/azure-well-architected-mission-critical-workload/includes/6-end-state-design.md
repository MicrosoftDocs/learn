...
It's highly recommended that dev/test environments shouldn't share resources with the production environment. Each environment has its own set of requirements for reliability, capacity, and security requirements. Can you identify services in this architecture that are shared across the two enviroments? How will you change the design so align with that recommendation.
