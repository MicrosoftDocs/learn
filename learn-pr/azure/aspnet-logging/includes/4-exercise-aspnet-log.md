...
1. Create an Application Insights instance.
    ...
    > [!NOTE]
    > Currently, Azure PowerShell and the Azure CLI don't have direct commands for creating this resource type. You must use the general purpose Azure resource creation command that's used in the following code. When prompted, select **Y**, and then press <kbd>Enter</kbd> to create the Application Insights instance.
...
1. Replace the contents of the file with the following text:
   ...
   - **azureAppendBlobAppender**: Writes to Azure Blob Storage in your storage account. In this appender, replace *YourStorageAccountConnectionString* with the value in the **$storageconnectionstring** PowerShell variable. You can use the command `Write-Host $storageconnectionstring` to see the value of this variable.
...
1. If you didn't previously deploy the web app and configure the connection to Azure SQL Database, complete the following substeps. Otherwise, go to step 7.
    ...
    1. In the **Destination Connection String** dialog box, enter the values that are described in the following table, and then select **OK**:
...
1. On the **Storage account** pane, under **Data storage**, select **Containers**.

    :::image type="content" source="../media/4-containers.png" alt-text="Screenshot of the Storage account pane, with Overview in the menu and the Containers tile highlighted.":::

    The **Containers** pane for your storage account appears.

1. Select the **workshopcontainer** container.

    :::image type="content" source="../media/4-workshop-container.png" alt-text="Screenshot of the Containers pane, with the workshopcontainer container highlighted.":::

1. On the container's blobs pane, select the **eShopModernizedLogs** folder.

    :::image type="content" source="../media/4-workshop-blob.png" alt-text="Screenshot of the container's pane, with the eShopModernizedLogs folder highlighted.":::
...
1. Select the append blob that is shown in this folder. The blob is named by using the current date, with the suffix *.entry.log.xml*.
    ...
    :::image type="content" source="../media/4-log-blob.png" alt-text="Screenshot of the Blobs pane, with the blob that contains the log entries that were generated by the web app highlighted.":::
...
1. Close Notepad and return to the Azure portal home page.
...
1. On the **All services** page, in the search box, enter **Application Insights**, and then press <kbd>Enter</kbd>.

1. On the **Application Insights** pane, select the Application Insights instance you created earlier. The **Overview** pane should display general statistics for the web app.

1. In the middle menu pane, under **Investigate**, select **Performance**.

    :::image type="content" source="../media/4-performance.png" alt-text="Screenshot of the Application Insights pane, with Performance highlighted in the menu.":::

    The **Performance** pane appears for your Application Insights instance.

1. Select any operation. In the **Drill into** section, select **Samples**.

    :::image type="content" source="../media/4-select-operation.png" alt-text="Screenshot of the Performance pane, with a GET operation and the Samples button highlighted.":::

    > [!NOTE]
    > It might take several minutes for the logging data to appear in Azure Blob Storage. If no operations are shown, refresh the page.

    The **Select a sample operation** pane appears.

1. Select any of the samples that are shown.
...
1. On the **End-to-end transaction details** pane, select **View all telemetry**.

    :::image type="content" source="../media/4-end-to-end-details.png" alt-text="Screenshot of the End-to-end transaction details pane, with the View all telemetry button highlighted.":::
...
1. On the detailed telemetry page, select any logging event. Logging events are identified as *Verbose* or *Information*. If the sample operation doesn't display any logging events, in the left pane, select a different sample.
    ...
    :::image type="content" source="../media/4-select-logging.png" alt-text="Screenshot of the End-to-end transaction details pane, with a logging event highlighted.":::
