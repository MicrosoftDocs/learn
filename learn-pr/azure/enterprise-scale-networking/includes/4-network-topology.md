...
-Azure Firewalls can be deployed into the VWAN hub, but third party firewalls have to be deployed into a spoke VNet. <https://docs.microsoft.com/azure/virtual-wan/scenario-route-through-nva> 
-There is no need to build a transit network on top of VWAN, as VWAN is the transit network. To move from hub and spoke to VWAN see: <https://docs.microsoft.com/azure/virtual-wan/migrate-from-hub-spoke-topology>
-Azure Virtual WAN is limited to two thousand virtual machines per spoke. If the deployment will be higher than this, consider hub and spoke. <https://docs.microsoft.com/azure/virtual-wan/virtual-wan-faq#is-there-a-network-throughput-or-connection-limit-when-using-azure-virtual-wan>
