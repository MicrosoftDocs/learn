...
:::image type="content" source="../media/4-virtual-wan-topology.png" alt-text="Diagram that illustrates a Virtual WAN network topology.":::
...
When multiple ExpressRoute circuits are connected to the same virtual network, connection weights or BGP techniques must be used to ensure an optimal path for traffic between on-premises and Azure. If connection weights are used, BGP AS prepending is ignored. 

The diagram below shows a basic hub and spoke topology, where ExpressRoute is deployed and connected to the hub. Application landing zones are deployed in virtual networks and connected to the hub with virtual network peering.
