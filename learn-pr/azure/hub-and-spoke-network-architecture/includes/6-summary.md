In this module, you've explored migrating your on-premises infrastructure to Azure by using the hub and spoke model. You covered the components that you need to create hub-spoke networks, including Azure ExpressRoute, and how to secure them in Azure.

A hub-spoke architecture in Azure allows your business to quickly and easily adapt to new requirements. You can add spokes to segregate workloads with network security groups and Azure Firewall.

[!include[](../../../includes/azure-sandbox-cleanup.md)]

## More information

- [Hub-spoke topology](/azure/architecture/reference-architectures/hybrid-networking/#hub-spoke-network-topology)
- [Connect to Azure by using ExpressRoute](/azure/architecture/reference-architectures/hybrid-networking/expressroute)
- [Azure DDoS protection](/azure/virtual-network/ddos-protection-overview)