...
It's a good example of how to do things and a great way to start for anyone who wants to make an identical feature for themselves. It is just not intended to be a comprehensive or utterly functional product.

## Pre-requisites

This project makes use of **StereoKit** to generate and drive this as a Mixed Reality application, allowing us to run it on the HoloLens 2 and VR headsets.
...
* Features and targets supported in **.Net Core Project** includes:
  * WMR VR Desktop
  * Flatscreen Desktop
  * Leap Motion articulated hands
  * No compile time

* Features and targets supported in **UWP Project** includes:
...
**StereoKit Ink** application utilizes a fully articulated hand to draw lines and models on the canvas. The accessing hand in the application is essential as it mimics the movement of the user's hands to draw effortlessly. Fully articulated hands are always available to work with as StereoKit uses a hands-first approach. Even when the hand sensors aren't available, the existing devices simulate the hand data.
