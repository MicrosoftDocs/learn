...
1. Open Azure Cloud Shell from the Azure portal by selecting the Cloud Shell icon.
...
1. Run the following command to define a variable called ACR_NAME.
    ...
    > [!IMPORTANT]
    > Before running the command, replace `<registry-name>` with the unique name you want to give your new container registry. The registry name must be unique within Azure, and contain 5-50 **alphanumeric** characters. For more information about naming, see [Naming conventions for Azure resources](/azure/architecture/best-practices/naming-conventions?azure-portal=true).

    ```azurecli
    ACR_NAME=<registry-name>
    ```

1. Run the following command in Cloud Shell to create our new container registry.
