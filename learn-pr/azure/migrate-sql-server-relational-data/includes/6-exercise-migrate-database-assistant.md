...
1. In the pane that appears, select **Migration** for the Project type.
...
1. Change the Migration scope set to **Schema only** and select **Create**.

## Select the Source

1. In the Connect to source server area, enter **admsdemovm** for the source server and accept the other defaults.

1. Select **Connect**. Once connected, you will see a list of databases.
...
1. Every Azure SQL Server must have a unique name. When you ran the environment setup, the script took care of it for you and generated a unique name beginning with **admsdemosql**, followed by a sequence of random characters, followed by **.database.windows.net**. 
   ...
   To find yours, go to the Azure Portal, open the **admsdemorg** resource group, and locate the _SQL server_ (note: not the SQL database, but the server). Enter that server name into the Server name box. As an example, it might look something like **admsdemosqlvsm4cdzuubghw.database.windows.net**.

1. The authentication type should automatically update to **SQL Server Authentication**.
...
1. In this exercise there should only be one database on the server, the empty **Social** database you created during the setup, and it should be selected by default. Select **Next** to proceed to the next step.
...
1. Select **Generate SQL script** to proceed.
