...
We discovered that the extension allows you to connect to a remote machine, such as a desktop PC or a virtual machine (VM), through a secure tunnel. We also found out that it eliminates the need for having the source code on your Visual Studio Code client machine by running commands and other extensions directly on the remote machine. We also covered the configuration and use of the extension through the Visual Studio Code UI and the code command-line interface (CLI).
