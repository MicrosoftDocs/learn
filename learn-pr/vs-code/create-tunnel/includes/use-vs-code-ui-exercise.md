...
1. Select **Copy Browser Link to Clipboard** in the popup for the tunnel link and save it.
...
1. Go to another computer (client) of your choice and retrieve the link. From a browser, paste the tunnel link from the previous step.
    - A *client* is any device that can run a web browser and has internet access, such as a phone, tablet, or another computer.
    
1. Confirm the account you used to start the tunnel and begin coding!
...
When you go to <a href="http://vscode.dev" target="_blank">vscode.dev</a>, you access a lightweight version of Visual Studio Code running fully in the browser. This version is known as Visual Studio Code for the Web (or `vscode.dev`). This step allows you to open a folder on your local machine and start coding without any installation. You can find official docs for <a href="http://vscode.dev" target="_blank">vscode.dev on this version of Visual Studio Code on the browser</a>.

> [!NOTE]
> The remote machine is only reachable through a tunnel while Visual Studio Code is running there. After you exit Visual Studio Code, tunneling to it won't be possible until you start Visual Studio Code there again or run the code tunnel command-line interface command.