...
Color filters are another type of tool that may allow players with certain types of color blindness to more easily distinguish certain visual elements on screen. They are often provided at the platform level to provide a base level of access to experiences that might not be otherwise accessible to some individuals.
...
As a general rule, however, color filters at the platform or game level should not be considered a solution for players with color blindness. This is because color filters can impact other visual aspects of a game in unintended ways. This can end up making some visual components more difficult to see for certain players depending on the severity of their color blindness. Instead, a better option is to offer players color palettes to choose from that individually change the color of specific elements.
