...
1. If the workload runs on a supported Azure resource, enable a system-assigned or user-assigned managed identity for that resource.
...
1. If federation is supported for the scenario, add a federated identity credential so the application trusts the external or managed identity token.
...
1. Test token acquisition for the chosen credential type before assigning broad permissions.
...
- Microsoft Graph application permissions for directory and Microsoft 365 operations.
- App roles for access to custom APIs or application-specific functions.
...
1. Scope the assignment to the smallest workable boundary, such as a specific resource, resource group, or application.
1. Remove overlapping or inherited permissions that the workload doesn't need.
1. Record why each permission exists so future reviews can confirm that the access is still justified.
