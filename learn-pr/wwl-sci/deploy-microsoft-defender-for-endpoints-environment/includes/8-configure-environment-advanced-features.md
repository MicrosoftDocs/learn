...
> [!TIP]
>For tenants created prior to that version, you'll need to manually turn on this feature from the Advanced features page.
...
> If a security operations analyst manually sets the status of an alert to "In progress" or "Resolved" the auto-resolve capability won't overwrite it.
...
This feature enables you to block potentially malicious files in your network. Blocking a file prevents it from being read, written, or executed on devices in your organization.
...
> [!NOTE]
> Network protection uses reputation services that process requests in locations that might be outside of the location you've selected for your Defender for Endpoint data.
...
> [!NOTE]
> When a device is being isolated from the network, there's a pop-up where you can choose to enable Outlook and Skype communications which allows communications to the user while they're disconnected from the network. This setting applies to Skype and Outlook communication when devices are in isolation mode.
...
This feature is only available if you have an active Office 365 E5 or the Threat Intelligence add-on. 
...
This feature is only available if you have the following prerequisites:
