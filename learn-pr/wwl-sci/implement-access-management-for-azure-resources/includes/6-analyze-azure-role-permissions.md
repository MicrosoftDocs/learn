...
| Register new applications | Read properites of registered and enterprise applications |
...
### Controling permissions - add and restrict
...
- Manage setings for external collaboration

By adding roles to a given user account or group, you can add permissions on to member users, guest users, and service principals.  Adding roles gives permissions to perform specific activities.  Actions are limited, which allows the rule of least prvilege.
...
If at all possible you only want to grant the permissions to a user they need.  So be care to know what all permissions are granted when you assign a role.  you can see the list of permissions in the **Attribute defintion reader**.  To open it, launch Azure AD, then open the **Roles and administrators** screen. Next select a role, and open its description page from the ellipsis (...) menu.  Depending on the role you chose, you will see a large number of permissions or possibly a very small number. Two sets of permissions:
- Role permissions
