...
Always remember that managed identities are assigned to an application.  So, you need to configure and managem them within the services they are being used. If you have an application running in a virtual machine (Lixus or Windows), then you add and configure the identity there.  If you are using a managed identity with a cloud-app, function, or app service.  Then you configure and manage it there.  Let's look at adding a managed identity to a cloud built using the App Service.
...
As stated at the being of this page, when you build and app you need a method to grant it access to resources.  The take advantage of the concepts of **zero trust** you can use managed identities.  You only assign the minimum privileges that the managed identity needs.  Then only assign access the minimum resources needed.  This will keep you applications and data protected.
