...
The speed that makes Perception useful is the same speed at which a wrong action lands. Approval gates are where people still own decisions with real consequences. They pause a playbook before high-impact actions. Each agent runs under its own dedicated Microsoft Entra Agent ID identity with least-privilege access scoped to what that agent does, so agent activity is attributable to that agent and not to a human. Agents can only take actions their scope allows, and where an approval gate is set, the agent pauses until a person signs off. Agent actions are logged, and Project Perception inherits the identity, permission, and audit controls already in use in the Defender portal rather than introducing a parallel governance model.
