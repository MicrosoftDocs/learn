...
Allowing the use of unsigned scripts may increase your exposure to threats.  Running unsigned scripts is not recommended as it can increase your exposure to threats. However, if you must use them, you'll need to enable the setting in the Advanced features settings page.
...
- If you are waiting for a file to download, you can move it to the background by using **Ctrl + Z**.
...
1. Specify whether you'd like to overwrite a file that has the same name.
...
Live response supports table and JSON format output types. For each command, there's a default output behavior. You can specify your preferred output format using the following command flags:
...
Select the Command log tab to see the commands used during a session. Each command is tracked with full details, including:
...
The following are command examples that demonstrate the use of the live response commands.
