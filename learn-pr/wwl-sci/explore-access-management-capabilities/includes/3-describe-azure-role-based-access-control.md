...
Microsoft Entra ID roles control permissions to manage Microsoft Entra ID resources. For example, allowing user accounts to be created, or billing information to be viewed.  Microsoft Entra ID supports built-in and custom roles. 

Managing access using roles is known as **role-based access control (RBAC)**. Microsoft Entra ID built-in and custom roles are a form of RBAC in that Microsoft Entra ID roles control access to Microsoft Entra ID resources.  This is referred to as Microsoft Entra ID RBAC.
...
### Categories of Microsoft Entra ID roles
...
Over time, some Microsoft 365 services, such as Exchange and Intune, have developed their own role-based access control systems (RBAC), just like the Microsoft Entra ID service has Microsoft Entra ID roles to control access to Microsoft Entra ID resources. Other services such as Teams and SharePoint don’t have separate role-based access control systems, they use Microsoft Entra ID roles for their administrative access.
...
:::image type="content" source="../media/role-overlap-diagram-v3-inline.png" lightbox="../media/role-overlap-diagram-v3-expanded.png" alt-text="Microsoft Entra ID role categories.":::

### Difference between Microsoft Entra ID RBAC and Azure RBAC
As described above, Microsoft Entra ID built-in and custom roles are a form of RBAC in that they control access to Azure AD resources.  This is referred to as Microsoft Entra RBAC.  In the same way that Microsoft Entra ID roles can control access to Microsoft Entra ID resources, so too can Azure roles control access to Azure resources.  This is referred to as Azure RBAC.  Although the concept of RBAC applies to both Azure AD RBAC and Azure RBAC, what they control are different.

- Microsoft Entra RBAC - Microsoft Entra ID roles control access to Microsoft Entra ID resources such as users, groups, and applications.
