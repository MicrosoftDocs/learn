...
After you've monitored the alerts for some time and adjusted the sensitivity of your policy, you should keep the policy in test mode and activate the policy tips for some time.  This will allow your users' time to help refine the policy further by reporting false positives.
...
1. In **Edit rule** pane, go down to the **User notifications** section, in **Use notifications to inform your users and help educate them on the proper use of sensitive info**, select **On**.
    ...
    ![image that shows a user notifications example](..\media\rule-user-notification.png)
...
1. Decide on the various other parameters available to fine-tune your incident reports.
    ...
    ![image that shows a rule incident example](..\media\rule-incident-report.png)
