...
| At scale | [Connect machines from Automation Update Management](/azure/azure-arc/servers/manage-agent?tabs=windows) to create a service principal that installs and configures the agent for multiple machines managed with Azure Automation Update Management to connect machines non-interactively. |
