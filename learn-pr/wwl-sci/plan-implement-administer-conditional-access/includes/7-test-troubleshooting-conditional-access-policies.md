...
**For all users, all cloud apps:**
 ...
 -  **Require device to be marked as compliant** \- For users who haven't yet enrolled their devices, this policy blocks all access including access to the Intune portal. If you're an administrator without an enrolled device, this policy blocks you from getting back into the Azure portal to change the policy.
 -  **Require Hybrid Microsoft Entra domain joined device** \- This access-blocking policy also has the potential to block access for all users in your organization if they don't have a Microsoft Entra hybrid joined device.
...
<a name='azure-active-directory-sign-in-events'></a>
...
1. Sign into the Azure portal as a Global Administrator, Security Administrator, or Global Reader.
1. Browse to **Microsoft Entra ID**, then **Sign-ins**.
1. Find the event for the sign-in to review. Add or remove filters and columns to filter out unnecessary information.
    1. Add filters to narrow the scope:
        ...
        1. Date scoped to the time frame in question.
            ...
            :::image type="content" source="../media/image-3-88d5dda0.png" alt-text="Screenshot of the error message screen. User is selecting the Conditional access filter in the sign-in log.":::
            
1. Once the sign-in event that corresponds to the user's sign-in failure has been found select the **Conditional Access** tab, the tab will show the specific policy or policies that resulted in the sign-in interruption.
