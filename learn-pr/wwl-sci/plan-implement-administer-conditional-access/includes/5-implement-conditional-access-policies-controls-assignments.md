...
A sign-in risk represents the probability that a given authentication request isn't authorized by the identity owner. Organizations with Microsoft Entra ID P2 licenses can create Conditional Access policies incorporating Microsoft Entra ID Protection sign-in risk detections.

This policy can be assigned either through Conditional Access itself or through Microsoft Entra ID Protection. Organizations should choose one of two options to enable a sign-in risk-based Conditional Access policy requiring a secure password change.
...
Like sign-in risk-based Conditional Access, this policy can be assigned either through Conditional Access itself or through Microsoft Entra ID Protection.

## Securing security info registration

Securing when and how users register for Microsoft Entra multifactor authentication and self-service password reset is now possible with user actions in Conditional Access policy. This preview feature is available to organizations that have enabled the combined registration preview. This functionality may be enabled in organizations where they want to use conditions like trusted network location to restrict access to register for Microsoft Entra multifactor authentication and self-service password reset (SSPR).
...
1. In the **Azure portal**, browse to **Microsoft Entra ID**, then **Security**, and then **Conditional Access**.
1. Select **New policy**.
...
1. Under **Conditions**, select **Locations**.
    1. Configure **Yes**.
    1. Include **Any location**.
    1. Exclude **All trusted locations**.
    1. Select **Done** on the **Locations** screen.
    1. Select **Done** on the **Conditions** screen.
1. Under **Conditions**, in **Client apps (Preview)**, set **Configure** to **Yes**, and select **Done**.
1. Under **Access controls**, select **Grant**.
    1. Select **Block access**.
    1. Then use the **Select** option.
...
1. Exclude **Device Microsoft Entra hybrid joined** and/or **Device marked as compliant.**
1. Select **Done** on the **Locations** screen.
1. Select **Done** on the **Conditions** screen.
...
1. Browse to **Microsoft Entra ID**, then **Security**, then **Conditional Access**, and then **Named locations**.
...
1. Under **Assignments**, select **Users and groups.**
    ...
    1. Under **Exclude**, select **Users and groups** and choose your organization's emergency access or break-glass accounts.
    1. Select **Done**.
1. Under **Cloud apps or actions**, then **Include**, and select **All cloud apps**.
1. Under **Conditions**, then **Location**.
    1. Set **Configure** to **Yes.**
    1. Under **Include**, select **Selected locations.**
    1. Select the blocked location you created for your organization.
    1. Choose **Select**.
1. Under **Access controls**, then select **Block Access**, and select **Select**.
...
1. Under **Cloud apps or actions**, then **Include**, and select **All cloud apps**.
    ...
    1. Select **Done**.
1. Under **Conditions**, then **Client apps (Preview)**, then **Select the client apps this policy will apply to**, leave all defaults selected and select **Done**.
1. Under **Access controls**, then **Grant**, select **Require device to be marked as compliant**.
    1. Select **Select**.
1. Confirm your settings and set **Enable policy** to **On**.
