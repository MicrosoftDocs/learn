...
The following table summarizes mitigations for sensitive data at different points in an application architecture.
...
| **Web Application** | <ul><li>Ensure that sensitive content isn't cached on the browser</li><li>Encrypt sections of Web App's configuration files that contain sensitive data</li><li>Explicitly disable the autocomplete HTML attribute in sensitive forms and inputs</li><li>Ensure that sensitive data displayed on the user screen is masked</li></ul> | 
...
| **Web API** | <ul><li>Ensure that sensitive data relevant to Web API isn't stored in browser's storage</li></ul> | 
...
| **Mobile Client** | <ul><li>Encrypt sensitive or personally identifiable data written to phones local storage</li><li>Obfuscate generated binaries before distributing to end users</li></ul> | 
| **WCF** | <ul><li>Set clientCredentialType to Certificate or Windows</li><li>WCF-Security Mode isn't enabled</li></ul> | 
