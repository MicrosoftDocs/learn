...
You can find the three reports in the **Azure portal**, then **Microsoft Entra ID**, and then **Security**.
...
If you allow users to self-remediate, with Microsoft Entra multifactor authentication (MFA) and self-service password reset (SSPR) in your risk policies, they can unblock themselves when risk is detected. These detections are then considered closed. Users must have previously registered for Microsoft Entra multifactor authentication and SSPR in order to use when risk is detected.
...
**Require the user to reset password** \- Requiring the users to reset passwords enables self-recovery without contacting help desk or an administrator. This method only applies to users who are registered for Microsoft Entra multifactor authentication and SSPR. For users who haven't been registered, this option isn't available.
...
Using the Microsoft Graph PowerShell SDK Preview module, organizations can manage risk using PowerShell. The preview modules and sample code are located in the [Microsoft Entra GitHub repo](https://github.com/AzureAD/IdentityProtectionTools).

## Use the Microsoft Graph API

Microsoft Graph is the Microsoft unified API endpoint and the home of Microsoft Entra ID Protection APIs. There are three APIs that expose information about risky users and sign-ins: `riskDetection, riskyUsers, and signIn`.
...
`signIn` allows you to query Microsoft Graph for information on Microsoft Entra sign-ins with specific properties related to risk state, detail, and level.
...
1. Browse to **Microsoft Entra ID**, then **Custom domain names**.
1. Take note of the .onmicrosoft.com domain. You'll need this information in a later step.

### Create a new app registration

1. In the Azure portal, browse to **Microsoft Entra ID**, then **App registrations**.
1. Select **New registration**.
1. On the **Create** page, perform the following steps:
    1. In the **Name** textbox, type a name for your application (for example: Microsoft Entra ID Risk Detection API).
