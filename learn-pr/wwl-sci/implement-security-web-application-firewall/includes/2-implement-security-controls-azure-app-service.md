...
- **Allowed token audiences**: Set this to your application's Entra ID app registration client ID. This rejects tokens issued for other applications, even if they were signed by a trusted issuer.
...
To configure EasyAuth in the Azure portal, navigate to your App Service, select **Settings** > **Authentication**, and then select **Add identity provider**. Choose **Microsoft** as the provider, then specify your Entra ID tenant and app registration.
...
All traffic to App Service should travel over HTTPS. The platform accepts HTTP connections by default, which means credentials, tokens, and application data can be transmitted over unencrypted connections without additional configuration. Two settings close this gap.
...
**Minimum TLS version** sets the lowest accepted protocol version. Set this to **1.2** to reject connections from clients using TLS 1.0 or 1.1, both of which have known vulnerabilities and are considered deprecated. The minimum TLS version setting is on the same **General settings** page.
...
- **IP-based rules**: Allow a specific IPv4 address or CIDR range. Use this when the Application Gateway's public IP address is static and known.
...
## Enable VNet integration for private outbound access

VNet integration allows your App Service to make outbound connections to resources inside an Azure virtual network without those resources having a public endpoint. Your App Service can reach a private Azure SQL Database, a Key Vault with public access disabled, or a private API endpoint — all over the virtual network's address space.
...
Configure VNet integration in the Azure portal under **Settings** > **Networking** > **VNet integration**. Select **Add VNet integration** and choose the virtual network and subnet. The subnet must be delegated to `Microsoft.Web/serverFarms` and must not be used for any other App Service resource.
