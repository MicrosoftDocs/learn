...
For Windows, Microsoft Defender integrates with Azure services to monitor and protect your Windows-based machines. Security Center presents the alerts and remediation suggestions from all of these services in an easy-to-use format.

For Linux, Microsoft Defender collects audit records from Linux machines by using auditd, one of the most common Linux auditing frameworks. auditd lives in the mainline kernel.
...
- **Vulnerability assessment scanning for VMs** - The vulnerability scanner included with Defender for Cloud is powered by Qualys.
...
- When you enable Defender for servers, you can use just-in-time VM access to lock down the inbound traffic to your VMs, reducing exposure to attacks while providing easy access to connect to VMs when needed.
