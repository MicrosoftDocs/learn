...
When you get an alert from Defender for Key Vault, it's important to investigate and respond to the alert as described in Respond to Defender for Key Vault. Defender for Key Vault protects applications and credentials. So even if you're familiar with the application or user that triggered the alert, it's important to check the situation surrounding every alert.
