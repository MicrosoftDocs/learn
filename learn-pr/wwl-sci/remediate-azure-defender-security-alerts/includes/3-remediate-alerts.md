From Defender for Cloud's overview page, select the Security alerts tile at the top of the page or the link on the sidebar.
...
- When a Microsoft Defender for Cloud Recommendation is created or triggered - If your logic app relies on a recommendation that gets deprecated or replaced, your automation will stop working. You'll then need to update the trigger. To track changes to recommendations, see Microsoft Defender for Cloud release notes.

- When a Microsoft Defender for Cloud Alert is created or triggered - You can customize the trigger so that it relates only to alerts with the severity levels that interest you.
