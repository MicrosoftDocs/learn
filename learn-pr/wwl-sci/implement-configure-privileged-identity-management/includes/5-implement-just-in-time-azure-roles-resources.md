...
## Understand the Azure resource scope in PIM
...
Navigate to **ID Governance** > **Privileged Identity Management** > **Azure resources**. Here, you're managing RBAC assignments on Azure control-plane objects rather than directory identities, and keeping this distinction in mind prevents you from inadvertently configuring the wrong target when managing resources. The updated PIM experience uses the latest ARM API and automatically surfaces Azure resources in your tenant — no manual onboarding step is required. The table below summarizes the key operational differences between the two planes.
...
|---|---|---|---|
| Critical | Production subscription, Key Vault, Azure AI services | Compromise grants broad lateral movement; Key Vault secrets unlock downstream systems; AI model weights and training data represent exfiltrable IP | multifactor authentication (MFA) required, justification required, approval required, 1–2 hour max |
...
Key Vault sits in the Critical tier because a compromised Owner on a Key Vault exposes every secret it holds — including credentials that authenticate downstream services. That cascading risk means a single Key Vault compromise can propagate silently across multiple dependent systems before your incident response team identifies the initial breach. Azure AI services carry similar weight: model weights and training data represent significant proprietary investment, and exfiltration of that data may not surface in logs immediately.
...
> [!NOTE]
> If you navigate to **Azure resources** and your subscriptions or resource groups don't appear, select **Discover resources** before troubleshooting permissions or other issues. Some tenants or older PIM configurations may require this one-time step to surface resources in the list.
...
To activate an eligible Azure resource role, navigate to **My roles** in PIM, select the **Azure resources** tab to see your eligible assignments, and select **Activate** — the activation steps follow the same flow as Entra roles.
...
Individual eligible assignments now protect both the identity plane and the resource plane. As your organization scales — with multiple engineers needing temporary access to the same production resources — managing assignments one by one creates overhead and increases the chance that you'll overlook an active assignment. PIM for groups addresses this by making group membership itself eligible, which you explore in the next unit.
