...
## Assign eligible access to an Entra role

When you assign a user to a role as eligible, you convert them from always holding the role to holding only the entitlement. The role grants no active permissions until the user initiates an activation. The procedure below makes that conversion for any Microsoft Entra ID role in PIM.
...
The following table shows recommended activation controls organized by impact tier.

| Impact tier | Example roles | MFA | Justification | Approval | Max duration |
|---|---|---|---|---|---|
...
## Activate an eligible Entra role
...
> [!NOTE]
> For roles that require approval, activation may be delayed depending on approver availability. For emergency response scenarios, use a dedicated break-glass account rather than an approval-gated activation.
