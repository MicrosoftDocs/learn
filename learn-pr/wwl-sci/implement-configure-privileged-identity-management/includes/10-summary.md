...
You started with the structural risk: standing privilege gives attackers an unlimited exploitation window, and blast radius expands with every over-privileged permanent assignment. PIM addresses this by making eligible access the default — granting access only when it's activated, for as long as it's needed, under conditions you define.
...
- **PIM for Groups** — you replaced per-user assignment sprawl with group-based JIT, enforcing a single activation policy across an entire team and reducing both configuration drift and audit surface.
