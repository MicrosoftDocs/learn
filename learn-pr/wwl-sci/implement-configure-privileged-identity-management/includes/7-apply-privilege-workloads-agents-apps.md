...
| **Typical use** | Azure-hosted workloads (virtual machines, functions, containers) | Applications hosted outside Azure, or tools needing custom auth flows |
| **PIM eligible?** | No—non-interactive, can't activate eligible roles | No—same limitation; service principals can't complete interactive activation |
