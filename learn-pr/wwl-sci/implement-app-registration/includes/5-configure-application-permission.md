...
> [!NOTE]
> This permission appears on all consent screens today, even for flows that don't provide a refresh token (the *implicit flow*). This is to cover scenarios where a client can begin within the implicit flow, and then move on to the code flow where a refresh token is expected. For more information, see [Microsoft identity platform and implicit grant flow](/azure/active-directory/develop/v2-oauth2-implicit-grant-flow).
