Microsoft Purview Audit, both Standard and Premium versions, includes the capability to export audit logs to CSV. This feature is essential for detailed data analysis and compliance reporting.
...
1. Begin by searching the audit log. Adjust your search criteria as needed to ensure you capture the data you require.
1. On the search results page, select **Export** to download all the audit records from your search into a CSV file. It takes a while to prepare the download file for a large search. Large files result when searching for all activities or using a wide date range.
...
1. Start by opening a blank workbook in Excel and navigate to the **Data** tab. Select **From Text/CSV** to open your exported CSV file.
...
1. When the CSV file opens, select **Transform Data** to begin editing in the Power Query Editor.
   :::image type="content" source="../media/json-open-power-query.png" alt-text="Screenshot showing the Transform Data button in Excel.":::

1. In the **Query Editor**, right-click **AuditData**, select **Transform**, then choose **JSON**. This step transforms the data into a readable format, creating separate columns for each property within the JSON object.
...
1. Select **Close & Load** on the **Home** tab to exit the Power Query Editor and open the transformed CSV file in an Excel workbook.

### Use PowerShell to search and export audit log records

Instead of using the audit log search tool in the Microsoft Purview portal or the Microsoft Purview compliance portal, you can utilize the Search-UnifiedAuditLog cmdlet in Exchange Online PowerShell for exporting audit log search results to a CSV file. This method allows for precise targeting of audit records by using the RecordType parameter. After exporting, you can apply the same steps as described previously to format the audit log using the Power Query editor in Excel, enhancing the accessibility and analysis of the data.
...
1. **Search for SharePoint sharing operations**:
   - To capture all relevant activities within a specific period, run:
...
1. **Append additional data**:
   - If you need to include file activities from the same date range, you can append these to the existing CSV:
