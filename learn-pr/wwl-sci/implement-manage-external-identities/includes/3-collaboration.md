...
Azure AD allows you to restrict what external guest users can see in your Azure AD directory. By default, guest users are granted a limited permission level. The guests are blocked from listing users, groups, or other directory resources. But, the guests can see membership of non-hidden groups. A setting also lets you restrict guest access even further, so that guests can only view their own profile information. For details, see [Restrict guest access permissions](/azure/active-directory/enterprise-users/users-restrict-guest-permissions).
