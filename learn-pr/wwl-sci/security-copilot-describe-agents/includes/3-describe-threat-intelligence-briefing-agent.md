...
The Threat Intelligence Briefing Agent addresses these challenges by applying dynamic automation and deep generative AI to produce customized, context-rich reports within minutes. It integrates signals from tools like Microsoft Defender External Attack Surface Management and Microsoft Defender for Endpoint to provide situational awareness for CISOs and managers, while offering a solid foundation for analysts to enhance threat defense strategies. The Threat Intelligence Briefing Agent is available through the Security Copilot standalone experience and is also available as an embedded experience in Microsoft Defender.

### Information about the agent and prerequisites

The following information describes the Threat Intelligence Briefing Agent as available through the Security Copilot standalone experience:

- **Trigger**: Runs every 7 days or can be triggered manually.
- **Permissions**: This agent can read data from Agents, Microsoft Defender External Attack Surface Management, Microsoft Threat Intelligence, and Microsoft Threat Intelligence Agents.
- **Identity**: Requires connection to an existing user account or creation of a new agent identity (recommended). Microsoft Entra Agent IDs are identities created specifically for AI agents. Using Agent IDs keeps access scoped, secure, and easier to manage.
- **Products**: Microsoft Security Copilot with provisioned capacity and a Microsoft Defender External Attack Surface Management (EASM) Standard license are necessary to run the agent.
- **Plugins**: The Microsoft Threat Intelligence and Microsoft Threat Intelligence Agent plugins are mandatory. The Microsoft Defender External Attack Surface Management plugin is optional but adds more context to the output.
