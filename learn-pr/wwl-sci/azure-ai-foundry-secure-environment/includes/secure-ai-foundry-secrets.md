AI workloads depend on credentials, keys, and connection strings to connect models, datasets, and services. In Microsoft Foundry, these secrets are used to authenticate to data sources, APIs, and managed compute. If credentials are stored in configuration files or shared directly between users, they might be exposed or misused.
...
Each Microsoft Foundry resource supports a single Key Vault connection across the resource and its associated projects. The connection is currently in preview and can only be created when no existing Key Vault connection is active.
...
## Integrate Key Vault with Microsoft Foundry

After permissions are configured, connect Foundry to your vault. In the **Microsoft Foundry portal**, go to **Management center** > **Settings** > **Connections**, select **Add connection**, and choose **Azure Key Vault**.
...
The diagram shows how Microsoft Foundry retrieves secrets from Azure Key Vault through a managed identity, allowing workloads to access required resources without embedded credentials.

:::image type="content" source="../media/foundry-key-vault-diagram.png" alt-text="Diagram showing Microsoft Foundry using a managed identity to retrieve secrets from Azure Key Vault for secure authentication." border="false" lightbox="../media/foundry-key-vault-diagram.png":::
...
Your organization's Microsoft Foundry resource already connects to several data sources. You've been asked to secure the connection strings and API keys used by model deployments.
...
1. On the **Members** tab, choose **Managed identity**, select the identity for **Microsoft Foundry** from the drop-down list, and select the appropriate Foundry resource.

1. On the **Review + assign** tab, review your settings, then select **Review + assign**.

   :::image type="content" source="../media/review-assign-identity-key-vault.png" alt-text="Screenshot showing a Key Vault role assignment granting the Microsoft Foundry resource the Key Vault Secrets User role." lightbox="../media/review-assign-identity-key-vault.png":::

1. In the **[Microsoft Foundry portal](https://ai.azure.com/?azure-portal=true)**, open **Management center** > **Connected resources**, select **+ New connection**, and choose **Azure Key Vault**.
