...
> [!IMPORTANT]
> The most important thing to remember about `while` loops is to ensure that the condition changes. If the condition is always true, Python will continue to run your code until the program crashes.
...
- The code you want to run for each item in the iterable, followed by a nested whitespace. For example:
...
Let's see how you can create code to prompt users to enter values, and then allow them to use *done* when they've finished entering the values. In our example, let's use the user input as the condition and then test it at the top of the `while` loop.
...
Notice that we're using `input` to prompt the users. Each time users enter a new value they're changing the condition, which means that the `while` loop will exit after they've entered *done*.

> [!NOTE]
> In our example, we're using `lower` to convert the input to lowercase to allow for a case-insensitive comparison.
...
Notice the `if` statement inside the `while` loop. This statement tests for a string value inside `user_input`. If the `while` loop is running for the first time, there is no value, so we won't store it in `inputs`. After the first time it runs, `user_input` always keeps the value that the user has just entered. Because `while` is testing to ensure that the value doesn't equal `'done'` (the word the user will use to exit), you know that the current value is one that you can add to the list.
