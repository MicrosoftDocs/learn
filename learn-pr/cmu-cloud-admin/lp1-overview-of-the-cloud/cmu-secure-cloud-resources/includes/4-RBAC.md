...
A key aspect of administering cloud services for an organization is controlling access to the resources that comprise those services -- that is, making sure users who require access to a service or one of its components have that access, while others do not. This is known as *identity and access management (IAM)*. In Azure, AWS, and GCP, IAM is achieved using a technique known as *role-based access control* (RBAC).
