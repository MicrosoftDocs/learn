...
A virtual network overlay has a tendency to mask changes in the underlying physical network to which its addresses are mapped. These physical changes are often the result of the evolution of a solution, and therefore the physical network that connects its components. Performance degradation, as well as alterations to the network's threat profile, may be evidence of discrepancies in the virtual network overlay that could lead to vulnerability and, in the worst case, exploitability.
