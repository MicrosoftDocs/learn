...
It is important to understand the concept of a transaction. By default, SQL Server and Azure SQL Database are in auto-commit mode, which means the changes made by the statement below would automatically be written to the database’s transaction log upon completion.
