Your search solutions performance can be affected by the size and complexity of your indexes. You also need to know how to write efficient queries to search it and choose the the right service tier. 
...
1. Try to avoid partial search terms like prefix search or regular expressions. As these kinds of searches are more computationally expensive.
1. Avoid using high skip values. As this forces the search engine to retrieve and rank larger volumes of data.
...
Adding extra partitions can help with performance as the search engine can run in parallel in each partition. The best improvements are seen for queries that return large numbers of documents and queries that use facets providing counts over large numbers of documents. This is a factor of how computationally expensive it is to score the relevancy of documents.

## Use the best service tier for your search needs

You've seen that you can scale out service tiers by adding additional partitions. You can scale out with replicas if you need to scale because of an increase in load. You can also *scale up* your search service by using a higher tier.
