In this excerise, you'll try out some of the techniques you've learned about in this module to build a cognitive search solution.

> [!NOTE]
> A virtual machine containing the client tools you need is provided. If you prefer to build you're own development environment, you can do so - these [setup notes](https://microsoftlearning.github.io/AI-102-AIEngineer/Instructions/00-setup.html) describe the software installed in the hosted virtual machine.
>
> You will also need an [Azure subscription](https://azure.microsoft.com/free) to complete the exercise.

When you finish the exercise, don't forget to come back and complete the knowledge check to earn points for completing this module!
