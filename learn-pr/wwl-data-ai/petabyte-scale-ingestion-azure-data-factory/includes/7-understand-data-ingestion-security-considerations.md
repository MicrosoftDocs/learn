...
Register Service principals within Azure Active Directory to take advantage of token management so that Azure Data Factory service can streamline its authentication across Azure resources. A data factory can be associated with a managed identity for Azure resources that represents the specific data factory. You can use this managed identity for Azure SQL Database authentication. The designated factory can access and copy data from or to your database by using this identity.

## Data protection

You may need to place special considerations for specific types of data, such as medical data, or financial data in the following areas.

### Use Role Based Access Control (RBAC) to control access to resources and Data Factory security and authentication

Use RBAC and Azure Data Factory Service principals to control access.  Using Azure RBAC, you can segregate duties within your organization and grant only the amount of access to users that they need to perform their jobs. Azure Data Factory supports Service principal and managed service identity (MSI) authentication 
...
Diagnostic setting for Azure Data Factory that send logs to Log Analytics can have alerts configured for a set of predefined set of conditions that can alert an administrator to activities.

### Follow logging and monitoring standards within your organization

Check your organization's standards for logging and monitoring and rely on the Azure standards for:
- Audit logging 
- Security logs
