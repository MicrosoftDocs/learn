...
SELECT oh.SalesOrderID, oh.OrderDate,
    od.ProductID, od.UnitPrice, od.Orderqty
FROM SalesLT.SalesOrderHeader AS oh
OUTER APPLY (SELECT productid, unitprice, Orderqty
        FROM SalesLT.SalesOrderDetail AS od
        WHERE oh.SalesOrderID = SalesOrderID
            ) AS od;
```

Use `CROSS APPLY` when you only want left-side rows that produce at least one right-side row, and use `OUTER APPLY` when you need every left-side row preserved regardless of what the right side returns.
