...
In this module, you learn how to use the `UNION` set operator to combine rows from two result sets, and the `INTERSECT` and `EXCEPT` set operators to compare rows between two result sets. You also learn how to use forms of the `APPLY` operator to use the result of one query to collect the output of a second query, returning the output as a single result set.
