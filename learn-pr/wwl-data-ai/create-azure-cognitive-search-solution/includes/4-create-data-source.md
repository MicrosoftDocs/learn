...
Choose the language you want to use, and follow the steps to create a data source.
...
4. Open the **Program.cs** file, and view the code it contains. The **Main** function creates a **SearchServiceClient** object for your Azure Cognitive Search service, and then calls a series of other functions to create various resources (some of the function calls are commented out for now).
5. View the **CreateOrUpdateDataSource** function, which creates a data source named **margies-docs** that references the Azure Storage blob container where the PDF documents are stored.
...
8. In the terminal for the **create-index** folder, enter the following command:
    ...
    dotnet run
    ```
9. Wait while the program creates the data source.
...
3. Review the JSON code, which defines an Azure blob data source named **margies-docs** that references the **margies** container in an Azure Storage account. The connection string for the blob container is null - you will address this in your code later.
