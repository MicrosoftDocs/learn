...
The model is very accurate in predicting which customers are thinking of buying your competitor's bikes. The marketing team can't wait to use this new model. Unfortunately, the data scientist is struggling with making the model available to the marketing team.

The data scientist who created the model, trained the model locally on their own computer. Training a machine learning model on your local computer is a typical approach. Ideally however, the model should be trained in Azure and the complete process should be tracked and reproducible. Not only do you want to make the model available for the marketing team, the goal is also to monitor the model to know when it needs to be retrained and redeployed.

To make the model available to the marketing team, the model needs to move from the data scientist's computer to Azure. As a first step you want to train the model with Azure Machine Learning. You want to create an Azure Machine Learning workspace, and make sure a compute instance is available to train the model in the cloud. Training the model in the cloud, will make it easier for the data science team to collaborate on this and other projects in the future.
