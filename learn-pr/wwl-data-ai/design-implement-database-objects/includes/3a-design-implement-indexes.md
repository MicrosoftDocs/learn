...
An index works like a book's index: instead of reading every page to find a topic, you consult the index to jump directly to relevant pages. The database uses indexes similarly, converting potentially millions of row comparisons into a handful of efficient lookups.
...
Nonclustered indexes are ideal when you need fast lookups for specific predicates, joins, or sorting patterns that do not align with the clustered key, or when you want to cover a query by including additional columns to avoid key lookups.

Choosing between them depends on how the data is accessed: use a clustered index for the primary access path and nonclustered indexes to support alternate, highly selective, or frequently queried patterns while balancing the cost they introduce on write operations.
...
A columnstore index organizes data into **rowgroups**, each containing up to 1,048,576 rows. Within each rowgroup, each column is stored separately as a **column segment** and compressed independently. This architecture enables the query optimizer to read only the columns needed for a query, skipping irrelevant data entirely.
...
| **Historical data** | Use columnstore | Archived data that's rarely updated but frequently analyzed achieves high compression ratios |
...
A Clustered Columnstore Index (CCI) is a type of columnstore index that becomes the primary storage structure for the entire table, replacing any existing clustered rowstore index. Unlike a non-clustered columnstore index (NCCI), which creates a secondary columnar copy alongside the rowstore table, a CCI stores all table data exclusively in columnar format.

This means the table has no traditional row-based storage—every column is compressed and stored separately. Both CCI and NCCI use the same columnar compression and batch processing optimizations, but a CCI is typically used when analytics is the primary workload and you don't need row-level transactional access patterns. In contrast, an NCCI allows you to maintain rowstore indexes for transactional queries while providing a columnar structure for analytical queries on the same table.
...
Index selection directly impacts both query performance and write throughput. Design indexes carefully during initial development to avoid costly rebuilds and performance issues in production.
