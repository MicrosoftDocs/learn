...
![Diagram of transformer model architecture with the encoder and decoder blocks.](../media/transformer-model.png)
...
5. For example, given an input sequence like :::no-loc text=""When my dog was"":::, the model can use the attention technique to analyze the input tokens and the semantic attributes encoded in the embeddings to predict an appropriate completion of the sentence, such as :::no-loc text=""a puppy."":::
...
The embedding vectors for :::no-loc text=""dog""::: and :::no-loc text=""puppy""::: describe a path along an almost identical direction, which is also fairly similar to the direction for “cat”. The embedding vector for “skateboard” however describes journey in a very different direction.
...
In an encoder block, each token is carefully examined in context, and an appropriate encoding is determined for its vector embedding. The vector values are based on the relationship between the token and other tokens with which it frequently appears. This contextualized approach means that the same word might have multiple embeddings depending on the context in which it's used - for example :::no-loc text=""the bark of a tree""::: means something different to :::no-loc text=""I heard a dog bark."":::

In a decoder block, attention layers are used to predict the next token in a sequence. For each token  generated, the model has an attention layer that takes into account the sequence of tokens up to that point. The model considers which of the tokens are the most influential when considering what the next token should be. For example, given the sequence :::no-loc text=""I heard a dog,""::: the attention layer might assign greater weight to the tokens :::no-loc text=""heard""::: and :::no-loc text=""dog""::: when considering the next word in the sequence:
