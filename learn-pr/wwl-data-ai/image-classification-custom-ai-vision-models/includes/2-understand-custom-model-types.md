...
Image classification is a computer vision technique in which a model is trained to predict a label for an image based on the contents of the entire image. Usually the class label relates to the main *subject* of the image, however individual use cases may vary.
...
Models can be trained for multi-class classification (in other words, there are multiple classes, but each image can belong to only one class) or multi-label classification (in other words, an image might be associated with multiple labels).
...
Product recognition works the same way object detection does, but has improved accuracy for product labels and brand names. The predictions for product recognition will have both the class label and location, enabling you to know where in the image the product is.
