...
1. Get a list of all the user accounts. In MySQL, for example, you could use this command:

    ```sql
    SELECT host, user FROM mysql.user;
    ```

1. For each user account, find out what access they have to the database. In MySQL, for example, you could use this command for the `dbadmin@on-premises-host` user account:

    ```sql
    SHOW GRANTS FOR 'dbadmin'@'on-premises-host';
    ```

1. Recreate each user account in the cloud database. In MySQL, for example, you could use this command to create a new account:

    ```sql
    CREATE USER 'dbadmin'@'cloud-host'
    ```

1. Authorize each user account to the correct level of access in the cloud database. In MySQL, for example, you could use this command to permit a user to access the complete database:
