...
Azure Synapse Pipelines has an _integration runtime_ that enables it to bridge between the activity and linked Services objects. It's referenced by the linked service, and provides the compute environment where the activity either runs on or gets dispatched from. This way, the activity can be performed in the region closest possible. There are three types of Integration Runtime, including Azure, and Self-hosted. Azure Data Factory supports the same integration runtimes, and in addition to this it also supports the and Azure-SSIS integration runtime.
