...
The following example uses the `@empid` variable in the predicate and changes its value in the `BEGIN...END` block. `SELECT` is used here (rather than `SET`) to demonstrate reading a column value from a query result into a variable:
