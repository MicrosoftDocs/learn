...
| Aspect                       | Advantages of Batch Processing                                                                           | Disadvantages of Batch Processing                                                 |
| ---------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| Processing Capacity          | Large volumes of data can be processed efficiently at a convenient time.                                 | All input data must be fully prepared before processing can begin.                |
| System Usage                 | Jobs can be scheduled during idle or off-peak times (such as overnight), improving resource utilization. | There's often a delay between data input and receiving results.                  |
| Reliability & Error Handling | —                                                                                                        | Errors in data, crashes, or program failures can stop the entire batch process.   |
| Data Validation              | —                                                                                                        | Input data must be carefully checked before rerunning the batch job.              |
| Impact of Minor Errors       | —                                                                                                        | Even small data errors can prevent the whole batch job from running successfully. |
...
![Diagram of cars being counted as they pass.](../media/stream-processing.png)
...
Even when real-time analysis or visualization of data isn't required, streaming technologies are often used to capture real-time data and store it in a data store for subsequent batch processing (this is the equivalent of redirecting all of the cars that travel along a road into a parking lot before counting them).

The following diagram shows a **lambda architecture**—a common pattern for combining batch and stream processing in a large-scale data analytics solution.
...
3. If real-time analytics isn't required, the captured streaming data is written to the data store for subsequent batch processing.
...
5. The nonstreaming data is periodically batch processed to prepare it for analysis, and the results are persisted in an analytical data store (often referred to as a **data warehouse**) for historical analysis.
