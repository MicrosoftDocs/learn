...
| Type 0 |
| -- |
|Dimension data cannot be changed. Any attempted changes fail.|
...
As an alternative to using multiple `INSERT` and `UPDATE` statement, you can use a single `MERGE` statement to perform an "*upsert*" operation to insert new records and update existing ones.

![Using the MERGE statement to upsert SCD](../media/4-merge-statement-scd.png)

> [!NOTE]
> MERGE is now Generally Available in Synapse Dedicated SQL Pool with version '10.0.17829.0' or above. Connect to your dedicated SQL pool (formerly SQL DW) and run SELECT @@VERSION. A pause and resume may be required to ensure your instance gets the latest version. See the [MERGE documentation for Azure Synapse Analytics](/sql/t-sql/statements/merge-transact-sql?view=azure-sqldw-latest&preserve-view=true) for more details.
