...
The transaction log records on the primary instance cannot be truncated until they have been replicated to the secondary instance. Regularly transaction log backups reduces the risk of running out of space on your primary instance.

Link feature can also be used as a hybrid disaster recover solution, where you have the option to failover your SQL Server databases hosted anywhere to a database running on SQL Database Managed Instance. Likewise, you also have the option to use link feature to provide a read-only secondary database in SQL Database Managed Instance to offload intensive read-only operations.
...
Instance pool provides a cost-efficient way to migrate smaller SQL Server instances to the cloud. When migrating to Azure, instead of consolidating smaller databases into larger managed instance, which requires additional governance and security planning, instance pools allow you to pre provision your resources based on your total migration resources and requirements.

The instance pool feature provides a fast deployment time of up to five minutes, which is a good option for scenarios where a deployment duration is important. Also, all instances in a pool share the same virtual machine, and the total IP allocation is independent of the number of instances deployed.
