...
* **Databricks Runtime for Machine Learning**: a variant that adds multiple machine learning libraries, TensorFlow, Keras, PyTorch
* **Databricks Light**: for jobs that don’t need the advanced performance, reliability, or autoscaling of the Databricks Runtime

To create and configure a new cluster, we have to click on the `Create Cluster` button and choose our options:
...
To launch the cluster, we have to click the `Start` button and then confirm to launch it. It is recommended to wait until the cluster is started.
...
Once the cluster is running, we can click `Edit` to change its properties. In case we want to provision your cluster with additional libraries, we can click on the `Libraries` and then choose `Install New`.
...
**More information:** for more information about provisioning clusters, see [libraries](/azure/databricks/libraries/) in the Azure Databricks documentation.
...
We can can cache, filter, and perform any operations supported by Apache Spark DataFrames on Azure Databricks tables. 
...
DBFS allows to mount storage objects so that you can seamlessly access data without requiring credentials.
...
For example, to mount a the remote Azure storage account as a DBFS folder, we can use the `dbutils` module
...
**More information:** for more information about DBFS, see the [Databricks File System](/azure/databricks/data/databricks-file-system) in the Azure Databricks documentation.
