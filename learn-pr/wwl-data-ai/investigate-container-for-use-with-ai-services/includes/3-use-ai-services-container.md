...
There are container images for Azure AI services in the Microsoft Container Registry that you can use to deploy a containerized service that encapsulates an individual AI service API.

To deploy and use a AI services container, the following three activities must occur:

1. The container image for the specific AI services API you want to use is downloaded and deployed to a container host, such as a local Docker server, an Azure Container Instance (ACI), or Azure Kubernetes Service (AKS).
2. Client applications submit data to the endpoint provided by the containerized service, and retrieve results just as they would from an AI services cloud resource in Azure.
3. Periodically, usage metrics for the containerized service are sent to an AI services resource in Azure in order to calculate billing for the service.

:::image type="content" source="../media/ai-services-container.png" alt-text="A diagram of an AI services container deployed to a container host and consumed by a client application.":::

Even when using a container, you must provision an AI services resource in Azure for billing purposes. Client applications send their requests to the containerized service, meaning that potentially sensitive data is not sent to the AI services endpoint in Azure; but the container must be able to connect to the AI services resource in Azure periodically to send usage metrics for billing.

## AI services container images

Each container provides a subset of AI services functionality. For example, not all features of the Azure AI Language service are in a single container. Language detection, translation, and sentiment analysis are each separate container images. However, the setup steps are similar for each container.
...
For a full list of currently available AI services container images, and specific notes for each one, see [Azure AI services container image tags and release notes](/azure/ai-services/cognitive-services-container-support#containers-in-azure-ai-services).

## AI services container configuration

When you deploy an AI services container image to a host, you must specify three settings.
...
## Consuming AI services from a Container

After your AI services container is deployed, applications consume the containerized AI services endpoint rather than the default Azure endpoint. The client application must be configured with the appropriate endpoint for your container, but does not need to provide a subscription key to be authenticated. You can implement your own authentication solution and apply network security restrictions as appropriate for your specific application scenario.
