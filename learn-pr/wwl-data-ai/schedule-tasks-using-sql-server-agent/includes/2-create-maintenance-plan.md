...
In a [multiserver environment](/ssms/agent/create-a-multiserver-environment?azure-portal=true), SQL Server Agent allows you to designate one server as a central server (known as MSX) that can execute jobs on other servers, known as target servers (TSX). The central server stores the main source of the jobs and distributes them to the target servers. Target servers periodically connect to the central server to update their job schedules. This setup enables you to define a job once and deploy it across your enterprise. For example, you can configure database maintenance tasks on the primary server and push them out to a group of target servers, ensuring consistent deployment.