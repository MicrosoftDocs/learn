...
In this unit, we discuss the typical machine learning lifecycle and its common challenges. It provides an overview of the process if you're building a custom model, either from the ground up or using a pre-trained model as a starting point. This knowledge should empower you to approach data science projects.

## Machine learning challenges and need of MLOps
...
5. **Deploy:** Finally, they deploy the model in the cloud (often through an API), on an on-prem server, or at the edge on devices like cameras, IoT gateways, or machinery.
...
1. **Validate:** Using a prebuilt model means organizations start just by checking if it works for them. It's key to understand as soon as possible which premade model to use and how viable it is.
   * **Repeat steps 1-3:** Data scientists repeat steps 1-4 until results are good enough. It may often take some effort for the premade model to deliver what is required.
...
5. **Package and deploy:** There are different ways to use a prebuilt model. In many cases, it may be enough to use it via API. This approach implies the organization doesn't own it and can't customize it, but it saves the time of packaging and deploying it. If there's been some level of custom training, the AI team will need to package and deploy this new version of the model.
...
Which of the two approaches is better? It depends on the scenario to solve. On the one hand, working with premade models has the advantage of requiring less resources and delivering results faster. On the other hand, prebuilt models are trained to solve a wide range of use cases, so they may struggle to meet very specific needs. In these cases, a full custom model may be a better idea. A flexible mix of both approaches is often preferrable and helps scaling. AI teams can save resources using premade models for the easiest use cases, while investing these resources in building custom AI models for the hardest scenarios. Further iterations can improve the prebuilt models by retraining them.
