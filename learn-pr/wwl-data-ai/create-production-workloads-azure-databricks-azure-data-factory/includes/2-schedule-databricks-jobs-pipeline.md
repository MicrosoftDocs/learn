...
1. Under the Access Tokens tab, select **Generate New Token**. In the Generate New Token dialog, add `Data Factory` for the Comment, then select **Generate**.
...
## Step 2: Navigate to Azure Databricks Linked Service
...
1. In the `New Linked Service` window, select the `Compute` tab, then select the `Azure Databricks` tile, and select `Continue`.
...
> [!IMPORTANT]
> To properly populate, this form should be completed from top to bottom. All fields not mentioned can be left with defaults.
...
1. Select the name of your cluster from the drop down list under `Choose from existing clusters`.
1. Select `Create`.
...
1. In the `Activities` panel to the right of the `Factory Resources` panel, select `Databricks` to expand this section.
