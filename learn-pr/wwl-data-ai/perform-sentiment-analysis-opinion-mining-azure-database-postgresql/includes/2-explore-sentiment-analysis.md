...
The `azure_ai` extension for Azure Database for PostgreSQL - Flexible Server relies on integration with the Azure Language service to perform sentiment analysis. The sentiment analysis capabilities of the extension are accessible using the `analyze_sentiment()` function within the `azure_cognitive` schema.
