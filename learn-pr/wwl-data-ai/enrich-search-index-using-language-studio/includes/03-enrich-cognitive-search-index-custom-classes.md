...
Azure Blob storage can be accessed from both Language Studio and Microsoft Foundry. The container needs to be accessible, so the simplest option is to choose Container, but it's also possible to use private containers with some additional configuration.
...
There are two ways to create your Azure Language project. If you start using the Language Studio without first creating a language service in the Azure portal, Language Studio will offer to create one for you.

The most flexible way to create a Azure Language project is to first create your language service using the Azure portal. If you choose this option, you get the option to add custom features.
