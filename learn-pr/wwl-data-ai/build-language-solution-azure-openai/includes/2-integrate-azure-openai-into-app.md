...
You'll notice the possible regions for an AOAI is currently quite limited. Choose the region closest to your physical location.

One the resource has been created, you'll have keys and an endpoint that you can use in your app.
...
This module will focus on general GPT models, with other models being covered in other modules.

The model family and capability is indicated in the name of the base model, such as `text-davinci-003`, which specifies that it's a text model, with `davinci` level capability, and identifier `3`. Details on models, capability levels, and naming conventions can be found on the [AOAI Models documentation](/azure/cognitive-services/openai/concepts/models) page.

To deploy a model for you to use, navigate to the [Azure OpenAI Studio](https://oai.azure.com/?azure-portal=true) and go to the **Deployments** page. The lab later in this module will cover exactly how to do that.
...
How the input prompt is written plays a large part in how the AI model will respond. For example, if prompted with a simple request such as "What is Azure OpenAI", you'll often get a generic answer similar to using a search engine.

However, if you give it more details about what you want in your response, you will get a more specific answer. For example, given this prompt: 
...
- **Completion** - model takes a input prompt, and generates one or more predicted completions
...
For example, the input for `Completion` or `Embeddings` endpoints might be "What is Azure OpenAI", where the input for `ChatCompletion` will be a conversation:
...
By giving the AI model a real conversation, it can generate a better response with more accurate tone, phrasing, and context. The `ChatCompletion` endpoint enables the ChatGPT model to have a more realistic conversation by sending the history of the chat with the next user message.
