...
It also includes custom-state passing and looping containers, that is, For-each iterators.
...
It enables you to iterate over a collection and runs specified activities in the defined loop. It works similar to the for each looping structure used in programming languages. 
...
Delta loads in ETL patterns will only load data that has changed since a previous iterations of a pipeline. Capabilities such as lookup activity, and flexible scheduling helps handling delta load jobs. 
