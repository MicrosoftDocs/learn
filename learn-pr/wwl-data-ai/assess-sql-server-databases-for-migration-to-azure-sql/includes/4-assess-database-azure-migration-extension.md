...
As we've seen, users can choose to run their migration online or offline depending on their business requirements, and can even configure a self-hosted integration runtime to access source SQL Server instance backup files in their on-premises environment. The extension also provides a secure and improved user experience for migrating Transparent Data Encryption (TDE) databases and SQL Server and Windows logins to Azure SQL.
...
1. Install the extension. Once installed, you will find the Azure SQL Migration extension in the list of installed extensions.
...
An Azure account is not required for assessments or SKU recommendations. One benefit of not requiring an Azure account for assessments or SKU recommendations is that it allows users to evaluate the feasibility and cost of migrating their databases to Azure before committing to creating an Azure account. This can save time and effort for users who are still in the decision-making process.
...
As we can see above, you have the option to select any Azure SQL target, where the results will get updated automatically based on your choice. 
The results are customized according to the chosen target platform. This can help you identify any potential roadblocks and determine if another target option may be more suitable for your environment.

You can also save the assessment report, this will generate a JSON file containing all the main properties about your database, and also the assessment results. Additionally, you can use the JSON file to programmatically extract specific data or information for further analysis or processing.

## Right-sized SKU recommendations (Preview)

The extension also provides recommended compute and storage configuration for your Azure SQL target based on the performance data collected from your source SQL instance, being on-premises or other cloud. This helps ensure that the Azure SQL target meets the needs of your workload.
...
As we can see above, once you initiate the Azure Migration extension wizard, you have the option to select **Get Azure recommendation** to enable performance data collection. 
...
In addition to running the assessment and Azure recommendation steps through the migration extension wizard, you also have the option to run them in either PowerShell or Azure CLI to perform these tasks at scale.
...
To learn more about the Azure migration extension PowerShell and Azure CLI commands available, please refer to the following links: [PowerShell module for data migration extension](https://learn.microsoft.com/en-us/powershell/module/az.datamigration) and [Azure CLI for data migration extension](https://learn.microsoft.com/cli/azure/datamigration).