...
- **The four-surface intervention model** covers input (user messages and uploads), tool-call (agent-generated invocations), tool-response (external system returns), and output (agent responses) as distinct guardrail surfaces requiring different detection mechanisms.
- **Input-surface defenses** (Unit 3) — structural delimiters, Prompt Shields, intent classification — are the first tier of a four-surface strategy, not standalone protection.
- **Custom guardrail design** follows a four-step pattern: plain-language policy → detection mechanism selection → surface integration → false-positive/false-negative measurement.
- **Coverage matrix testing** maps threat categories × intervention surfaces × adversarial variations to identify untested gaps before production deployment.
- **Synthetic adversarial data** generated at scale and the Microsoft Foundry AI Red Teaming Agent provide the test coverage that manual test authoring alone cannot achieve.
- **Operational instrumentation** (OpenTelemetry spans per guardrail invocation) and audit trails (immutable records of blocked outputs) are required for regulated environments and guardrail effectiveness monitoring.
