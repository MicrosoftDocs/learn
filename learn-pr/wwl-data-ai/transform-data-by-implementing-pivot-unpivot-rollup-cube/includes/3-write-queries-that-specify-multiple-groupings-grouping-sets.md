...
> [!TIP]
> If you want to know whether a `NULL` marks a placeholder or comes from the underlying data, use the `GROUPING()` function — it returns `1` for a placeholder and `0` for an actual `NULL` from the source data. `GROUPING_ID()` extends this to compute the grouping level across multiple columns. For details, see [`GROUPING`](/sql/t-sql/functions/grouping-transact-sql) and [`GROUPING_ID`](/sql/t-sql/functions/grouping-id-transact-sql).
