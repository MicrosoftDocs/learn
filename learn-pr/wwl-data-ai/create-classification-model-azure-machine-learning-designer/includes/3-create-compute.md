...
> [!NOTE]
> In a production environment, you'd typically set the **minimum number of nodes** value for a compute cluster to 0 so that compute is only started when it is needed. However, compute can take a while to start, so to reduce the amount of time you spend waiting for it in this module, you've initialized it with two permanently running nodes.
>
