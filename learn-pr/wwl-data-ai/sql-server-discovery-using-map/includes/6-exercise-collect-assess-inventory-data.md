...
1. In the **Inventory and Assessment Wizard**, in the **Inventory Scenarios** tab, under **Choose your scenario**, select **Windows Computers**, **SQL Server**, and **SQL Server with Database Details**, then click **Next**.
1. In the **Discovery Methods** tab, clear the **Use Active Directory Domain Services (AD DS)** check box, then select **Manually enter computer names and credentials**, and click **Next**.
...
1. In the **Credentials Order** tab, in the **Technology** list, click each item to ensure it has a value in the **Credential** field, and then click **Next**.
1. In the **Enter Computers Manually** tab, click **Create**.
1. In the **Specify Computers and Credentials** dialog box, in the **Computer Name** field, type the name `ReportingServer`, and click **Add**.
...
1. Look at the **Developer and Free Editions** section at the bottom of the tab (you might need to scroll down). Of the instances, 110 aren't production servers so, when planning upgrades, you can optionally return to the Express and Desktop editions later, and potentially ignore the Evaluation edition. Or you might want to upgrade certain development databases before upgrading the production version, to test your procedures.

## Assess SQL Server components

Next, examine which SQL Server services are present in your organization:

1. Look at the **SQL Server Components** chart. You'll instantly see that **Database Engine Services** is the dominant service, as you would expect to find in most setups. Notice that there's six SQL Server R Services components in use. If this was your organization's chart, it would suggest that data science hasn't yet been widely adopted across the business.
1. In the **Options** box in the top-right corner of the tab, click **Generate SQL Server Assessment Report**. The **Report Generation Status** dialog opens. When the report has been created, click **Close**.
1. Again, in the **Options** box in the top-right corner of the tab, click **Generate SQL Server Database Details Report**. The **Report Generation Status** dialog opens. When the report has been created, click **Close**.
1. A **File Explorer** window should open automatically to display the two new files. If not, click **Start**, type **File Explorer**, and go to **C:\Users\azureadmin\Documents\MAP\MapTrainingDatabase**.
...
1. Click the **DatabaseInstances** tab. On the **SQL Server Product Name** column header, click the filter, and clear **Microsoft SQL Server 2012**, **Microsoft SQL Server 2014**, **Microsoft SQL Server 2016**, and **Microsoft SQL Server 2017**.
1. Click **OK**.
1. On the **SQL Server Edition** column header, click the filter, and clear **Desktop Engine**, **Enterprise Evaluation**, and **Express**. Click **OK**. A list of production servers requiring an upgrade will be shown.
1. Click the **Components** tab. On the **SQL Server Component Name** column header, click the filter. Click **(Select All)** to clear all items, then click **SQL Server R Services**.
...
1. On the **Overview** tab, notice the counts of database engines across the different versions. Look at the **Total SQL Server Databases that were Connected to Successfully** column. You'll see that the **SQL Server 2008** and **SQL Server 2008 R2** instances have the highest number of operational databases.
...
1. On the **DatabaseSummary** tab, on the **Database Size (MB)** column header, click the filter, and then click **Sort Z to A** to order by the largest to smallest databases.
...
1. On the **DBInstanceProperties** tab, on the **clr enabled** column header, click the filter, clear the **(Select All)** check box, select the **1** check box, and then click **OK** to return only instances that have the Common Language Runtime (CLR) enabled.
1. On the **SQL Server Database Engine Instance Name** column header, click the filter, clear the **MAPS**, and **MICROSOFTSCM** check boxes, and then click **OK**.
