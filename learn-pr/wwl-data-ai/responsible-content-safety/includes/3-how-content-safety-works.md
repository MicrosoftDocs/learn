...
- **Moderate images** scans for inappropriate content across four categories: violence, self-harm, sexual, and hate. A numeric severity level is returned for each category: 0 (safe), 2 (low), 4 (medium), or 6 (high). You then configure filters by setting allowed severity thresholds for each category, which determines whether the image is allowed or blocked.
...
- **Task adherence** detects when tool use by AI agents is misaligned, unintended, or premature in the context of a user interaction. This helps system developers proactively mitigate misaligned actions by blocking them or escalating for human review.
