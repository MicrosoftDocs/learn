...
Databases can also be semi-structured or unstructured, comprising a mass of raw, unprocessed data. These databases are typically referred to as *non-relational*. Databases are managed using a database management system (DBMS). The DBMS handles the physical aspects of a database, such as where and how it's stored, who can access it, and how to ensure that it's available when required.
