...
Embeddings can apply to text and any kind of data such as images or audio. The key part is transforming data into *n*-dimensional embedding vectors based on some model or function. The numerical similarity of embeddings proxies the semantic similarity of their corresponding data.
...
The following image from OpenAI visualizes embeddings. Embeddings were generated from input documents with 2,048 dimensions. Using PCA, the 2,048 dimensions were projected down to three dimensions. Each resulting 3D vector is represented as a point in space and colored by category. Rendering these points as a 3D scatterplot shows that articles with the same category form clusters in space.
...
1. Sort the dot products, which are numbers from -1 to 1.
1. The most relevant (semantically similar) documents have the highest scores. The least relevant (semantically different) documents have the lowest scores.
