Parameters enable you to create flexible and reusable Bicep templates. Define a parameter for any aspect of your deployment that might change, such as environment-specific settings, pricing and capacity configuration for your Azure resources, and API keys to access external systems. Parameters can be secured using parameter decorators and Key Vault references, so that they can handle sensitive data. In this module, you'll learn about how parameters can be defined and configured, and how you can specify parameter values during your deployments.

## Example scenario

Suppose you're responsible for deploying and configuring the Azure infrastructure at a toy company. The Human Resources department is migrating an on-premises web application to Azure. The application will handle information about all of the employees of the toy company, so security is important.

You have been asked to prepare infrastructure for three environments: dev, test, and production. You should build this using Infrastructure as Code (IaC) techniques so that you can reuse the same templates to deploy across environments. You also want to create parameter files that contain input values for each environment, while securely retrieving database credentials from Azure Key Vault.
