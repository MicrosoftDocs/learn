...
In the terminal, run the following Azure CLI command. Replace `{storage-account-name}` with a name that's likely to be unique. The name must be all lowercase, contain 24 or fewer characters, including hyphens, and have no other special characters. Keep a note of the name for later use
...
In the terminal, run the following Azure PowerShell command. Replace `{storage-account-name}` with a name that's likely to be unique. The name must be all lowercase, contain 24 or fewer characters, including hyphens, and have no other special characters.
