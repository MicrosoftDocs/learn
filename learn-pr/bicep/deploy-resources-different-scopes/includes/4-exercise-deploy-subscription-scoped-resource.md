...
1. At the bottom of the file, add the following Azure Policy definition:
   ...
   * The `sku.name` property *either* begins with `Standard_F` or `Standard_G`.
   ...
   > [!WARNING]
   > Be careful when you use the **deny** policy effect for your policy definitions, especially at wide scopes like subscriptions and management groups. If they aren't created correctly, they can have unexpected effects that lead to outages. We recommend you start with the **audit** policy effect, and then only switch to the **deny** effect once you've seen this work well over a period of time.
