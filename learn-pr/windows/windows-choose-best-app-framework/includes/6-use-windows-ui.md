Your company wants to build a consumer application to help people manage their shopping lists, which must be deployed via the Microsoft Store.

In this scenario, we discuss how each Windows UI framework can be used to create the Shopping List app and whether it's the best option, based on the latest requirements.
...
The shopping list application is a consumer-facing application. The differentiating requirements to focus on for this application are:

- **Appearance and style of the application.** This application supports the latest Fluent design system guidance and matches the UX of the latest Windows releases.
- **The device compatibility.** The shopping app should run on the latest Windows devices, including desktops and tablet PCs, and take advantage of modern hardware.
...
In this section, we're going to analyze the requirements for the Shopping List app and see how each framework can be used to meet those requirements. We also look at the benefits and limitations of the frameworks and how they may affect the decision of which to use for the Shopping List app.

### Appearance and style of the application

WinUI 3 applications implement the latest Fluent design system. The Fluent design system is a set of guidelines for creating apps that look and feel like they belong on Windows. The WinUI design language evolves alongside Fluent to create a design that is human, universal, and truly feels like Windows. WinUI provides the best experience for users on the latest version of Windows. This requirement is a perfect match for the Shopping List app's requirement to implement the latest Fluent design system guidance. Neither WPF nor Windows Forms implement the Fluent design system.

### Tablet usability (touch) and device compatibility

WinUI 3 is designed to work seamlessly with touch and pen input. The WinUI team works to ensure that WinUI 3 apps adapt and provide the best experience on all form factors, including tablets, 2-in-1 devices, and desktop PCs. WinUI 3 apps are built to run on Windows 10, version 1809 or later and Windows 11. Again, this capability is an excellent match for the Shopping List app's requirement to be usable with touch and pen input. WPF and Windows Forms can be used with touch and pen input, but they don't provide the same experience as WinUI 3.
...
The team is familiar with the latest version of .NET and C#. The developers are also familiar with XAML, so they can use the XAML designer or editor in Visual Studio to build the UI for the application. They're comfortable developing with any of the frameworks being considered for this project.

### Application performance and responsiveness

WinUI 3 apps have a vector-based UI that uses the GPU. XAML-based UIs make the apps performant and responsive. WinUI 3 apps are built on the .NET 8 runtime, which is the fastest version of .NET to date. There's no concern about the performance of the application with WinUI 3.
...
WinUI 3 seems like a great match for the Shopping List app. It works seamlessly with touch and pen input. It also implements the latest Fluent design system. Windows App SDK is built on the .NET 8 runtime, which is the fastest version of .NET to date. It's also easy to deploy via the Microsoft Store. WinUI 3 is the best choice for this project.
