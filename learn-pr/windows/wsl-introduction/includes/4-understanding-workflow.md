The ability to use both Linux and Windows on a single computer means you can use your favorite tools from either operating system. In this section, we'll recommend some of the tools, command-line utilities, coding languages and frameworks that best support a smooth development environment when using WSL. You'll need to keep in mind that you're working between two different file systems. To achieve the best performance from your environment, there are a few best practices to consider for your workflow.
...
Your choice of tools is highly dependent on what type of development work you plan to do. There are a few that we specifically recommend for a WSL workflow, especially if you're just getting started using WSL, including:
...
- **Code languages and app frameworks**: Install project code on the Linux file system using a package manager
    ...
    On Windows, software has traditionally been installed using executable (.exe) files and using the Settings GUI to manage or remove software (Settings > Apps & Features). On Linux, _package managers_ handle the process of installing, upgrading, configuring, and removing software. When working in Linux with WSL, you will use a package manager to install code languages and app frameworks on the Linux file system directory associated with the distribution you're using. The default WSL distribution, Ubuntu, uses the `apt` package manager (short for Advanced Packaging Tool). You should always install the languages/frameworks/tools that you'll be using on the same file system where you'll create your project code.
...
2. Install your code language and framework using the package manager for your distribution by entering commands like `sudo apt install <software>`.

3. You may also need to use a command-line utility to download software if it's not available from the distribution's package manager. cURL (short for Client URL) is a useful utility that comes with most Linux distributions. It can be used from the command line to download software (or any sort of data) from a server that uses URL syntax to the local filesystem. It's also commonly used for automated tasks, since it is designed to work without user interaction.
