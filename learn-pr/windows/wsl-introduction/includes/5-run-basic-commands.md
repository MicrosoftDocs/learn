We've already looked at a few basic WSL administrative commands, including `wsl --install` and `wsl --list --online`, as well as a few basic Linux commands, including `mkdir` to make a directory and `sudo apt install <package>` to install software, but let's explore a few more and try accomplishing some common tasks.

## Try some basic WSL commands

The WSL commands in this training are listed in a format supported by PowerShell or Windows Command Prompt (Cmd.exe). To run these commands from a Bash command line on a Linux distribution, you must replace `wsl` with `wsl.exe`. This is because WSL is managed from the Windows file system as an executable file... but Windows executable files can *also* be run from Linux as long as you include the file extension.
...
Bash is the default command line interface (CLI) for Linux. A deep-dive introduction to Bash is linked in the summary at the end of this module. PowerShell is the default command line for Windows (replacing the previous Windows Command Prompt). Both can be hosted by the Windows Terminal, which can be customized to suit your preferences.
