...
Next, run the app in debug mode (F5 or **Debug / Start Debugging**). Type something in the `TextBox` and note that the breakpoint doesn't get hit. If you press the _Tab_ key, it will move the input focus to the next control (in our case, the button). Losing focus on the `TextBox` will update the binding, so the breakpoint will be hit.
