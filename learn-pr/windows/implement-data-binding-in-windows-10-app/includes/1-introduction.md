...
The connection between the app's logic and the UI is essential. Frameworks like Universal Windows Platform (UWP) and Windows Presentation Foundation (WPF) have incorporated an extremely efficient and versatile method of transferring data to and from the UI called *data binding*.
...
- Basic concepts of UWP layout, including `StackPanel` and `Grid`.
