The **Databricks app for Microsoft Teams** gives business users direct access to Genie Spaces without leaving Teams. Once a Teams administrator installs the app and users connect their accounts, they can ask data questions in chat—in a DM (direct message) or in a channel—and Genie responds with tables, analysis, and source citations.
...
You can use the Databricks app in two surfaces in Teams.
...
**Channel:** Post `@Databricks` in a Teams channel. The query runs as you, but the response **appears in the channel and is visible to all channel members**. This is useful when your whole team needs to see the same data in context, such as during a discussion about a sales target or a supply chain issue.
...
| Feedback | **Not available in Teams.** Users must follow the source link to the Databricks Genie Space UI to submit thumbs up/down, "fix it," or "request review" feedback. |
...
| Customizations required | No. A Teams administrator installs the app once. Each user then connects their own account. |
