The Azure Maps web SDK has a wide variety of tools you can use to depict spatial data on an interactive map on a webpage. Before we get started adding a map to a webpage, let's look at some of the capabilities of the SDK.
...
You can augment maps by adding layers. A layer depicts data on top of a map. In the app you create, you'll use a layer on a map to depict air quality data.
