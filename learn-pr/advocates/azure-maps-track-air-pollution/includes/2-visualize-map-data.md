...
More styles are described in [Azure Maps supported map styles](https://docs.microsoft.com/azure/azure-maps/supported-map-styles?azure-portal=true).

## Navigate around the map

Azure Maps controls can be configured to be fully interactive, so the user can zoom, move, rotate, and tilt the map view. When zoomed in and tilted, the map can show 3D representations of the shape of buildings. Navigate by using a mouse, trackpad, or touchscreen, and supports pinching to zoom and dragging to move the map around. The map control has zoom, compass, pitch, and style picker controls that can be enabled and shown on the map.

Navigation also can be set in code. For example, the view can be set to a specific location. Also, the various user navigation capabilities can be enabled or disabled by settings on the map control.
