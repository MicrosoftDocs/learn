So far, this module discussed ways that you can both detect harmful content and mitigate harmful content generation from the model. Now, you can prevent prompt injections by adding another layer of security to the model.
...
Now, test for an indirect prompt injection with fictional data provided by a manufacturer:
...
Suppose a customer submits a DAN prompt via chat to Contoso Camping Store's AI-powered customer support assistant. Later, the customer submits an indirect prompt injection via email to the company's AI-moderated customer support inbox. The customer executes such actions in hopes of manipulating the model into doing their bidding.
...
The model was able to detect jailbreaks in both **User prompt** and **Document** despite the attacks deriving from different sources.
