...
Throughput is pre-provisioned, and you use a metric called *Request Units* to measure this throughput. Request Units (RUs) are a blended measure of memory, CPU, and IOPS, giving you a single metric by which you can plan capacity. You provision RUs in increments of 100 RUs per second.

Every DB operation is measured in RUs. Reads are simple: 1-KB read is a single Request unit. Other operations are calculated based on several factors, such as item size, data consistency, query patterns, and so forth. 
...
:::image type="content" source="../media/application-usage.png" alt-text="Graph showing monthly active user growth over time from July to October.":::
...
:::image type="content" source="../media/application-storage.png" alt-text="Graph comparing database storage usage against total capacity over time.":::

From a throughput perspective, it's sitting at 300/1000 and growing at 10%/month:

:::image type="content" source="../media/application-throughput.png" alt-text="Graph comparing throughput usage in Request Units against provisioned capacity over time.":::

Now that we understand our system's resource metrics, we know when we're likely to have to scale our throughput, and also what our costs will be over time.

We can now produce a graph that helps us with capacity planning.

:::image type="content" source="../media/application-planning.png" alt-text="Graph showing projected throughput intersecting provisioned capacity, indicating when scaling is needed.":::
...
Let’s look at another hypothetical situation as an example of planning for inorganic growth. There's an upcoming marketing event for the launch of a high-profile innovative new product at Tailwind Traders. They're expecting that to drive 5,000 more users to their sales site.
...
You know from the previous scenario that for 2,500 users you need approximately 50 GB of storage and 100 RUs. You can now use that data and determine whether you're ready for this event. If we can expect 5,000 users, that's going to require 100 GB of storage and 200 RU/s.

:::image type="content" source="../media/application-storage-prediction.png" alt-text="Graph showing predicted storage growth after a marketing event compared to total capacity.":::
...
:::image type="content" source="../media/application-planning-cost.png" alt-text="Bar graph comparing Cosmos DB storage, throughput, and total costs before and after a marketing event.":::

You can see that the 100 GB of extra storage is going to cost an extra $25/month. The throughput price stays the same as customers pay for provisioned RUs, and they already have more than enough. The bottom line is that this marketing event is likely to increase their Cosmos DB bill by $25 USD.
