...
We will automate the deployment of the `todo-service` microservice that we developed earlier. It is exactly the same configuration that would need to be done for the gateway, so if you want to automate it too, you can just copy/paste what is being done in the current module.
...
Then, in your project's GitHub repository, select `Settings > Secrets` and add a new secret called `AZURE_CREDENTIALS`. Paste the JSON text you just copied into that secret.
