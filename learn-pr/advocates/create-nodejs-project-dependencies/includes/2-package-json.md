...
The `npm init` and `npm init -y` commands will both generate a package.json file. A package.json file might look like this file:
...
You invoke actions by entering `npm run <action>`. But there are *special* actions: `start` and `test`. They're special because you can omit `run`. So instead of entering `npm run start`, you can instead save a few characters and enter `npm start`.
