...
- **Get better code**: Does your program need to support a specific task, such as security, where you need to implement authentication and authorization? Security is a type of task that you need to *get right* to protect your data and your customer's data. There are standard security patterns and libraries used by many developers. These libraries implement features that you most likely will always need, and issues are patched in the libraries as they arise. You should use these libraries instead of reinventing the wheel. The main reason is you're not likely to do as good of a job of writing the code yourself because there are so many edge cases to consider.  
