...
Why are we looking at these examples? Each of them represents an application that potentially took a business huge amounts of time, energy, and resources to create. But what if the application isn’t up, operational when a customer needs to access it, or isn’t reliable? In this case, it does no one any good, especially the business. In fact, a lack of reliability can do actual harm (reputational, economic, contractual, morale, and so on) to a business.
