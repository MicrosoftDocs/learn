...
The way transfer learning works with the Custom Vision service in Azure is that it adds a layer in the neural network that's represented by a pretrained model. The trained model gives us a head start when we train new data. It starts with a general-knowledge domain, and then new layers are added to the neural network to solve a specific problem. In this case, the problem we want to solve is identifying the species of a bird in an image of a bird. By starting with a pretrained model, we can get better results with adding less data.
