Tailwind Traders has tasked you with working on an app with some outdated dependencies. The app is fairly small and only have a couple of dependencies. Updating it should be straight forward. See if you can update the app to take advantage of the latest features and if you find any vulnerabilities fix them while you are at it.
...
1. Open up the `index.js` file. It should look like so:
   ...
   The above code is pulling some data from a REST API using the `node-fetch` library. Then it goes on to process the response by sorting it and taking the top 3 results using the library `lodash`. The result is stored in a file.
...
1. Have a look at the `package.json` file and locate the `dependencies` section. It should look like so:
   ...
   You have vulnerabilities, 2 on high and 1 low. The high vulnerabilities says you should upgrade. It tells you that *3 vulnerabilities require semver-major dependency updates*. This means you need to upgrade the major version of the dependency to get rid of these vulnerabilities. The audit report suggests how to resolve these issues. It states the command to run at the top Run `npm install lodash@4.17.15`.
...
1. Type the command in the terminal the audit report told you to:
   ...
   This means you from a vulnerability standpoint is now safe. You might have some other reasons to update though so let's look at that next.
...
Congratulations you've managed to upgrade the two dependencies in your app. In doing so you've managed to fix 3 vulnerabilities, 2 of them on high, nice. You've also managed to upgrade the second dependency because you wanted the latest and greatest features. Well done !
