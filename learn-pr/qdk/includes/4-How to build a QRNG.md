...
To do it you can do right click in the VS Code file explorer and select "New
File" or click in the interactive "New File" icon. Alternatively, you can invoke
...
The next thing that we need to do after creating the namespace is to open all
the libraries we need. Q# is a modular programming language that makes extensive
...
libraries that include all sort of basic operations. In this case we only need
the Intrinsic library that includes the most basic quantum operations. We can
open the library using the `open` directive:
...
### Step 6: Write a Q# operation and save

Now we can start to write the operation. To declare a Q# operation we need to
specify a name for the operation and the input and output format. We will call
...
And we save the file. To do it you can press `Ctrl` + `S`. Alternatively,
right click in the `my-random-bit.qs` file in the explorer and click on 
...
2. We can put our qubit in a quantum superposition by applying the `H` operation
   from the Intrinsic library. To apply the we only need to specify the target
...
And that's it, we have now a quantum program that takes a qubit, throw's it in a
...
## Creating the host program

Ok, you have created a Q# quantum program, how do we execute it? The Q# program
allow us to communicate bidirectionally with the quantum computer. However, we
...
a host program, typically C# or Python. This allows us to call the quantum 
...
to create the string of random bits in the step 3. We are going to invoke `N`
...
### Step 2: Import Q# and the operations

When writing a Python host program we first need to import the `qsharp` module
and all the Q# operations we are going to use. To do it, we treat the Q#
namespaces as if they where Python modules. Let's import Q# operation we just
created.
...
Python script. To do it you can go to the command line and type:
...
Congratulations! Now you know how to combine Q# with a host program.