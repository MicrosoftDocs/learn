Now we'll consider whether Azure Data Lake Storage is the right choice for your organization's big data requirements. Your organization wants to control the costs of big data storage, especially the hardware and software expenditures associated with huge data volumes. Management wants to store every data type generated by your organization and is especially keen to remove all data silos that keep data sources separate. Your organization's management wants to make it easier to generate business insights from any data. They also want the ability to run analytics workloads on real-time data streams.

## Decision criteria

If your organization stores big data, it most likely uses a *data warehouse*<!-- If we have this in italics, this indicates we are going to define what the term means. -->. Administrators and data engineers know that traditional data warehouses:
...
- Data that comes in relatively slowly.

To help you decide whether Azure Data Lake Storage is a good fit for your organization, let's review the decision criteria in more detail.
<!-- I moved this sentence and bulleted list from line two, as it introduces the following heading 3's. -->
Some of the criteria that indicate whether Azure Data Lake Storage will meet your performance and functional goals are:
...
Data warehouses are purpose-built for managing large volumes of data. But there are hard limits on the total amount of data they can handle<!-- "manage?" -->:
...
Is Azure Data Lake Storage a better choice when managing a diverse collection of data types?<!-- Unless this is specific to formatting, try to avoid single-sentence pgphs. Can we combine the next sentence w/ this sentence? -->
...
Azure Data Lake Storage overcomes these data access barriers, thereby enabling so-called *data democratization*<!-- Can we define this here? -->. It does this by enabling your organization to store all your data formats, including raw data, in a single location. The elimination of data silos enables users to open a tool such as Azure Data Explorer and use it to access and work with every data item in their storage account.

### Data completeness

Does Azure Data Lake Storage facilitate the storage of complete datasets?<!-- Single-sentence pgph. -->

Traditional data warehouses aren't designed to store every piece of data generated by the organization:

- The incoming data is highly processed, and much of an organization's generated data is either incompatible with this processing or gets lost.
...
- Doesn't process data at all during ingestion, so all data is compatible, and none gets lost.
- Stores all the available data regardless of <!-- Is "stage" the right word? I wanted to avoid the word "format," because that might mean something different to different people. -->stage&mdash;raw, semi-structured, or structured.
...
Traditional data warehouses usually aren't ideal for managing high-speed, streaming data because:<!-- I broke this out to match the format of the other two sections above. If you change the previous sections, please change here as well. -->
...
- Can ingest real-time data directly from an instance of HDInsight Storm<!-- - Apache Storm on Azure HDInsight? Refer to Mod 3. Also, if you change it, please change in the knowledge-check.yml file as well. -->, Azure IoT Hub, Azure Event Hubs, or Azure Stream Analytics.
...
For example, a national package delivery business generates large amounts of structured and real-time data, so data costs are a major concern. Azure Data Lake Storage would be a good fit to achieve the organization's goal of controlling data costs.
...
- Wants users to be able to build models and sandboxes that use any or even all the available data.
...
- Ingests streaming data that comes in quickly.
