Now we'll consider whether Azure Data Lake Storage is the right choice for your organization's big data requirements. Your organization wants to control the costs of big data storage, especially the hardware and software expenditures associated with huge data volumes. Management wants to store every data type generated by your organization. It's<!-- I like this better as one sentence, but Acrolinx... --> especially keen to remove all data silos that keep data sources separate. Your organization's management wants to make it easier to generate business insights from any data. They also want the ability to run analytics workloads on real-time data streams.
...
Azure Data Lake Storage overcomes these data access barriers, thereby enabling so-called *data democratization*<!-- Can we define this here? -->. It does this by enabling your organization to store all your data formats (including raw data) in a single location. Eliminating data silos enables users to use a tool such as Azure Data Explorer to access and work with every data item in their storage account.

### Data completeness

Does Azure Data Lake Storage facilitate the storage of complete datasets?<!-- Single-sentence pgph. Also, can we change the word "facilitate" for Acrolinx?-->

Traditional data warehouses aren't designed to store every piece of data generated by the organization:<!-- This first bulleted sentence didn't really go with the first bulleted sentence, which is why I changed it. -->

- When an organization's incoming data is processed, much of it is either incompatible with this processing<!-- With whose processing? --> or is lost.
...
Traditional data warehouses usually aren't ideal for managing high-speed, streaming data. This is because:<!-- I broke this out to match the format of the other two sections above. If you change the previous sections, please change here as well. -->
