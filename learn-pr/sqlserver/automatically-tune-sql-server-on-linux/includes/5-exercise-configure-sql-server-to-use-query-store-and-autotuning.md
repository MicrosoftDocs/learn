...
1. Log on to the **RHELSQLServer** image, by clicking **Administrator**, typing the password **Pa$$w0rdLinux** and clicking **Sign In**.
...
You must now execute a script that creates a query plan regression problem. Follow these steps:
...
Because the query store is enabled, SQL Server 2017 gathers information about the regression problem that you have just created. To view this information, follow these steps:
...
By executing the recommended script that you obtained from the query store, you can manually fix any regression problems that develop. However, by enabling automatic tuning, you can configure SQL Server to apply such fixes automatically. To do this, follow these steps:
