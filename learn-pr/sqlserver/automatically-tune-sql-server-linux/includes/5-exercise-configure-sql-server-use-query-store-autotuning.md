...
1. Select **initialize.sql**, and then select **Open**. This script ensures that the Query Store is empty and that auto tuning is disabled.
1. In the top-left of the script window, select **Run**. Azure Data Studio executes the script.
...
1. Switch to the **batchrequests_perf_collector.sql** tab, and then select **Run**.
1. Switch to the **report.sql** tab, and then select **Run**. Wait for about 20 seconds to store some data before the next step.
...
1. Switch to the **batchrequests.sql** tab, and then select **Run**.
1. On the right of the **RESULTS** window, select the lower icon, which displays the **CHART VIEWER** window.
...
1. In **Azure Data Studio**, on the **File** menu, select **Open File** and then browse to the **~/mslearn-automatically-tune-sql-server-linux/** folder.
1. Select **regression.sql**, and then select **Open**. Examine the contents of the script, which executes the **regression** stored procedure to create a query plan regression problem.
1. In the top-left of the script window, select **Run**.
1. Switch to the **batchrequests.sql** tab, and then select **Run**.
1. On the right, on the **RESULTS** window, select the lower icon, which displays the **CHART VIEWER** window.
...
1. In **Azure Data Studio**, on the **File** menu, select **Open File**, and then browse to the **~/mslearn-automatically-tune-sql-server-linux/** folder.
1. Select **recommendations.sql**, and then select **Open**. Examine the contents of the script, which queries the Query Store to obtain tuning recommendations.
1. In the top-left of the script window, select **Run**.
...
1. Select **auto_tune.sql**, and then select **Open**. Examine the contents of the script, which calls the **auto_tune** stored procedure to enable automatic tuning on the current database.
1. In the top-left of the script window, select **Run**.
...
1. In **Azure Data Studio**, switch to the **batchrequests_perf_collector.sql** tab, and then select **Run**.
1. Switch to the **report.sql** tab, and then select **Run**.
1. Wait for about 20 seconds to store some data before the next step. Switch to the **regression.sql** tab, and then select **Run**.
...
1. Switch to the **recommendations.sql** tab, and then select **Run**.
1. In the **RESULTS** window, notice that:
    - The **reason** column describes the observed problem
