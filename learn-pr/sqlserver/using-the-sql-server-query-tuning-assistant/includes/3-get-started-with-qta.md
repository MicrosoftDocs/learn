...
The Query Store feature was introduced in SQL Server 2016 to continuously collect information regarding the execution and performance of queries within a database. It operates like a flight data recorder, by gathering runtime information on queries and plans. If you save this data, you can track performance over time. By default, the Query Store is not enabled on a database, but any database on an instance of SQL Server 2016 or higher can take advantage of it. This includes databases with an earlier compatibility level than the instance, so if you migrate a database from SQL Server 2008 to SQL Server 2017 and leave the compatibility level at 100, Query Store can still operate on the database.
