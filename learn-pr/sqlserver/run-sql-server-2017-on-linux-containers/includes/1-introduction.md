Containers are a light-weight way to deploy and manage virtualized servers for hosting services such as databases. They are smaller and more agile than Virtual Machines (VMs) because they share an operating system and hardware with the host computer.
...
You are evaluating using containers to solve both this issue.

In the testing environments, you want to deploy containers that include SQL Server, then script the creation of the database, its schema, and sample data. Because a container is light-weight, it can be deployed quickly and provide a clean environment in which to run tests.
