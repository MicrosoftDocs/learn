...
1. Set the **Authentication Type**, and provide a database name.
1. You can now start the replay. When the replay has completed, you will see the message **This replay trace is ready for analysis**.
...
1. Provide a database name, and start this replay. When this replay has completed, you will again see the message **This replay trace is ready for analysis**.

## Summary

The Database Experimentation Assistant leverages Distributed Replay technology to create an A/B testing solution for applying a single change to SQL Server. An A/B test measures only one change, enabling you see the affect of applying the change. Using the DEA to test a change before it goes into production can highlight potentially adverse performance results, that could negatively impact the business. While it may not be the right tool for an emergency fix or smaller alterations, it is useful for planned changes such as upgrading your databases or testing a new feature.
