...
Although the compatibility level remains in the previous version, you can enable the Query Store on the database, as it's a server level feature. You enable the Query Store through the database properties window in SQL Server Management Studio (SSMS), or by running the following statement:
