Suppose you are the database administrator for a law company, and you know that you have databases running on SQL Server 2005 and SQL Server 2000 that are no longer supported by Microsoft. While the databases may function effectively with applications, you cannot guarantee to the business that they are as secure as they could be, because patches and upgrades are no longer provided. Furthermore, you are unable to take advantage of the features in newer versions of SQL Server that could offer enhanced productivity.

Here, we will look at the main questions an organization will ask before it begins a database migration operation.

## Which versions of SQL Server can be upgraded?

If you want to ensure you upgrade your database to the most secure and performant possible standard, do not upgrade to a version earlier than SQL Server 2014. Versions of SQL Server from 2005 onwards can be upgraded to SQL Server 2014, 2016, or 2017. If you discover earlier versions such as SQL Server 2000 that are still in existence, you must first upgrade to SQL Server 2008 to overcome the compatibility level obstacle. You can then upgrade to a later version.

## Reasons to upgrade to SQL Server 2017

If your company is running any SQL Server 2005 databases, these are no longer supported by Microsoft, so if you are bound to any rules of compliance, you should check that you still adhere to these. Furthermore, security patches and updates are not provided, so your databases are more prone to malicious attack, which is the most compelling reason to upgrade.
...
- SQL Server 2000 and 2005 are no longer supported by Microsoft and no updates are published. This means your databases may be vulnerable to security attacks that have been addressed by recent upgrades. You should certainly upgrade these database server unless you can afford to lose them or allow the data they contain to fall into malicious hands.
