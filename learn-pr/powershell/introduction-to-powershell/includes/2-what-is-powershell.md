...
- You can store commands and scripts in a text file and use a source-control system. This benefit is probably one of the biggest because your commands are repeatable and auditable. In many systems, especially government systems, everything must be traced and evaluated, or _audited_. Audits cover everything from database changes to changes done by a script.
...
- **It has cmdlets.** Commands in PowerShell are called cmdlets (prounounced *commandlets*). Unlike many other shell environments, in PowerShell, cmdlets are built on a common runtime rather than separate executables. This characteristic provides a consistent experience in parameter parsing and pipeline behavior.
