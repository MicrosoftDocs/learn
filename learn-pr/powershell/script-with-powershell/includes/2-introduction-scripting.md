...
- **Requirement to run scripts by using a full path or relative path**. When you run a script, you always need to provide the path of the script. Providing the path helps you to know exactly what you're running. There could, for example, be commands and aliases on your computer that you don't intend to run but that have the same name as your script. Including the path provides an extra check to ensure you run exactly what you want to run.
- **Execution policy**. An execution policy is a safety feature. Like requiring the path of a script, a policy can stop you from doing unintentional things. The policy can be set on various levels, like the local computer, current user, or particular session. You can also use a Group Policy setting to set execution policies for computers and users.  
...
1. Run the script by invoking it by its name and path:
   ...
   We recommend that you include the file extension in the invocation, but it's not required.
...
When you output text via `Write-Host` or `Write-Output`, you use single or double quotation marks. Your choice depends on whether you want to interpolate the values. There are three mechanisms that you should know about:
...
- **Script scope**. When you run a script file, a script scope is created. For example, a variable or a function defined in the file is in the script scope. It will no longer exist after the file is finished running. You can create a variable, for example, in the script file and target the global scope. But you need to explicitly define that scope by prepending the variable with the `global` keyword.

- **Local scope**. The local scope is the current scope. It can be the global scope or any other scope.
...
- **Items are visible in the current and child scopes**. An item, like a variable or a function, is visible in the scope it's created in. By default, it's also visible in any child scopes. You can change that behavior if you want by making the item private within the scope. Here's an example that uses a variable defined in the console:
...
- **Items can be changed only in the created scope**. By default, you can change an item only in the scope it was created in. You can change this behavior by explicitly specifying a different scope.
