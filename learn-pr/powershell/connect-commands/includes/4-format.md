...
What exactly these types are, are not important for this lesson. What is important is to realize is that when you use any type of formatting command, your data is different and when it's different it might no longer contain the columns you care about. Let's illustrate this with an example:

```powershell
Get-Process 'some process' | Select-Object Name, Cpu
```

The preceding command gives you a result with the columns `Name` and `CPU` that looks like the following:
...
It's empty, because `Format-Table` transformed your object by placing data into other properties. Your data isn't gone, only your properties, and the preceding PowerShell command makes an honest attempt at resolving non existing properties. Formatting commands is supposed to be the last thing you do because they are meant for formatting things nicely for screen presentation, not for you to keep massaging the data via commands like `Select-Object`.
