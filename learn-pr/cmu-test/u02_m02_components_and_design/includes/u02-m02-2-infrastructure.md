...
A server room can vary in size from a single rack in a closet to several hundred square feet to a warehouse the size of a football field. Some authors use the phrases server room and data center interchangeably. For the purpose of the course, we define a _server room_ as the actual room that houses all of the racks full of IT equipment, and a _data center_ as the server room plus all of the power and HVAC equipment that may be located outside of that room.
