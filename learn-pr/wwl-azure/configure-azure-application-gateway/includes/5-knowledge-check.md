...
(x) The hostname, port, and path in the URL of the request. {{That's correct.}}
...
(x) Distributes requests to each available server in a backend pool in turn, round-robin. {{That's correct.}}
...
(x) Install the Web Application Firewall {{That's correct. The web application firewall (WAF) is an optional component that handles incoming requests before they reach a listener. The web application firewall checks each request for many common threats, based on the Open Web Application Security Project (OWASP).}}
