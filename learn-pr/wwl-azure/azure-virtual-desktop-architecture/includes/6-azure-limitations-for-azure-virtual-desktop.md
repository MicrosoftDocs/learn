...
Numbers in the following sections are approximate. The following numbers are based on various large customer deployments, and they might change over time.
 ...
 -  You **can currently deploy 399 VMs per Azure Virtual Desktop Azure Resource Manager template deployment without Availability Sets**, or 200 virtul machines per Availability Set. You can increase the number of VMs per deployment by switching off Availability Sets in either the Azure Resource Manager template or the Azure portal host pool enrollment.
