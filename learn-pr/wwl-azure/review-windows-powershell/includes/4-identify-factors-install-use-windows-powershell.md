...
- **Bypass**. Nothing is blocked and there are no warnings or prompts. Use this policy when a PowerShell script is built into a larger application with its own security model.
- **Default**. Sets the default execution policy. In Windows PowerShell 5.1, this is **Restricted** for Windows clients and **RemoteSigned** for Windows servers. In PowerShell 7, this is **RemoteSigned** for both Windows clients and servers.
