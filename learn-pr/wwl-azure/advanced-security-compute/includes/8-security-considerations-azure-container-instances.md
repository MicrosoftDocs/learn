...
For example, Azure Container Registry [integrates with Microsoft Defender for Cloud](/azure/security-center/defender-for-container-registries-introduction) to automatically scan all images pushed to a registry. Microsoft Defender for Containers uses Microsoft Defender Vulnerability Management (MDVM) to detect image vulnerabilities, classify them, and provide remediation guidance with exploitability insights.

Other security monitoring and image scanning solutions are also available through Azure Marketplace and partner ecosystems.
...
New vulnerabilities are discovered frequently, so scanning for and identifying vulnerabilities is a continuous process. Incorporate vulnerability scanning throughout the container lifecycle:
 ...
 -  You should continue to scan container images in the registry. Look to identify any flaws that were missed during development. Then address any newly discovered vulnerabilities that might exist in the code used in the container images.
...
It's a lot easier to manage a safe list when implemented from the beginning. A safe list provides a measure of control and manageability as you learn what files and executables are required for the application to function correctly.
...
Modern container security platforms provide automated approaches for nano-segmentation. These platforms monitor container network activities in runtime, identifying all inbound and outbound network connections to/from other containers, services, IP addresses, and the public internet. Nano-segmentation can be automatically created based on monitored traffic patterns.
...
As with any IT environment, you should consistently monitor activity and user access to your container ecosystem to quickly identify any suspicious or malicious activity. Azure provides container monitoring solutions including:
 ...
 -  [Container insights](/azure/azure-monitor/containers/container-insights-overview), a feature of Azure Monitor, monitors the performance of your workloads deployed to Kubernetes environments hosted on Azure Kubernetes Service (AKS) and Azure Arc-enabled Kubernetes. Container insights give you performance visibility by collecting memory and processor metrics from controllers, nodes, and containers that are available in Kubernetes through the Metrics API.

> [!IMPORTANT]
> The **Azure Container Monitoring Solution** was deprecated on August 31, 2024. Migrate to **Container insights** (built into Azure Monitor) for comprehensive container monitoring:
> - Uses the Azure Monitor Agent
> - Supports Azure Monitor metrics
> - Provides both log-based and metric-based alerts
> - Available in both Azure Monitor and in the AKS/Arc resource screen
> - Automatic updates via Azure Arc extension
...
Maintain an accurate audit trail of administrative access to your container ecosystem, including your Kubernetes cluster, container registry, and container images. These logs might be necessary for auditing purposes and will be useful as forensic evidence after any security incident. Azure solutions include:
 ...
 -  [Container insights](/azure/azure-monitor/containers/container-insights-analyze), a feature of Azure Monitor
