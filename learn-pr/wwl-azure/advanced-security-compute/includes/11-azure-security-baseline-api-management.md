...
Configuration Guidance: Deploy network security groups (NSG) to your API Management subnets to restrict or monitor traffic by port, protocol, source IP address, or destination IP address. Create NSG rules to restrict your service's open ports (such as preventing management ports from being accessed from untrusted networks). By default, NSGs deny all inbound traffic but allow traffic from virtual network and Azure Load Balancers.

> [!CAUTION]
> When configuring an NSG on the API Management subnet, there's a set of ports that are required to be open. If any of these ports are unavailable, API Management might not operate properly and might become inaccessible.
...
Feature: Microsoft Entra ID Authentication Required for Data Plane Access

Description: Service supports using Microsoft Entra ID authentication for data plane access.
...
Configuration Guidance: Set up integration of API Management with Azure Key Vault. Ensure that secrets for API Management (Named values) are stored in Azure Key Vault so they can be securely accessed and updated.
...
Description: Azure Role-Based Access Control (RBAC) can be used to managed access to service's data plane actions.

Configuration Guidance: Use Azure role-based access control (RBAC) for controlling access to Azure API Management. Azure API Management relies on Azure role-based access control to enable fine-grained access management for API Management services and entities (for example, APIs and policies).
