You can share SharePoint Online content with external users. Because Microsoft OneDrive storage is part of SharePoint Online, there are similar settings for both. When you configure settings for external sharing, the OneDrive settings must be the same or more restrictive than the SharePoint Online settings. The following table lists the commonly used external sharing settings for SharePoint Online.
...
When a user shares content from a SharePoint site, SharePoint Online sends a sharing link to the recipient. This link is unique and includes information, such as permissions, to edit the file and who can use it.
...
| ----- | ----------------------------------------------- |
| `None`  | Uses the value set at the tenant level.         |
| `View`  | Sets the default value to **View** permissions.  |
| `Edit`  | Sets the default value to **Edit** permissions. |

You can configure an expiration time for external sharing links. After a link expires, users can no longer use it to access the linked content. You can set the expiration time independently for anonymous and external users. The following table lists parameters that you can use to configure link expiration for a site.
...
| `-AnonymousLinkExpirationInDays`               | This parameter sets the number of days that an anonymous link is valid.          |
...
| `-ExternalUserExpirationInDays`                | This parameter sets the number of days that an external user link is valid.      |
