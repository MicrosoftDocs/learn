...
- The same is true for the shared sapmnt directory, which is a shared disk (Windows), a CIFS share (Windows), or an NFS share (Linux). You need to use a technology that replicates these shared disks or shares between the zones. These technologies are supported:
  ...
  - For SUSE Linux, an NFS share built as documented in High availability for NFS on Azure Virtual Machines on SUSE Linux Enterprise Server.
...
- You should deploy dormant virtual machines in the passive zone (from a DBMS point of view) so you can start application resources if there's a zone failure.

> [!NOTE]
> Azure Site Recovery is currently unable to replicate active virtual machines to dormant virtual machines between zones.
