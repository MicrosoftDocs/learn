...
If you don't want to deploy additional Azure VMs, you can use the Azure Fence agent. When using the Azure Fence Agent, consider setting the `skipShutdown` flag to forcefully skip the power off command graceful shutdown period. When the skipShutdown flag isn't set, a failover can take between 10 to 15 minutes.
