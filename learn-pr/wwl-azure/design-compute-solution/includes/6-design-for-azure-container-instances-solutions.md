...
Multi-container groups are useful when you want to divide a single functional task into several container images. These images have separate resource requirements, and different teams can deliver them. Some example scenarios include:
...
| **Fault tolerance** | If a cluster node fails in Azure Container Instances, the orchestrator on another cluster node rapidly recreates any containers running on the failed node. | A virtual machine can fail over to another server in a cluster with the operating system of the virtual machine restarting on the new server. |