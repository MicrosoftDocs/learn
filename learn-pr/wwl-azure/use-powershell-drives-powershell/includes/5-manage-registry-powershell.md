You may already be familiar with the graphical Registry Editor, which you can use to manage registry keys, entries, and values. You can also manage the registry by using PowerShell and the Registry provider.
...
You access registry keys by using cmdlets with the `Item` and `ChildItem` nouns, whereas you access entries and values by using cmdlets with the `ItemProperty` and `ItemPropertyValue` nouns. This design is because PowerShell considers registry entries to be properties of a key item.
...
> [!NOTE]
> For more information about transactions in PowerShell, refer to the `about_Transactions` help topic.
