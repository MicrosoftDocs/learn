...
- Integration into an existing environment. You can control access to Azure file shares by using identities from Microsoft Entra ID or AD DS, which  sync to Microsoft Entra ID. This helps ensure that users can have the same experience accessing an Azure file share as when they access an on-premises file server.
