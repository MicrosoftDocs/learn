...
5. To access content from the SAP Fiori application cluster and SAP HANA database, Irwin's request proceeds through a firewall. The **Azure Firewall** ensures the connection is asking for the right ports and that Irwin's IP address is on the allow list.
