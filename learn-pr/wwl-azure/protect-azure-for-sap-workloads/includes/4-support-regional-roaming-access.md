...
4. Once authenticated, Tim’s request proceeds through an **Azure Firewall**. The firewall ensures Tim is asking for the right ports and confirms Tim's IP address is on the allow list to access the web servers. Like Amita's request, Tim's request is still in the hub Azure Virtual Network and the Azure network security group.
