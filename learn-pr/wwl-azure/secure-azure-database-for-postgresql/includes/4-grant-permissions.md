...
1. As an example, the follow query:
...
1. You can then grant additional privileges to objects within the database. For example:
...
- **SUPERUSER | NOSUPERUSER** - You cannot assign SUPERUSER privileges within Azure Database for PostgreSQL. If not specified, NOSUPERUSER is the default.
- **CREATEDB | NOCREATEDB** - whether the role can create databases. The default is NOCREATEDB.
- **CREATEROLE | NOCREATEROLE** - whether the role can create new roles, ie execute CREATE ROLE. If CREATEROLE privilege if granted, the role can also alter and drop other roles. NOCREATEROLE is the default.
- **INHERIT | NOINHERIT** - whether the role inherits privileges of roles it is a direct or indirect member of. INHERIT is the default.
- **LOGIN | NOLOGIN** - whether a role is allowed to log in. A role with the LOGIN attribute is a database user. Roles without the LOGIN attribute may be used for managing database privileges. NOLOGIN is the default.
...
- **CONNECTION LIMIT** connlimit - specifies how many concurrent connections a role that can login can make. -1 is the default. This does not place a limit on concurrent connections.
- **[ ENCRYPTED ] PASSWORD 'password'| PASSWORD NULL** - sets the role's password. A password is only used by roles with the LOGIN attribute. If no password is specified, the password is NULL and password authentication will fail. Alternatively, explicitly state PASSWORD NULL.
- **VALID UNTIL** 'timestamp'- the date and time after which the role's password is no longer valid. If omitted, there is no time limit on the password.
- **IN ROLE role_name** - one or more roles that the new role is added to as a new member. There is no option to add a new role as an administrator; use the GRANT command.
