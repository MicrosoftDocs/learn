...
Let's perform a common task: creating an **Azure Storage account**. Here are the four steps you need to follow:

1. **Connect to Azure** using `az login`.

1. **Work with Azure subscriptions** using `az account`.

1. **Create a resource group** (if needed) using `az group create`.

1. **Create a storage account** using `az storage account create`.

### Step 1: Connect to Azure

Because you're working with a local **Azure CLI** installation, you need to authenticate before you can execute **Azure** commands. Sign into **Azure** by using the `az login` command:
...
After a successful sign-in, you're connected to your **Azure subscription**.
...
If you want to change your default subscription:
...
> [!TIP]
> The backslash (`\`) is the **line continuation character** in **Bash**. It allows you to split long commands across multiple lines for better readability.
...
> [!NOTE]
> Format the `--query` parameter string using **JMESPath**, which is a standard query language for JSON requests. Learn more at [JMESPath Tutorial](https://jmespath.org/tutorial.html).

## Clean up resources

Using random IDs and running these module steps creates test **resource groups** that can be removed. The easiest way to clean up **Azure resources** is to delete the **resource group**. However, when you delete a **resource group**, you delete every object inside the group, so it's important to delete the correct **resource group** name!
...
> [!TIP]
> The `--no-wait` parameter allows the delete operation to run in the background, freeing up the command line for other tasks.

Complex or repetitive tasks can be time-consuming and error-prone when performed manually. Many organizations prefer to automate these tasks to reduce costs and avoid mistakes. Using an **Azure CLI** script to automate the creation of **Azure resources** is much more efficient and reliable than manually creating them one at a time.
