...
By making this list, you'll see that the **Stop-Process** command has more than one parameter that accepts pipeline input by using **ByPropertyName**. Those parameters are **–Name** and **–Id**. The objects that Get-LocalUser produces don't have an ID property, so the **–Id** parameter isn't considered. The objects that **Get-LocalUser** produces have a **Name** property. Therefore, the contents of the **Name** property attach to the **–Name** parameter of **Stop-Process**. This means that **Stop-Process** will try to stop a process with a name that is the same as a user. If you try this step in Windows PowerShell, you'll notice that any errors that you receive are because a process can't be found with the target name. However, you wouldn't want to do this step in a real-world scenario. 
