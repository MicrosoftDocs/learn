...
Azure Database PostgreSQL flexible server allows us to create migration projects to help plan and perform database migrations from source servers to our flexible server. They're scoped to a single source server and the flexible server that the project is created on. However, we can specify up to eight (8) databases to be part of a migration project, making it easier to migrate applications where there are multiple databases associated with them.

When setting up the project, we're able to validate the source server connectivity so that we can ensure that we have the appropriate connectivity in place. Notice that we can perform this test in the following screenshot.

[![Azure portal showing the migration project source page.]](../media/10-pgflex-migation-project-connect-source.png)

Once we validated the source information, then we can move on to the next steps of validating the destination. In this case, the server details are all provided for us and we need to add the authentication information for the username and password needed to access the Azure Database for PostgreSQL Flexible Server.

[![Azure portal showing the migration project target page.]](../media/10-pgflex-migation-project-connect-target.png)
...
[![Azure portal showing the migration project database selection.]](../media/10-pgflex-migation-project-select-database.png)
...
[![Azure portal showing the migration project database validation and migration status.]](../media/10-pgflex-migation-project-start.png)
...
[![pgAdmin with menus showing various admin options.]](../media/3-migrationTools-pgAdmin.png)
...
## Next: Select the migration mode and tools
