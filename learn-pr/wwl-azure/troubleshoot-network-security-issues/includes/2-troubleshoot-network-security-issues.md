...
There are occasions where the requests that should pass through your Web Application Firewall (WAF) are blocked.

To tune the strict Open Web Application Security Project (OWASP) rulesets as per the needs of the application or organization, WAF helps you to customize, or disable the rules, or create exclusions, that may be causing issues or false positives. This can be done on a per-site and per-URI basis, that is, the changes to the policies will only affect specific sites/URIs and wouldn’t affect other sites that may not be having the same issues.

The following articles help gain understanding of how the WAF functions and how the WAF rule sets and logs work:
...
WAF logs work as a statement of all evaluated requests that are matched or blocked. It shows every request that is matched or blocked by the WAF. If you notice a false positive, meaning, if the WAF blocks a request that it shouldn’t, you can perform the following steps:

- First, find the specific request.

- Next, examine the logs to find the specific URI, timestamp, or transaction ID of the request.

- Finally, fix the false positives.

## Viewing WAF logs

To view WAF logs, preform the following steps:
...
Anomaly Scoring mode is one of the modes used by OWASP when deciding whether to block traffic. In Anomaly Scoring mode, traffic that matches any rule isn't instantly blocked when the firewall is in Prevention mode. Rules have a certain criterion: Critical, Error, Warning, or Notice. Each criterion has a numeric value associated with it, which is called Anomaly Score. The numeric value indicates the severity of a request.

For more information, see [Anomaly scoring mode](/azure/web-application-firewall/ag/ag-overview).

## Fixing false positives

To fix the false positives and to avoid the issues of blocked traffic, you can:

Use an Exclusion List: Using an Exclusion List is applicable to only a specific part of a request, or a ruleset that is being disabled. You can decide to exclude either body, headers, or cookies for a certain condition instead of excluding the whole request. In a global setting environment, the specific exclusion applies to all traffic passing through your WAF.

![Screenshot showing Web application firewall gateway](../media/2-wafgateway.png)
...
- **Disable the rule**: Disabling a rule allows to treat a certain condition as a non-threat, that would otherwise be flagged as malicious and be blocked. In a global setting environment, disabling a rule for the entire WAF is a risk and can weaken your security.
...
![Screenshot showing Web application firewall gateway](../media/2-firewallgateway.png)
...
There is a third-party tool called Fiddler that can provide additional information. Fiddler is a useful tool that can help you in:

- **Finding request attribute names**: Review individual requests and determine what specific fields of a web page are called. It also helps to exclude certain fields from inspection using Exclusion List.

- **Finding request header names**: View request and response headers inside the developer tools of Chrome or see the headers for the GET request.

- **Finding request cookie names**: View cookies, by selecting the Cookies tab in Fiddler.

## Restrict global parameters to eliminate false positives

- **Disable request body inspection**: Certain bodies that are not a threat to your application can be turned off from being evaluated by your WAF by setting **Inspect request body** to off. This way only the request body is not inspected. The headers and cookies will still be inspected, unless excluded under the exclusion list functionality.

![Screenshot showing WAF policy. The option of Inspect request body to off](../media/2-wafpolicy.png)

- **File size limits**: The possibility of an attack to your web servers and applications can be reduced by limiting the file size for your WAF. Permitting large files increases the risk of your backend being exhausted. To prevent attacks, it is advisable to limit the file size to a typical case for your application.
...
If the client is using a version of Transport Layer Security (TLS)that is lower than the minimum required version, all calls to Azure Storage will fail. Therefore, from the security point of view, an Azure Storage account may require that clients use a minimum version of TLS to send requests. For example, a request sent by a client who is using TLS 1.1 will fail, if a storage account requires TLS 1.2.

The article [Configure minimum required version of Transport Layer Security (TLS) for a storage account](/azure/storage/common/transport-layer-security-configure-minimum-version?tabs=portal) explains how to configure the minimum TLS version for an Azure Storage account that may affect client applications.
...
> [!NOTE]
> Note: Fiddler or a similar tool can help you verify the specified version of TLS was used by the client to send a request.
...
P2S VPN supports SSTP (Secure Socket Tunnelling Protocol), and IKEv2. You can securely connect different clients running Windows, Linux, or macOS to an Azure VNet through point-to-site connection.
...
It is recommended to generate a unique certificate for each client instead of using the same certificate. This is because if you want to revoke a particular client certificate, you don’t need to generate and install a new certificate for every client that uses the same certificate. To generate client certificate, consider the following steps:

1. Use the following example if the PowerShell console session still open.
...
```
   ...
   1. In this example, a client certificate 'P2SChildCert' is generated. The client certificate that you generate is automatically installed in 'Certificates - Current User\Personal\Certificates' on your computer.
...
Azure Private Endpoint is a network interface that uses a private IP address from your virtual network and connects you privately and securely to a private link service.

The following are the connectivity scenarios available with Private Endpoints:
...
The given steps will guide you to ensure that all the required configurations are in place, to resolve connectivity problems with your private endpoint setup. For detailed steps, see [Diagnose connectivity problems](/azure/private-link/troubleshoot-private-endpoint-connectivity).
...
1. If the Private Endpoint is linked to a Private Link Service which is linked to a Load Balancer, check if the backend pool is reporting healthy. Fixing the Load Balancer health will fix the issue with connecting to the Private Endpoint.

1. Contact the Azure Support team if your problem is still unresolved and a connectivity problem still exists.
...
The IPsec and IKE protocol standard supports a wide range of cryptographic algorithms in various combinations. The article [IPsec/IKE parameters](/azure-stack/user/azure-stack-vpn-gateway-settings?view=azs-2102) explains which parameters are supported in Azure Stack Hub to meet your compliance or security requirements.

Note:

The following important considerations when using these policies:
...
Following are the steps on how to create and configure an IPsec/IKE policy and apply it to a new or existing connection. For detailed step-by-step instructions, follow [Steps to configure an IPsec/IKE policy for site-to-site (S2S) VPN connections in Azure Stack Hub](/azure-stack/user/azure-stack-vpn-s2s?view=azs-2102&tabs=az1%2Caz2%2Caz3%2Caz4%2Caz5%2Caz8%2Caz6%2Caz7).
