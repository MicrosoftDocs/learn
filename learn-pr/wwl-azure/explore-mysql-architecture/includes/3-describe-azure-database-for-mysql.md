...
Microsoft have implemented **Azure Database for MySQL** in Azure as Platform as a Service (Paas) database. Azure Database for MySQL is a fully managed database service that is ready installed, ready configured with automatic backups and in-built security.

For applications that require a fast response from the database, the number of CPU cores, amount of IOPS, and amount of RAM are important factors. When buying on-premises hardware it can be difficult to predict future requirements as your needs change. Azure offers the flexibility of scaling up or down as needed. You can even stop Azure Database for MySQL when it is not required, and you are not charged for the resource whilst it is stopped. With Azure Database for MySQL Flexible server you can increase IOPS without increasing storage.
...
- **Single Server** - runs on Windows. Single server is designed for minimal customization and is best suited to existing applications that already leverage single server. Azure Database for MySQL Single server is not available in all regions and you might need to alter the deployment region or use Azure Database for MySQL Flexible server.
...
> [!NOTE]
> Azure Database for MySQL does not allow access to the underlying operating system or file system, and some data manipulation commands are not supported.
...
Azure Database for MySQL provides zone independent high availability with automatic failover. This ensures that data is not lost in the event of a failure. When high availability is configured, a standby replica is provisioned and maintained with every database modification.

There are two high availability options:

- **Zone-redundant HA**. This option provides the highest level of availability, with redundancy across zones. This option can be enabled only when the server is created. This is not supported by Azure Database for MySQL single server.
