...
The **AzureEnhancedMonitoring** Windows service collects performance metrics in Azure. If the service isn't installed correctly or if it isn't running in your VM, no performance metrics can be collected.
...
If the service doesn't exist, the Azure Extension for SAP isn't installed correctly. Reinstall the extension by using the steps described earlier. After you deploy the extension, after one hour, check again whether the Azure performance counters are provided in the Azure VM.
