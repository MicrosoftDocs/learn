...
- Application instance A has a cache with a snapshot of the data at time `X`. It retrieves data at time `X` and caches it in-memory.

- Application instance B has a cache with a snapshot of the data at time `Y`. It retrieves data at time `Y` and caches it in-memory.

- Information in the SQL database changes between time `X` and time `Y`.
...
| Pattern | Description |
| --- |--- |
| **Data cache** | Databases are often too large to load directly into a cache. It's common to use the _cache-aside_ pattern to only load data into the cache as needed. When the system makes changes to the data, the system can also update the cache, which is then distributed to other clients. Additionally, the system can set an expiration on data, or use an eviction policy to trigger data updates into the cache. |
