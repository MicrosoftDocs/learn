...
Some significant differences between the two protocols include:
 ...
 -  No mutual authentication: Kerberos allows for mutual authentication, meaning both the client and server can verify each other’s identity. NTLM does not allow for client verification of server identity.
 ...
 -  Dependence on trust: In a multi-domain environment, Kerberos does not require all domains to trust each other, using existing trusts to find a path to the target. NTLM’s trust model is less flexible.
