...
1. Start a new build and select agent pool.
...
1. Open the source code for the PartsUnlimited package in Visual Studio in a different solution.
...
1. Change the patch version: `1.0.1`. Make a small edit to the code for illustrative purposes, and check in the new code.
1. Change the **exception type** check-in, commit, and push the new code. We go back to the Azure DevOps pipeline. Since there's a trigger on the code we just changed, the build will automatically start.
1. Go back to build pipeline for **PartsUnlimited Security Package**.
...
1. Open **Artifacts** and show the feed.
...
1. Scroll down and click on the **PartsUnlimited.Security 1.0.1** package. By clicking on it, we can inspect the details and the versions.
1. Edit the build pipeline **PartsUnlimited E2E**.
1. Click **NuGet restore**.
