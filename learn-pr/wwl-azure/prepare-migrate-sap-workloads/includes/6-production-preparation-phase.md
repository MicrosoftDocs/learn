...
3. Test production migration process into Azure. In cases where you are not moving all production systems to Azure within the same timeframe, identify groups of production systems that need to reside in the same hosting location. Test data migration. Common migration methods include:
    ...
    * Using DBMS methods like backup/restore in combination with SQL Server AlwaysOn, HANA System Replication or Log shipping to seed and synchronize database content into Azure.
    ...
    * Use the SAP DMO process if you need to combine with an SAP release upgrade. Keep in mind that not all combinations between source and target DBMS are supported. 
...
6. Adapt your deployment scripts to leverage new Azure VM SKU types and incorporate new features from which you can benefit.
...
7. After deployment of infrastructure and application, step through the additional checks to validate the following:
    ...
    * Guest operating systems of Azure VMs are at the intended version and patch level. The intended application releases and patches have been installed.

    * Guest operating systems of Azure VMs are at the intended version and patch level. Azure Premium Storage has been used for the latency sensitive disks or where the single VM availability SLA of 99.9% is required.

    * Guest operating systems of Azure VMs are at the intended version and patch level. Whenever applicable, Azure Write Accelerator has been enabled.

    * Guest operating systems of Azure VMs are at the intended version and patch level. Make sure that within the VM, storage spaces, or stripe sets have been created across disks that need Azure Write Accelerator support. 
    ...
    * Network latency between SAP application layer VM and DBMS layer complies within the guidelines provided in SAP Note #500235 and SAP Note #1100926.
...
8. Create a playbook that facilitates Azure planned maintenance events. The playbook should define the order in which the systems and Azure VMs to be taken offline and brought online in case of planned maintenance.