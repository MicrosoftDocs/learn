In the production phase, you are starting to deploy non-production SAP systems into Azure following a successful pilot, leveraging all the testing and validation tasks. All the criteria and steps applicable to the pilot do apply in this phase as well. The non-production environment typically includes development, unit tests, and business regression tests systems. It is recommended that at least one of them implements high availability configuration that will be used for the future production system. Additional steps you need to consider during that phase include:
