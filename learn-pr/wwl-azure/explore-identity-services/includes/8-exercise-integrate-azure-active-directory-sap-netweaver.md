...
3.  Log on to the business client of SAP System (T01), where SSO is required, and activate HTTP Security session Management.
...
7.  Go to **Transaction code SAML2** in the business client of SAP system \[T01/122\]. It will open a user interface in a browser.
...
11. Generate Service Provider Metadata: once you are done with configuring the Local Provider and Trusted Providers settings on SAML 2.0 User Interface, the next step involves generating the service provider’s metadata file (which would contain all the settings, authentication contexts, and other configurations in SAP).
12. On the **Local Provider** tab, select **Metadata**.
13. Save the generated Metadata XML file on your computer and upload it in Basic SAML Configuration section to auto-populate the **Identifier** and **Reply URL** values in the Azure portal.
...
17. On the **Basic SAML Configuration** section, perform the following steps:
    ...
    3.  After the metadata file is successfully uploaded, the **Identifier** and **Reply URL** values get auto populated in **Basic SAML Configuration** section textbox as shown below:
...
1.  Log on to the SAP system and go to transaction code SAML2. It opens a new browser window to the SAML configuration screen.
...
13. From the pop-up window select **Unspecified** from the **Supported NameID** formats and select **OK**. Note that user ID Source and user ID mapping mode values determine the link between SAP user and Azure AD claim.
