...
- The Storage Account used needs to be in the same Azure Region as the one that's used to deploy the VM SQL Server.
...
- As a result of pushing file I/O through the network quota, you're stranding the storage quota mostly, and so only partially using the overall bandwidth of the VM.
