There are four separate stages to executing the query, in the order of execution these are:
...
The parser builds a query tree, which separates the query into identifiable parts to understand which tables are involved, what filters have been applied, etc. The parts of a query tree are:

- **Command type** - SELECT, INSERT, UPDATE, or DELETE.
- **Range table entry (RTE)** - a list of relations, ie tables, subqueries, results of joins, etc. In a SELECT statement these appear after the FROM key word.
...
- **Join tree** - this may be a list of the FROM items. Joins can be done in any order, or done in a specific order, such as Outer joins.
- **Others** - items that are not relevant at this stage, such as the ORDER BY clause.
...
For example, rules on SELECT are always applied as the last step, including for INSERT, UPDATE and DELETE queries. Rules also mean that UPDATE queries do not overwrite existing rows, instead a new row is inserted, and the old row is hidden. After the transaction has committed, the vacuum process can remove the hidden row.
...
ANALYZE collects statistics about database tables and stores the results in the **pg_statistic** system catalog. You will need to run ANALYZE, if:

- You have disabled **autovacuum** (which normally analyses tables automatically)
- You have disabled **autovacuum**, and haven't run ANALYZE recently
- Either or the above, and there have been a lot of INSERTS, UPDATES, or DELETE statements.

Cost estimates reply on up-to-date statistics, and if statistics are out of date and an inefficient plan may be chosen. When no parameter is passed to ANALYZE, every table in the database will be examined.
...
For **SELECT**, the executor returns each row back to the client as the resultset.

For **INSERT**, each returned row is inserted into the specified table. This is done in a special top-level plan node called ModifyTable.
...
For **DELETE**, the only column that is actually returned by the plan is the row ID. The ModifyTable node uses the row ID to mark the row as deleted.
