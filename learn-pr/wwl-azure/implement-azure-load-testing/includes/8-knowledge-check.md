...
- content: What is the purpose of the test engine in Azure Load Testing?
    choices:
  - content: It is a computing infrastructure that runs the Apache JMeter test script.
      isCorrect: true
      explanation: Correct. The test engine runs the Apache JMeter test script and can be scaled out for larger tests.
  - content: It is used to specify the list of URL endpoints to test.
      isCorrect: false
      explanation: Incorrect. The test engine does not specify URL endpoints, it runs the Apache JMeter test script.
  - content: It is used to store the results of the load test.
      isCorrect: false
      explanation: Incorrect. The test engine does not store results, it runs the Apache JMeter test script.
...
- content: What is the purpose of the 'engineInstances' key in a load test configuration?
  ...
  - content: It specifies the number of virtual users in the test script.
      isCorrect: false
      explanation: Incorrect. The 'engineInstances' key does not specify the number of virtual users in the test script.
  - content: It determines the load pattern in the test script.
      isCorrect: false
      explanation: Incorrect. The 'engineInstances' key does not determine the load pattern in the test script.
...
- content: What is the purpose of defining fail criteria in Azure Load Testing?
  ...
  - content: To ensure that the total number of requests per second doesn't exceed a threshold.
      isCorrect: false
      explanation: Incorrect. While this could be a part of the fail criteria, it is not the overall purpose. The purpose is to define performance and quality expectations for an application under load.
  - content: To automatically start a load test when the error rate surpasses a given threshold.
      isCorrect: false
      explanation: Incorrect. Fail criteria are not used to start a load test, but rather to define performance and quality expectations for an application under load.
- content: What is the purpose of using secrets in Azure Load Testing?
  ...
  - content: To store non-sensitive information and make it available as environment variables in the load test engine.
      isCorrect: false
      explanation: Incorrect. Secrets are used to store sensitive information, not non-sensitive information.
- content: What is the role of a service principal in the context of automating Azure load testing using GitHub Actions?
  ...
  - content: It represents the Azure Load Testing resource in Microsoft Entra ID.
      isCorrect: false
      explanation: Incorrect. The service principal does not represent the Azure Load Testing resource, but rather the GitHub Actions workflow.
  - content: It is used to create and manage test plans, test results, and related artifacts.
      isCorrect: false
      explanation: Incorrect. The service principal does not directly manage test plans or results. It provides access for the GitHub Actions workflow to the Azure Load Testing resource.
