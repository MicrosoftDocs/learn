...
Both planes use Microsoft Entra ID for authentication. For authorization, the management plane uses Azure role-based access control and the data plane uses a Key Vault access policy and Azure role-based access control for Key Vault data plane operations.<br>
