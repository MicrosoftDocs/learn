...
- Assign a coadministrator to provide access when the first administrator isn't available.
- Limit administrator accounts to the fewest number that's practical. If an administrator account is compromised, a hacker has full access to the server.
...
- There's no **Root user** in Azure Database for MySQL. Instead, use the **Administrator** user role or **Owner**.
