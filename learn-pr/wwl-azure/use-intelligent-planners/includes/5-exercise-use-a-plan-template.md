You can tune the plans generated by the Handlebars planner. In this exercise, you'll generate a plan, tune the template, and use it as a function prompt. Let's try it out!
...
1. In your 'Program.cs' file, update your code to the following:
    ...
    Now you're able to see the generated plan. Next, you'll add a quick function to suggest a song to the user.
...
1. Enter `dotnet run` in the terminal to see the output of the plans you created.
    ...
    You should see a template similar to the following:
...
1. Create a new file named 'handlebarsTemplate.txt' with the following text:
    ...
    Notice the `{{#if ...}}` syntax. This acts as a conditional statement that the Handlebars planner can use, similar to a traditional `if`-`else` block in C#. The `if` statement must be closed with `{{/if}}`. You also add a `{{set ...}}` statement that sets the user's location to the `location` variable.
...
1. Enter `dotnet run` in the terminal to see the output of your planner template.
    ...
    ```output
    Based on your recently played music, I recommend you listen to the song "Luv(sic)" by Nujabes. It falls under the genres of hiphop and rap, which aligns with some of your recently played songs. Enjoy!  
    ```
...
> [!IMPORTANT]
> B Be sure not to delete any of the code you wrote so far since it's needed for the next exercise.