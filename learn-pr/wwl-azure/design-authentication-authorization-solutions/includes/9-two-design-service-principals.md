...
### User permissions and consent for applications

The Microsoft identity platform implements the [OAuth 2.0](https://oauth.net/2/?azure-portal=true) authorization protocol. A third-party app can use OAuth 2.0 to access web-hosted resources on behalf of a user. The web-hosted resources can define a set of permissions that you can use to implement functionality in smaller chunks. Developers can allow one of two types of permissions supported by the Microsoft identity platform depending on the app scenario.

By understanding the different types of permissions supported in Azure AD apps, you can design an access strategy that works for your organization. There are different consent framework models for obtaining permissions from users for use in custom apps.

Microsoft identity platform supports two types of permissions: delegated permissions and application permissions.

- **Delegated permissions**: For apps that have a signed-in user present. These permissions are provided to the app by the user so the app can perform actions on their behalf. This option doesn't give permissions to the app. Instead, the user is simply allowing the app to act on their behalf by using the user's permissions.

- **Application permissions**: For apps that run without a signed-in user present.

There's a difference between the delegated and application permissions granted to an app, and the _effective_ permissions of the app when making calls to the target resource. Effective permissions are the permissions your app has when making requests to the target resource.

- **Effective permissions for delegated permissions**: The effective permissions of your app are the intersection of the delegated permissions granted to the app and the privileges of the currently signed-in user. The app can never have more privileges than the privileges of the signed-in user. In the Tailwind Traders organization, the privileges of the signed-in user might be determined by policy, or by membership in one or more administrator roles.

   Assume your app has been granted the `User.ReadWrite.All` delegated permission. This permission enables your app to be used to read and update the profile of every user in an organization. If the signed-in user is a global admin, your app can update the profile of every user in the organization. However, if the signed-in user isn't in an administrator role, your app can update only the profile of the signed-in user. Your app can't update the profiles of other users in the organization because the user it has permission to act on behalf of doesn't have those privileges.

- **Effective permissions for application permissions**: The effective permissions of your app are the full level of privileges implied by the permission. An app that has the `User.ReadWrite.All` application permission can update the profile of every user in the organization.