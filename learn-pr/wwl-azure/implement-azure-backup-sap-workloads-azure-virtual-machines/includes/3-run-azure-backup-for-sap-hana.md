This image shows an architecture for backing up SAP HANA databases that are running on Azure virtual machines (VMs) to an Azure Backup Recovery Services vault.

![Architecture for backing up SAP HANA databases that are running on Azure VMs to an Azure Backup Recovery Services vault](../media/backup-architecture.png)

SAP HANA databases are critical workloads that require a low recovery-point objective (RPO) and long-term retention. You can use Azure Backup to back up SAP HANA databases running on Azure VMs.
...
- **Soft Delete**: Even if a malicious actor deletes a backup (or if backup data is accidentally deleted), the data is retained and can be recovered for 14 additional days.
...
When you have run the pre-registration script on each VM that you want to back up, you can then discover databases. During discovery, Azure Backup registers the VM with the vault and installs an extension on the VM.
...
Backup policy defines when backups are taken and how long they are retained. Policies are created at the vault level, and while multiple vaults can use the same backup policy, you will need to apply the backup policy to each vault. Policies can be set for full and differential backups.
...
- **RPO** indicates how much data loss is acceptable for the user/customer. This is determined by the log backup frequency: more frequent log backups provide lower RPO (to a minimum of 15 minutes).
...
- All changes are applied retroactively. The most recent backup policy is also applied to any recovery points that have already been taken.

- The scope of change also includes the day of backup and type of backup, as well as retention. For example: If you change a policy  from **daily full** to **weekly full on Sundays**, all earlier daily full backups that aren't from Sundays will be marked for deletion.
...
- If the initial backup is pending, an on-demand backup will create a full copy of the VM in the Recovery Services vault.
...
1. Azure Backup executes a pre-snapshot script that puts SAP HANA in a consistent state.

1. When the consistent state is confirmed, Azure Backup executes the disk snapshots.

1. After completing the snapshots, Azure Backup undoes the activity it did in the pre-snapshot script.

1. After a successful execution, Azure Backup streams the data into the Backup vault.
