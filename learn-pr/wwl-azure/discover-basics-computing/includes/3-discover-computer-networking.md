...
![image of a MAC address](../media/mac-address.svg)

Okay, so there are lots of computer with unique IDs on the internet. Let's talk about how they all talk to each other.
...
![image showing internet and subnets](../media/person-diagram.svg)
...
Let's go back to the question, then, on how that text message sent by your friend ends up showing up on your mobile device. As you might guess by now, MAC addresses are not the only IDs involved when you send that text message to a friend. For that text message to find your friend, the systems that run the internet have to know how to find your unique machine in a secure way.
