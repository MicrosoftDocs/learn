Azure Center for SAP solutions (ACSS) creates several required resources during system registration, but for ACSS to fully deploy it also requires an SAP workload in Azure infrastructure and the SAP system prerequisites listed in this topic.
...
These resources do not have any billing or cost associated with them.

Azure Center for SAP solutions system registration also creates:

- A managed resource group which is used by the ACSS service.
- A Storage account within the managed resource group which contains blobs.
