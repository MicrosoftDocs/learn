...
To view or read an account's access keys, the user must either be a Service Administrator, or must be assigned an Azure role that includes **Microsoft.Storage/storageAccounts/listkeys/action**. Some Azure built-in roles that include this action are the **Owner**, **Contributor**, and **Storage Account Key Operator Service Role**.

## Use Azure Key Vault to manage your access keys

Microsoft recommends using Azure Key Vault to manage and rotate your access keys. Azure Key Vault provides several benefits:

- **Centralized key management**: Store all your storage account keys in a single, secure location.
- **Secure access**: Your application can securely access keys in Key Vault without storing them in application code or configuration files.
- **Automated rotation**: Configure automatic key rotation using Azure Key Vault's storage account key management feature, which can rotate keys on a defined schedule.
- **Access logging**: Monitor and audit all key access through Azure Monitor logs.
- **Integration with managed identities**: Applications running in Azure can use managed identities to access keys from Key Vault without storing any credentials.

For automated key rotation, you can also use Azure Automation runbooks or Azure Functions to implement custom rotation logic that updates both the storage account key and the Key Vault secret.
...
Rotating or regenerating access keys revokes shared access signatures (SAS) that are generated based on that key. After access key rotation, you must regenerate account and service SAS tokens to avoid disruptions to applications. Note that **user delegation SAS** tokens are secured with Microsoft Entra credentials and aren't affected by key rotation, making them a more secure choice when working with Blob storage.
...
To rotate an account's access keys, the user must either be a Service Administrator, or must be assigned an Azure role that includes the **Microsoft.Storage/storageAccounts/regeneratekey/action**. Some Azure built-in roles that include this action are the **Owner**, **Contributor**, and **Storage Account Key Operator Service Role**.
...
Key expiration policies are particularly useful in organizations with compliance requirements that mandate regular key rotation. Combined with Azure Policy, you can enforce key rotation across your entire Azure environment.
...
4. To bring a storage account into compliance, rotate the account access keys following the manual rotation process described earlier.
