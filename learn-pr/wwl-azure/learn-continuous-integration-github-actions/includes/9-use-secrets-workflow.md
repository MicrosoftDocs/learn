...
It is important to follow this approach, which minimizes the risk of their accidental exposure. Secrets passed between processes from the command line can be viewed by using the ps command and captured in security audit logs.

## Using secrets in if: conditionals

Since secrets cannot be directly referenced in if: conditionals, as a workaround, consider setting secrets as job-level environment variables, then referencing the environment variables to conditionally run steps in the job, as in the following example:
...
Note that with the exception of GITHUB\_TOKEN, secrets are not passed to the runner when a workflow is triggered from a forked repository.<br>
