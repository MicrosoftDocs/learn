...
The cluster master includes the following core Kubernetes components:
 ...
 -  kube-scheduler. The Scheduler determines what nodes can run the workload and starts when you create or scale applications.
...
An *AKS cluster*, which is an Azure virtual machine (VM) that runs the Kubernetes node components and container runtime, contains one or more nodes:
 ...
 -  The *container runtime* is the component that allows containerized applications to run and interact with more resources such as the virtual network and storage. In AKS, Docker is used as the container runtime.

Nodes of the same configuration are grouped into *node pools*. A Kubernetes cluster contains one or more node pools.

The initial number of nodes and size is defined when you create an AKS cluster, making a default node pool.
