A PowerShell provider, or just *provider*, is an adapter that makes some data stores resemble hard drives within PowerShell. Because you're already familiar with managing hard drives using command-line commands, PowerShell providers help you manage other forms of data storage using the same familiar commands.
...
- Alias. Provides access to aliases for PowerShell cmdlets.
...
- Function. Provides access to PowerShell functions loaded into memory.
- Variable. Provides access to PowerShell variables and their values loaded into memory.
