...
For example, you can choose to create Microsoft Sentinel incidents automatically only from high-severity Microsoft Defender for Cloud alerts.

> [!div class="mx-imgBorder"]
> ![Create Microsoft Sentinel incidents automatically only from high-severity Microsoft Defender for Cloud alerts](../media/az500-incidents.png)

You can create more than one Microsoft Security analytic rule per Microsoft security service type. This does not create duplicate incidents, since each rule is used as a filter. Even if an alert matches more than one Microsoft Security analytic rule, it creates just one Microsoft Sentinel incident.
