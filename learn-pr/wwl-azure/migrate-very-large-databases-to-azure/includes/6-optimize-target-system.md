 ...
 -  Customers using SQL Server TDE are recommended to pre-create the database and transaction log files, then enable TDE prior to starting the import. TDE will run for a similar amount of time on a DB that is full of data or empty. Enabling TDE on a VLDB can lead to blocking/locking issues and it is recommended to import into a TDE database. The overhead importing to a TDE database is relatively low.

## Cost optimizations

 -  When you shut down an Azure VM and the VM is deallocated, you no longer have to pay for it (only for storage). With pay-as-you-go pricing, snoozing a typical SAP VM will provide considerable savings, as much as ~60-70%.
 -  For Virtual Machines that need to run all or most of the time, use Azure Reserved Virtual Machine Instances. Reserving an instance for 1 year can provide savings of ~40-45% or as much as ~60-65% for a 3-year reservation.
