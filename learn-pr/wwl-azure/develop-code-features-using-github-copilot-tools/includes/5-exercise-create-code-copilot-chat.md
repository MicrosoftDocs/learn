Skills in the APL-2007 credential:

- Create a new feature by using inline chat.
- Create a new code block by using inline chat.
- Create regular expressions to validate data patterns by using inline chat.
...
The Chat view and Quick Chat window enable interactive multi-turn conversations with the AI. Both of these interfaces provide a way to ask questions, get help with a coding problem, and generate code. During a conversation, GitHub Copilot responses can include source code (in the form of code blocks) as well as natural language. When code blocks are provided in a response, you can either copy them or inject them directly into your code editor.

The inline chat interface is designed to provide contextual help and code suggestions while you're coding.

In this exercise, you'll use GitHub Copilot's inline chat feature to update existing code and generate new code features.

### Review the coding tasks and project goals

> [!NOTE]
> To ensure that the instructions align to your code, you'll start this exercise with a sample code project. The project code from the previous exercise.

This exercise focuses on the following tasks:

1. Update the `SalesData` data structure and `GenerateSalesData` method to produce a data sample resembles "actual" data.
    ...
    - Increase the number of records generated to 200,000.
...
### Develop questions for inline chat

The prompts that you develop for GitHub Copilot's inline chat feature should be natural language questions or statements that encapsulate the task or problem you're trying to solve.

You can use the following process to create prompts and interact with inline chat:

1. Create a natural language prompt that you believe encapsulates your task or problem. Remember to "be specific and keep it simple".
1. Select the code block (or larger file context) that you want to update or replace, and then open the inline chat interface (press "Ctrl" + "I").
1. Submit your prompt to GitHub Copilot using the inline chat feature.
1. Review GitHub Copilot's suggestions.
1. Accept the suggestions that provide the best match for what you intended. If none of the suggestions are suitable, reject the suggestions and update your prompt.
1. If you are unable to construct a prompt that produces a useful suggestion, move on a more traditional development approach.

### Generate data structures using inline chat

It's important to start a development project by working on the features or parameters that you know. Selecting or creating a data source is often a good place to start.
...
> [!NOTE]
> In an actual business project, you'd probably use historical data rather generating simulated data. In this training, generating simulated data provides an opportunity to practice using the GitHub Copilot tools. Simulating data is not suggested as a best practice for business projects.
...
1. Ensure that you're using the intended code sample.
...
1. Enter the following prompt:
    ...
    ```output
    I need a public struct ProdDepartments that contains static fields for 8 clothing industry department names. For each department, create a 4-character abbreviation. The abbreviation must be unique.
    ```
...
1. Review the suggestions provided by GitHub Copilot.
    ...
    As long as the prompt is clear and specific, GitHub Copilot should provide a useful suggestion. If the suggestion is not what you expected, you can reject it and try again. In this case, the names of the departments are not important, so you can probably accept the suggestion.
...
1. Enter the following prompt:
    ...
    ```output
    I need a public struct ManufacturingSites that contains static fields for 10 manufacturingSites. Manufacturing sites should be represented by a 3-character code that includes a 2-letter ISO country code followed by a digit. Use 3 country codes.
    ```
...
1. Select the `SalesData` data structure, and then press "Ctrl" + "I" to open the inline chat interface.
    ...
    You need to add fields for `departmentCode` and `manufacturingSiteCode` to the `SalesData` data structure (both fields are string data types). You also need to change the data type for `productID` from `int` to `string`.
...
- unitPrice: Raise the lower bound of the price range to 25 and the upper bound to 300. Assume that size and color do not affect the unit price.
...
1. Enter the following prompt:
    ...
    ```output
    Update the departmentName assignment to randomly select a department name from the ProdDepartments data structure. 
    ```

1. Review the suggestions provided by GitHub Copilot and then select **Accept**.

1. Create a blank code line after the `departmentName` assignment.
...
1. Take a minute to consider how you want to construct the value assigned to `productID`.
    ...
    It is worth noting that the index number of the selected departmentName can be used to select the departmentAbbreviation and to calculate the first digit of the product number. With that in mind, here is the list of variables that you can use to construct the productID:
...
1. Copy the following variable declarations and paste them at the location of the blank code line that you created.
    ...
    Variable declarations aren't required for inline chat to generate code update suggestion from a prompt, but these code lines do help anchor GitHub Copilot to a specific line of code where the update belongs.

1. Select the `int indexOfDept = 0;` code line, open the inline chat (press the "Ctrl" + "I" keys), and then enter the following prompt:
...
1. Select the `string deptAbb = "";` code line, open the inline chat (press the "Ctrl" + "I" keys), and then enter the following prompt:
...
1. Select the `salesData[i].baseCost = random.Next(10, 100) + random.NextDouble();` code line, open the inline chat, and then enter the following prompt:
...
1. Select the `salesData[i].volumeDiscount = random.Next(0, 6);` code line, open the inline chat, and then enter the following prompt:

    ```output
    Assign 10 percent of quantitySold volumeDiscount. Truncate any fractional values.
    ```
...
1. To verify the current quarterly sales report output, run the application.
    ...
    Notice that the quarters are not listed in order and the currency values are not formatted properly.
