...
Microsoft offers the M-Series VM SKU that is certified for an SAP HANA scale-out configuration. The VM type M128s got certified for a scale-out of up to 16 nodes. Of the 16-node scale-out certification:
...
The basic configuration of a SUSE Linux VM node for SAP HANA scale-out has the following characteristics:
 ...
 -  All other disk volumes are not shared among the different nodes and are not based on NFS.
