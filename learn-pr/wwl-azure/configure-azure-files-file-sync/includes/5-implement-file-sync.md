...
- When a file is tiered, the Azure File Sync file system replaces the file locally with a pointer (or _reparse point_). The reparse point represents a URL to the file in Azure Files.
...
- **Consider file archiving with cloud tiering**. Azure File Sync stores only recently accessed data on local servers. When you implement cloud tiering, non-used data moves to Azure Files.