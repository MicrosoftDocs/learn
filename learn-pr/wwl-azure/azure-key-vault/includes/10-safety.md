...
You always want to make sure you have the ability to backup keys, certificates, and even secrets from your Key Vault in case of emergency.  There are two primary features to do this:
...
> ![Diagram of the soft delete flow in Key Vault.  Left side, no Soft Delete - Key is deleted and no recovery option exists.  Right side, Soft Delete enabled - Key is deleted and goes to deleted state, but not purged.  User can recover or purge the key from this point.](../media/az500-key-vault-soft-delete.png)
...
This screenshots shows how to download a backup of a primary key.  Similar process can be used to backup other items in Key Vault.
