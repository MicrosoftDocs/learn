Application templates in Azure IoT Central are a tool to help solution builders kickstart their IoT solution development. You can use app templates for everything from getting a feel for what is possible, to fully customizing your application for resale to your customers.

Application templates consist of:

 -  Sample operator dashboards.
 -  Sample device templates.
 -  Simulated devices producing real-time data.
 -  Pre-configured rules and jobs.
 -  Rich documentation including tutorials and how-tos.

You choose the application template when you create your application. You can't change the template after the application is created.

## Custom templates

If you want to create your application from scratch, choose the Custom application template.

## Industry focused templates

Azure IoT Central is an industry agnostic application platform. The following application templates are industry focused examples available for these industries today:

:::row:::
  :::column:::
    Name
  :::column-end:::
  :::column:::
    Industry
  :::column-end:::
  :::column:::
    Description
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Smart Meter Analytics
  :::column-end:::
  :::column:::
    Energy
  :::column-end:::
  :::column:::
    Creates an application to monitor energy consumption, network status, and identify trends to improve customer support and smart meter management.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Solar Power Monitoring
  :::column-end:::
  :::column:::
    Energy
  :::column-end:::
  :::column:::
    Creates an application to monitor solar panel status and energy generation trends.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Water Consumption Monitoring
  :::column-end:::
  :::column:::
    Government
  :::column-end:::
  :::column:::
    Creates an application to monitor and control water flow.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Water Quality Monitoring
  :::column-end:::
  :::column:::
    Government
  :::column-end:::
  :::column:::
    Creates an application to digitally monitor water quality.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Connected Waste Management
  :::column-end:::
  :::column:::
    Government
  :::column-end:::
  :::column:::
    Creates an application to monitor waste bins and dispatch field operators.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Continuous Patient Monitoring
  :::column-end:::
  :::column:::
    Health
  :::column-end:::
  :::column:::
    Creates an application to extend patient care, reduce readmissions, and manage diseases.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    In-store Analytics – Condition Monitoring
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Creates an application to connect and monitor a store environment.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Digital Distribution Center
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Creates an application to improve warehouse output efficiency by digitizing key assets and actions.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Smart Inventory Management
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Creates an application to automate receiving, product movement, cycle counting, and tracking.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Connected Logistics
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Creates an application to track your shipments in real time across air, water, and land with location and condition monitoring.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Micro-fulfillment Center
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Creates an application to digitally connect and manage a fully automated fulfillment center.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    In-store Analytics – Checkout
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Creates an application to monitor and manage the checkout flow inside your store.
  :::column-end:::
:::row-end:::
:::row:::
  :::column:::
    Video analytics - object and motion detection
  :::column-end:::
  :::column:::
    Retail
  :::column-end:::
  :::column:::
    Use cameras as a sensor in intelligent video analytics solutions powered by Azure IoT Edge, AI, and Azure Media Services.
  :::column-end:::
:::row-end:::


For more information about these templates, see the documentation at the following location: [https://docs.microsoft.com/azure/iot-central/core/overview-iot-central-admin](/azure/iot-central/core/overview-iot-central-admin)
