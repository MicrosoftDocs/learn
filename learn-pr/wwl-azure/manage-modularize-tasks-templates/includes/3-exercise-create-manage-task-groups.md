...
1. Click **Pipelines,** click **Releases** and click **Edit** to open the pipeline we worked on in the main menu.

    :::image type="content" source="../media/07-03-release-editor-2beaf055.png" alt-text="Release editor.":::


1. The **Development** stage currently has a single task. We'll add another task to that stage. Click the **View stage tasks** link to open the stage editor.
...
1. Click the **+** sign to the right of the **Agent job** line to add a new task, in the **Search** box, type **database**.
...
1. Hover over the **Azure SQL Database Deployment** option and click **Add**. Click the **Azure SQL DacpacTask** when it appears in the list to open the settings pane.

    :::image type="content" source="../media/07-07-azure-sql-dacpactask-55d53a05.png" alt-text="Azure SQL DacPac Task.":::


1. Set the **Display name** to **Deploy devopslog database**, and from the **Azure Subscriptions** drop-down list, click **ARM Service Connection**.
...
1. In the **SQL Database** section, set a unique name for the SQL Server, set the **Database** to **devopslog**, set the **Login** to **devopsadmin**, and set any suitable password.

    :::image type="content" source="../media/07-09-database-settings-6a816244.png" alt-text="Database settings.":::


1. In the **Deployment Package** section, set the **Deploy type** to **Inline SQL Script**, set the **Inline SQL Script** to:
...
1. Click **Save** then **OK** to save the work.

    Now that we have two tasks let's use them to create a task group.

1. Click to select the **Backup website zip file** task and select the **Deploy devopslog database** task, then right-click either task.

    :::image type="content" source="../media/07-11-double-select-8d81b4dc.png" alt-text="Double select.":::


1. Click **Create task group**, then in the **Create task group** window, set **Name** to **Backup website zip file and deploy devopslog**. Click the **Category** drop-down list to see the available options. Ensure that **Deploy** is selected, and click **Create**.
...
1. From the **Task** drop-down list, select the **Test Team A** stage.
...
1. Click the **+** sign to the right of the **Agent job** to add a new task. In the **Search** box, type **backup** and notice that the new task group appears like any other task.

    :::image type="content" source="../media/07-16-search-backup-111d236c.png" alt-text="Search backup.":::


1. Hover on the task group and click **Add** when it appears.
...
1. From the **Tasks** drop-down list, select the **Development** stage. Again click **Remove** to remove the task group from the **Development** stage.
