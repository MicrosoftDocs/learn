...
When you first deploy Azure SQL, you can specify a login name and a password for a special type of administrative login, the **Server admin**. The following configuration of logins and users in the master and user databases occurs during deployment:
 ...
 -  This login is granted full administrative permissions on all databases as a **server-level principal**. The login has all available permissions and can't be limited. In a SQL Managed Instance, this login is added to the **sysadmin fixedserver role** (this role does not exist in Azure SQL Database).
