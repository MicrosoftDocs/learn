This unit covers implementation details for SAP NetWeaver deployment scenarios with AnyDB on Azure VMs. In particular, it describes the steps to prepare Azure infrastructure for installing a high-availability SAP system on a Windows failover cluster (with a shared disk) to host an SAP ASCS instance. Azure Resource Manager templates for SAP are used to simplify deployment of required resources.

## Single SID 3-tier SAP NetWeaver HA AnyDB deployment

The three-tier SAP Azure Resource Manager templates referenced earlier in this module support high-availability scenarios. To prepare a highly available infrastructure, when deploying the templates, set the SYSTEMAVAILABILITY parameter to HA.
...
## Multi-SID 3-tier SAP NetWeaver HA AnyDB deployment
