...
The load balancer offers an option of DirectServerReturn. If that option is configured, the traffic directed from the DBMS VM to the SAP application layer it's routed through the load balancer. Instead, it goes directly to the application layer. When DirectServerReturn isn't configured, the return traffic to the SAP application layer is routed through the load balancer.
