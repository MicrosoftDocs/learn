...
In highly available configurations, the incoming traffic to the DBMS virtual machine is always routed through the load balancer. The outgoing traffic route from the DBMS virtual machine to the application layer virtual machine depends on the configuration of the load balancer.

The load balancer offers an option of DirectServerReturn. If that option is configured, the traffic directed from the DBMS virtual machine to the SAP application layer isn't routed through the load balancer. Instead, it goes directly to the application layer. When DirectServerReturn isn't configured, the return traffic to the SAP application layer is routed through the load balancer.
