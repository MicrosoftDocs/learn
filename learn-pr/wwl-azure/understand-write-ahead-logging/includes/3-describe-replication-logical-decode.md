...
Refer to [pglogical extension documentation](/azure/postgresql/flexible-server/concepts-logical#pglogical-extension) online documentation for more details.
...
1. If PostgreSQL restarts, a slot may re-emit changes, which the client needs to handle.
1. Slots must be monitored. Unconsumed slots will hold on to all the WAL files for those unconsumed changes. This can lead to storage full or transaction ID wraparound.
