...
There are three different ways for controlling access to IoT Hub:
 ...
 -  Microsoft Entra integration for service APIs. Azure provides identity-based authentication with Microsoft Entra ID and fine-grained authorization with Azure role-based access control (Azure RBAC). Microsoft Entra ID and RBAC integration is supported for IoT hub service APIs only.
