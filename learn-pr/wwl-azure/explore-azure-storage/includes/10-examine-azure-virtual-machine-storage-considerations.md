When using Azure Virtual Machine storage, consider the following:
...
The use of managed disks is recommended for all SAP workloads. Managed disks are required to implement Write Accelerator. As explained earlier, Write Accelerator is a disk capability of M-Series Azure Virtual Machines with Premium storage-based Azure managed disks. Its purpose is to improve the I/O latency of writes. Write Accelerator is ideally suited where log file updates are required to persist to disk in a highly performant manner for modern databases.
...
- Read
- Read/Write
- None + Write Accelerator, which is only for Azure M-Series virtual machines
- Read + Write Accelerator, which is only for Azure M-Series virtual machines
...
- **/hana/log** \- no caching (with exception for M-Series virtual machines)
- **/hana/shared** \- read caching

For M-Series deployments, Microsoft recommends that you use Azure Write Accelerator for your DBMS deployment. As a matter of fact, SAP HANA certification for Azure M-Series virtual machines requires that Azure Write Accelerator be enabled for the **/hana/log** volume. There are limits of Azure Premium Storage VHDs per virtual machine that can be supported by Azure Write Accelerator. The current limits are:
