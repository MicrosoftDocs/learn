...
2. You need write permissions on the management group ("Owner", "Contributor", or "Management Group Contributor"). To see what permissions you have, select the management group and then select **IAM**.
...
`Remove-AzManagementGroup -GroupId 'Contoso'`<br>
...
`az account management-group show --name 'Contoso'`<br>

To return a specific management group and all the levels of the hierarchy under it, use -Expand and -Recurse parameters.

`Azure CLI`

`az account management-group show --name 'Contoso' -e -r`<br>
