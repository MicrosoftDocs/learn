...
- Develop and test complex code in one place, then call it wherever it's needed.
- Execute code efficiently. Stored procedures that return the same result are more efficient because they're pre-compiled and stored in the Azure Database PostgreSQL server.
...
- **argmode** - the mode of the argument. May be IN, INOUT, or VARIADIC. The default is IN. OUT isn't supported; use INOUT instead. VARDIADIC is an undefined number of input arguments of the same type, and must be the last input arguments.
