...
When it comes to planning an application, you start by thinking about what you want to build. You then plan the design and build out your application from there. Essentially, you want to think about the kind of application you want to build and then identify the classes (and their corresponding objects) needed to create it. 
...
>[!NOTE] 
>As mentioned previously, classes are the design or blueprint for an object, detailing the members (variables, properties, and methods)  of an object. Objects are the “live” embodiment, so to speak, of that class in your computers Random Access Memory or RAM. 

To tell it another way, each object represents a class. The class is the building block of an object-oriented application. And, in order to use a class in an application, you must create an object based on the class, as it's the live, in-memory, representation of that class. 
The purpose of this lesson is to differentiate between a class and an object, and to create a class. 
