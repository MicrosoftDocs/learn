...
4. Validate that intra-node communication between VMs is not routed through an NVA. The same must be true for traffic between the VMs and the highly available NFS cluster.
5. Install the SAP HANA leading node according to SAP's documentation.
6. After the installation, add the parameter 'basepath\_shared = no' to the global.ini file. This parameter enables SAP HANA to run in scale-out without 'shared' **/hana/data** and **/hana/log** volumes between the nodes. Details are documented in [SAP Note \#2080991]https://aka.ms/az-120-sapnote-2080991).
...
Specify the internal network for SAP HANA inter-node communication during the installation or afterward using, for example, the local hdblcm. For details, refer to [SAP Note \#2183363](https://aka.ms/az-120-sapnote-2183363).
