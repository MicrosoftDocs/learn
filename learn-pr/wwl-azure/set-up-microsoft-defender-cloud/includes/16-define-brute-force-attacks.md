A brute force attack is a type of hacking technique in which an attacker tries to gain access to a network or system by guessing the username and password combination through an automated process. The attacker typically uses a program that generates a large number of sign in attempts in a short period of time to try every possible combination of characters until the correct one is discovered. This type of attack can be effective against weak passwords and security systems with no protection against brute force attacks, but it's time-consuming and is detected by security measures such as account lockouts after several failed sign in attempts.

## Management services, ports, and protocols

Adversaries without prior knowledge of legitimate credentials within the system or environment guess passwords to attempt access to accounts. Without knowledge of the password for an account, an adversary tries to systematically guess the password using a repetitive or iterative mechanism. An adversary guesses sign in credentials without prior knowledge of system or environment passwords during an operation by using a list of common passwords. Password guessing often takes into account the target's policies on password complexity or use policies that lock out accounts after many failed attempts.
...
| SSH brute force tools                | used to guess SSH sign in credentials.                |
| Remote desktop brute force tools     | used to guess RDP sign in credentials.                |
| FTP brute force tools                | used to guess FTP sign in credentials.                |
...
## Indications of an attack
 ...
 -  No successful authentication from multiple remote desktop protocol (RDP) connections or from new source IP addresses
