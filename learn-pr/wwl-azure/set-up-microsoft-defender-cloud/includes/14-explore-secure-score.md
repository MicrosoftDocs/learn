...
> [!NOTE]
> The secure score described in this unit is the classic secure score available in the Azure portal. A newer risk-based Cloud Secure Score is available in the Defender portal, which factors in asset risk and criticality for more accurate prioritization.
...
- Current score - The current score for this control.
    ...
    Current score = **Score per resource** X **Number of healthy resources**
...
- Potential score increase - The remaining points available to you are within your control. If you remediate all the recommendations in this control, your score increases by 9%.
    
    Potential score increase = **Score per resource** X **Number of unhealthy resources**
...
- Insights - Gives you extra details for each recommendation, such as:
     ...
     - :::image type="content" source="../media/enforce-icon-cd64da95.png" alt-text="Screenshot of enforce icon.":::
         **Enforce** - From within the recommendation details page, you can automatically deploy a policy to fix this issue whenever someone creates a noncompliant resource.
...
- Only built-in recommendations change the secure score.
- Recommendations flagged as Preview aren't included in the calculations of your secure score. They should still be remediated wherever possible so that when the preview period ends, they contribute towards your score.
...
You can also configure the Enforce and Deny options on the relevant recommendations. Customizing these options improves your score and make sure your users don't create resources that lower your score.
...
In some cases, you see a control max score greater than zero, but the value is zero. When the incremental score for fixing resources is negligible, it's rounded to zero. Don't ignore these recommendations because they still bring security improvements. The only exception is the "**Additional Best Practice**" control. Remediating these recommendations has no change on your score, but it enhances your overall security.
