This unit provides an overview of how encryption is used in Microsoft Azure. It covers the major areas of encryption, including encryption at rest, encryption in flight, and key management with Azure Key Vault. Each section includes links to more detailed information.
...
For a more detailed discussion of how data at rest is encrypted in Azure, see [Azure Data Encryption-at-Rest](/azure/security/fundamentals/encryption-atrest).
...
To learn more about client-side encryption with Key Vault and get started with how-to instructions, see [Tutorial: Encrypt and decrypt blobs in Azure Storage by using Key Vault](/azure/storage/blobs/storage-encrypt-decrypt-blobs-key-vault).
...
[TDE](/sql/relational-databases/security/encryption/transparent-data-encryption-tde) is used to encrypt [SQL Server](https://www.microsoft.com/sql-server), [Azure SQL Database](/azure/azure-sql/database/sql-database-paas-overview), and [Azure Synapse Analytics](/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is) data files in real time, using a Database Encryption Key (DEK), which is stored in the database boot record for availability during recovery.
...
Shared Access Signatures ([SAS](/azure/storage/common/storage-sas-overview)), which can be used to delegate access to Azure Storage objects, include an option to specify that only the HTTPS protocol can be used when you use Shared Access Signatures. This approach ensures that anybody who sends links with SAS tokens uses the proper protocol.

[SMB 3.0](/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn551363%28v=ws.11%29#BKMK_SMBEncryption), which used to access Azure Files shares, supports encryption, and it's available in Windows Server 2012 R2, Windows 8, Windows 8.1, and Windows 10. It allows cross-region access and even access on the desktop.
...
You can use an [Azure VPN gateway](/azure/vpn-gateway/vpn-gateway-about-vpn-gateway-settings) to send encrypted traffic between your virtual network and your on-premises location across a public connection, or to send traffic between virtual networks.
...
Point-to-site VPNs allow individual client computers access to an Azure virtual network. [The Secure Socket Tunneling Protocol (SSTP)](/previous-versions/technet-magazine/cc162322%28v=msdn.10%29) is used to create the VPN tunnel. It can traverse firewalls (the tunnel appears as an HTTPS connection). You can use your own internal public key infrastructure (PKI) root certificate authority (CA) for point-to-site connectivity.
...
[Configure a point-to-site connection to a virtual network by using certification authentication: Azure portal](/azure/vpn-gateway/point-to-site-certificate-gateway)

[Configure a point-to-site connection to a virtual network by using certificate authentication: PowerShell](/azure/vpn-gateway/vpn-gateway-howto-point-to-site-rm-ps)
...
[Create a site-to-site connection in the Azure portal](/azure/vpn-gateway/tutorial-site-to-site-portal)

[Create a site-to-site connection in PowerShell](/azure/vpn-gateway/vpn-gateway-create-site-to-site-rm-powershell)

[Create a virtual network with a site-to-site VPN connection by using CLI](/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-cli)
...
To learn more about encryption of data in transit in Data Lake, see [Encryption of data in Data Lake Store](/azure/data-lake-store/data-lake-store-encryption).
