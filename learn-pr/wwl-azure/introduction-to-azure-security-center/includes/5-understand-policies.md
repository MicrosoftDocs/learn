...
A **policy definition** specifies the resources to be evaluated and the actions to take on them. For example, you could prevent VMs from deploying if they're exposed to a public IP address. You could also contain a specific hard disk from being used when deploying VMs to control costs. Policies are defined in the JavaScript Object Notation (JSON) format.
...
The following list is example policy definitions:
 ...
 -  Allowed Locations. This policy restricts the locations that new resources can be deployed. It's used to enforce geographic compliance requirements.
...
Child resources will inherit any policy assignments that have been applied to their parents.
