...
**my.cnf** is a text file that contains the huge number of parameters, defaults, and options that MySQL uses. Azure Database for MySQL provides a user interface to these configuration settings in the **Server Parameters** blade. In the Azure portal, navigate to your Azure Database for MySQL. In the left menu, under **Settings**, select **Server Parameters**. There are around 22 pages of dynamic and static parameters you can view or modify.
