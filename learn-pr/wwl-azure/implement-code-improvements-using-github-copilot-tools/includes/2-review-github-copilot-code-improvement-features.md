...
The Fix smart action is a contextual action that can be used to fix coding errors without writing a prompt. To use it, right-click selected code in the editor and select **Generate Code** > **Fix**.

### Review smart action

The Review smart action provides a way to review and analyze your code. It can help you identify areas for improvement, suggest refactoring updates, and provide guidance on best practices. To use it, right-click selected code in the editor and select **Generate Code** > **Review**. Review comments appear in the **Comments** panel and inline in the editor. Where possible, Copilot's feedback includes suggested code changes that you can apply or discard with a single click.

### Additional smart actions

Several other smart actions support the code improvement workflow:

- **Explain**: Right-click and select **Explain** to understand unfamiliar code before refactoring.
- **Generate Tests**: Right-click and select **Generate Code** > **Generate Tests** to create tests that validate your refactored code.
- **Generate Docs**: Right-click and select **Generate Code** > **Generate Docs** to add documentation to improved code.

> [!NOTE]
> The Visual Studio Code context menu for GitHub Copilot is undergoing updates. Some smart actions may not appear in the context menu locations described in this unit. If a smart action isn't available in the context menu, you can perform the same action by entering a prompt directly in the Chat view. For example, select your code and ask GitHub Copilot to "fix this code" or "review this code." Additionally, the **Review** option may appear at the top level of the context menu rather than under **Generate Code**.
