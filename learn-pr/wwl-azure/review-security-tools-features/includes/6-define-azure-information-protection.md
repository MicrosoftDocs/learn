![MSIP icon.](../media/icon-azure-information-protection.png)
...
The following screen capture is an example of information protection in action on a user's computer. In this example, the administrator has configured a label with rules that detect sensitive data, like a credit card. When a user saves the document containing a credit card number, a custom tooltip is displayed. The tooltip recommends labeling the file as *Confidential/ All Employees*, which is a label that the administrator has configured. This label classifies the document and protects it.

:::image type="content" source="../media/azure-information-protection-call-out.png" alt-text="Screenshot of a Microsoft Word window with the previously described message displaying.":::
