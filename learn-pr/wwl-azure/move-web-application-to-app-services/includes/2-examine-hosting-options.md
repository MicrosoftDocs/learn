When it comes to hosting your web app on Azure, you have a few ways to do it. Each has its advantages and deciding which way you'll want to go depends on a number of factors like cost, ease of maintenance, and performance among others. Let's review some of these and examine the advantages of each.

![chart of the traditional tenants of hosting](../media/azure-service-types.png)
...
With Azure App Service, you can develop in a variety of languages, run your app on Windows or Linux, start with templated apps (e.g. WordPress and Drupal), and integrate with Azure DevOps services like continuous deployment. In addition, updates to the underlying architecture are handled for you, the service ISO, SOC, and PCI compliant, and the cost model is elastic: you only pay for the compute resources you use. For a full introduction to this service, see the [Microsoft Docs article on Azure App Service](/azure/app-service/overview).

![chart showing Azure App Service benefits](../media/azure-app-service-benefits.png)

## Static Architecture Options

As the name implies, this type of web app deployments are useful if you don't need server-side rendering. These essentially are fully-rendered web files that are delivered to the end user. While they can take advantage of some PaaS (Platform as a Service) options like Azure Functions, they are really designed for specific scenarios where you don't need a web server to perform operations.
...
Similar to Static Web Apps, you can also serve static web files (HTML, CSS, JavaScript, images, documents) from Azure Storage (e.g. blog storage). There are [some additional limitations](/azure/storage/blobs/storage-blob-static-website) relative to Static Web Apps but this approach can be a quick and relatively inexpensive way to deliver your web app to your users.

## Virtual Machine

You may already be familiar with this technology as it's been around for many years. A virtual machine (VM) approach to hosting is essentially a computer in the cloud where you have a lot of control over the type of operating system you can deploy (Windows or Linux), the settings on that computer, the software you install, and how all if it is configured. This type of hosting gives you the most control, but it also can be the most expensive depending on how much compute power you need, networking needs etc.
