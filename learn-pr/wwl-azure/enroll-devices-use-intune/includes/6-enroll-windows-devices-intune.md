...
This enrollment method will only enroll the device in Intune and not Azure AD join the device. You'll only use this form of enrollment in environments that don't have Azure AD Premium licenses that are required to enable auto-enrollment of devices into Intune.
...
This method of enrollment is similar to method 3, except it's performed by IT admins using a special type of account - A Device Enrollment Manager (DEM) account. A DEM account is useful for scenarios where devices are enrolled and prepared before handing them out to the users of the devices. The DEM would enroll the device, log on to the company portal and install the apps required by the user. This account can be used to enroll up to 1000 devices into Intune. The IT administrator who performs the enrollment must have access to local administrator credentials to complete the enrollment from the Settings menu. For more information about DEM, refer to the topic **Enrollment Rules** later in this lesson.
