...
The `StopService` method in the previous example doesn't require any parameters to be passed to it, so there's no value within the parentheses. If you invoke a method that requires parameters, place their values within the parentheses. The following example sets the start mode parameter to `Manual`:
...
The `Invoke-WmiMethod` cmdlet is another way to invoke a method for a WMI object with different syntax. The `-Name` parameter specifies the method to invoke, and the `-ArgumentList` parameter specifies the parameter values passed to the method. If required, pass multiple parameters as a comma-separated list or array. The parameter values need to be in a specific order, just as they were for direct invocation.
