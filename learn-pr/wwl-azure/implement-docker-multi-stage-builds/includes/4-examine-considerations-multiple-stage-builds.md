...
To help minimize image sizes, it is also essential to avoid including packages that you suspect might be needed but are not yet sure if they are required.

Only include them when they are required.

## Choose an appropriate base

While optimizing the contents of your Dockerfiles is essential, it is also crucial to choose the appropriate parent (base) image. Start with an image that only contains packages that are required.

## Avoid including application data

While application data can be stored in the container, doing it will make your images more prominent.
