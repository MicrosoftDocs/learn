...
3. Determine what processes can be used to supply authoritative information for visitors. It might be necessary to find an alternate way to determine when a digital identity for a visitor is no longer needed.
...
- **Leave** - when an individual leaves the scope of needing access, access might need to be removed, and the identity might no longer be required by applications other than for audit or forensics purposes.
