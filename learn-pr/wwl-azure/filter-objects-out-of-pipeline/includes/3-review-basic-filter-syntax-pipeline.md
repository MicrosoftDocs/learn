...
You start this basic syntax with the `Where` alias (you can also specify the full command name, `Where-Object`), followed by the property that you want PowerShell to compare. Then you specify a comparison operator, followed by the value that you want PowerShell to compare. PowerShell keeps every object that has the specified value in the specified property.
