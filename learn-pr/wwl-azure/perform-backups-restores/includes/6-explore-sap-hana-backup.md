SAP HANA on Azure Virtual Machines can be backed up by using the following two methods:

- HANA backup to the file system in an Azure Linux virtual machine
- HANA backup based on storage snapshots using either the Azure storage blob snapshot feature manually or Azure Virtual Machine Level Backup
