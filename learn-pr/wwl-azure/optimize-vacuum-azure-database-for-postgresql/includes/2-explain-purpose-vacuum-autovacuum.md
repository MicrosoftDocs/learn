When a row is updated, each update is made up of an insert and a delete. The modified data is inserted, and the old row is deleted. In PostgreSQL, the original row is retained so that a transaction can roll back in the event of a problem. Rather than deleting the original row, it's marked for deletion later, resulting in "dead" rows (or tuples) that are still on disk but no longer required.
