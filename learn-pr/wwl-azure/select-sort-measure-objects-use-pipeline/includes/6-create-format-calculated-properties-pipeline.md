`Select-Object` can create custom, or *calculated*, properties. Each of these properties has a label, or name, that PowerShell displays in the same way it displays any built-in property name. Each calculated property also has an expression that defines the contents of the property. You create each calculated property by entering the values in a hash table.

## What are hash tables?

Some programming and scripting languages refer to a *hash table* as an *associative array* or *dictionary*. One hash table can contain multiple items, and each item consists of a key and a value, or a *name-value pair*. PowerShell uses hash tables many times and for many purposes. When you create your own hash table, you can specify your own keys.
...
This structure means that each hash table contains only one name-value pair. However, you can use more than one hash table to create multiple calculated properties.
...
The previous example uses a semicolon to separate the two key-value pairs, and it uses the keys `n` and `e`. PowerShell expects these keys. The label (or name) is a string, and therefore enclosed in single quotation marks. PowerShell accepts either double quotation marks (`" "`) or single quotation marks (`' '`) for this purpose. The expression is a small piece of executable code called a *script block* and is contained within braces (`{ }`).

> [!NOTE]
> `$PSItem` is a special variable created by PowerShell. It represents whatever object was piped into the `Select-Object` command. In the previous example, that's a `Process` object. The period after `$PSItem` lets you access a single member of the object. In this example, one calculated property uses the `VM` property, and the other uses the `PM` property.
...
A comma separates each property, such as `Name`, `ID`, and each calculated property from the others in the list.
...
This example uses the PowerShell `-f` formatting operator. When used with a string, the `-f` formatting operator instructs PowerShell to replace one or more placeholders in the string with the specified values that follow the operator.

In this example, the string that precedes the `-f` operator instructs PowerShell what data to display. The string '{0:N2}' signifies displaying the first data item as a number with two decimal places. The original mathematical expression comes after the operator. It's in parentheses to make sure that it runs as a single unit. You can enter this command exactly as displayed, press the Enter key on a blank line, and then review the results.
...
> [!NOTE]
> You can learn more about the `-f` operator by running `Help About_Operators` in PowerShell.
