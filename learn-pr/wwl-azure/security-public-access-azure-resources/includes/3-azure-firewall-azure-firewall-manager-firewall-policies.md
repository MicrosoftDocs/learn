...
Azure Firewall Standard provides layer 3 to layer 7 (L3-L7) filtering and threat intelligence feeds directly from Microsoft Cyber Security. Threat intelligence-based filtering can alert and deny traffic from/to known malicious IP addresses and domains. The malicious sites are updated in real time to protect against new and emerging attacks.
...
Azure Firewall Premium provides advanced capabilities that include signature-based intrusion detection and prevention system (IDPS) to allow rapid detection of attacks by looking for specific patterns. These patterns can include byte sequences in network traffic or known malicious instruction sequences used by malware. There are more than 67,000 signatures in over 50 categories that are updated in real time to protect against new and emerging exploits. The exploited categories include malware, phishing, coin mining, and Trojan attacks.
...
Azure Firewall Basic is intended for small and medium size business (SMB) customers to secure their Azure cloud environments. It provides the essential protection SMB customers need at an affordable price point.
...
### Integrated with external security-as-a-service for advanced security

In addition to Azure Firewall, you can integrate external security as a service (SECaaS) providers to provide other network protection for your virtual network and branch Internet connections.

This feature is available only with secured virtual hub deployments.

- Virtual network to Internet (V2I) traffic filtering
  -  Filter outbound virtual network traffic with your preferred external security provider.
  -  Apply advanced user-aware Internet protection for your cloud workloads running on Azure.
- Branch to Internet (B2I) traffic filtering
- Use your Azure connectivity and global distribution to easily add external filtering for branch to Internet scenarios.
...
You can use external providers for Branch to Internet (B2I) traffic filtering, side by side with Azure Firewall for Branch to virtual network (B2V), virtual network to virtual network (V2V) and virtual network to Internet (V2I).
