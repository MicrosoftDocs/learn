...
Within the **Show** **Command Window**, each parameter set for the specified command displays on a separate tab. This tab layout makes it visually clear that you can't mix and match parameters between sets.
...
The **Get-Help** command is useful for finding commands. It accepts wildcard characters (\*, \?), notably the asterisk (\*) wildcard character. When you ask for help and use wildcard characters with a partial command name, PowerShell displays a list of matching help topics.
