...
5. Create striped volumes using the attached disk. Use either logical volume management (LVM) or the multiple-devices administration (mdadm) tool at the OS level inside the VM.
...
15. Share the **/sapmnt** directory using NFS. The application server VM is the NFS server.
