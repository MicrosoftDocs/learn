...
It's helpful to add the nofail parameter to the **fstab** file. This way, if something goes wrong with the disks, the VM doesn't stop responding in the boot process. But remember that extra disk space might not be available, and processes might fill up the root file system. If **/hana** is missing, SAP HANA won't start.
