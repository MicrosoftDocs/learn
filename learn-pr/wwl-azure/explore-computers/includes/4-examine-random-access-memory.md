...
![Illustration of a person having a conversation with another person with lists hovering above their heads](../media/conversing.svg)

Random access memory works similarly. The CPU can think about only certain data at a time, so it keeps the rest in memory until it needs it. It's referred to as "random" because the CPU can think about anything stored in memory any time it wants.
