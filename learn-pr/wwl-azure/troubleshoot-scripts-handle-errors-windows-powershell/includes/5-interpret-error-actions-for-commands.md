PowerShell errors are either *terminating* or *nonterminating*. A terminating error stops the command immediately—PowerShell can't recover and continue. With a nonterminating error, the script or pipeline keeps running. Nonterminating errors are far more common.
...
If **LON-SVR1** isn't available on the network, **Get-CimInstance** generates an error. However, the command continues with the next computer, **LON-DC1**. That behavior makes the error nonterminating.

## $ErrorActionPreference

`$ErrorActionPreference` is a built-in global variable. When a nonterminating error occurs, PowerShell checks this variable to determine how to proceed. The variable accepts six values:
...
- **Ignore** suppresses the error message and doesn't record the error in `$Error`. Use this value only when you're certain the error is expected and irrelevant.
...
> [!NOTE]
> Be selective about using **SilentlyContinue** for `$ErrorActionPreference`. This setting might seem to make your script friendlier, but it makes troubleshooting much harder.
...
All PowerShell commands have the `-ErrorAction` parameter (alias: `-EA`). It accepts the same values as `$ErrorActionPreference` and overrides it for that command. If you expect an error, set `-ErrorAction Stop` on that command. This parameter lets you trap and manage errors there while all other commands continue to use `$ErrorActionPreference`. For example:
