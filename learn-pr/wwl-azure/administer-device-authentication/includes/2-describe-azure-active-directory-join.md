...
The scope of devices that are capable of joining Azure AD is larger than for AD DS. Typically, only devices running Windows Pro or Enterprise Edition can join an AD DS domain. However, devices running operating systems such as Windows Home (including Pro &amp; Enterprise), iOS or Android can join Azure AD. When joining a device to Azure AD, it is fully capable of accessing cloud-based resources and Azure-based resources by using SSO. From a management perspective, you cannot manage these devices by using Group Policy, but you can use Intune to manage and provision them.
...
You may want to join devices to Azure AD in these scenarios:
 ...
 -  **If you want to enable users to join their own devices to the organizational environment**. If you support the Bring Your Own Device (BYOD) concept and you want to enable users to join their devices to your business environment, Azure AD might be the right solution. This is particularly the case in situations where users are utilizing non-Microsoft devices such as iPads or Android tablets that cannot join an AD DS domain, but they can enroll in Intune and Azure AD.
