...
The following reference architecture exhibits a set of proven practices for running SAP NetWeaver in a Windows environment on Azure with high availability. The database is AnyDB, the SAP term for any supported DBMS besides SAP HANA. This architecture is deployed with specific virtual machine (VM) sizes that can be changed to accommodate your organization's needs.

## High-level architecture 

![Reference architecture for SAP NetWeaver (Windows) for AnyDB on Azure VMs](../media/sap-netweaver-anydb.png)
...
* **Virtual machines**. Azure VMs host the application tier and database tier components, grouped as follows:
...
* **Load balancers**. Azure Load Balancer instances are used to distribute traffic to Azure VMs in the application and database tiers. 

* **Availability sets**. In highly available configurations, Azure VMs serving the same role can be grouped into distinct availability sets, with at least two VMs per availability set. Alternatively, in multi-zone deployments, individual VMs serving the same role can be deployed into distinct availability zones.
...
* **Azure Storage**. Azure Storage provides a range of storage services. It is also used by Cloud Witness to implement quorum for Windows Server Failover Clustering.
...
Azure does not support natively shared disks, but you can use third party solutions (such as SIOS DataKeeper Cluster Edition, which replicates synchronously independent disks owned by individual cluster nodes) to implement this functionality on Azure VMs running Linux or Windows Server. 
...
* **Database servers**. The database tier is another potential single point of failure (SPOF) when deployed to a single VM — a typical deployment when high availability is not a requirement. High availability and disaster recovery for database servers may be achieved using built-in SAP load balancers, Azure Load Balancer, or other mechanisms, depending on the DBMS.
