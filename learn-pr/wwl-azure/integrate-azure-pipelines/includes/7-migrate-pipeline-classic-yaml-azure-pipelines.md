...
There are, however, two features which are not converted:

 -  Variables: Variables in a classic pipeline are excluded from the conversion process. Instead, they are mentioned by name in the comments of the autogenerated YAML pipeline as a reminder that you need to configure them manually.
...
There is no equivalent option to view the YAML at the job or stage level. Effectively, this part of the conversion relies on your familiarity with the syntax of YAML pipelines.
