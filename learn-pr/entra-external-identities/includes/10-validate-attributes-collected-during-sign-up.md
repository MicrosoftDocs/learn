...
1. Start by registering a custom authentication extension that is used by Microsoft Entra External ID to call your REST API. The custom authentication extension contains information about your REST API endpoint, the attribute collection start action that it parses from your REST API, and how to authenticate to your REST API. Sign in to the [Microsoft Entra admin center](https://entra.microsoft.com/) and browse to **External Identities** > **Custom authentication extensions**. Note, you can also browse to **Applications** > **Enterprise applications** > **Custom authentication extensions**. Then, select **Create a custom extension**.
    ...
    [ ![Screenshot of External Identities Custom authentication extensions blade highlighting a button in the toolbar titled Create a custom extension.](../media/validate-attributes-collected-during-sign-up/1_inline.png) ](../media/validate-attributes-collected-during-sign-up/1.png#lightbox)

1. In **Basics**, select the **AttributeCollectionSubmit** event, and then select **Next**.

    [ ![Screenshot of Basics tab of Create new custom extension blade highlighting event type titled Attribute Collection Submit.](../media/validate-attributes-collected-during-sign-up/2_inline.png) ](../media/validate-attributes-collected-during-sign-up/2.png#lightbox)
...
1. In **Endpoint Configuration**, fill in the following properties:
    ...
    To continue, select **Next**.
        ...
        [ ![Screenshot of Endpoint Configuration tab of Create new custom extension blade showing fields Name, Target URL, Timeout in milliseconds, Maximum Retries, and Description.](../media/validate-attributes-collected-during-sign-up/3_inline.png) ](../media/validate-attributes-collected-during-sign-up/3.png#lightbox)
...
1. Select the application you configured for the other custom authentication extensions, and choose **Select**.
    ...
    [ ![Screenshot of Select application pane open on the right that highlights an application titled Custom authentication extension security app checked.](../media/validate-attributes-collected-during-sign-up/5_inline.png) ](../media/validate-attributes-collected-during-sign-up/5.png#lightbox)
...
1. Review your configuration and select **Create**, which registers the custom authentication extension you configured and the associated application registration you selected.
    ...
    [ ![Screenshot of Review tab of Create new custom extension blade showing Endpoint Configuration and API Authentication configurations.](../media/validate-attributes-collected-during-sign-up/6_inline.png) ](../media/validate-attributes-collected-during-sign-up/6.png#lightbox)
...
1. At this point the custom authentication extension is configured in your tenant, but not used in any of the sign-up pages. Next, you'll associate the custom authentication extension with one or more of your user flows. To do so, browse to **Identity** > **External Identities** > **User flows**. Then, select the user flow from the list.
    ...
    [ ![Screenshot of External Identities User flows blade highlighting a user flow titled Default from the grid list of user flows.](../media/validate-attributes-collected-during-sign-up/7_inline.png) ](../media/validate-attributes-collected-during-sign-up/7.png#lightbox)
...
1. From the menu, select **Custom authentication extensions**. To associate your custom authentication extension, select the **Before collecting information from the user** event.
    ...
    [ ![Screenshot of Custom authentication extensions blade for user flow titled Default that highlights edit button next to When a user submits their information.](../media/validate-attributes-collected-during-sign-up/8_inline.png) ](../media/validate-attributes-collected-during-sign-up/8.png#lightbox)
...
1. The custom extensions configured for the **OnAttributeCollectionStart** event will be displayed. Select one of them for the attribute collection start event, and then choose **Select** to save the changes.
    ...
    [ ![Screenshot of Custom extensions pane open on the right with Validate sign-up user input and its URL highlighted in the dropdown menu for Select a custom extension.](../media/validate-attributes-collected-during-sign-up/9_inline.png) ](../media/validate-attributes-collected-during-sign-up/9.png#lightbox)
