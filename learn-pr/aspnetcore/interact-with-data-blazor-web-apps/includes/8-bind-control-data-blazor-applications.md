...
> [!NOTE]
> At the time of writing, format strings are only supported with date values. Currency formats, number formats, and other formats might be added in the future. To check the latest information on binding formats, see [Format strings](/aspnet/core/blazor/components/data-binding?view=aspnetcore-9.0#format-strings) in the Blazor documentation.
