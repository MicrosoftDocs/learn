...
1. In _Pizza.jsx_, replace the code with the following code:
    ...
    The data is now pulled from the API instead of a front-end app's in-memory array. Notice that the URL used is `/pizza` without the reference to the back-end server. This is because you'll use a proxy to make requests toward the mocked API.
...
1. Navigate up one directory from the _PizzaClient_ directory, and then download the back-end project:
...
1. In the file explorer, browse to the _PizzaStore_ directory, and open _Program.cs_. Add the code to enable CORS (the code you need to add is highlighted):
