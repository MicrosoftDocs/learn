We discussed screen readers briefly in the **Web accessibility fundamentals** unit. A screen reader does what the name implies: it reads the screen to you. They do more than that plus, plus provide audible cues, context, and control to navigate and interact with web applications and complex content.
...
When you're designing visually, it can be tempting to just use `<div>` everywhere. A `<div>` can be styled to look like anything using CSS, and Bootstrap certainly doesn't discourage using `<div>`'s liberally.
...
- `<nav>` - Represents a portion of the page with navigational links.
...
- `<section>` - Represents a block of content that doesn't fit in one of the other groupings above.

All of these items can generally be styled the same as a `<div>`, so you can just replace the tag. The default ASP.NET Core layout page (found in `/Pages/Shared/_Layout.cshtml`) uses many of these, including `header`, `nav`, `main`, and `footer`. 
