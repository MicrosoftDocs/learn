Most HTML elements expose events that are triggered when something significant happens, such as a page has finished loading, the user has clicked a button, or the contents of an HTML element have changed. An app can handle an event in several ways:
...
> [!NOTE]
> This example uses the JavaScript `alert` function to display the message because there is no equivalent function in Blazor. You use JavaScript interop to call JavaScript from Blazor code. The details of this technique are the subject of a separate module.

## Override default DOM actions for events

Several DOM events have default actions that run when the event occurs, regardless of whether there is an event handler available for that event. For example, the `@onkeypress` event for an \<input\> element always displays the character that corresponds to the key that the user has pressed, and handling the key press. In the next example, the `@onkeypress` event is used to convert the user's input to uppercase. Additionally, if the user types an `@` character, the event handler displays an alert:
...
Some events in a child element in the DOM can trigger events in their parent elements. In the example below, the \<div\> element contains an `@onclick` event handler. The \<button\> inside the \<div\> has its own `@onclick` event handler. Additionally, the \<div\> contains an \<input\> element:
