...
- Deploy the microservices application to an AKS that's using the [Azure Application Gateway](/azure/application-gateway/overview).

- Expose BFF API outside the Kubernetes cluster outside using [Application Gateway Ingress Controller](/azure/application-gateway/ingress-controller-overview) and access that external Swagger Client.
