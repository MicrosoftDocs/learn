...
In this unit, learn how to create a new .NET Aspire app and understand the differences between the two starter project templates. Then explore the structure of the solution generated.

## .NET Aspire prerequisites

Before you can create a new .NET Aspire app, there are some prerequisites you need to install locally:
...
## Creating a new .NET Aspire app by using a .NET Aspire template

You can use the Visual Studio launch dialog to create a new .NET Aspire applications, or **File** > **New** > **Project**. You can also use .NET CLI commands. To create a solution with the *.NET Aspire Application* template, you would use this command:
...
A benefit of using Visual Studio is you choose your configuration options through dialogs. The .NET CLI commands are useful when you want to create a new .NET Aspire app with default settings, but you can alter the defaults with optional flags. In the next exercise, you'll see how to create a new .NET Aspire app using both methods.
...
Both templates add **AppHost** and **ServiceDefaults** projects. These projects are the core of the .NET Aspire application. The **AppHost** project is the entry point and is responsible for acting as the orchestrator.
