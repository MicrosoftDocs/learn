...
Monitoring refers to gathering platform and application metrics that are meant to track the application health. Metrics are numerical values that can be used to observe the system in near real-time or analyze performance trends over time. The metrics are collected from various architecture levels, from the physical infrastructure to the application, including node-level metrics, container metrics, application metrics, and dependant service metrics.

## Implement logging in ASP.NET Core microservice

Logging in a microservices app is challenging due to the sheer volume of logs generated by many services, network calls, and dependant services working together to complete a business transaction. Other challenges in a microservices architecture include:
...
2. Every name in curly braces for e.g. `{TransactionId}` defines a **property** that gets it's value from the parameter `transaction.TransactionId`.
