Logging refers to application-level diagnostic traces that developers include in the app code. The diagnostic information helps troubleshoot issues when the app doesn't behave as expected. Logs have text-based records of events collected while the app is running and includes app traces and web server traces.

Monitoring refers to gathering platform and application metrics that are meant to track the application health. Metrics are numerical values that can be used to observe the system in near real-time or analyze performance trends over time. The metrics are collected from various architecture levels, from the physical infrastructure to the app, including node-level metrics, container metrics, application metrics, and dependant service metrics.

## Implement logging in ASP.NET Core microservice

Logging in a microservices app is challenging because of the sheer volume of logs generated by many services, network calls, and dependant services working together to complete a business transaction. Other challenges in a microservices architecture include:
...
In a microservices architecture, we need a solution that can aggregate log traces from several microservices, correlate them, and carry additional contextual information like the hosted infrastructure to efficiently debug the app.

In this unit, you'll review how the *eShopOnContainers* app implements [Serilog](https://serilog.net/) for structured logging in .NET and [Seq](https://datalust.co/seq) to centralize all log traces. In the subsequent unit, you'll learn how the challenges mentioned above are addressed using code instrumentation with Application Insights and near real-time monitoring of the telemetry data using Azure Monitor.

### Structured logging

Structured logging is an approach where the app writes logs in a structured format, such as JSON, rather than outputting unstructured text strings. Unstructured text strings make it harder to query logs for useful information. Especially in a complex application using microservices architecture, you'll need a quicker way to filter the traces by a particular property, say `TransactionId`. .NET supports a logging API that works with various built-in and third-party logging providers. To implement structured logging in *eShopOnContainers*, [Serilog](https://github.com/serilog/serilog) - an open-source third-party logging provider is used.

Structured formats like JSON can easily be parsed by any application to search and filter the relevant data based on their correlation. For example, take a look at the logging statement in .NET that uses the Serilog library:
...
The code above is similar to what you've seen in the `string.Format` method, with three significant differences:
...
2. Every name is in curly braces. For example, `{TransactionId}` defines a property that gets its value from the parameter `transaction.TransactionId`.

3. The `@` operator in front of `{@Command}` tells Serilog to serialize the object passed in, rather than convert it using `ToString()`.
...
The same trace can be visualized in a centralized logging system like Seq and further filtered to narrow down the issue while doing root cause analysis:
...
Logging context allows you to define a scope, so you can trace and correlate a set of events, even across the boundaries of the applications involved. Correlation Ids establishes a link between two or more contexts or applications; for example, every incoming request is assigned a unique Id (like a GUID or similar). Serilog allows logging additional context values across all logging statements using [Enrichment](https://github.com/serilog/serilog/wiki/Enrichment).  

These are some of the custom context properties used in eShopOnContainers:

* `ApplicationContext` is defined on app startup and adds the `ApplicationContext` property to all events.
* `SourceContext` identifies the class's full name where the event is logged, and it is usually defined when creating or injecting the logger.
...
* `TransactionContext` covers the events from the beginning of the database transaction until it is committed.
...
Serilog provides [sinks](https://github.com/serilog/serilog/wiki/Provided-Sinks) for writing log events to storage in various formats. In eShopOnContainers, by default Seq is configured as the centralized log monitoring system because Seq includes a free single-user license to use in production and can be run locally.
...
* `.ReadFrom.Configuration(configuration)` allows you to override the configuration from values in `appsettings.json`, or environment variables, which becomes useful for containers.

The JSON below shows the default configuration for `appsettings.json` in eShopOnContainers microservice:
...
Later in this module, you will learn to configure the Serilog sink to **Application Insights** and monitor the telemetry data using **Azure Monitor**.
...
[Azure Monitor for containers](/azure/azure-monitor/insights/container-insights-overview) is a feature within Azure Monitor designed to monitor the performance of container workloads deployed to Azure Kubernetes Service(AKS). It gives you performance visibility by collecting memory and processor metrics from controllers, nodes, and containers available in Kubernetes through the Metrics API. Container logs are also collected. Metrics are written to the metrics store, and log data is written to the logs store associated with your Log Analytics workspace.

In this module, you will:

* Configure eShopOnContainers application to use **Application Insights** for logging and monitor live metrics using Azure Monitor
* Enable **Azure Monitor for Containers** on the Azure Kubernetes cluster, monitor health of nodes, controllers, and containers and implement a **Prometheus** metric

### What is Application Insights

Application Insights is an extensible Application Performance Management (APM) service. To use it, you install an instrumentation package in your application. This package monitors the app and sends telemetry data to the Application Insights service. The data is then sent to Azure Monitor. Application Insights also provides built-in correlation and dependency tracking.

### What is Prometheus

Generally, in a large microservices application, the telemetry data rate is high enough to trigger throttling. In such cases, you should consider exporting metrics to a **time-series database such as Prometheus**. [Prometheus](https://prometheus.io/) is a popular open-source metric monitoring solution and is a part of the Cloud Native Compute Foundation. Azure Monitor for containers provides a seamless onboarding experience to collect Prometheus metrics. To use Prometheus, you typically need to set up and manage a Prometheus server with a store. By integrating with Azure Monitor, a Prometheus server isn't required. You just need to expose the Prometheus metrics endpoint through your exporters or pods (app), and the containerized agent for Azure Monitor for containers can scrape the metrics for you.
