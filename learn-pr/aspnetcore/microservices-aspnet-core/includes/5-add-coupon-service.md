...
The preceding script compiles and builds the containers in Azure Container Registry using the `az acr build` command with the provided *Dockerfile* files. ACR isn't required to use AKS. Other public container repositories, such as Docker Hub, are supported. **%TODO%** - Nish, is there more to say here? 
