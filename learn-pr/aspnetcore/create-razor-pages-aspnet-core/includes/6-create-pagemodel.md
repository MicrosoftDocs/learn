...
If the `ModelState` is valid, the `OnPostAsync` handler will proceed to call upon an instance of `ProductService`. `ProductService` is responsible for HTTP requests and responses for the web API.
