...
If we were running the app on our own machine, we would be able to direct a browser to 'https://localhost:5001' to view the resulting page that was created. To make this accessible from outside of our own machine, we will deploy the app to a public endpoint hosted on Azure App Service.
...
The preceding Azure CLI command provisions a public endpoint for this project. The first time this is provisioned it will take longer, please be patient as it may take several minutes.
...
Now that your *:::no-loc text="ContosoPets.UI":::* application has been hosted on a public endpoint, let's tour the unfinshed application as it is.
