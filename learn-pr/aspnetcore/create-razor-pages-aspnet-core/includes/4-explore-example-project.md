...
The *ContosoPets.UI* project directory should be currently open in the [Azure Cloud Shell editor](https://docs.microsoft.com/azure/cloud-shell/using-cloud-shell-editor). If it is not, then repeat the set up steps in the previous unit.
...
Models in ASP.NET Core often make use of DataAnnotations to help define model properties. DataAnnotations are simple attributes used to specify behavior that you want to enforce on the model properties they're applied to. For example a range of minimum and maximum acceptable values. The *:::no-loc text="ContosoPets.UI":::* project has a *:::no-loc text="Model":::* class file named *:::no-loc text="Product.cs":::* that uses DataAnnotations to define contraints for its data properties. For example, a `Product.Name` property is always required, and a `Product.Price` property must have a value between the range of '.01' and '9999.99'. Since this module often refers to the `Product` Model and its data properties, it is important to understand where that model class is stored, and that it contains DataAnnotations that define its data property constraints.
...
The *:::no-loc text="ContosoPets.UI":::* project's purpose is to provide a very basic UI client to execute Create, Read, Update and Delete (CRUD) operations through your company's web API endpoint to manage product listings.
