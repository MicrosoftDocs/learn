...
Multiple strategies exist for generating the QR code. One example includes using a third-party JavaScript library. In this module, however, a third-party NuGet package is used to generate the QR code with C#. The resulting QR code image is injected into an HTML placeholder element as a base-64 encoded string.
