...
- **MainPage.xaml**. This file contains the user interface definition. The sample app generated by the .NET MAUI Blazor App template comprises of a `BlazorWebView` that loads the `Main` component in the specified host HTML page (`wwwroot/index.html`) at location specified by the CSS selector (`#app`).
