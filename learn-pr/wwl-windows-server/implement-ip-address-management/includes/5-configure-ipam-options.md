Contoso IT administrators can configure IPAM to suit their environment and provide the level of manageability that they require. They have the option to configure all the managed servers manually by creating and configuring the necessary firewall rules, security groups, scheduled tasks, and file shares. However, to simplify management, they should consider using GPOs to perform provisioning.

## GPO provisioning

When you select Group Policy provisioning, you're prompted to provide a prefix for the GPOs so that they can be easily identified. The names of the GPOs display after you provide the prefix. You need to create these GPOs after completing the provisioning wizard.  
