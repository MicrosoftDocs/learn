...
| The Group Policy template| This template is a collection of files stored in the SYSVOL of each domain controller in the `%SystemRoot%\SYSVOL\Domain\Policies\GPOGUID` path, where GPOGUID is the globally unique identifier (GUID) of the Group Policy container. The Group Policy template contains the Group Policy settings. |
