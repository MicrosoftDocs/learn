...
> [!NOTE]
> Admittedly, we'd prefer to not see so many values after the decimal point. Ideally, the value would be formatted to a single value after the decimal point: `34.4`.  In the module [Format alphanumeric data for presentation in C#](/training/modules/csharp-format-strings/), we'll learn how to format numbers for proper display to the user.
