...
This exercise has data stored in older files that is fixed length, and character positions are allocated for certain fields of information. The first five digits represent a customer identification number. The next 20 digits contain a customer's name. The next six positions represent the customer's latest invoice amount, and the last three positions represent the number of items ordered on that invoice.

In the following steps you will need to remove the customer's name to format the data so that it can be sent to a separate process. Since you know the exact position and length of the user's name, you can easily remove it using the `Remove()` method.
