...
( ) `decimal myInputDecimal = (decimal)(myInput);` {{Incorrect. A `string` can't be cast into a `decimal`.}}
( ) `decimal myInputDecimal = myInput + 0;` {{Incorrect. A `string` can't be implicitly converted into a `decimal`.}}
(x) `decimal.TryParse(myInput, out myInputDecimal)` {{Correct. Using `TryParse` (or `Parse()`) is a valid technique.}}

## Multiple Choice

Which best describes the return type of `decimal.TryParse()`?
( ) `decimal` {{Incorrect. `TryParse()` returns a `bool` }}
(x) `bool` {{Correct. `TryParse()` returns a `bool` }}
( ) `out` {{Incorrect. `TryParse()` returns a `bool`. `out` is a parameter used in `TryParse()` for signifying the output variable to use.}}
