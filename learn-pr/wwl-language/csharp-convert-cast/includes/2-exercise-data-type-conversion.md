...
(x) cast {{Correct. `decimal` to `int` is a narrowing conversion so `cast` is the best answer.}}
( ) narrowing {{That’s incorrect. "Narrowing" describes the data precision change in a conversion.}}
( ) implicit conversion {{That’s incorrect. `decimal` to `int` is a narrowing conversion and implicit conversion can't be used because data loss is possible.}}
...
(x) `int cost = Convert.ToInt32(3.75m);` {{That’s correct. `Convert.ToInt32()` rounds values with fractional precision}}
