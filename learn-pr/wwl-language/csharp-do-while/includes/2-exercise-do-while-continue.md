...
1. Type the following code into the Visual Studio Code Editor.
   ```c#   Random random = new Random();   int current = 0;      do   {       current = random.Next(1, 11);       Console.WriteLine(current);   } while (current != 7);   ```
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   Random random = new Random();   int current = random.Next(1, 11);      /*   do   {       current = random.Next(1, 11);       Console.WriteLine(current);   } while (current != 7);   */      while (current >= 3)   {       Console.WriteLine(current);       current = random.Next(1, 11);   }   Console.WriteLine($"Last number: {current}");   ```
   > [!NOTE]   > In this case, we position the `while` keyword and the Boolean expression before the code block. This changes the meaning of the code and acts as a "gate" to only allow the flow of execution to enter if the Boolean expression evaluates to true.
...
1. Take a minute to review your code.
   ...
   - If `current` is initialized to a value greater than or equal to `3`, then the Boolean expression will return `true`, and the flow of execution will enter the code block. Inside the code block, the first thing that we do is write the value of `current` to the console. Next, still inside the code block, we update the value of `current` with a new random value. At this point, control goes back to the top of the `while` statement where the Boolean expression is evaluated. This process continues until the Boolean expression returns `false` and the flow of execution breaks from the code block.   - If `current` is initialized (at the top of our code) to a value less than `3`, then the Boolean expression will return `false`, and the code block will never execute.  
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   Random random = new Random();   int current = random.Next(1, 11);      do   {       current = random.Next(1, 11);          if (current >= 8) continue;          Console.WriteLine(current);   } while (current != 7);      /*   while (current >= 3)   {       Console.WriteLine(current);       current = random.Next(1, 11);   }   Console.WriteLine($"Last number: {current}");   */   ```
