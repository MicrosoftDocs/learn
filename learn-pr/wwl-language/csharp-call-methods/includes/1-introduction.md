...
Suppose you need to generate a daily report that's based on a thousands of data files. To save time, your code will use a random sample of the data files to calculate an approximate result without analyzing all the data files. How can you generate a random number? Is this supported by C#? If so, how can you write code to set the value range for a random number, and then generate and retrieve the result?

In this module, you'll learn about class libraries, and write code that calls methods in the .NET Class Library. You'll learn the characteristics of methods, and why methods of the .Net Class Library are designed and implemented differently across the .NET Class Library. Most importantly, you'll write code that uses the .NET Library methods to achieve a specific programming task. Finally, you'll use built-in tools and online documentation to help you gather information about the methods you want to work with.
