...
1. Type the following code into the Visual Studio Code Editor.
   ```c#   for (int i = 0; i < 10; i++)   {       Console.WriteLine(i);   }   ```
...
1. Take a minute to identify the six parts of the `for` statement.
   ...
   1. The `for` keyword.   1. A set of parenthesis that defines the conditions of `for` iteration. The parentheses contain three distinct parts, separated by the end of statement operator, a semi-colon.   1. The first part defines and initializes the iterator variable. In this example: `int i = 0`. This section is referred to as the **initializer**.   1. The second part defines the completion condition. In this example: `i < 10`. In other words, the runtime will continue to iterate over the code in the code block below the `for` statement while `i` is less than `10`. When `i` becomes equal to `10`, the runtime stops executing the `for` statement's code block. The docs refer to this section as the **condition**.   1. The third part defines the action to take after each iteration. In this case, after each iteration, `i++` will increment the value of `i` by 1. The docs refer to this section as the **iterator**.   1. Finally, the code block. The code block contains the code that will be executed for each iteration. Notice that the value of `i` is referenced inside of the code block. The docs refer to this section as the **body**.
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   for (int i = 10; i >= 0; i--)   {       Console.WriteLine(i);   }   ```

1. Take a minute to review your updated code.
   By changing the three parts of the `for` statement, we change its behavior.
   - We initialize the iteration variable to 10.   - We change the completion condition to exit the `for` statement when `i` is less than `0`.   - We change the pattern of the iterator to subtract `1` from `i` each time we complete an iteration.
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   for (int i = 0; i < 10; i += 3)   {       Console.WriteLine(i);   }   ```
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   for (int i = 0; i < 10; i++)   {       Console.WriteLine(i);       if (i == 7) break;   }   ```
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   string[] names = { "Alex", "Eddie", "David", "Michael" };   for (int i = names.Length - 1; i >= 0; i--)   {       Console.WriteLine(names[i]);   }   ```
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   string[] names = { "Alex", "Eddie", "David", "Michael" };   foreach (var name in names)   {       // Can't do this:       if (name == "David") name = "Sammy";   }   ```
...
1. Use the Visual Studio Code Editor to update your code as follows:
   ```c#   string[] names = { "Alex", "Eddie", "David", "Michael" };   for (int i = 0; i < names.Length; i++)       if (names[i] == "David") names[i] = "Sammy";      foreach (var name in names) Console.WriteLine(name);    ```
