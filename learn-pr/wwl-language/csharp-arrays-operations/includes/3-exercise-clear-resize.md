...
(x) `Array.Clear(pallets, 0, 2);` removes `2` array elements starting from item `0`. {{Correct. `Clear` is a method of arrays that removes (replaces with `Null`) specific elements in an array.}}
( ) `Array.Clear(pallets, 0, 2);` removes the data from array element locations `pallets[0]` and `pallets[2]`. {{Incorrect. `Clear` is a method of arrays that removes (replaces with `Null`) specific elements in an array.}}
( ) `Array.Clear(pallets, 0, 2);` removes array elements that are equal to `0` or `2`. {{Incorrect. `Clear` is a method of arrays that removes (replaces with `Null`) specific elements in an array.}}
