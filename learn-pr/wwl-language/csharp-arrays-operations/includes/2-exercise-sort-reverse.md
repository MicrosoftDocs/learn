...
( ) `Sort` represents a cast operation {{Incorrect. `Sort` is a method of arrays that sorts alphanumerically.}}
(x) `Sort` is a method of Arrays. {{Correct. `Sort` is a method of Arrays.}}
( ) `Sort` is used to sort array amount of precision. {{Incorrect. `Sort` is a method of arrays that sorts alphanumerically.}}
