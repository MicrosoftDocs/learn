...
The .NET runtime is the code library that's required to run your C# applications. You may also see the .NET runtime referred to as the Common Language Runtime, or CLR. The .NET runtime isn't required to *write* your code, but it's required to actually *run* your application.
...
1. On the **Terminal** menu, select **New Terminal**.
    ...
    You use the Terminal panel to enter a command.

1. At the Terminal command prompt, type: `dotnet --version` and then press the Enter key.

    If a version of .NET is installed on your PC, this command displays the version number.
...
1. Take a moment to view the response to your `dotnet --version` command.
    ...
    If a version of .NET is installed, you should see a response message listing the version number similar to the following (depending on your version):
...
1. Ensure that a .NET version number is listed.
    ...
    The version number is similar to: `7.0.102`
