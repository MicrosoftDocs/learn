In addition to branching code execution, often it's necessary to repeat code execution until a certain condition is met.  This is called iteration in programming terminology.

Suppose you need to prompt a user for input, and continue prompting them until they enter a value that is within an acceptable range of values.  Or suppose you need to read each line of a data file until you reach the end of the file.  In these situations, you'll use the `while` statement, which creates a looping structure in your code.  With a `while` statement, the code execution will iterate until a condition is met, at which point the path of execution will continue through the remainder of your program.
...
- You should have your Python development environment set up, know how to create a working folder and code files, and how to execute the code in a code file.
- You should understand data types like `str` and `int`, how to define, set and get values from a variable.
