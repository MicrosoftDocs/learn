...
We're now ready to train our model. In this exercise, you'll perform a simple training operation in order to test your model. The testing will take place using the built-in testing panel in the LUIS portal.
...
1. After training is completed, select **Manage** in the top toolbar. The following options will appear on the left toolbar:

    > [!NOTE]
    > The categories on the left pane may change as the portals are updated.  As a result, the keys and endpoints may appear under a different category than the one listed here.
...
1. Under **Choose your publishing slot and settings**, select **Production Slot**, and then select **Done**.
    ...
    Publishing creates an endpoint to call the LUIS model. The endpoint URL will appear. Copy the endpoint URL and add it to your list of keys for future use.

1. In the top bar, select **Test**. Try entering a few utterances and see which intents are returned. Here are some examples you can try:
...
1. To retrain the model for utterances with low scores, perform the following steps:

    - Beside the low-scoring utterance (in this case, **Send to Tom**), select **Inspect**.

    - Beside **Top-scoring intent**, select the dropdown, and select **SharePic** from the list.

    - Close the **Test** panel.

    - Select the **Train** button to retrain your model.

    - Test the **Send to Tom** utterance again. It should now return the **SharePic** intent with a higher score.
