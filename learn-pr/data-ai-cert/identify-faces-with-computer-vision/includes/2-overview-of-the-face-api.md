...
The Face API requires special authorization through a *subscription key*. Every programmatic call to the Face API requires a subscription key to be passed to the service through a value in either  the query string parameter or the request header.

Authorizing an API call to the Face service is just a matter of providing a subscription key. The code you write depends on your language of choice.

Now that you have an idea of what the Face API provides and how authorization works, you're ready to subscribe to the Face API and generate subscription keys.