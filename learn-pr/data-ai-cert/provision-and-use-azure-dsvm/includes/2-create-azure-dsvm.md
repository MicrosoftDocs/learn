...
1. On the marketplace screen search for **Data Science Virtual Machine - Windows**

1. Select the results entry for **Data Science Virtual Machine - Windows 2016**
...
1. The browser window will take you to the Azure portal. Make sure you are signed in with your subscription.
...
1. Select **Create** to begin configuring a virtual machine (VM). Set the appropriate values into each of the fields using the following suggestions.
    - **Subscription**. Verify that your subscription is selected.
    - **Resource group**. A *resource group* is a container that holds related resources for an Azure solution. Create a new resource group with the name **Learn-dsvm-rg** so that it will be easier to clean up these resources when you are finished with the module.
    ...
    - **Password**. Enter an administration account password. You will need this later as well to access the VM.
