...
It is very common for AI practitioners to decide the number of clusters by running k-means with several different numbers of clusters. The results are then visualised for the users decision. There is a cost function to determine an error rate but analysing cost for k-means is often ambiguous as to the correct number of clusters.
