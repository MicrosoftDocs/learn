...
![three circular nodes are aligned horizontally, with each joined to the node to its right with a black arrow. The left-hand node is labeled 'input' with an 'i' in the centre of the circle, and an 'n'. The middle node is labeled 'RNN layer' with an 'h', and an 'n' in the centre of the circle. The right-hand node is labeled 'output' with an 'o' and an 'n' in the centre of the circle. The 'n' in each cell indicates that these three cells are all on the same position in the RNN. There is also an arrow, which loops back from the right-hand side of the centre 'RNN layer' node, around to the left'hand side of the RNN node. This arrow is labeled 'loop'.](../media/65-l-1.png)
