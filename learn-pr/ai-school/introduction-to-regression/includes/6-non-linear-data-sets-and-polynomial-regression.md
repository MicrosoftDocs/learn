...
A polynomial is a type of mathematical expression than can be expensive to compute, but can fit very complex trends. Although using polynomial regression is very similar to multiple linear regression, behind the scenes the model is more elaborate. As training time increases when models get more complex, it is vital to ensure your data is in an appropriate format.
...
Next up we will build and use a polynomial regression model in Azure Notebooks.
