...
1. Try filtering to show only failed tests with the failed test button in the Test Explorer toolbar. This button also summarizes how many tests are failing in the current view.
...
1. Click the test flask button to go back to viewing tests with all outcomes. This button also lists the total number of tests and how many tests are visible in the current view as a fraction. There are only two tests visible out of a total of six tests with the current filtering.
...
1. Let's explore some of the Group By settings. Try a new grouping by de-selecting the presets and only grouping by State (passed, failed, not run, and so on) and then Class.
...
`DivideByZeroTest` will fail until you update the `Divide` method in the `Calculator.cs` to gracefully handle when the divisor is zero.
...
1. Now rerun your `DivideByZero` test to confirm it now passes! The `Divide` method now returns null when the divisor is zero. Note the output that warns the user they cannot divide by zero appears in the test detail summary in the Test Explorer.
...
1. Now that your test is passing don't forget to clean up your product and test code with a bit of refactoring. Consider scenarios that might need to be updated because of your recent changes. Feel free to add comments to describe new functionality.

This exercise was a basic example of the Red, Green, Refactor pattern. We started with the red failing `DivideByZeroTest` and modified the product code until it got to green. The last step before moving on was refactoring.
