Xamarin.Android enables .NET developers to create fully native Android apps. Developers can take full advantage of the power, features, and performance of the Android platform. In this module, you've explored Xamarin.Android and its development tools, and you've built an Android application.

Use these resources to learn more about developing apps with Xamarin.Android:

- [Get started](/xamarin/android/get-started/index) with Xamarin.Android.

- Learn [application fundamentals](/xamarin/android/app-fundamentals/) such as working with files, databases, and more.

- Learn more about building [user interfaces](/xamarin/android/user-interface).

- Work with Android [platform features](/xamarin/android/platform/).

- Find out how to [build, deploy, and test](/xamarin/android/deploy-test/) your applications.

- Learn how to build apps for [wearable Android devices](/xamarin/android/wear/).

## Learn more with a Channel 9 video series

- [Xamarin Tutorial 101](https://channel9.msdn.com/Series/Xamarin-101/?WT.mc_id=Educationalmobile-c9-niner&azure-portal=true)
- [Intro to Visual Studio for Mac](https://channel9.msdn.com/Series/Intro-to-Visual-Studio-for-Mac?azure-portal=true)