Now it's time to get our proof-of-concept working. We'll build a simple UI that takes advantage of not just Xamarin.Forms, but also the Xamarin.Essentials NuGet package which will dial the phone.

The app will let the user type text into an input field, and will translate that text to numeric digits. It will use the letters that show up on a telephone keypad as the basis for translation. For example, the letters "cab" translates to "222" because the digit "2" has all three letters "abc".
...
Place the following static class code into the new PhonewordTranslator.cs source file. This code is a single `public static` method, which you will use to translate the number from alphanumeric text into a regular numeric phone number.
...
1. In Solution Explorer, right-click on the existing MainPage.xaml file, and choose **Delete**. This also deletes the code-behind MainPage.xaml.cs file.
...
When you're done, your code looks similar to this:
...
Subscribe a handler to the `Clicked` event of the **Call** `Button`. This should be added after the current `translateButton.Clicked` handler.
...
Next, we'll use a cross-platform library called Xamarin.Essentials, which abstracts several platform-specific functions through a single API. Xamarin.Essentials offers a phone dialing API to make calls on iOS and Android platforms. This library is provided as a NuGet package that we add to all our projects. Because it provides implementations for all of our platforms and the shared abstraction, we will add it to all of our projects.

### Add permissions to the Android project

In order to make calls on Android an additional step is required. We must explicitly declare the calling permission.
...
1. If you have time, run the app on each supported platform, and try stepping through the program to see it jump into the platform-specific code. For some extra fun, try running this on a real device!
