...
When you're creating a `Grid`, you need to define each row and column individually. This system gives you full control over the height of each row and the width of each column. Every `Grid` has a collection of `RowDefinition` and `ColumnDefinition` objects that define the shape of the grid. Your job is to populate those collections with instances of `RowDefinition` and `ColumnDefinition`, each representing a row or column in your UI.
...
Notice that `RowDefinition` has a property called `Height` and `ColumnDefinition` has a property called `Width`. You use these properties to set the height of a row and the width of a column.

### What is GridLength?

The data type for `Width` and `Height` is `GridLength`. This type contains two properties: `GridUnitType` and `Value`. Here's a snippet of code that shows a portion of the type definition.
...
`Absolute` specifies that the row or column will be fixed in size. You use the `Value` property to indicate what the size will be. Here's an example that shows how you would set the height of a row to be a fixed size of `100` device units in C#. Notice how you would use the `GridLength` constructor, which takes a numeric value. This constructor will set `GridUnitType` to `Absolute` for you automatically.

```cs
var row = new RowDefinition() { Height = new GridLength(100) };
```

In XAML, we just use a numeric value. The XAML parser will invoke a type converter to create the `GridLength` instance. Here's an example that shows the same thing in XAML:
...
`Auto` will automatically size the row or column to fit your child views. The `Grid` will scan all child views in that row or column, select the largest view, and then make the row or column large enough to fit that child. When you create a row definition in code, the numeric value is ignored. You can use any value. Here's an example that shows how you would set the height of a row to be automatically sized in C#. Notice that we arbitrarily chose `1` for the value.
...
`Star` gives you proportional sizing. The size is determined by the total available space and ratio that each row or column asks for. In conversation, people often call this "star sizing" instead of "proportional sizing".
...
Now that you understand how to define rows and columns by using `RowDefinition` and `ColumnDefinition`, you need to add them to a `Grid`. You add them by using the `RowDefinitions` and `ColumnDefinitions` collection properties of the `Grid`. Populating these collections is most commonly done in XAML, so that's what we'll use in our examples.
...
When we add a view to a `Grid`, we add it to a specific cell. Cells are created at positions where rows and columns intersect. To position a view in a cell, we need to know the location of the cell. You use a combination of a row number and a column number to identify a cell.
...
We need a way to specify a view's row and column number when we add it to a grid. One solution would be to define `Row` and `Column` properties on the `View` base class so that we could specify the position on the view directly. This technique would work, but it's not the most efficient approach. Views aren't always going to be in a `Grid`, so sometimes those properties wouldn't be needed. A better approach is to use attached properties.

An *attached property* is a property that's defined in one class but set on objects of other types.

Think of attached properties as a collection of key-value pairs that is part of a view. Let's use `Grid` as an example. When we add a view to a `Grid`, we need to specify the row and column. By using attached properties, we can add a key-value pair with the key `Grid.Row` and a value that's the row position. When the `Grid` is ready to position the view, it will check the collection to see if there's a key called `Grid.Row`. If there is, the `Grid` will use the value to position the view.
