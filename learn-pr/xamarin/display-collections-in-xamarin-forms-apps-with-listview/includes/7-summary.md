...
Our app's goal was to show a list of countries/regions and their flags. Each row needed a thumbnail image of the flag, the name of the country/region, and the date the flag was adopted. We started with Xamarin.Forms `ListView`, which gave us a scrollable list. Then we added the predefined cell type `ImageCell` and used data binding to load the flag data into the rows.
