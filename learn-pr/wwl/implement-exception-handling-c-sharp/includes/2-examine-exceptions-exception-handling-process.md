...
The `try` code block contains the guarded code that may cause an exception. If the code within a `try` block causes an exception, the exception is handled by the corresponding `catch`.
...
Once a `catch` clause is found that can handle the exception, the runtime prepares to transfer control to the first statement of the `catch` clause. However, before execution of the `catch` clause begins, the runtime executes any `finally` clauses associated with `try` statements found during the search. If more than one `finally` clause is found, they are executed in order, starting with the one closest to the code that caused the exception to be thrown.
...
The following code sample includes a `try-finally` pattern nested inside a `try-catch` pattern:
...
- When an exception occurs, the .NET runtime searches for the nearest `catch` clause that can handle it. The search starts with the method where exception was thrown, and moves up the call stack if necessary.