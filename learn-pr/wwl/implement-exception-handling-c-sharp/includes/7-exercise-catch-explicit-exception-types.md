...
1. Take a minute to examine the information describing the exception object `ex`.
    ...
    Notice that the `StackTrace` property reports the method and line number where the error occurred, along the sequence of method calls (and line numbers) that led to the error.
...
1. Take a minute to review this code.
    ...
    First, the code creates a string array named `inputValues`. The data in the array is intended to represent the input values entered by a user who was instructed to enter a numeric value. Depending on the value entered, different exception types may occur.
