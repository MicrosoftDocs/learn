...
The foundation of the platform is based on Software as a Service (SaaS), which provides a simple and integrated approach. The image below shows the Fabric architecture, with OneLake as the foundation, and each *workload* built on top.

![Screenshot of Fabric architecture, show with OneLake as the foundation, with each workload built on top.](../media/fabric-overview.png)

All of the Fabric workloads use OneLake as their native store without needing any additional configuration. OneLake is hierarchical in nature to simplify management across your organization. There's only one OneLake per tenant and it provides a single-pane-of-glass file-system namespace that spans across users, regions and even clouds. The data in OneLake is divided into manageable containers for easy handling.
...
A *domain* is a collection of items that are logically grouped together. Domains are used to organize items in a way that makes sense for your organization. For example, you might have a domain for sales, another for marketing, and another for finance. Domains are also used to control access to items. For example, you might want to restrict access to the sales domain to only users in the sales organization.
...
Understanding Fabric's architecture is important for you as an admin, because it helps you understand how to manage the Fabric environment.
