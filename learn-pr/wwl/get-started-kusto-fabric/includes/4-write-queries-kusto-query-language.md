...
The next example returns the first five rows from the sales table in the KQL database by using the `take` statement, which is a simple and quick way to view a small sample of records when browsing data.
...
This time, the results include only the first five rows:
...
This query uses the `sort` operator to retrieve sales of "Mountain-100" items sorted so that the most recent sales are sown first:
