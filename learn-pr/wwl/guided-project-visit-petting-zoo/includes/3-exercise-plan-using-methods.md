...
In this first step of your development process, you'll begin planning your application through pseudo-code. You'll identify the steps and methods needed to complete the task, and you'll need to take some time to think about the input and output of these methods as well. Preparing methods via pseudo-code is a helpful step in any coding task, and can help you develop bug-free code quickly.
...
In this task, you'll turn the steps defined in the application specification into pseudo-code. By preparing pseudo-code, developing the full application becomes a much easier task. Let's get started!
...
1. The first task is to randomize the animals. Create a pseudo-code method to randomize the petting zoo animals by entering the following code into the editor:
    ...
    You won't be filling in any logic to randomize the animals just yet. For now, you will focus on planning the application as a whole. You will assume that this `RandomizeAnimals()` method exists and will correctly randomize the `pettingZoo` array.
...
1. Next, create a pseudo-code method to assign the animal groups. Enter the following code into the editor:
    ...
    Like before, you will assume that this method correctly assigns the animals to a subset group and continue to the next task.
...
1. Since you have completed pseudo-code for the general tasks, you can take a moment to consider what data type to assign to the `group` variable.
    ...
    You want School A to have animals assigned into 6 groups. And you know that there are 18 animals. You also know that the animals are represented using `string`. So you can use a 2D array that will contain 6 groups of 3 animals each.
...
1. Assign the `group` data type by updating the `AssignGroup` line of code to the following:
    ...
    You won't modify the `AssignGroup` method for now since 6 groups is the default option.

## Check your work

In this task, you'll check that your pseudo-code will perform the general tasks listed for each school. You'll also run the code to ensure there are no issues.
