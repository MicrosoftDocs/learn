...
Another approach is the use to the Blazor framework by Microsoft which will allow you to use a mixture of C# and JavaScript while compling to wasm, web assembly. Assembly language that your browser knows how to run.
...
A feature in Visual Studio you can use to improve this workflow is "Hot reload". The idea is that you do a change, save said change and the application will rebuild and relaunch in the browser.

## Blazor components

When you create apps using Blazor your app will consist of many components, each with their own area of responsibility. Creating the components, you will use something called Razor, so what is Razor?
...
In this example, `@product.Name` will be resolved to the string "Blazor".
...
The top part consist of HTML markup and everything within `@code {}` contains C# code. Razor will continue interpreting C# code after the @ until it identifies a tag that it will then render normally.

## Styling a Blazor component

There are two approaches we will look at on how you could style a Blazor component:
...
- Styling Blazor component, inline. Blazor components can define content to be added to the HTML head of the page using a special HeadContent tag.
    ...
    This style tag and contents will be delivered inside the head tag on the page.
