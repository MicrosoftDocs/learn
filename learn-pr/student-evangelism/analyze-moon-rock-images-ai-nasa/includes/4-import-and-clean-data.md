...
Because the the rock images come in different sizes (small, medium, and large), we will crop the images so that they are all the same size (224 &times; 224 pixels). We need to resize all the images because computers expect images to be all same size, otherwise they get confused about differences in sizes.
