...
1. Run the following command to deploy the website to Azure. Replace `APP_NAME` with the name that you want to assign to the site. The name must be *unique with Azure*, so you probably won't be able to use a common name such as "contoso" or "contosotravel" unless you append some random characters to the end.
    ...
    If you get an error like `"Could not auto-detect the runtime stack of your app, see 'https://go.microsoft.com/fwlink/?linkid=2109470' for more information"`, make sure that you're in the correct directory. You should be in the **starter** directory within **mslearn-build-ai-web-app-with-python-and-flask** where you've been writing all your code.
