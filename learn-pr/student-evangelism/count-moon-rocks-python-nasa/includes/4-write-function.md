The next step in our application is to add a function that will look at a line of text, tell us which type of rock it is, and then increment the corresponding variable. To start, copy this code:
...
As you recall, these lines will create a function that has a parameter of a string variable `rockToID` and returns nothing. We'll be calling the function and passing in one of four a rock names: Basalt, Breccia, Highland, or Regolith. To ensure that the variables will remember how many rocks they've counted, we'll need to make each rock type a global variable. To increment the correct variable based on the types of rocks that Artemis encountered, we'll use if statements.
