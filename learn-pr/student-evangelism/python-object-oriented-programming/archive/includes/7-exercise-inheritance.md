...
One characteristic that differentiates a missing person from another person is the date that the person went missing. Let's write a missing person class that includes a `missing_since` attribute and a `get_years_missing()` method that computes the number of years the person has been missing.
...
1. Return to your MissingPersons.py file and enter the following class definition under the definition of `Person`. Feel free to remove the `print` statements and definition of `aPerson` at the bottom of the file.
    ...
    You defined a new class (subclass) named `MissingPerson` that inherits from `Person`. When created, a `MissingPerson` object creates a `Person` object by calling the latter's `__init__()` method. Then, it defines an instance attribute of its own (`missing_since`) and a method that subtracts the date the person went missing from today's date to compute how long that person has been missing.
...
1. Add the following code at the bottom of the file to create a `MissingPerson` object to show how long the person has been missing:
    ...
    Observe that the `MissingPerson` object contains the `name` attribute inherited from `Person` in addition to the `get_years_missing()` method it added itself. When you create a `MissingPerson` object, you must provide the date that the person went missing, as well as a name, a photo, and a date of birth. That data comes from the `MissingPerson` object's `__init__()` method, which serves the same purpose in an inherited class as it does in a base class.
...
You can include calls to Python's `delattr()` function in a class definition to remove attributes that are inherited from the base class. Here's how an `AnonymousPerson` class might look:
...
Here's how you would create an instance:
