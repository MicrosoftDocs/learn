Instance methods are associated with a particular object rather than with the class. Instance methods are defined by adding functions that receive `self` as the first parameter to the class. Internally, instance methods are free to access the object's attributes and even other methods.
...
Python supplies a default `__str__()` method that works well with Python's rather primitive built-in object types. But for custom types, it rarely provides the results you want. The default method tells you the object's name and where the object is located in memory. It would be a lot more useful if it could tell you something specific about the object's content. 
...
1. To demonstrate, return to the `Person` class and add the following method to the class by using the correct indentation:
...
1. Add the following code to the end of your file, and then run it again to test the new `__str__()` method:
