...
* A **reference implementation** is an instructor-written solution to a problem. These are created using **annotations**, which assert conditions on the student implementation. Reference implementations can be used in concert to allow students various different methods of implementing solutions.
...
* The **grading pipeline** is the process by which the student implementations are produced and checked against the reference implementations. This is a very freeform component, and can be adapted to suit the technological needs of the instructor. This can be anything from automation via GitHub Actions to a notebook that runs each submission in sequence using PyBryt's API and spits out a report.

## Building an assignment

The process for building an assignment with PyBryt is not too complicated:
