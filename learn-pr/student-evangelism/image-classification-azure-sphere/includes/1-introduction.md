...
By the end of this module, you will be able to implement local whole-image classification on an Azure Sphere real-time capable core by using a pre-trained, pre-quantized CIFAR-10 model.

## Learning objectives

In this module, you will:

- Implement local image classification on a microcontroller device using a pre-trained, pre-quantized neural network model.
...
- On Linux, `sudo` permission. Install `net-tools` and `curl` before running the SDK installer, answer `Y` when it asks to set up the default `udev` rule and `azsphere` group, then log out and back in (or restart) before running `az sphere` commands.
