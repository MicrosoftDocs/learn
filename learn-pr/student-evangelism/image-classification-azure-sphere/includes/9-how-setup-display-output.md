...
**USB-to-serial adapter:** Use an adapter that supports 3.3 V logic levels. For the dedicated RT-core logging UART, the adapter receives output from the board; it does not power the board or send data to the board.
...
> [!IMPORTANT]
> Use 3.3 V logic. For this TX-only logging UART, connect only GND and adapter RX to board TX. Do not connect adapter VCC or adapter TX.
