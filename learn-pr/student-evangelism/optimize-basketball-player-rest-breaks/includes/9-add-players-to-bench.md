...
- **className:** In the styles.css file, we can see a `playerButton` class, so we can assign that here.
- **onclick:** This will be the function that we want our app to call when the button is selected.
- **style:** There are a couple of style attributes, like background color and width, that we can set.
- **playerImage:** We'll add the player's image to the button, rather than to their name.

After we've created the button, we can add the button to the bench area that we identified earlier, and then repeat this for each player.
...
Before continuing, let's make sure that our app is working so far. If we were to try to test our app at this point, we should be able to upload our *game_stats.csv* file. Then, the 16 characters should be added as buttons to the bench area. 

We can test our app in two ways:
* We can commit our changes to the main branch on GitHub, triggering the GitHub Action and redeploying our code to our Azure static web app. We'll do this one in a later unit.
* We can save the main.js file and open index.html locally in a browser. We'll do this one this time to save on testing time while we are still building our app. 

To test your web app locally, right-click the index.html file, and then select **Copy Path**. 

:::image type="content" source="../media/local-copy-path.png" alt-text="Screenshot that shows copying the local path to the index.html file.":::
