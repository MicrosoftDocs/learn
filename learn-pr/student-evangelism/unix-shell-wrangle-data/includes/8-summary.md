In this module, you were introduced to some tools and commands that you can use on the UNIX shell to transform your data from one format into another.

You were first introduced to the commands `head`, `tail`, `wc`, and `nl`. Using these commands will help you to do quick inspections of your text files and get a better understanding of the raw data. The next command you were introduced to was `cat`. This command allows you to peek into the contents of your files. You can also use it to create new files by appending or concatenating files' contents.

Then, you learned the basics of regular expressions, also known as regex. Regular expressions are useful for pattern matching and can be used in IDEs and on the command line with other filters and commands.

You were then introduced to `sed` and `grep`, which can be used to search for and replace patterns in text streams and files.

Finally, you learned about pipes and output redirection. Using pipes allows you to put together all the commands and filters you learned in the previous units to build data processing pipelines.

You should now have a good understanding of what these commands do and how to use them to transform data efficiently from the UNIX shell.


### Resources

- [Filters](http://www.linfo.org/filters.html)
- [Pipes](http://www.linfo.org/pipes.html)
- [Regular Expressions 101](https://regex101.com)
- [UNIX shell commands](https://www.educba.com/unix-shell-commands/)
- [UNIX (and Linux) shell commands](https://www.geeksforgeeks.org/essential-linuxunix-commands/)
- [Visual Studio Code documentation](/visualstudio/ide/using-regular-expressions-in-visual-studio/)


### Commands and filters

- [cat command](https://www.techonthenet.com/unix/basic/cat.php)
- [grep command](https://www.techonthenet.com/unix/basic/grep.php)
- [head command](https://www.tutorialspoint.com/unix_commands/head.htm)
- [mkdir command, and other basic UNIX commands](https://www.techonthenet.com/unix/basic/mkdir.php)
- [nl filter](https://www.geeksforgeeks.org/nl-command-in-linux-with-examples/)
- [sed command](https://www.educba.com/sed-command-in-unix/)
- [sort command](https://www.tutorialspoint.com/unix_commands/sort.htm)
- [stderr, stdin, and stdout commands](https://www.howtogeek.com/435903/what-are-stdin-stdout-and-stderr-on-linux/)
- [tail command](https://www.tutorialspoint.com/unix_commands/tail.htm)
- [wc command](https://www.tutorialspoint.com/unix_commands/wc.htm)
- [wget command](https://www.tutorialspoint.com/unix_commands/wget.htm)
