This unit summaries the main steps completed in this module.

In this module, you:

* You created the Altair IoT Central application.
* You configured your IoT Central to trust messages signed by certificates generated from the Azure Sphere tenant Certificate Authority certificate.
* Your Azure IoT Central application will automatically provision Azure Sphere devices in the trusted Azure Sphere tenant.

## Resources

* [Quickstarts to set up your Azure Sphere device](/azure-sphere/install/overview)
* [Quickstart - Create an Azure IoT Central application and use your smartphone to send telemetry](/azure/iot-central/core/quick-deploy-iot-central)
* [Azure Sphere Samples on GitHub](/azure/azure-sphere-samples)