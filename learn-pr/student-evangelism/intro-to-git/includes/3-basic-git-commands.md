...
`git status` displays the state of the working tree (and staging area—we'll talk more about the staging area later). It lets you see which changes are currently being tracked by Git, so you can decide whether you want to ask Git to take another snapshot.
...
You've already tried out the `git help` command, but it's worth reminding you about. This command lets you easily get information about all the commands you've learned so far, and more.
