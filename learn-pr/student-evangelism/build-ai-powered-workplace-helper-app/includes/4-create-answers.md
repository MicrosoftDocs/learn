We have quickly been able to create a chatbot that provides helpful information that would take a long time to find ourselves. However, in an emergency, we must provide specific information very quickly. In this section, you will add Copilot Studio Topics to answer emergency-related questions. 
...
- When the topic is triggered, it should provide relevant local contact numbers for fire, police, and ambulance services (e.g., 000, 911, 111, as per your country). List the street address of the building so it can be provided to authorities.
...
- After providing the above contact information, add a node to your conversation flow that uses generative answers to provide other related information from the uploaded data sources.
  - When you click to add a new node, select the Advanced menu option and then select Generative Answers.
  - Next, set the Input; in the System tab, select the LastMessage.Text variable.
- When it is working, publish your Chatbot so it can be used in your Power App. 
...
1. Ask your bot what you should do if there is a fire. See that it provides both the specified answers and generative answers.
...
**For more information on Generative Answers Nodes**, see this documentation [Generative Answers with Search and Summarize Content](https://learn.microsoft.com/microsoft-copilot-studio/nlu-boost-node)
