...
![png](../media/88-nearest-neighbors-11-0.png)
...
whether she has CKD? It sure looks like it! You can see a very clear
...
measurements are pretty similar, so we might expect them to receive the
...
![png](../media/88-nearest-neighbors-16-0.png)
...
The scatterplot suggests that this nearest neighbor classifier should be
pretty accurate. Points in the lower-right will tend to receive a "no
...
capture our intuition pretty well, for this example.
...
![png](../media/88-nearest-neighbors-19-0.png)
...
![png](../media/88-nearest-neighbors-22-0.png)
...
![png](../media/88-nearest-neighbors-26-0.png)
...
However, the separation between the two classes won't always be quite so
...
![png](../media/88-nearest-neighbors-29-0.png)
...
closest to her, we can look at the 3 points that are closest to her, and
use the diagnosis for each of those 3 points to predict Alice's
diagnosis. In particular, we'll use the majority value among those 3
diagnoses as our prediction for Alice's diagnosis. Of course, there's
nothing special about the number 3: we could use 4, or 5, or more. (It's
