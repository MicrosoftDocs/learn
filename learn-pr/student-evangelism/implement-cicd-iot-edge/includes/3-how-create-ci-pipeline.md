Azure Pipelines supports continuous integration (CI) and continuous delivery (CD). CI/CD allows you to regularly and consistently test, build, and ship code. You accomplish this by defining pipelines. You define a *build pipeline* to build and test your code, and then to publish artifacts. You also define a *release pipeline* to consume and deploy those artifacts to deployment targets. You define pipelines using the YAML syntax or through the user interface (Classic).

YAML is a human-readable data-serialization language. It is commonly used for configuration files and in applications where data is being stored or transmitted. A pipeline is one or more stages that describe a CI/CD process. Pipelines comprise a hierarchy of Stages, Jobs. Stages are the primary divisions in a pipeline. A stage comprises of is one or more jobs, which are units of work assignable to the same machine. You can arrange both stages and jobs into dependency graphs. A job is a linear series of steps. 
