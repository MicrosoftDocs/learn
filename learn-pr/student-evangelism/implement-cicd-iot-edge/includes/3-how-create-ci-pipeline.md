...
You can define pipelines using YAML syntax or through the user interface (classic). YAML is a human-readable data-serialization language which  is commonly used for configuration files and in applications where data is being stored or transmitted. A pipeline is one or more stages that describe a CI/CD process. Pipelines have a hierarchy of stages and jobs. Stages are the primary divisions in a pipeline. A stage has one or more jobs, which are units of work assignable to the same machine. You can arrange both stages and jobs into dependency graphs. A job is a linear series of steps.
