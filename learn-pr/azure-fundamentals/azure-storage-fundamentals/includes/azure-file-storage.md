:::row:::
  :::column span="3":::
Azure Files offers fully managed file shares in the cloud, and the shares are accessible via the industry standard Server Message Block (SMB) protocol. You can mount Azure file shares concurrently, by cloud or on-premises deployments of Windows, Linux, and macOS. Applications that run in Azure virtual machines or cloud services can mount a file storage share to access file data, just as a desktop application mounts a typical SMB share. Any number of Azure virtual machines or roles can mount and access the file storage share simultaneously. You might use this service to share files anywhere in the world, to share diagnostic data, or for application data sharing.
  :::column-end:::
  :::column:::
...
- Many on-premises applications use file shares. This feature makes it easier to migrate those applications that share data to Azure. If you mount the file share to the same drive letter that the on-premises application uses, the part of your application that accesses the file share should work with minimal changes, if any.
...
![Screenshot of components of a service SAS URI.](../media/sas-storage-uri.png)
