...
While there is more you'll need to know to program production level smart contracts, these are the four things that will get you off on the right foot.
...
**Pragma** is the keyword that you use to ask the compiler to check whether its version of Solidity matches the one required. If it matches, that means your source file can run successfully. If it doesn't match, the compiler will error.

You should always make sure to include the latest version of Solidity in your contract definition. To find the current version of Solidity, visit the [Solidity website](https://solidity.readthedocs.io/?azure-portal=true). Use the most recent version in your source file.
...
Functions are executable units of code within a contract. Functions describe a single action for achieving one task. They are reusable and can also be called from other source files like libraries. They behave very similarly to functions in other programming languages.
...
This shows a function with the name `buy` that has a public visibility, meaning that it can be accessed by other contracts. Functions can use one of the following visibility specifiers: **public**, **private**, **internal** and **external**.
