...
Consider a `servicedBy` relationship that has a `reportedCondition` property. In the example, this relationship is given an alias of `R` in order to reference its property.
...
When constructing a query, consider the following elements of the query language.

- Use the scalar functions: IS_OF_MODEL, IS_BOOL, IS_DEFINED, IS_NULL, IS_NUMBER, IS_OBJECT, IS_PRIMITIVE, IS_STRING, STARTS_WITH, ENDS_WITH.
- Use query comparison operators: IN/NIN, =, !=, <, >, <=, >=.
- Use any combination of AND, OR, and NOT operators, with the scalar functions, and comparison operators.
- To select only the top _N_ results, use the TOP operator.
