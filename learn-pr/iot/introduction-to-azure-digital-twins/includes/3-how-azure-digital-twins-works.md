...
The role of the model is to define the concept of what a robotic arm is, so you would only create one model file to use for all the robotic arms in your environment. You might create additional models for other types of entity in your environment, such as a factory temperature sensor or the distribution center as a whole.
...
Then, when you create the digital twin for the distribution center, you can add a *contains* relationship from the distribution center twin to any existing robot arm twins, using the Azure Digital Twins APIs and SDKs, the Azure CLI, or Azure Digital Twins Explorer (a visual developer tool that's accessible from your instance page in the Azure portal).
...
1. Your Azure function listen on the endpoint for updates from the robot arm twins, and receives the new data when any of them changes this value.
