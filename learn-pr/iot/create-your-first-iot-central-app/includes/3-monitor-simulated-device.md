...
1. Select the dropdown under **Telemetry**, and select  **Location**, and then select **Add tile**. Dashboards are made up of tiles. The reason we choose the location tile first, is that we want to expand it from its default size. Drag the lower right-hard corner of the tile, so that the tile is at least twice the default size. This tile is the most fun, it will show the location of the truck on a map of the world.

1. Before adding more tiles, change the **View name** to something more specific, "Truck view", or something similar.

1. Now, select each of the rest of the telemetry and property capabilities in turn, starting at the top, and **Add tile** for each one. We are going for function over form here. We can beautify the dashboard later. For now, we just want a dashboard that will confirm all the telemetry being sent from our remote device. There's no need to add the commands to the dashboard, though that option does exist.

    > [!NOTE]
    > If you select the checkboxes for two, or more, telemetry values, and then select **Add tile**, all the values will appear in _one_ tile. This feature is a good way of saving dashboard space.
...
1. To save this view, select **Save**.
...
1. Select **Views**, then select the **Editing device and cloud data** tile.
...
1. Select the **Optimal Temperature** property checkbox, and then select **Add section**.

1. Verify that your view looks similar to the following image, and then select **Save**.

    [![Screenshot showing the view for writable properties](../media/set-properties-view.png)](../media/set-properties-view.png#lightbox)

1. When you are satisfied with your views, from the top menu bar, select **Publish**. You'll now notice that in the dialog that appears, that the **Views** entry is **Yes**. Select **Publish** in the dialog.
...
1. In the **Devices** menu, select **RefrigeratedTruck** to ensure the device we create uses this device template.

1. Select **+ New**. In the **Create a new device** dialog, verify that the device template is **RefrigeratedTruck**.
...
1. Select **Create**. Wait a few seconds, and then your device list should be populated with a single entry. Note the **Device status** is **Registered**. Not until the device status is **Provisioned** will the IoT Central app accept a connection to the device. The coding unit that follows shows how to provision a device.

1. Select the **RefrigeratedTruck - 1** name, and the live dashboard appears with lots of **Waiting for data** messages.

1. Select the **Commands** entry in the bar that includes **Truck view**. Notice that the two commands you entered are ready to be run.
...
1. In the top-right menu, select **Connect**. Do _not_ click **Attach to gateway**.

1. In the **Device connection** dialog that follows, carefully copy the **ID scope**, **Device ID**, and **Primary key** to a text file. The **ID scope** identifies the app, the **Device ID** identifies the real device, and the **Primary Key** provides permission for the connection. Typically, use a tool like Notepad, and save the text file with a meaningful name, say "Truck keys.txt".

1. Leave the **Authentication type** as **Shared access signature (SAS)**.

1. When you've saved off the IDs and key, select **Close**.
