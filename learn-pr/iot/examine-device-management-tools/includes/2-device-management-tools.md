...
Imagine you work as an operator in a central contral facility for an electrical utility. One day you notice that a field power line voltage measurement IoT device is malfunctioning and is giving erratic readings that could affect the stability of the grid. You use the Azure portal IoT Hub portal interface to send a direct method to the malfunctioning device, instructing it to go offline and stop sending messages. You receive a confirmation that the device has executed the command successfully. You then update the device twin of the malfunctioning device, setting the  status property to `offline` and add a reported property reason with the value `malfunction`. You then dispatch a technician to maintenance the manfunctioning device.

Here, you will learn about the capabilities of the three device management tools for Azure IoT.
