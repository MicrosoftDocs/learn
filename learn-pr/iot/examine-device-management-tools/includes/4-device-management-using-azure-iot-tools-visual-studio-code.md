...
Suppose you work as a field technician for an oil and gas company. You need to test some IoT devices by monitoring IoT Hub-to-device messages in real-time. Your company does not allow you to use it's Azure IoT Hub account which is limited to a select group of administrators, so you use Visual Studio and the Azure IoT extension to run a menu-based command to monitor the IoT Hub-to-device messages.

Here, you will learn about some of the Visual Studio with Code Azure IoT Hub extension capabilities that you can use to manage IoT devices.
...
After a few seconds, the Visual Studio Code Explorer pane will be updated to show a Devices section under Azure IoT Hub. The Devices section will display a list of the devices connected to the IoT Hub that you selected.
...
3. Enter the method name in the input box, and then the associated payload value. The results will be shown in OUTPUT &gt; Azure IoT Hub extension view. If the direct method that you specify does not exist on the device, you will see message similar to the following: Failed to invoke Direct Method: Not found.
...
1. In the Visual Studio Code Explorer pane, right-click the device that you are interested in.
2. On the context menu for your device, click **Edit Device Twin**. An azure-iot-device-twin.json file will be opened in Visual Studio Code showing the contents of the device twin document.
...
2. Right-click anywhere within the azure-iot-device-twin.json document. This will open the Visual Studio Code context menu for the open document.
...
3. Enter the message in input box. Results will be shown in OUTPUT &gt; Azure IoT Hub extension view.
