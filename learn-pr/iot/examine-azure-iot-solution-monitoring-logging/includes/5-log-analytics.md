Log Analytics is a tool you access from the IoT Hub **Logs** menu item to edit and run log queries against data in the Azure Monitor Logs store.
...
To perform Log Analytics, navigate to the Azure portal and open your IoT hub, then select **Logs** under **Monitoring**. These Log Analytics queries are scoped, by default, to the logs and metrics collected in Azure Monitor Logs for your IoT hub.
