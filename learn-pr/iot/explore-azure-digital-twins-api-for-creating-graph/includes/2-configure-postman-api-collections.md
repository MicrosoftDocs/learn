...
Start by navigating to [Postman](https://web.postman.co/) in a browser window. Create a free account or log in to an existing account. Select **Workspaces > Create Workspace**.

:::image type="content" source="../media/2-create-workspace.png" alt-text="Screenshot showing the Create Workspace button in Postman." border="false" lightbox="../media/2-create-workspace.png":::
...
:::image type="content" source="../media/2-create-workspace.png" alt-text="Postman screenshot showing the Create Workspace button." border="false" lightbox="../media/2-create-workspace.png":::

Enter the following URL, which represents the Azure Digital Twins control plane Swagger: `https://raw.githubusercontent.com/Azure/azure-rest-api-specs/main/specification/digitaltwins/resource-manager/Microsoft.DigitalTwins/stable/2023-01-31/digitaltwins.json`. Continue the prompts to import the Swagger as a Postman collection.

Next, select **Import** again to create the *Data plane* collection.

Enter the following URL, which represents the Azure Digital Twins data plane Swagger: `https://raw.githubusercontent.com/Azure/azure-rest-api-specs/main/specification/digitaltwins/data-plane/Microsoft.DigitalTwins/stable/2023-10-31/digitaltwins.json`. Continue the prompts to import the Swagger as a Postman collection.
...
:::image type="content" source="../media/2-rename.png" alt-text="Postman screenshot showing the Rename collection option." border="false" lightbox="../media/2-rename.png":::

#### Configure authentication

Select the *Control plane* collection in the workspace and switch to the **Authorization** tab. Make sure **Type** is set to *OAuth 2.0*. In the box next to **Token**, paste your **control plane token** value from earlier in this unit. The change should save automatically.

:::image type="content" source="../media/2-token.png" alt-text="Postman screenshot showing the Authorization tab for a collection." border="false" lightbox="../media/2-token.png":::
...
:::image type="content" source="../media/2-variables.png" alt-text="Postman screenshot showing the Variables tab for a collection." border="false" lightbox="../media/2-variables.png":::

Switch to the *Data plane* collection and create the following variable:

| Variable | Initial value |
| --- | --- |
| `digitaltwins-hostname` | The host name of your Azure Digital Twins instance. You can find this value on the overview page for your instance in the Azure portal. |
...
This section provides more detail about the APIs used in this module.
...
Control plane APIs are ARM APIs used to manage Azure resources related to Azure Digital Twins, so they cover operations like creating/deleting your entire instance and creating/deleting endpoints. An **endpoint** is a recognized destination outside of Azure Digital Twins that can receive digital twin events, so we'll use these requests later in the module to prepare your graph to export data externally. Although the Postman collection imported from the control plane Swagger contains many folders and operations, this module only uses the endpoint ones.

In your Postman collection, expand the following folder path: **Control plane** > **subscriptions** > **{subscriptionId}** > **resourceGroups** > **{resourceGroupName}** > **providers** > **Microsoft.DigitalTwins** > **digitalTwinsInstances** > **{resourceName}** > **endpoints**.

This folder contains request templates for endpoint operations, including create/update, get, delete, and list.

### Data plane APIs

Data plane APIs are used to manage the elements within your Azure Digital Twins instance, such as models, twins, relationships, and event routes. The Postman collection imported from the data plane Swagger has been organized into folders for different API areas that are used in this module. Here's some more information about each.
...
* **eventroutes** - This folder contains request templates for managing event routes from an Azure Digital Twins instance. Event routes are used to direct digital twin data both throughout an Azure Digital Twins graph and to external endpoints, capable of sending data along to downstream services.
