In this unit, you'll use the Model and Digitaltwin APIs to make manual updates to the sample graph that you imported in the previous unit.
...
In this example scenario, say you've decided you also want your graph to keep track of who owns and operates each substation. There are three substation-type models in your instance: *Delivery Substation*, *Generator Substation*, and *Base Receiver* (the base model from which the other two inherit). By adding an `Operator` property to the base model, you can make the new property available on all three.

Copied below is the original text of the *Base Receiver* model that was uploaded to your instance with the sample scenario. (You can also view the [energy grid models in GitHub](https://github.com/Azure-Samples/azure-digital-twins-getting-started/tree/main/models/energy-grid-example).)
...
Below is the updated text for the model after adding a new `Operator` property. You'll use this code in the body of an API request to upload the new model.
...
However, the other substation models *Delivery Substation* and *Generator Substation* depend on this model, so *Base Receiver* can't be deleted without removing those models first. So, in this section, you'll remove all three substation models, reupload the edited *Base Receiver* model, and reupload the *Delivery Substation* and *Generator Substation* models without any changes.
...
Finally, change the **id** value to *dtmi:example:grid:transmission:baseReceiver;1*. This is the ID of the *Base Receiver* model. Send the request.

Now, all substation-type models have been deleted from your instance.
...
Next, return to the **Body** tab and paste in the definition of the *Delivery Substation* model. It's copied below from its [GitHub source file](https://github.com/Azure-Samples/azure-digital-twins-getting-started/blob/main/models/energy-grid-example/DeliverySubStation.json) for simplicity.
...
Next, replace the **Body** contents with the definition of the *Generator Substation* model. It's copied below from its [GitHub source file](https://github.com/Azure-Samples/azure-digital-twins-getting-started/blob/main/models/energy-grid-example/GeneratorSubStation.json) for simplicity.
...
Now all of the substation models have been reuploaded to the instance, including the updated *Base receiver* model and two unchanged inherited models for *Delivery Substation* and *Generator Substation*. The inherited models, as well as any digital twins referencing the *Base Receiver* model or any of its inherited models, will now reference the updated definition and are capable of supporting an `Operator` property.
...
>[!TIP]
>To get a list of all twins that use a certain model, use the Query APIs covered in Unit 5 of this module.
...
You can verify the relationship was deleted by listing the relationships from the  *sub_farmcluster_2* twin and confirming that the *feeds* relationship to farm consumer *c_farm_05* is not in the list.
