...
- **Event flags group**: ThreadX resource that provides a powerful tool for thread synchronization. Threads can operate on all 32 event flags in a group at the same time. There are two ThreadX services that deal with event flags groups, and they're set flags and get flags.
