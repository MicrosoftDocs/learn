...
To meet this requirement, we can employ firewall settings that explicitly authorize pre-determined ports for data transmission then block everything else.  This will ensure that our IoT Edge Gateway is locked down and running only the services that we have granted permission to.  
