...
If a thread attempts to send a message to a full queue, then its behavior will depend on the specified wait option. These options will cause the thread either to abort the message transmission or to suspend, either indefinitely or for a specific number of timer ticks, until adequate space is available in the queue.
