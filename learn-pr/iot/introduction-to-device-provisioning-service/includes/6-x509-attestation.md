The following provides an overview of the Device Provisioning Service (DPS) concepts involved when provisioning devices using X.509 certificate attestation.

> [!NOTE]
> X.509 certificates can be stored in a hardware security module (HSM). We strongly recommend using an HSM with devices to securely store secrets, like the X.509 certificate, on your devices in production.
