...
Each instance of DPS has a unique *ID scope* assigned to it which is used to identify the specific provisioning service that a device will register through. The ID scope is generated by the service and is immutable, which guarantees uniqueness. Uniqueness is important for long-running deployment operations and merger and acquisition scenarios.
...
A *device ID* is how a device appears in IoT Hub. The desired device ID may be set in the an individual enrollment entry, but it is not required to be set. Setting the desired device ID is only supported in individual enrollments. If no desired device ID is specified in the enrollment list, the registration ID is used as the device ID when registering the device.
