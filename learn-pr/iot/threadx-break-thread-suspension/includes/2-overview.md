...
We'll investigate the concepts or the wait abort service, a counting semaphore, and a monitor thread, and then take a look at a project in the code editor. 

If the wait is successfully aborted, a `TX_WAIT_ABORTED` value is returned from the service that the thread was waiting on. This service doesn't release explicit suspension that's made by the `tx_thread_suspend` service. 
