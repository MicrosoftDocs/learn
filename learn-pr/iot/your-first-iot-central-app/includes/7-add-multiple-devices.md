...
1. When you have completed connecting all four new trucks, notice that the **Provisioning Status** is still **Registered**. Not until we actually make the connection will this change.
...
1. Open up Azure Cloud Shell, and navigate to the right folder by entering `cd refrigerated-truck`.
...
1. Remember to verify that each connection string ends with an exact copy of the **Primary Key**. Just in case there is spurious text at the end of the connection.txt file.
...
1. Exciting isn't it. You now have five console windows open, with five simulated refrigerated trucks waiting for their commands!
