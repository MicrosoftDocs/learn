...
In addition to this list, there are edge-cases we could also investigate, such as what happens when the truck's contents start to melt. However, as we have left this state up to chance in our simulation (note the use of random numbers in our code in the previous unit) we can perhaps leave this test out of our first round of testing.
...
1. If the next line on the console is **Device successfully connected to Azure IoT Central** you have made the connection. This line should be followed by some text verifying the settings and properties were sent successfully.
