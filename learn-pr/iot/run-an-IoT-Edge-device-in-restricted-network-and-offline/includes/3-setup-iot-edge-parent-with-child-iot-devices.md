This exercise will detail steps on registering an IoT Edge Device within Azure IoT Hub, and configuring an IoT Device in a child relationship. The child IoT Device will be able to send messages through to the IoT Edge Device as a gateway prior to communicating to the Azure IoT Hub.
...
1. Notice when the command completes, there is a blog of JSON returned to the terminal. This JSON includes a few details for the configuration of the IoT Edge Device that was created. The device details also contains the symmetric keys that were autogenerated by the service for the device.
