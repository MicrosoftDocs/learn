In this unit, you're going to create a programming project to simulate a sensor device in a refrigerated truck. This simulation enables you to test the code long before requiring a real truck! 
...
1. After you've entered the code below into the Program.cs file, you can run the app with the command `dotnet run`. This command will run the Program.cs file in the current folder, so ensure you're in the `RefrigeratedTruck` folder.
...
1. Watch the telemetry for a short while.
...
1. Try giving the truck a command to go to a customer. Valid customer numbers are 0 through 9. Click on **Commands**, then enter a customer ID, and click **Run**.

If all is well, this is great progress. The truck is responsive, and awaiting more commands.
