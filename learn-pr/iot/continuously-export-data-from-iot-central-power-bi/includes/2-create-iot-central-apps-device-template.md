...
In this unit, you'll first download the required files, then import the capability model. Then we create a dashboard showing all the capabilities of the device template. Next, you'll create a real device, and record the connection settings needed for the remote device app. Finally, an Azure Maps account is created, if needed.
...
1. When you're satisfied with your dashboard, click **Save**, then click **Publish**. You'll now notice that in the dialog that appears, that the **Views** entry is **Yes**. Click **Publish** in the dialog.
...
You don't need an Azure Maps account to _render_ a map, as the rendering is done by an IoT Central dashboard. You need the Azure Maps account to use its API calls to calculate the best route to any given destination.
