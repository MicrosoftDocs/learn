...
To create, manage and monitor Automatic Device Management configurations with Azure IoT Hub, you can use the portal like you did in previous exercises, but to integrate the device management operations into your IoT application and process, dealing with multiple configurations in parallel and automate part of the process, you will want to use a more programmatic method.

The IoT extension for the Azure CLI allows you to easily create, update, monitor, and delete configurations as you can see in the [documentation](/azure/iot-hub/iot-hub-automatic-device-management-cli).

We want to prepare a firmware update campaign to update all devices installed with two of our customers: Lewis and Smith. But, as described in the previous unit, we don't want to take the risk of disrupting cheese-aging processes for too long, and we can't afford a failure of all devices at the same time.
...
We will create a script file that we will execute from Azure Cloud Shell to create the device identities and start the simulators.

1. In Cloud shell, and in the same folder you created the device simulator app, create, and open a script file called *createdevices.sh* by running the following command:
...
As an administrator of the IoT solution, you could use the Azure portal to create the device management configurations (as done in previous units), but to automate things at scale, it's simpler to create and use [Azure CLI scripts](/azure/iot-hub/iot-hub-automatic-device-management-cli).
...
1. Copy and paste the following code into *firmwareupdatecontent.json*:
...
1. Copy and paste the following code into *firmwareupdatemetricslewis.json*:
...
1. Copy and paste the following code into *firmwareupdatemetricssmith.json*:
...
1. Now that we have our configuration content ready, we want to create and monitor the first configuration. After the first one will terminate successfully, we will create and monitor the second one.
    ...
    > [!NOTE]
    > Remember the trick to use a text editor to prepare the commands before pasting them in Cloud Shell.
...
1. The firmware update campaign started and you should see the five devices installed at the Lewis' start the firmware update. We will now monitor the progress of the configuration to make sure everything went well before starting the second one. In the terminal, run the following command (replacing {your iot hub name} with your hub name):
    ...
    This command will list all the devices on which the firmware update has been successfully applied. You should see five of them. If that's not the case, check the progress on each device simulator output, and try the monitoring command again.

1. After the first part of the firmware update campaign was successful, you can start the second one by running the following command in Cloud Shell:
    ...
    And then run the following command (eventually several times) to monitor progress.
...
Congratulations. You successfully updated all devices' firmware at both customers' locations without disrupting their production.
