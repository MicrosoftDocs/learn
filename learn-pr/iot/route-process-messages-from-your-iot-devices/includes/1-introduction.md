Azure Stream Analytics can analyze and process large volumes of streaming IoT data in real-time with sub-millisecond latencies. Important patterns and relationships can be identified in the data which can be used to fine-tune and improve a system.
...
You are developing an IoT-based solution to improve product packaging speed for your company.
 ...
 An efficiency metric is the number of packages leaving the conveyor belt system per work shift. Belt speed is critical in this process, but belt vibration tends to increase in relation to belt speed.
 
 There are three belt speeds: **stopped**, **slow**, and **fast**.

* Slow speed vibration is much lower, but less packages are transported.
...
It's not easy for operators to visually detect abnormal vibration levels, so you are prototyping an Azure IoT solution that will help measure vibration levels and data anomalies. IoT vibration sensors will be attached to the conveyor belt at various locations and send telemetry to an IoT hub. The IoT hub uses Azure Stream Analytics and a Machine Learning (ML) model to alert operators of vibration anomalies in real time.
