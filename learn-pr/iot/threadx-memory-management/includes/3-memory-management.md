...
:::image type="content" alt-text="Diagram that lists the attributes of a memory byte pool in a table." source="../media/memory-byte-pool-attributes.png" border="false" loc-scope="Azure":::

Every memory byte pool must have a control block that contains essential system information. Every memory byte pool is assigned a name, which is used primarily for user identification purposes. The starting address of the byte pool must be provided, as well as the total number of bytes to be allocated to the memory byte pool.
