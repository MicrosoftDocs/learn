This module will guide you through the steps required to use the Azure IoT Hub Device Provisioning Service (DPS) to group enroll IoT devices securely using X.509 certificate authentication, and deprovision devices after use. As part of this exercise, you will generate an X.509 root certificate, device certificates signed by the root certificate, and run C# Azure IoT device SDK code samples to provision multiple devices to an Azure IoT hub.

## Example scenario

Suppose you are working on an update to your company's cheese shipping container monitoring and tracking solution. The proposed system update will use IoT devices to reduce manual data entry and provide more advanced monitoring during the container shipping process. The IoT devices connect to Azure IoT Hub using vehicle-provided WiFi. The new system will provide continuous monitoring of the product environment and enable a variety of notification scenarios when issues are detected. The IoT devices are equiped with integrated sensors to track the location, temperature, and pressure of shipping containers during transit.
