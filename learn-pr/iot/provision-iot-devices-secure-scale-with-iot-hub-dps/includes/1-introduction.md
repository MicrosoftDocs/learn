...
Create and test two C# programs that use X.509 certificates to authenticate with the Azure IoT Hub Device Provisioning Service (DPS). The programs generate sample telemetry readings that are sent to an IoT hub. Use the Azure Cloud Shell to create an instance of the DPS service, IoT hub, set device twin properties, and initiate group and single device enrollment and disenrollment to test program enrollment and device twin update scenarios.
