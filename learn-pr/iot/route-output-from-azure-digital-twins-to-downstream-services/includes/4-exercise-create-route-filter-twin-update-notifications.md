...
1. Create a [route](/azure/digital-twins/concepts-route-events#create-an-event-route) in Azure Digital Twins to send twin update events to your endpoint. The filter in this route will allow only twin update messages to be passed to your endpoint:
    ...
    >
    > To resolve this issue, either run `az login` in Cloud Shell before you run the command or use the [local CLI](/cli/azure/install-azure-cli?preserve-view=true&view=azure-cli-latest) instead of Cloud Shell. For more information, see [Known issues in Azure Digital Twins](/azure/digital-twins/troubleshoot-known-issues#400-client-error-bad-request-in-cloud-shell).
