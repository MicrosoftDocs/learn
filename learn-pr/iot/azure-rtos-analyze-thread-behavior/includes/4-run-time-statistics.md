ThreadX provides several services to extract cumulative performance data for a variety of ThreadX resources. We wish to analyze the performance of our system, so we need cumulative performance data for our two threads, which we can do with the `tx_timer_performance_info_get` service.
...
- `&Routine_reumptions` — Pointer to destination for the number of resumptions of this thread
...
There are other parameters for this service that we do not need, so we enter `TX_NULL` to ignore them.
