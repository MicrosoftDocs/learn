ThreadX provides several services to extract cumulative performance data for various ThreadX resources. We wish to analyze the performance of our system, so we need cumulative performance data for our two threads, which we can do with the `tx_timer_performance_info_get` service.
