...
The second property of the `LayoutOptions` struct is `Expands`. The `Expands` property is a `bool` that allows a view in a `StackLayout` to request extra space if any is available. You'll explore the `Expands` property in more detail in the unit on the `StackLayout` layout.
