Each device type (iOS/iPadOS, Windows, Android, macOS) provides a different set of enrollment methods. These methods range from users adding their own device to using a special user account to enroll and manage multiple corporate-owned devices. A user adding their own device to Intune is called, Bring your own device (BYOD). When you use a special user account to enroll and manage multiple corporate-owned devices, you use the Device enrollment manager (DEM).
...
Additionally, you can choose to enroll corporate owned devices in Intune based on an international mobile equipment identifier (IMEI) number or serial number (SN), and several other enrollment methods.
...
- **Enable Apple device enrollment**: A mobile device management (MDM) push certificate is required for iOS/iPadOS and macOS device enrollment.
...
- **Multi-factor authentication**: Require users to use an extra verification method such as a phone, PIN, or biometric data when they enroll a device.
