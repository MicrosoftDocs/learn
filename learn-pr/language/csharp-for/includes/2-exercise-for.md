...
- The second part defines the completion condition.  In this example: `i < 10`.  In other words, the runtime will continue to iterate over the code in the code block below the `for` statement while `i` is less than `10`.  Once `i` is greater or equal to `10`, the runtime will stop executing code after the `for` statement's code block.  The docs refer to this section as the **condition**.
