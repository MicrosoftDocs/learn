...
A microservices architecture helps address those needs by building a single application that is composed of a set of small services. Each microservice implements a specific end-to-end domain or business capability within a certain context boundary, and each must be developed autonomously and be deployable independently.
...
Finally, .NET was built to be cloud native first. It runs cross-platform, so your Docker image can be based on a flavor of Linux, and your .NET code will still run. In addition, there are already .NET images created for Docker by Microsoft. Plus, .NET is extremely fast, with the ASP.NET Kestrel web server routinely outperforming other web servers.
