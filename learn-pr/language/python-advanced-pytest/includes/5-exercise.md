In this exercise, you'll use Pytest with _parametrize_ to test a function. Then, you’ll update a test class to use a fixture instead of a `setup()` and `teardown()` method. Using _parametrize_ and working with fixtures will allow you to become more flexible when creating or updating tests.
...
1. Create a new test file called _test_advanced.py_ and add the following code:
    ...
    The function `str_to_bool()` accepts a string as input and depending on its contents it will return a true or a false value. 
...
3. Next, append another test with the false values:
    ...
    There are now two tests are covering all the possible inputs for both the true and the false return values. 
...
After adding tests, the next step is to run `pytest` and inspect the output. Use the increased verbosity flag (`-v`) so that you can see all input values being treated as a separate test. 
...
1. Add a new class-based test to the _test_advanced.py_ file. This test should use a `setup()` and `teardown()` function that creates a temporary file with some text on it. After each test, the file gets removed. It should look like this:
   ...
   The test class is ensuring that a file is created, but this is problematic because it’s using the _/tmp/_ path, which isn’t guaranteed to be present on every system. 

2. Create a fixture that uses the `tmpdir()` fixture from Pytest to produce the content and return the path:
...
- Two parametrized tests for the `str_to_bool()` function, one that tests the true values and the other one that test the false values.
