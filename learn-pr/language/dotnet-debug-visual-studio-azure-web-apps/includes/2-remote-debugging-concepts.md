...
When an application is compiled and deployed to Azure, Visual Studio does not automatically have access to the necessary symbol files, so you must provide them. This can be accomplished by loading a local copy of the symbol files during debugging. The local source code and symbol files must exactly match the deployed code so that the instructions line up correctly.
