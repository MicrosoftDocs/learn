...
`props` are read-only values available to a single instance of a component. They can be set programmatically or with the same syntax you use when setting attributes for HTML elements. Unlike normal HTML, `props` are not limited to strings or primitive values; they can be complex objects or arrays. `props` is available as a property of `this` inside a component. `props` can contain as many properties as your component needs, and is capable of storing objects or any other data type.
