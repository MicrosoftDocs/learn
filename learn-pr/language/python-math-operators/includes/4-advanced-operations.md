...
The built-in Python function called `round` is also helpful. Use it to round up to the nearest integer if the decimal value is greater than `.5`, or down if it's less than `.5`. However, if the decimal value is equal to .5, the function rounds to the nearest **even** integer.
