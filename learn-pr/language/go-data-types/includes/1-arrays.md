...
To declare an array in Go, you have to define the data type of its elements and the number of elements the array can hold. You can then access each element in the array with the subscript notation where zero is it's the first element, and the last element is one less than the array length (length - 1).
