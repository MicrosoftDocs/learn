...
A typical programming exercise is to write a program to calculate the Fibonacci sequence from a number. Write a function that returns a slice with all the numbers in a Fibonacci sequence that results from doing the calculation from a number that users input greater than two. Let's assume that numbers lower than `2` will result in an error and return a nil slice.
