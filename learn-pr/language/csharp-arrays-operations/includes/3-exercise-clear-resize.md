...
Focus on the line `Array.Resize(ref pallets, 6);`. Here, we're calling the `Resize()` method passing in the `pallets` array by reference, using the `ref` keyword. In some cases, methods require that you pass arguments by value (the default) or by reference (using the ref keyword). The reasons this is necessary would require a lot of explanation about of how objects are managed in .NET; unfortunately, that's beyond the scope of this module. When in doubt, we recommend looking at Intellisense or Microsoft Docs for examples on how to properly call a given method.
