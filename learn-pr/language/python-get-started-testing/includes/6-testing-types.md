...
Test functions and methods that are very long, with multiple logical conditions will be difficult to unit test. Unit testing forces developers to think about complexity in code and keeping it to a minimum. In general, the shorter the function or method is, the easier it will be to test it.
...
Although CI (Continuous Integration) isn't a type of testing, it's an important piece that has to do with every type of testing. When a testing plan is put into place, it's essential to have _something_ that will run the tests in an automated way. A CI environment is the right place to automatically run tests. These tests can run on a schedule, triggered by an event, or manually executed. This environment will usually be consistent with where the code or project needs to run.
...
For example, if a developer wants to merge his changes to the main branch, it's ideal to ensure that the changes won't cause breakage. A CI system can automatically run tests from a developer's branch and alert when there's a failure. Preventing code that has failed CI to be merged is a good way to increase confidence and robustness in a project.
