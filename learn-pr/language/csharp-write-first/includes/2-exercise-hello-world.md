...
> [!NOTE] 
> You might be tempted to select `Copy` or `Run`  and skip all the typing.  However, we encourage you to enter this line of code yourself. Entering the code yourself builds muscle memory and helps you gain insights that you wouldn't get otherwise.
...
To print an entire message to the output console, the first technique used `Console.WriteLine()`. At the end of the line, it added a line feed similar to how to create a new line of text by using the `Enter` or `Return` key on your keyboard.  
