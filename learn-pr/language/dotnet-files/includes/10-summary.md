...
- `Directory.EnumerateDirectories` and `Directory.EnumerateFiles` accept a parameter that enables you to specify a search condition for names to return, and a parameter to recursively traverse all child directories.
- `System.Environment.SpecialFolder` is an enumeration that enables you to access system-defined folders, such as the desktop or user profile, in a cross-platform manner without having to memorize the exact path for each operating system.
