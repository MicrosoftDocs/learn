...
You typically write Rust syntax in a text file and save it to your local hard drive. You can write code by using a simple text file editor, like Notepad in Windows. Notepad edits ASCII text, a simple standard text file format.
...
Go to the [Visual Studio Code download page](http://code.visualstudio.com/download?azure-portal=true). The web page displays logos for Windows, Linux, and Mac.
...
The web page displays logos for Windows, Linux, and Mac. Under the Linux logo, select either the .deb installer (for .deb-based package managers) or the .rpm installer (for .rpm-based package managers).
