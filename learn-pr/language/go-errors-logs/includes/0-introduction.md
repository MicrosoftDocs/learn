There are times when the programs you write don't behave the way you expect. Sometimes there are external factors that you can't control, like other processes blocking a file, or an attempt to access a memory address that's not available anymore. Failures are simply another type of behavior that your programs can have. It's better if you anticipate those failures so you can troubleshoot problems when they happen.
