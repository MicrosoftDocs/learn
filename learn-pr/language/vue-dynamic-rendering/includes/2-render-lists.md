...
- **Expose an array** via your data object. To be able to use the `v-for` directive, ensure that there's data of type array that your `data()` method is returning.
...
If the data is modified, Vue.js needs to be able to refresh the appropriate parts of the display. When you're working with a list, it's best to update the individual item rather than the entire list. To allow Vue.js to find an individual item, each displayed item should have a key. The key does *not* need to be part of the data; you can generate it by using the **index** of the array.
