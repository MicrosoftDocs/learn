In the introduction, we described a scenario that required us to match a number representing the employee's level to a title.   In this exercise, let's use a `switch-case` construct implement this branching logic.
