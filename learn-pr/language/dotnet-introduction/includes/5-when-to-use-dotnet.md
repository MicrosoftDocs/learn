...
.NET is also popular when used for smaller projects due to a long history of improvements to the design of the languages, APIs, and tools based on customer feedback and telemetry. New full-featured software can be developed quickly for prototyping purposes. Furthermore, .NET can run on inexpensive hardware and cloud services, and out-perform other popular platforms, which makes it a great fit for start-up businesses.
...
.NET can be used to build IoT applications for IoT devices and scenarios. IoT applications typically interact with sensors, displays, and input devices that require the use of GPIO pins, serial ports, or similar hardware. For example, there are libraries available for working with popular boards like Raspberry Pi and Hummingboard.
