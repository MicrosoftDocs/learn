This unit focuses on the technical aspects of building an application. We'll use .NET languages, tools, and frameworks to outline the general workflow of software developers who build .NET applications.
...
The first decision developers make is selecting the tools they'll use to create their applications. In other words, they first select their development environment.  The decision is based purely on whether they prefer a visual workflow that features a graphical user interface, or a command-line interface where they'll mostly rely on the keyboard to navigate the development environment and issue commands.
...
If you prefer a command-line environment, then separately download and install Visual Studio Code and the .NET SDK. This choice is popular for beginners who want to get started quickly. Both tools require less bandwidth to download.
...
Sometimes the code can be compiled because it follows the syntax rules, but the application's logic has problems. If the logic prevents the .NET runtime from running a command, then the program "crashes." The .NET runtime then removes the program from the computer's memory.  These errors are called *runtime errors* or *exceptions*.
...
Some logic errors don't result in a "crash." But they also don't produce the results that your users expect. You can remedy these errors only by testing and debugging your application.
...
When you debug, you watch your application as it runs to understand what's going on. You can set break points that pause the application and allow you to follow the code as it runs. You can observe the value that's stored in variables, take control of the running path through the code to skip over or rerun lines, change the value of variables, and so on.

Both the visual and command-line debugging tools allow you to observe and take control of the running of the application. Use this functionality to see what's going on in your application as it's running.

### Distribute the application

When the application is ready to be released, you create a release version of the build. In a release version, you remove the code that's required for debugging. Again, in both the visual and the command-line tools, you can compile a release version. 
