...
A revenue model is a strategy for how to assign value to your product or service, how to target customers to buy your product or service, what to charge those customers, and how to deliver the value of your product or service in such a way that yields increasing sales.  In other words, it encompasses all the factors you need to consider when thinking about how your business will generate and scale revenue.
...
**Ideal for:** Infrastructure businesses that provide services that users can't afford to own but need to access (for example, Azure or Twilio).
...
**Ideal for:** Software products that customers will use regularly and continue using for an extended period (for example, Microsoft 365 or Adobe Photoshop).

**Practical considerations:**

- Enables you to generate recurring revenues, which provide many benefits.
...
- Allows you to offer new customers a free trial that converts to a paid plan after a set period. This can be a good way of getting customers to start using your product, particularly if they're uncertain about the features, value, or pricing. However, it can also be a distraction if too few users convert.
- Enables you to deliver a *freemium* offering in which users can access basic features for free (usually forever), and unlock premium features by paying (for example, LinkedIn Premium, Strava, or Spotify). You need to ensure that a large enough percentage of users convert to paid plans and that free users can be serviced at very low cost.
...
**Ideal for:** High-value products or services. Enterprise customers where relationships are crucial, or sales require interaction with multiple decision-makers and influencers over time (like SAP).
...
**Ideal for:** Marketplace or platform businesses that connect a buyer with a seller (for example, Airbnb or Upwork).
...
**Ideal for:** Physical products that can be purchased online via an e-commerce store (for example, most Shopify stores).

**Practical considerations:**

- Revenue generation is an isolated event and only occurs when the customer makes a purchase, so there's limited scope for recurring revenues. Exceptions: products that are purchased often (such as groceries) or subscription boxes such as Dollar Shave Club or meal delivery services.
...
**Ideal for:** Any item that users want to use sporadically, but not enough that it makes sense for them to buy and maintain their own (for example, Lime electric scooter rental). Expensive items or those that require complex maintenance (for example, UberAIR helicopter rental).
...
**Ideal for:** Advanced technologies that could be used by multiple licensees, or in applications where your technology is only part of a solution and relies on an existing product or infrastructure that you don't own (for example, image correction software for use on MRI scanners).

**Practical considerations:**

- Licensees generally make a build-or-buy decision in which they weigh up the cost of creating the technology in-house versus acquiring the right to use your existing technology.
...
**Ideal for:** Products or services that naturally sit alongside another company's offering as an up-sell or specialist product (for example, companies that partner with Microsoft via the Microsoft Partner Network).
...
- You'll have limited control over the sales process, you don't own the customer relationship, and you may never interact directly with the customer.
